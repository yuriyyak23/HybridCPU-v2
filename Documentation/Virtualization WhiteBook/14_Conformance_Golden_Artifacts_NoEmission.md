...
The one exception to broad no-emission is the default-off, exact-operation compiler producer for `HybridCPU.VMCALL.Runtime.v1 / 0x0001 / PROBE_NO_STATE_V1`. It emits a carrier, not backend success. `VmxExactProbeCompilerEmissionGateTests` proves exact identity/ABI/scheduling, raw-ingress and forgery denial, and independent runtime denial for missing capability, foreign domain and revoked/stale grant. Every adjacent or broad virtualization operation remains under the no-emission contract.

Compiler intent, legality facts, metadata, plans and bindings are deliberately absent from runtime sources. Static tests also forbid compiler calls to `SafetyVerifier`, runtime admission, capability/restore owners, backend execution, completion and retire owners.

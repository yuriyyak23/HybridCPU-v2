...
## CompletionPublishedRetireDenied

Neutral publication-fence result containing a published `CompletionRecord` while `RetirePublicationAllowed` remains false. It is completion success, not architectural retirement.
...
## TrapCompletionMigrationClass

Neutral classification used by the completion fence. `Unclassified` and `HostOwnedNonMigratable` cannot grant retire; positive retire classification must be explicit.

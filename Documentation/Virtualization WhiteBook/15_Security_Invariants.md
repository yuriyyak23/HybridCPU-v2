...
Route authorization is also not publication. `RuntimeOwnedCompletionPublication` may authorize only the completion route flag; the neutral fence must still create the record. When it does, `CompletionPublishedRetireDenied` keeps retire false.

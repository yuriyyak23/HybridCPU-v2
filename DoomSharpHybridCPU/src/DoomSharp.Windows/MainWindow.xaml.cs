...
namespace DoomSharp.Windows
{
    ...
    public partial class MainWindow : Window
    {
        ...
        protected override void OnInitialized(EventArgs e)
        {
            ...
            var wadPath = Path.Combine(AppContext.BaseDirectory, "DOOM.WAD");
            if (!File.Exists(wadPath))
            {
                DoomGame.Console.WriteLine($"WAD file not found: {wadPath}");
                return;
            }

            var wadImage = File.ReadAllBytes(wadPath);
            var gameTask = Task.Run(() => DoomGame.Instance.Run(GameMode.Retail, wadImage));
            _ = gameTask.ContinueWith(
                task => DoomGame.Console.WriteLine($"Game startup failed: {task.Exception?.GetBaseException()}"),

...
public class IntermissionController
{
    ...
    private readonly Patch[] _levelNames = new Patch[36];
    ...
    {
        public Animation(AnimationType type, int period, int frameCount, Point location, int data1 = 0)
        {
            Type = type;
            Period = period;
        ...
        // ALWAYS: n/a, RANDOM: period deviation (<256), LEVEL: level
        public int Data1 { get; set; }

        // ALWAYS: n/a, RANDOM: random base period, LEVEL: n/a
    ...
    public void Start(WorldMapInfo wmInfo)
    {
        ...
        if (DoomGame.Instance.Game.DeathMatch != 0)
        {
            InitDeathmatchStats();
        }
        else
    ...
    {
        var name = DoomGame.Instance.GameMode == GameMode.Commercial
            ? "INTERPIC"
            : string.Concat("WIMAP", DoomString.DecimalDigit(_wbs.Episode));
        ...
        if (DoomGame.Instance.GameMode == GameMode.Commercial)
        {
            ...
            {
                name = string.Concat("CWILV", DoomString.DecimalTwoDigits(i));
                _levelNames[i] = Patch.FromBytes(DoomGame.Instance.WadData.GetLumpName(name, PurgeTag.Static)!);
            }
        }
        else
        {
            ...
            {
                name = string.Concat(
                    string.Concat("WILV", DoomString.DecimalDigit(_wbs.Episode)),
                    DoomString.DecimalDigit(i));
                _levelNames[i] = Patch.FromBytes(DoomGame.Instance.WadData.GetLumpName(name, PurgeTag.Static)!);
            }
            ...
            if (_wbs.Episode < 3)
            {
                for (var j = 0; j < NumberOfAnimations[_wbs.Episode]; j++)
                {
                    ...
                    for (var i = 0; i < a.FrameCount; i++)
                    {
                        ...
                        {
                            // animations
                            var animationName = string.Concat(
                                string.Concat("WIA", DoomString.DecimalDigit(_wbs.Episode)),
                                string.Concat(DoomString.DecimalTwoDigits(j), DoomString.DecimalTwoDigits(i)));
                            a.Patches[i] = Patch.FromBytes(DoomGame.Instance.WadData.GetLumpName(animationName, PurgeTag.Static)!);
                        }
                        else
                        {
                            // HACK ALERT!
        ...
        {
            // numbers 0-9
            _num[i] = Patch.FromBytes(DoomGame.Instance.WadData.GetLumpName(
                string.Concat("WINUM", DoomString.DecimalDigit(i)), PurgeTag.Static)!);
        }
        ...
        {
            // "1,2,3,4"
            _p[i] = Patch.FromBytes(DoomGame.Instance.WadData.GetLumpName(
                string.Concat("STPB", DoomString.DecimalDigit(i)), PurgeTag.Static)!);

            // "1,2,3,4"
            _bp[i] = Patch.FromBytes(DoomGame.Instance.WadData.GetLumpName(
                string.Concat("WIBP", DoomString.DecimalDigit(i + 1)), PurgeTag.Static)!);
        }
    }
    ...
    private void DrawOnLevelNode(int level, Patch?[] patches)
    {
        var i = 0;
        var fits = false;
        var node = LevelNodes[_wbs.Episode][level];

        do
        {
            var left = node.X - patches[i]!.LeftOffset;
            var top = node.Y - patches[i]!.TopOffset;
        ...
        {
            DoomGame.Console.WriteLine("Could not place patch on level " + DoomString.DecimalInt(level + 1));
        }
    }

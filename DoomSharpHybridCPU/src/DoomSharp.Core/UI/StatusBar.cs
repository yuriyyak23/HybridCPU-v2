...
public class StatusBar
{
    ...
    // Cheat sequences
    private readonly CheatSequence _cheatGod = new("iddqd");
    private readonly CheatSequence _cheatAmmo = new("idkfa");
    private readonly CheatSequence _cheatAmmoNoKey = new("idfa");
    private readonly CheatSequence _cheatNoClip = new("idspispopd");
    private readonly CheatSequence _cheatCommercialNoClip = new("idclip");
    private readonly CheatSequence[] _cheatPowerup = new CheatSequence[]
    {
        new("idbeholdv"),
        new("idbeholds"),
        new("idbeholdi"),
    ...
    private readonly CheatSequence _cheatChoppers = new("idchoppers");
    private readonly CheatSequence _cheatClev = new("idclev\x01\x01");
    private readonly CheatSequence _cheatMyPos = new("idmypos");
    ...
    private void LoadGraphics()
    {
        ...
        for (var i = 0; i < 10; i++)
        {
            name = string.Concat("STTNUM", DoomString.DecimalDigit(i));
            _tallNum[i] = Patch.FromBytes(DoomGame.Instance.WadData.GetLumpName(name, PurgeTag.Static)!);

            name = string.Concat("STYSNUM", DoomString.DecimalDigit(i));
        ...
        {
            name = string.Concat("STKEYS", DoomString.DecimalDigit(i));
        ...
        for (var i = 0; i < 6; i++)
        {
            name = string.Concat("STGNUM", DoomString.DecimalDigit(i + 2));
        ...
        name = string.Concat("STGNUM", DoomString.DecimalDigit(DoomGame.Instance.Game.ConsolePlayer));
        ...
        for (var i = 0; i < ST_NUMPAINFACES; i++)
        {
            ...
            {
                name = string.Concat("STFST", DoomString.DecimalDigit(i), DoomString.DecimalDigit(j));
                _faces[facenum++] = Patch.FromBytes(DoomGame.Instance.WadData.GetLumpName(name, PurgeTag.Static)!);
            }

            name = string.Concat("STFTR", DoomString.DecimalDigit(i), "0"); // turn right
            _faces[facenum++] = Patch.FromBytes(DoomGame.Instance.WadData.GetLumpName(name, PurgeTag.Static)!);
            name = string.Concat("STFTL", DoomString.DecimalDigit(i), "0"); // turn left
            _faces[facenum++] = Patch.FromBytes(DoomGame.Instance.WadData.GetLumpName(name, PurgeTag.Static)!);
            name = string.Concat("STFOUCH", DoomString.DecimalDigit(i)); // ouch!
            _faces[facenum++] = Patch.FromBytes(DoomGame.Instance.WadData.GetLumpName(name, PurgeTag.Static)!);
            name = string.Concat("STFEVL", DoomString.DecimalDigit(i)); // evil grin ;)
            _faces[facenum++] = Patch.FromBytes(DoomGame.Instance.WadData.GetLumpName(name, PurgeTag.Static)!);
            name = string.Concat("STFKILL", DoomString.DecimalDigit(i)); // pissed off
            _faces[facenum++] = Patch.FromBytes(DoomGame.Instance.WadData.GetLumpName(name, PurgeTag.Static)!);
        }
    ...
    private void RefreshBackground()
    {
        ...
        DoomGame.Instance.Video.CopyStatusBarRectangle(X, 0, Width, Height, X, Y);
    }

    private void CreateWidgets()
    {
        _weaponWidget = new NumberWidget(this, ST_AMMOX, ST_AMMOY, _tallNum!, NumberSource.ReadyWeaponAmmo, OnSource.StatusBar, ST_AMMOWIDTH);
        _weaponWidget.Data = (int)_player!.ReadyWeapon;

        _healthWidget = new PercentWidget(this, ST_HEALTHX, ST_HEALTHY, _tallNum!, NumberSource.Health, OnSource.StatusBar, _tallPercent!);
        _armsBackgroundWidget = new BinaryIconWidget(this, ST_ARMSBGX, ST_ARMSBGY, _armsBackground!, BinaryValueSource.NotDeathMatch, OnSource.StatusBar);

        for (var i = 0; i < 6; i++)
        {
            _armsWidgets[i] = new MultiIconWidget(this, ST_ARMSX + (i % 3) * ST_ARMSXSPACE,
                ST_ARMSY + (i / 3) * ST_ARMSYSPACE, _arms[i], (IconNumberSource)((int)IconNumberSource.Weapon1 + i), OnSource.Arms);
        }

        _fragsWidget = new NumberWidget(this, ST_FRAGSX, ST_FRAGSY, _tallNum!, NumberSource.Frags, OnSource.Frags, ST_FRAGSWIDTH);
        _faceStatusWidget = new MultiIconWidget(this, ST_FACESX, ST_FACESY, _faces!, IconNumberSource.Face, OnSource.StatusBar);
        _armorWidget = new PercentWidget(this, ST_ARMORX, ST_ARMORY, _tallNum!, NumberSource.Armor, OnSource.StatusBar, _tallPercent!);

        _keyBoxWidgets[0] = new MultiIconWidget(this, ST_KEY0X, ST_KEY0Y, _keys!, IconNumberSource.Key0, OnSource.StatusBar);
        _keyBoxWidgets[1] = new MultiIconWidget(this, ST_KEY1X, ST_KEY1Y, _keys!, IconNumberSource.Key1, OnSource.StatusBar);
        _keyBoxWidgets[2] = new MultiIconWidget(this, ST_KEY2X, ST_KEY2Y, _keys!, IconNumberSource.Key2, OnSource.StatusBar);

        _ammoWidgets[0] = new NumberWidget(this, ST_AMMO0X, ST_AMMO0Y, _shortNum!, NumberSource.Ammo0, OnSource.StatusBar, ST_AMMO0WIDTH);
        _ammoWidgets[1] = new NumberWidget(this, ST_AMMO1X, ST_AMMO1Y, _shortNum!, NumberSource.Ammo1, OnSource.StatusBar, ST_AMMO1WIDTH);
        _ammoWidgets[2] = new NumberWidget(this, ST_AMMO2X, ST_AMMO2Y, _shortNum!, NumberSource.Ammo2, OnSource.StatusBar, ST_AMMO2WIDTH);
        _ammoWidgets[3] = new NumberWidget(this, ST_AMMO3X, ST_AMMO3Y, _shortNum!, NumberSource.Ammo3, OnSource.StatusBar, ST_AMMO3WIDTH);

        _maxAmmoWidgets[0] = new NumberWidget(this, ST_MAXAMMO0X, ST_MAXAMMO0Y, _shortNum!, NumberSource.MaxAmmo0, OnSource.StatusBar, ST_MAXAMMO0WIDTH);
        _maxAmmoWidgets[1] = new NumberWidget(this, ST_MAXAMMO1X, ST_MAXAMMO1Y, _shortNum!, NumberSource.MaxAmmo1, OnSource.StatusBar, ST_MAXAMMO1WIDTH);
        _maxAmmoWidgets[2] = new NumberWidget(this, ST_MAXAMMO2X, ST_MAXAMMO2Y, _shortNum!, NumberSource.MaxAmmo2, OnSource.StatusBar, ST_MAXAMMO2WIDTH);
        _maxAmmoWidgets[3] = new NumberWidget(this, ST_MAXAMMO3X, ST_MAXAMMO3Y, _shortNum!, NumberSource.MaxAmmo3, OnSource.StatusBar, ST_MAXAMMO3WIDTH);
    }

    private void DrawWidgets(bool refresh)
    {
        var deathMatch = DoomGame.Instance.Game.DeathMatch != 0;
    ...
    {
        // The ready weapon source is evaluated dynamically, since its ammo type can change.
        _weaponWidget!.Data = (int)_player!.ReadyWeapon;
    ...
    private enum BinaryValueSource
    {
        NotDeathMatch
    }

    private bool EvaluateOn(OnSource source) => source switch
    {
        OnSource.StatusBar => _statusBarOn,
        OnSource.Arms => _armsOn,
        OnSource.Frags => _fragsOn,
        _ => false
    };

    private int EvaluateNumber(NumberSource source) => source switch
    {
        NumberSource.ReadyWeaponAmmo => WeaponInfo.GetByType(_player!.ReadyWeapon).Ammo == AmmoType.NoAmmo
            ? 1994 : _player.Ammo[(int)WeaponInfo.GetByType(_player.ReadyWeapon).Ammo],
        NumberSource.Health => _player!.Health,
        NumberSource.Frags => _fragsCount,
        NumberSource.Armor => _player!.ArmorPoints,
        NumberSource.Ammo0 => _player!.Ammo[0],
        NumberSource.Ammo1 => _player!.Ammo[1],
        NumberSource.Ammo2 => _player!.Ammo[2],
        NumberSource.Ammo3 => _player!.Ammo[3],
        NumberSource.MaxAmmo0 => _player!.MaxAmmo[0],
        NumberSource.MaxAmmo1 => _player!.MaxAmmo[1],
        NumberSource.MaxAmmo2 => _player!.MaxAmmo[2],
        NumberSource.MaxAmmo3 => _player!.MaxAmmo[3],
        _ => 0
    };

    private int EvaluateIconNumber(IconNumberSource source) => source switch
    {
        IconNumberSource.Face => _faceIndex,
        IconNumberSource.Key0 => _keyBoxes[0],
        IconNumberSource.Key1 => _keyBoxes[1],
        IconNumberSource.Key2 => _keyBoxes[2],
        >= IconNumberSource.Weapon1 and <= IconNumberSource.Weapon6 => _player!.WeaponOwned[(int)source - (int)IconNumberSource.Weapon1 + 1] ? 1 : 0,
        _ => -1
    };

    private bool EvaluateBinaryValue(BinaryValueSource source) => source switch
    {
        BinaryValueSource.NotDeathMatch => _notDeathMatch,
        _ => false
    };

    private class Widget
    {
        protected Widget(StatusBar statusBar, int x, int y, OnSource onSource)
        {
            Owner = statusBar;
            X = x;
            Y = y;
        ...
        protected StatusBar Owner { get; }
        protected OnSource OnSource { get; }
        public int X { get; }
        public int Y { get; }
    ...
    {
        public NumberWidget(StatusBar statusBar, int x, int y, Patch?[] digits, NumberSource numberSource, OnSource onSource, int width)
            : base(statusBar, x, y, onSource)
        {
            Digits = digits;
        ...
        public NumberSource NumberSource { get; }
        public Patch?[] Digits { get; }

        public void Update(bool refresh)
        {
            if (!Owner.EvaluateOn(OnSource))
            {
                return;
            }
        ...
        private void Draw(bool refresh)
        {
            var numdigits = Width;
            var num = Owner.EvaluateNumber(NumberSource);

            var w = Digits[0]!.Width;
            var h = Digits[0]!.Height;

            OldNum = Owner.EvaluateNumber(NumberSource);
            ...
            DoomGame.Instance.Video.CopyStatusBarRectangle(x, Y - StatusBar.Y, w * numdigits, h, x, Y);
    ...
    {
        public PercentWidget(StatusBar statusBar, int x, int y, Patch?[] digits, NumberSource numberSource, OnSource onSource, Patch percentSign)
            : base(statusBar, x, y, digits, numberSource, onSource, 3)
        {
            PercentSign = percentSign;
        }
        ...
        public new void Update(bool refresh)
        {
            if (refresh && Owner.EvaluateOn(OnSource))
            {
    ...
    {
        public MultiIconWidget(StatusBar statusBar, int x, int y, Patch?[] icons, IconNumberSource iconNumberSource, OnSource onSource)
            : base(statusBar, x, y, onSource)
        {
            Icons = icons;
            IconNumberSource = iconNumberSource;
        }

        public int OldIconNumber { get; set; }
        public IconNumberSource IconNumberSource { get; }
        public Patch?[] Icons { get; }

        public void Update(bool refresh)
        {
            var iconNumber = Owner.EvaluateIconNumber(IconNumberSource);
            if (Owner.EvaluateOn(OnSource) && (OldIconNumber != iconNumber || refresh) && iconNumber != -1)
            {
                Draw(refresh);
            }
        }
        ...
        private void Draw(bool refresh)
        {
            if (OldIconNumber != -1)
            {
                ...
                DoomGame.Instance.Video.CopyStatusBarRectangle(x, y - StatusBar.Y, w, h, x, y);
            }

            var iconNumber = Owner.EvaluateIconNumber(IconNumberSource);
            DoomGame.Instance.Video.DrawPatch(X, Y, FG, Icons![iconNumber]!);
            OldIconNumber = iconNumber;
        }
    }

    private class BinaryIconWidget : Widget
    {
        public BinaryIconWidget(StatusBar statusBar, int x, int y, Patch icon, BinaryValueSource valueSource, OnSource onSource)
            : base(statusBar, x, y, onSource)
        {
            Icon = icon;
        ...
        public BinaryValueSource ValueSource { get; }
        public Patch? Icon { get; }

        public void Update(bool refresh)
        {
            var value = Owner.EvaluateBinaryValue(ValueSource);
            if (Owner.EvaluateOn(OnSource) && (OldValue != value || refresh))
            {
                Draw(refresh);
            }
        }
        ...
        private void Draw(bool refresh)
        {
            ...
            if (Owner.EvaluateBinaryValue(ValueSource))
            {
            ...
            OldValue = Owner.EvaluateBinaryValue(ValueSource);
        }
    }
    ...
    public bool HandleEvent(InputEvent currentEvent)
    {
        ...
        if (currentEvent.Type == EventType.KeyDown)
        {
                ...
                var plyr = _player!;
                var key = (char)currentEvent.Data1;

                // 'dqd' cheat for toggleable god mode
                if (_cheatGod.CheckCheat(key))
                {
                    plyr.Cheats ^= Cheat.GodMode;
                    if ((plyr.Cheats & Cheat.GodMode) != 0)
                    {
                        if (plyr.MapObject != null)
                        {
                            plyr.MapObject.Health = 100;
                        }

                        plyr.Health = 100;
                        plyr.Message = Messages.STSTR_DQDON;
                    }
                    else
                    {
                        plyr.Message = Messages.STSTR_DQDOFF;
                    }
                }
                // 'fa' cheat for killer fucking arsenal
                else if (_cheatAmmoNoKey.CheckCheat(key))
                {
                    ...
                    plyr.Message = Messages.STSTR_FAADDED;
                }
                // 'kfa' cheat for key full ammo
                else if (_cheatAmmo.CheckCheat(key))
                {
                    plyr.ArmorPoints = 200;
                    plyr.ArmorType = 2;
                    ...
                    plyr.Message = Messages.STSTR_KFAADDED;
                }
                // Simplified, accepting both "noclip" and "idspispopd".
                // no clipping mode cheat
                else if (_cheatNoClip.CheckCheat(key)
                         || _cheatCommercialNoClip.CheckCheat(key))
                {
                    plyr.Cheats ^= Cheat.NoClip;

                    if ((plyr.Cheats & Cheat.NoClip) != 0)
                    {
                        plyr.Message = Messages.STSTR_NCON;
                    }
                    else
                    {
                        plyr.Message = Messages.STSTR_NCOFF;
                    }
                }
                ...
                for (var i = 0; i < 6; i++)
                {
                    if (_cheatPowerup[i].CheckCheat(key))
                    {
                        if (plyr.Powers[i] == 0)
                        {
                            plyr.GivePower((PowerUpType)i);
                        }
                        else if (i != (int)PowerUpType.Strength)
                        {
                            plyr.Powers[i] = 1;
                        }
                        else
                        ...
                        plyr.Message = Messages.STSTR_BEHOLDX;
                    }
                }

                // 'behold' power-up menu
                if (_cheatPowerup[6].CheckCheat(key))
                {
                    plyr.Message = Messages.STSTR_BEHOLD;
                }
                // 'choppers' invulnerability & chainsaw
                else if (_cheatChoppers.CheckCheat(key))
                {
                    plyr.WeaponOwned[(int)WeaponType.Chainsaw] = true;
                    plyr.Powers[(int)PowerUpType.Invulnerability] = 1; // true in original (non-zero)
                    plyr.Message = Messages.STSTR_CHOPPERS;
                }
                // 'mypos' for player position
                else if (_cheatMyPos.CheckCheat(key))
                {
                    var mo = game.Players[game.ConsolePlayer].MapObject!;
                    var position = string.Concat("ang=0x", DoomString.HexUInt(Angle.RawValue(mo.Angle)));
                    position = string.Concat(position, ";x,y=(0x");
                    position = string.Concat(position, DoomString.HexInt(Fixed.RawValue(mo.X)));
                    position = string.Concat(position, ",0x");
                    position = string.Concat(position, DoomString.HexInt(Fixed.RawValue(mo.Y)));
                    plyr.Message = string.Concat(position, ")");
                }
            

            // 'clev' change-level cheat
            if (_cheatClev.CheckCheat((char)currentEvent.Data1))
            {
                var buf = new char[3];
                _cheatClev.GetParam(buf);
                ...
                if (DoomGame.Instance.GameMode == GameMode.Shareware
                    && (epsd > 1 || map > 9))
                    return false;
                ...
                // So be it.
                _player!.Message = Messages.STSTR_CLEV;
                DoomGame.Instance.Game.DeferedInitNew(
                    DoomGame.Instance.Game.GameSkill, epsd, map);
            }
        }

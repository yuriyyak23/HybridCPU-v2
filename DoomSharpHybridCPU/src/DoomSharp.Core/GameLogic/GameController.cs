...
public class GameController
{
    ...
    private Thinker? _thinkers;

    public const int BodyQueueSize = 32;
    private MapObject[] _bodyQueue = new MapObject[BodyQueueSize];
    private int _bodyQueueSlot = 0;

    public const int ItemQueueSize = 128;
    private MapThing[] _itemRespawnQueue = new MapThing[ItemQueueSize];
    private int[] _itemRespawnTime = new int[ItemQueueSize];
    private int _iqueueHead;
    private int _iqueueTail;
    ...
    private int[] _sightNodeStack = Array.Empty<int>();
    private int[] _sightSideStack = Array.Empty<int>();
    ...
    private readonly int[] _tmBoundingBox = new int[4];
    ...
    public Thinker? Thinkers => _thinkers;
    ...
    public Fixed BlockMapOriginX => _blockMapOriginX;
    public Fixed BlockMapOriginY => _blockMapOriginY;
    ...
    public void Ticker()
    {
        ...
        for (var i = 0; i < Constants.MaxPlayers; i++)
        {
            if (PlayerInGame[i])
            {
                var cmd = i == ConsolePlayer ? DoomGame.Instance.LocalCommand(buf) : new TicCommand();
                ...
                if ((Players[i].Command.Buttons & ButtonCode.Special) != 0)
                {
                    switch (Players[i].Command.Buttons & ButtonCode.SpecialMask)
                    {
                        ...
                        case ButtonCode.SaveGame:
                            if (DoomString.IsNullOrSpaces(_saveDescription))
                            {
    ...
    private bool CheckSpot(int playernum, MapThing mthing)
    {
        ...
        var fineAngle45 = unchecked((int)(Angle.RawValue(Angle.Angle45) >> DoomMath.AngleToFineShift));
        var an = fineAngle45 * (mthing.Angle / 45);
    ...
    private void DeathMatchSpawnPlayer(int playerNum)
    {
        ...
        if (selections < 4)
        {
            DoomGame.Error(string.Concat(
                string.Concat("Only ", DoomString.DecimalInt(selections)),
                " deathmatch spots, 4 required"));
            return;
        }
        ...
        for (var j = 0; j < 20; j++)
        {
            ...
            {
                _deathMatchStarts[i] = _deathMatchStarts[i].WithType((short)(playerNum + 1));
    ...
    private void DoPlayDemo()
    {
        ...
        if (_demoData.Length == 0 || _demoData[_demoDataIdx++] != DoomGame.DoomVersion)
        {
    ...
    public void InitNew(SkillLevel skill, int episode, int map)
    {
        ...
        if (FastParm || (skill == SkillLevel.Nightmare && GameSkill != SkillLevel.Nightmare))
        {
            for (var i = (int)StateNum.S_SARG_RUN1; i <= (int)StateNum.S_SARG_PAIN2; i++)
                State.GetSpawnState((StateNum)i).Tics >>= 1;
            MapObjectInfo.SetSpeed(MapObjectType.MT_BRUISERSHOT, 20 * Constants.FracUnit);
            MapObjectInfo.SetSpeed(MapObjectType.MT_HEADSHOT, 20 * Constants.FracUnit);
            MapObjectInfo.SetSpeed(MapObjectType.MT_TROOPSHOT, 20 * Constants.FracUnit);
        }
        else if (skill != SkillLevel.Nightmare && GameSkill == SkillLevel.Nightmare)
        {
            for (var i = (int)StateNum.S_SARG_RUN1; i <= (int)StateNum.S_SARG_PAIN2; i++)
                State.GetSpawnState((StateNum)i).Tics <<= 1;
            MapObjectInfo.SetSpeed(MapObjectType.MT_BRUISERSHOT, 15 * Constants.FracUnit);
            MapObjectInfo.SetSpeed(MapObjectType.MT_HEADSHOT, 10 * Constants.FracUnit);
            MapObjectInfo.SetSpeed(MapObjectType.MT_TROOPSHOT, 10 * Constants.FracUnit);
        }
    ...
    private void BeginRecording()
    {
        ...
        _demoData[_demoDataIdx++] = DoomGame.DoomVersion;
    ...
    public bool CheckDemoStatus()
    {
        if (TimingDemo)
        {
            ...
            DoomGame.Error(string.Concat(string.Concat("timed ", DoomString.DecimalInt(GameTic), " gametics in "),
                DoomString.DecimalInt(endTime - StartTime), " realtics"));
            return true;
        }
        ...
        if (DemoRecording)
        {
            ...
            //Z_Free(demobuffer);
            DemoRecording = false;
            DoomGame.Error(string.Concat("Demo ", DemoName, " recorded"));
        }
    ...
    public void AddThinker(Thinker thinker)
    {
        ...
        var current = _thinkers;
        while (current.Next != null)
        {
            current = current.Next;
        }

        current.Next = thinker;
    }
    ...
    {
        thinker.Action = ThinkerAction.None;
    }

    private void ExecuteThinkerAction(ThinkerAction action, ActionParams parameters)
    {
        switch (action)
        {
            case ThinkerAction.MapObjectThinker: P_MapObjectThinker(parameters); break;
            case ThinkerAction.VerticalDoor:
            case ThinkerAction.MoveFloor:
            case ThinkerAction.MoveCeiling:
            case ThinkerAction.PlatformRaise:
            case ThinkerAction.StrobeFlash:
            case ThinkerAction.LightFlash:
            case ThinkerAction.GlowingLight:
            case ThinkerAction.FireFlicker:
                Trigger.ExecuteAction(action, parameters);
                break;
            case ThinkerAction.None:
                break;
            default:
                DoomGame.Error("Invalid thinker action");
                break;
        }
    }
    ...
    private void RunThinkers()
    {
        ...
        var thinker = _thinkers;

        while (thinker != null)
        {
            var next = thinker.Next;
            if (thinker.Action == ThinkerAction.None)
            {
                if (previous == null)
                {
                    _thinkers = next;
            ...
            {
                ExecuteThinkerAction(thinker.Action, new ActionParams(thinker as MapObject, Thinker: thinker));
                P_DrainCrossedSpecialLines();
                previous = thinker;
            }
    ...
    public void P_Ticker()
    {
        ...
        if (DoomGame.Instance.MenuActive
            && !DemoPlayback
    ...
    private void RespawnSpecials()
    {
        ...
        if (_iqueueHead == _iqueueTail)
        {
            return;
        }

        // wait at least 30 seconds
        if (LevelTime - _itemRespawnTime[_iqueueTail] < 30 * 35)
        {
            return;
        }

        var mthing = _itemRespawnQueue[_iqueueTail];

        var x = Fixed.FromRaw(mthing.X << Constants.FracBits);
        var y = Fixed.FromRaw(mthing.Y << Constants.FracBits);

        // spawn a teleport fog at the new spot
        var ss = DoomGame.Instance.Renderer.PointInSubSector(x, y);
        var mo = P_SpawnMapObject(x, y, ss.Sector!.FloorHeight, MapObjectType.MT_IFOG);

        // find which type to spawn
        var moInfo = MapObjectInfo.FindByDoomedNum(mthing.Type);
        if (moInfo == null)
        {
            // pull it from the queue even if we can't find the type
            _iqueueTail = (_iqueueTail + 1) & (ItemQueueSize - 1);
            return;
        }

        // spawn it
        Fixed z;
        if ((moInfo.Flags & MapObjectFlag.MF_SPAWNCEILING) != 0)
        {
        ...
        mo = P_SpawnMapObject(x, y, z, moInfo.Type);
        mo.Spawnpoint = mthing;
        mo.Angle = Angle.Angle45 * (uint)(mthing.Angle / 45);
    ...
    private void P_LoadVertexes(int lump)
    {
        ...
        var reader = new DoomSharp.Core.Data.ByteReader(data);
    ...
    private void P_LoadNodes(int lump)
    {
        ...
        _sightNodeStack = new int[_numNodes];
        _sightSideStack = new int[_numNodes];
    ...
    private void P_LoadThings(int lump)
    {
        ...
        for (var i = 0; i < numThings; i++)
        {
            ...
            if (!ShouldSpawnThing(DoomGame.Instance.GameMode, mt.Type))
                continue;
    ...
    internal static bool ShouldSpawnThing(GameMode gameMode, short type)
    {
        if (gameMode == GameMode.Commercial)
            return true;

        return type is not (68 or 64 or 88 or 89 or 69 or 67 or 71 or 65 or 66 or 84);
    }
    ...
    private Player?[] _playerSpriteStatePlayers = new Player?[8];
    private PlayerSpriteType[] _playerSpriteStatePositions = new PlayerSpriteType[8];
    private StateNum[] _playerSpriteStateNumbers = new StateNum[8];
    private int _playerSpriteStateRead;
    private int _playerSpriteStateCount;

    internal void P_QueuePlayerSpriteState(Player player, PlayerSpriteType position, StateNum state)
    {
        if (_playerSpriteStateCount == _playerSpriteStatePlayers.Length)
        {
            if (_playerSpriteStatePlayers.Length > int.MaxValue / 2)
                DoomGame.Error("Player-sprite state queue capacity overflow");
            var newLength = _playerSpriteStatePlayers.Length * 2;
            var players = new Player?[newLength];
            var positions = new PlayerSpriteType[newLength];
            var states = new StateNum[newLength];
            Array.Copy(_playerSpriteStatePlayers, players, _playerSpriteStateCount);
            Array.Copy(_playerSpriteStatePositions, positions, _playerSpriteStateCount);
            Array.Copy(_playerSpriteStateNumbers, states, _playerSpriteStateCount);
            _playerSpriteStatePlayers = players;
            _playerSpriteStatePositions = positions;
            _playerSpriteStateNumbers = states;
        }

        _playerSpriteStatePlayers[_playerSpriteStateCount] = player;
        _playerSpriteStatePositions[_playerSpriteStateCount] = position;
        _playerSpriteStateNumbers[_playerSpriteStateCount] = state;
        _playerSpriteStateCount++;
    }

    public void P_SetPlayerSprite(Player player, PlayerSpriteType position, StateNum stnum)
    {
        P_QueuePlayerSpriteState(player, position, stnum);
        P_DrainPlayerSpriteStates();
    }

    private void P_DrainPlayerSpriteStates()
    {
        while (_playerSpriteStateRead < _playerSpriteStateCount)
        {
            var queuedPlayer = _playerSpriteStatePlayers[_playerSpriteStateRead]!;
            var queuedPosition = _playerSpriteStatePositions[_playerSpriteStateRead];
            var queuedState = _playerSpriteStateNumbers[_playerSpriteStateRead];
            _playerSpriteStatePlayers[_playerSpriteStateRead] = null;
            _playerSpriteStateRead++;
            P_SetPlayerSpriteImmediate(queuedPlayer, queuedPosition, queuedState);
        }
        _playerSpriteStateRead = 0;
        _playerSpriteStateCount = 0;
    }

    private void P_SetPlayerSpriteImmediate(Player player, PlayerSpriteType position, StateNum stnum)
    {
        ...
        do
        {
            ...
            {
                var actionTransition = State.ExecutePlayerSpriteAction(state.Action,
                    new ActionParams(Player: player, PlayerSprite: psp));
                if (psp.State == null)
                {
                    break;
                }
    ...
    public void P_SetPlayerFlashSprite(Player player, StateNum stnum)
    {
        var psp = player.PlayerSprites[(int)PlayerSpriteType.Flash];
        ...
        do
        {
            ...
            if (state.Action != StateAction.None)
            {
                State.ExecutePlayerFlashAction(state.Action,
                    new ActionParams(Player: player, PlayerSprite: psp));
                if (psp.State == null)
                {
                    break;
    ...
    internal StateNum P_CheckAmmoTransition(Player player)
    {
        ...
        return WeaponInfo.GetByType(player.ReadyWeapon).DownState;
    }

    public bool P_CheckAmmo(Player player)
    {
        var transition = P_CheckAmmoTransition(player);
        if (transition == StateNum.NUMSTATES)
        {
            return true;
        }

        P_SetPlayerSprite(player, PlayerSpriteType.Weapon, transition);
        return false;
    }

    internal StateNum P_FireWeaponTransition(Player player)
    {
        var transition = P_CheckAmmoTransition(player);
        if (transition != StateNum.NUMSTATES)
        {
        ...
        player.MapObject!.SetStateWithoutAction(StateNum.S_PLAY_ATK1);
        return WeaponInfo.GetByType(player.ReadyWeapon).AttackState;
    }

    public void P_FireWeapon(Player player)
    {
        P_SetPlayerSprite(player, PlayerSpriteType.Weapon, P_FireWeaponTransition(player));
    }
    ...
    private void P_SpawnMapThing(MapThing mapThing)
    {
        ...
        {
            DoomGame.Error(string.Concat(
                string.Concat("P_SpawnMapThing: Unknown type ", DoomString.DecimalInt(mapThing.Type), " at ("),
                string.Concat(DoomString.DecimalInt(mapThing.X),
                    string.Concat(", ", DoomString.DecimalInt(mapThing.Y), ")"))));
            return;
        }

        // don't spawn keycards and players in deathmatch
        if (DeathMatch != 0 && (moInfo.Flags & MapObjectFlag.MF_NOTDMATCH) != 0)
        {
            return;
        }
        ...
        if (NoMonsters
            && (moInfo.Type == MapObjectType.MT_SKULL
                || (moInfo.Flags & MapObjectFlag.MF_COUNTKILL) != 0))
        {
            return;
        }
        ...
        var mapObject = P_SpawnMapObject(x, y, z, moInfo.Type);
    ...
    public void P_UnsetThingPosition(MapObject thing)
    {
        ...
        if ((thing.Flags & MapObjectFlag.MF_NOBLOCKMAP) == 0)
        {
            ...
            {
                var blockx = Fixed.RawValue(thing.X - _blockMapOriginX) >> Constants.MapBlockShift;
                var blocky = Fixed.RawValue(thing.Y - _blockMapOriginY) >> Constants.MapBlockShift;
    ...
    {
        var deathState = BeginExplodeMissile(mo);
        mo.SetState(deathState);
        CompleteExplodeMissile(mo);
    }

    private static StateNum BeginExplodeMissile(MapObject mo)
    {
        mo.MomX = mo.MomY = mo.MomZ = Fixed.Zero;
        return MapObjectInfo.GetByType(mo.Type).DeathState;
    }

    internal void CompleteExplodeMissile(MapObject mo)
    {
    ...
    private bool ExecuteLineTraversal(TraversalAction action, Line line)
    {
        return action switch
        {
            TraversalAction.CheckLine => PIT_CheckLine(line),
            TraversalAction.AddLineIntercepts => PIT_AddLineIntercepts(line),
            _ => true
        };
    }

    private bool ExecuteThingTraversal(TraversalAction action, MapObject thing)
    {
        return action switch
        {
            TraversalAction.CheckThing => PIT_CheckThing(thing),
            TraversalAction.RadiusAttack => PIT_RadiusAttack(thing),
            TraversalAction.ChangeSector => PIT_ChangeSector(thing),
            TraversalAction.AddThingIntercepts => PIT_AddThingIntercepts(thing),
            TraversalAction.StompThing => PIT_StompThing(thing),
            _ => true
        };
    }

    private bool ExecuteInterceptTraversal(TraversalAction action, Intercept intercept)
    {
        return action switch
        {
            TraversalAction.Use => PTR_UseTraverse(intercept),
            TraversalAction.Slide => PTR_SlideTraverse(intercept),
            TraversalAction.Aim => PTR_AimTraverse(intercept),
            TraversalAction.Shoot => PTR_ShootTraverse(intercept),
            _ => true
        };
    }
    ...
    /// to it.
    /// </summary>
    private bool P_BlockLinesIterator(int x, int y, TraversalAction lineAction)
    {
        if (x < 0
            || y < 0
            || x >= _blockMapWidth
        ...
        for (var i = offset; _blockMapLump[i] != -1; i++)
        {
            ...
            if (!ExecuteLineTraversal(lineAction, line))
            {
                return false;
            }
        }
    ...
    private MapObject? P_FirstBlockThing(int x, int y)
    {
        if (x < 0
            || y < 0
            || x >= _blockMapWidth
        ...
        return _blockLinks[y * _blockMapWidth + x];
    }

    private bool P_BlockCheckThingsIterator(int x, int y)
    {
        var thing = P_FirstBlockThing(x, y);
        for (; thing != null; thing = thing.BlockNext)
        {
            if (!PIT_CheckThing(thing))
                return false;
        }
        return true;
    }

    private bool P_BlockPassiveCheckThingsIterator(int x, int y)
    {
        var candidate = P_FirstBlockThing(x, y);
        for (; candidate != null; candidate = candidate.BlockNext)
        {
            if ((candidate.Flags & (MapObjectFlag.MF_SOLID | MapObjectFlag.MF_SPECIAL | MapObjectFlag.MF_SHOOTABLE)) == 0)
                continue;

            var blockDist = candidate.Radius + _tmThing!.Radius;
            if (Fixed.Abs(candidate.X - _tmX) >= blockDist ||
                Fixed.Abs(candidate.Y - _tmY) >= blockDist ||
                candidate == _tmThing)
                continue;

            // This iterator is valid only for a non-missile, non-skull-fly,
            // non-pickup mover. Such a mover has no collision side effects.
            if ((candidate.Flags & MapObjectFlag.MF_SOLID) != 0)
                return false;
        }
        return true;
    }

    private void P_BlockRadiusAttackThingsIterator(int x, int y)
    {
        var thing = P_FirstBlockThing(x, y);
        for (; thing != null; thing = thing.BlockNext)
            PIT_RadiusAttack(thing);
    }

    private void P_BlockChangeSectorThingsIterator(int x, int y)
    {
        var thing = P_FirstBlockThing(x, y);
        for (; thing != null; thing = thing.BlockNext)
    ...
    private bool P_BlockStompThingsIterator(int x, int y)
    {
        var thing = P_FirstBlockThing(x, y);
        for (; thing != null; thing = thing.BlockNext)
        {
            if (!PIT_StompThing(thing))
                return false;
        }
        return true;
    }

    private bool P_BlockAddInterceptThingsIterator(int x, int y)
    {
        var thing = P_FirstBlockThing(x, y);
        for (; thing != null; thing = thing.BlockNext)
        {
            if (!PIT_AddThingIntercepts(thing))
                return false;
        }
        return true;
    }

    public bool P_BlockVileThingsIterator(int x, int y)
    {
        if (x < 0
            || y < 0
            || x >= _blockMapWidth
    ...
    ///  numspeciallines
    /// </summary>
    public bool P_CheckPosition(MapObject thing, Fixed x, Fixed y)
    {
        _tmThing = thing;
        ...
        _tmBoundingBox[BoundingBox.BoxTop] = Fixed.RawValue(y + _tmThing.Radius);
        _tmBoundingBox[BoundingBox.BoxBottom] = Fixed.RawValue(y - _tmThing.Radius);
        _tmBoundingBox[BoundingBox.BoxRight] = Fixed.RawValue(x + _tmThing.Radius);
        _tmBoundingBox[BoundingBox.BoxLeft] = Fixed.RawValue(x - _tmThing.Radius);
        ...
        var xl = unchecked(_tmBoundingBox[BoundingBox.BoxLeft] - Fixed.RawValue(_blockMapOriginX) - Fixed.RawValue(Constants.MaxRadius)) >> Constants.MapBlockShift;
        var xh = unchecked(_tmBoundingBox[BoundingBox.BoxRight] - Fixed.RawValue(_blockMapOriginX) + Fixed.RawValue(Constants.MaxRadius)) >> Constants.MapBlockShift;
        var yl = unchecked(_tmBoundingBox[BoundingBox.BoxBottom] - Fixed.RawValue(_blockMapOriginY) - Fixed.RawValue(Constants.MaxRadius)) >> Constants.MapBlockShift;
        var yh = unchecked(_tmBoundingBox[BoundingBox.BoxTop] - Fixed.RawValue(_blockMapOriginY) + Fixed.RawValue(Constants.MaxRadius)) >> Constants.MapBlockShift;

        for (var bx = xl; bx <= xh; bx++)
        {
            for (var by = yl; by <= yh; by++)
            {
                if (!P_BlockCheckThingsIterator(bx, by))
                {
                    return false;
                }
            }
        ...
        // check lines
        xl = unchecked(_tmBoundingBox[BoundingBox.BoxLeft] - Fixed.RawValue(_blockMapOriginX)) >> Constants.MapBlockShift;
        xh = unchecked(_tmBoundingBox[BoundingBox.BoxRight] - Fixed.RawValue(_blockMapOriginX)) >> Constants.MapBlockShift;
        yl = unchecked(_tmBoundingBox[BoundingBox.BoxBottom] - Fixed.RawValue(_blockMapOriginY)) >> Constants.MapBlockShift;
        yh = unchecked(_tmBoundingBox[BoundingBox.BoxTop] - Fixed.RawValue(_blockMapOriginY)) >> Constants.MapBlockShift;

        for (var bx = xl; bx <= xh; bx++)
        {
            for (var by = yl; by <= yh; by++)
            {
                if (!P_BlockLinesIterator(bx, by, TraversalAction.CheckLine))
                {
                    return false;
                }
            }
    ...
    /// </summary>
    public bool P_TryMove(MapObject thing, Fixed x, Fixed y)
    {
        _floatOk = false;
    ...
    {
        var damage = P_RadiusAttackDamage(thing);
        if (damage != 0)
            MapObject.DamageMapObject(thing, _bombSpot, _bombSource, damage);
        return true;
    }

    internal bool P_CheckPositionPassive(MapObject thing, Fixed x, Fixed y)
    {
        _tmThing = thing;
        _tmFlags = thing.Flags;
        _tmX = x;
        _tmY = y;
        _tmBoundingBox[BoundingBox.BoxTop] = Fixed.RawValue(y + thing.Radius);
        _tmBoundingBox[BoundingBox.BoxBottom] = Fixed.RawValue(y - thing.Radius);
        _tmBoundingBox[BoundingBox.BoxRight] = Fixed.RawValue(x + thing.Radius);
        _tmBoundingBox[BoundingBox.BoxLeft] = Fixed.RawValue(x - thing.Radius);
        ...
        for (var bx = xl; bx <= xh; bx++)
        {
            for (var by = yl; by <= yh; by++)
            {
                if (!P_BlockPassiveCheckThingsIterator(bx, by))
                    return false;
            }
        }
    ...
    internal bool P_TryMovePassive(MapObject thing, Fixed x, Fixed y)
    {
        _floatOk = false;
        if (!P_CheckPositionPassive(thing, x, y))
            return false;
        ...
        if ((thing.Flags & (MapObjectFlag.MF_TELEPORT | MapObjectFlag.MF_NOCLIP)) == 0)
        {
            while (_numSpecHit-- != 0)
            {
                ...
                var side = P_PointOnLineSide(thing.X, thing.Y, line);
                var oldSide = P_PointOnLineSide(oldX, oldY, line);
                if (side != oldSide && line.Special != 0)
                    P_QueueCrossedSpecialLine(line, oldSide, thing);
            }
        }
        return true;
    }

    private Line?[] _crossedSpecialLines = new Line?[8];
    private int[] _crossedSpecialSides = new int[8];
    private MapObject?[] _crossedSpecialThings = new MapObject?[8];
    private int _crossedSpecialRead;
    private int _crossedSpecialCount;

    private void P_QueueCrossedSpecialLine(Line line, int side, MapObject thing)
    {
        if (_crossedSpecialCount == _crossedSpecialLines.Length)
        {
            if (_crossedSpecialLines.Length > int.MaxValue / 2)
                DoomGame.Error("Crossed-special queue capacity overflow");
            var newLength = _crossedSpecialLines.Length * 2;
            var lines = new Line?[newLength];
            var sides = new int[newLength];
            var things = new MapObject?[newLength];
            Array.Copy(_crossedSpecialLines, lines, _crossedSpecialCount);
            Array.Copy(_crossedSpecialSides, sides, _crossedSpecialCount);
            Array.Copy(_crossedSpecialThings, things, _crossedSpecialCount);
            _crossedSpecialLines = lines;
            _crossedSpecialSides = sides;
            _crossedSpecialThings = things;
        }

        _crossedSpecialLines[_crossedSpecialCount] = line;
        _crossedSpecialSides[_crossedSpecialCount] = side;
        _crossedSpecialThings[_crossedSpecialCount] = thing;
        _crossedSpecialCount++;
    }

    private void P_DrainCrossedSpecialLines()
    {
        while (_crossedSpecialRead < _crossedSpecialCount)
        {
            var line = _crossedSpecialLines[_crossedSpecialRead]!;
            var side = _crossedSpecialSides[_crossedSpecialRead];
            var thing = _crossedSpecialThings[_crossedSpecialRead]!;
            _crossedSpecialLines[_crossedSpecialRead] = null;
            _crossedSpecialThings[_crossedSpecialRead] = null;
            _crossedSpecialRead++;

            // A preceding queued one-shot activation may already have consumed
            // this line before the later crossing reaches the drain point.
            if (line.Special != 0)
                P_CrossSpecialLine(line, side, thing);
        }

        _crossedSpecialRead = 0;
        _crossedSpecialCount = 0;
    }

    /// <summary>
    /// Revalidates a freshly spawned object at its current coordinates. The
    /// unlink/relink and floor/ceiling updates match P_TryMovePassive, while the
    /// current-position invariant means that no line can have been crossed.
    /// Keeping that invariant explicit also avoids exposing the full trigger
    /// dispatcher to state actions which only perform a spawn fit check.
    /// </summary>
    internal bool P_TryMoveAtCurrentPositionPassive(MapObject thing)
    {
        var x = thing.X;
        var y = thing.Y;
        _floatOk = false;
    ...
    private int P_RadiusAttackDamage(MapObject thing)
    {
        if ((thing.Flags & MapObjectFlag.MF_SHOOTABLE) == 0)
        {
            return 0;
        ...
        var distance = Fixed.RawValue(dist);
        if (distance >= _bombDamage)
        {
            return 0; // out of range
        }

        if (P_CheckSight(thing, _bombSpot))
            return _bombDamage - distance;
        return 0;
    }
    ...
    {
        var dist = Fixed.FromInt(damage + Fixed.RawValue(Constants.MaxRadius));
        var yh = Fixed.RawValue(spot.Y + dist - _blockMapOriginY) >> Constants.MapBlockShift;
        var yl = Fixed.RawValue(spot.Y - dist - _blockMapOriginY) >> Constants.MapBlockShift;
        var xh = Fixed.RawValue(spot.X + dist - _blockMapOriginX) >> Constants.MapBlockShift;
        var xl = Fixed.RawValue(spot.X - dist - _blockMapOriginX) >> Constants.MapBlockShift;
        _bombSpot = spot;
        _bombSource = source;
        _bombDamage = damage;
        ...
        for (var y = yl; y <= yh; y++)
        {
            for (var x = xl; x <= xh; x++)
            {
                P_BlockRadiusAttackThingsIterator(x, y);
            }
        }
    }
    ...
    private bool PIT_ChangeSector(MapObject thing)
    {
        ...
        if (thing.Health <= 0)
        {
            thing.SetStateWithoutAction(StateNum.S_GIBS);
    ...
    public bool P_ChangeSector(Sector sector, bool crunch)
    {
        ...
        for (var x = sector.BlockBox[BoundingBox.BoxLeft]; x <= sector.BlockBox[BoundingBox.BoxRight]; x++)
        {
            ...
            {
                P_BlockChangeSectorThingsIterator(x, y);
            }
        }
    ...
    private const int MaxIntercepts = 1024;
    ...
    private bool PIT_AddThingIntercepts(MapObject thing)
    {
        ...
        var tracePositive = (Fixed.RawValue(_trace.Dx) ^ Fixed.RawValue(_trace.Dy)) > 0;
    ...
    /// for all lines.
    /// </summary>
    private bool P_TraverseIntercepts(TraversalAction traverseAction, Fixed maxFrac)
    {
        ...
        while (count-- != 0)
        {
            ...
            if (!ExecuteInterceptTraversal(traverseAction, intercept!))
            {
    ...
    /// for all lines.
    /// </summary>
    private bool P_PathTraverseCollect(Fixed x1, Fixed y1, Fixed x2, Fixed y2, int flags)
    {
        Fixed xStep;
        Fixed yStep;
        ...
        var mapBlockMask = Fixed.RawValue(Constants.MapBlockSize) - 1;
        if ((Fixed.RawValue(x1 - _blockMapOriginX) & mapBlockMask) == 0)
        {
            x1 = x1 + Fixed.Unit; // don't side exactly on a line
        }

        if ((Fixed.RawValue(y1 - _blockMapOriginY) & mapBlockMask) == 0)
        {
            y1 = y1 + Fixed.Unit; // don't side exactly on a line
        }
        ...
        x1 = x1 - _blockMapOriginX;
        y1 = y1 - _blockMapOriginY;
        var xt1 = Fixed.RawValue(x1) >> Constants.MapBlockShift;
        var yt1 = Fixed.RawValue(y1) >> Constants.MapBlockShift;

        x2 = x2 - _blockMapOriginX;
        y2 = y2 - _blockMapOriginY;
        var xt2 = Fixed.RawValue(x2) >> Constants.MapBlockShift;
        var yt2 = Fixed.RawValue(y2) >> Constants.MapBlockShift;

        if (xt2 > xt1)
        {
            mapXStep = 1;
            partial = Fixed.FromRaw(Constants.FracUnit - ((Fixed.RawValue(x1) >> Constants.MapBlockToFrac) & (Constants.FracUnit - 1)));
            yStep = (y2 - y1) / Fixed.Abs(x2 - x1);
        }
        else if (xt2 < xt1)
        {
            mapXStep = -1;
            partial = Fixed.FromRaw((Fixed.RawValue(x1) >> Constants.MapBlockToFrac) & (Constants.FracUnit - 1));
            yStep = (y2 - y1) / Fixed.Abs(x2 - x1);
        }
        else
        {
        ...
        {
            mapYStep = 1;
            partial = Fixed.FromRaw(Constants.FracUnit - ((Fixed.RawValue(y1) >> Constants.MapBlockToFrac) & (Constants.FracUnit - 1)));
            xStep = (x2 - x1) / Fixed.Abs(y2 - y1);
        }
        else if (yt2 < yt1)
        {
            mapYStep = -1;
            partial = Fixed.FromRaw((Fixed.RawValue(y1) >> Constants.MapBlockToFrac) & (Constants.FracUnit - 1));
            xStep = (x2 - x1) / Fixed.Abs(y2 - y1);
        }
        else
        {
        ...
        for (count = 0; count < 64; count++)
        {
            ...
            {
                if (!P_BlockLinesIterator(mapX, mapY, TraversalAction.AddLineIntercepts))
                {
            ...
            {
                if (!P_BlockAddInterceptThingsIterator(mapX, mapY))
                {
                    return false;   // early out
                }
            }
            ...
            if ((Fixed.RawValue(yIntercept) >> Constants.FracBits) == mapY)
            {
                yIntercept = yIntercept + yStep;
                mapX += mapXStep;
            }
            else if ((Fixed.RawValue(xIntercept) >> Constants.FracBits) == mapX)
            {
    ...
    public bool P_PathTraverse(Fixed x1, Fixed y1, Fixed x2, Fixed y2, int flags, TraversalAction traverseAction)
    {
        return P_PathTraverseCollect(x1, y1, x2, y2, flags) &&
               P_TraverseIntercepts(traverseAction, Fixed.Unit);
    }

    private bool P_PathTraverseAim(Fixed x1, Fixed y1, Fixed x2, Fixed y2, int flags)
    {
        if (!P_PathTraverseCollect(x1, y1, x2, y2, flags))
            return false;
    ...
    private bool P_PathTraverseShoot(Fixed x1, Fixed y1, Fixed x2, Fixed y2, int flags)
    {
        if (!P_PathTraverseCollect(x1, y1, x2, y2, flags))
            return false;
        ...
        while (count-- != 0)
        {
            ...
            if (dist > Fixed.Unit)
                return true;
            if (!PTR_ShootTraverse(intercept!))
                return false;
            intercept!.Frac = Fixed.MaxValue;
        }
        return true;
    ...
    private bool P_PathTraverseShootForAction(
        ActionParams actionParams,
        Fixed x2,
        Fixed y2,
        MapObjectActionContinuation continuation,
        HitscanAttackWork? continuationWork)
    {
        if (!P_PathTraverseCollect(_shootThing!.X, _shootThing.Y, x2, y2, PT_AddLines | PT_AddThings))
            return false;
        ...
        while (count-- != 0)
        {
            ...
            if (dist > Fixed.Unit)
                return true;
            if (!PTR_ShootTraverseForAction(actionParams, intercept!, continuation, continuationWork))
                return false;
            intercept!.Frac = Fixed.MaxValue;
        }
        return true;
    ...
    private bool PTR_SlideTraverse(Intercept intercept)
    {
        ...
        if ((line.Flags & Constants.Line.TwoSided) == 0)
        {
            ...
            SetBlockingSlideLine(intercept, line);
            return false;
        }
        ...
        {
            SetBlockingSlideLine(intercept, line);        // doesn't fit
            return false;
        }

        if (_openTop - _slideMapObject.Z < _slideMapObject.Height)
        {
            SetBlockingSlideLine(intercept, line);        // mobj is too high
            return false;
        }

        if (_openBottom - _slideMapObject.Z > Fixed.FromInt(24))
        {
            SetBlockingSlideLine(intercept, line);        // too big a step up
            return false;
        }
    ...
    private void SetBlockingSlideLine(Intercept intercept, Line line)
    {
        // The line blocks movement; keep the closest two blocking lines.
    ...
    retry:
        ...
        P_PathTraverse(leadx, leady, leadx + mo.MomX, leady + mo.MomY,
            PT_AddLines, TraversalAction.Slide);
    ...
    private void P_XYMovement(MapObject mo)
    {
            ...
            && (player == null || (player.Command.ForwardMove == 0 && player.Command.SideMove == 0)))
        {
            ...
            if (player != null && (uint)(State.GetStateNum(player.MapObject!.State!) - StateNum.S_PLAY_RUN1) < 4)
            {
    ...
    private int P_PointOnLineSide(Fixed x, Fixed y, Line line)
    {
        ...
        var left = Fixed.FromRaw(Fixed.RawValue(line.Dy) >> Constants.FracBits) * dx;
        var right = dy * Fixed.FromRaw(Fixed.RawValue(line.Dx) >> Constants.FracBits);
    ...
    /// </summary>
    private int P_BoxOnLineSide(int[] boundingBox, Line line)
    {
        int p1 = -1, p2 = -1;
        ...
        switch (line.SlopeType)
        {
            case SlopeType.Horizontal:
                p1 = boundingBox[BoundingBox.BoxTop] > Fixed.RawValue(line.V1.Y) ? 1 : 0;
                p2 = boundingBox[BoundingBox.BoxBottom] > Fixed.RawValue(line.V1.Y) ? 1 : 0;
                if (line.Dx < Fixed.Zero)
                {
                    p1 ^= 1;
                    p2 ^= 1;
            ...
            case SlopeType.Vertical:
                p1 = boundingBox[BoundingBox.BoxRight] < Fixed.RawValue(line.V1.X) ? 1 : 0;
                p2 = boundingBox[BoundingBox.BoxLeft] < Fixed.RawValue(line.V1.X) ? 1 : 0;
                if (line.Dy < Fixed.Zero)
                {
                    p1 ^= 1;
                    p2 ^= 1;
            ...
            case SlopeType.Positive:
                p1 = P_PointOnLineSide(Fixed.FromRaw(boundingBox[BoundingBox.BoxLeft]), Fixed.FromRaw(boundingBox[BoundingBox.BoxTop]), line);
                p2 = P_PointOnLineSide(Fixed.FromRaw(boundingBox[BoundingBox.BoxRight]), Fixed.FromRaw(boundingBox[BoundingBox.BoxBottom]), line);
                break;

            case SlopeType.Negative:
                p1 = P_PointOnLineSide(Fixed.FromRaw(boundingBox[BoundingBox.BoxRight]), Fixed.FromRaw(boundingBox[BoundingBox.BoxTop]), line);
                p2 = P_PointOnLineSide(Fixed.FromRaw(boundingBox[BoundingBox.BoxLeft]), Fixed.FromRaw(boundingBox[BoundingBox.BoxBottom]), line);
                break;
        }
    ...
    private int P_PointOnDivlineSide(Fixed x, Fixed y, DividerLine line)
    {
        ...
        var lineDy = Fixed.RawValue(line.Dy);
        var lineDx = Fixed.RawValue(line.Dx);
        var dxRaw = Fixed.RawValue(dx);
        var dyRaw = Fixed.RawValue(dy);
        if (((lineDy ^ lineDx ^ dxRaw ^ dyRaw) & unchecked((int)0x80000000)) != 0)
        {
            if (((lineDy ^ dxRaw) & unchecked((int)0x80000000)) != 0)
            {
        ...
        var left = Fixed.FromRaw(lineDy >> 8) * Fixed.FromRaw(dxRaw >> 8);
        var right = Fixed.FromRaw(dyRaw >> 8) * Fixed.FromRaw(lineDx >> 8);
    ...
    public bool P_TeleportMove(MapObject thing, Fixed x, Fixed y)
    {
        ...
        for (var bx = xl; bx <= xh; bx++)
        {
            for (var by = yl; by <= yh; by++)
            {
                if (!P_BlockStompThingsIterator(bx, by))
                {
                    return false;
                }
            }
    ...
    internal void P_TeleportMoveForAction(
        ActionParams actionParams,
        MapObject thing,
        Fixed x,
        Fixed y,
        MapObject? cube)
    {
        ...
        var work = new TeleportMoveWork
        {
            ...
            YPosition = y,
            FloorZ = _tmFloorZ,
            CeilingZ = _tmCeilingZ,
            DropOffZ = _tmDropOffZ,
            XLow = unchecked(_tmBoundingBox[BoundingBox.BoxLeft] - Fixed.RawValue(_blockMapOriginX) - Fixed.RawValue(Constants.MaxRadius)) >> Constants.MapBlockShift,
            XHigh = unchecked(_tmBoundingBox[BoundingBox.BoxRight] - Fixed.RawValue(_blockMapOriginX) + Fixed.RawValue(Constants.MaxRadius)) >> Constants.MapBlockShift,
            YLow = unchecked(_tmBoundingBox[BoundingBox.BoxBottom] - Fixed.RawValue(_blockMapOriginY) - Fixed.RawValue(Constants.MaxRadius)) >> Constants.MapBlockShift,
            YHigh = unchecked(_tmBoundingBox[BoundingBox.BoxTop] - Fixed.RawValue(_blockMapOriginY) + Fixed.RawValue(Constants.MaxRadius)) >> Constants.MapBlockShift
        };
        work.X = work.XLow;
        work.Y = work.YLow;
        ContinueTeleportMoveForAction(actionParams, work, false);
    }

    internal void ContinueTeleportMoveForAction(
        ActionParams actionParams,
        TeleportMoveWork work,
        bool advanceCurrentThing)
    {
        RestoreTeleportMoveContext(work);
        if (advanceCurrentThing)
        {
            work.CurrentThing = work.NextThing;
            work.NextThing = null;
        }

        while (work.X <= work.XHigh)
        {
            while (work.Y <= work.YHigh)
            {
                if (!work.BlockLoaded)
                {
                    work.CurrentThing = P_FirstBlockThing(work.X, work.Y);
                    work.BlockLoaded = true;
                }

                while (work.CurrentThing is not null)
                {
                    var candidate = work.CurrentThing;
                    work.NextThing = candidate.BlockNext;
                    if ((candidate.Flags & MapObjectFlag.MF_SHOOTABLE) != 0)
                    {
                        var blockDist = candidate.Radius + work.Thing.Radius;
                        if (Fixed.Abs(candidate.X - work.XPosition) < blockDist &&
                            Fixed.Abs(candidate.Y - work.YPosition) < blockDist &&
                            candidate != work.Thing)
                        {
                            if (work.Thing.Player == null && GameMap != 30)
                            {
                                CompleteTeleportMoveForAction(work, false);
                                return;
                            }

                            var transition = MapObject.DamageMapObjectCore(
                                candidate,
                                work.Thing,
                                work.Thing,
                                10000);
                            ...
                            if (transitionState != StateNum.NUMSTATES)
                            {
                                actionParams.ScheduleStateTransition(
                                    ...
                                    continuationTeleportMoveWork: work);
                                return;
                            }
                        }
                    }
        ...
        CompleteTeleportMoveForAction(work, true);
    }

    private void RestoreTeleportMoveContext(TeleportMoveWork work)
    {
        _tmThing = work.Thing;
        _tmFlags = work.Thing.Flags;
        _tmX = work.XPosition;
        _tmY = work.YPosition;
        _tmBoundingBox[BoundingBox.BoxTop] = Fixed.RawValue(work.YPosition + work.Thing.Radius);
        _tmBoundingBox[BoundingBox.BoxBottom] = Fixed.RawValue(work.YPosition - work.Thing.Radius);
        _tmBoundingBox[BoundingBox.BoxRight] = Fixed.RawValue(work.XPosition + work.Thing.Radius);
        _tmBoundingBox[BoundingBox.BoxLeft] = Fixed.RawValue(work.XPosition - work.Thing.Radius);
        _tmFloorZ = work.FloorZ;
        _tmCeilingZ = work.CeilingZ;
        _tmDropOffZ = work.DropOffZ;
    }

    private void CompleteTeleportMoveForAction(TeleportMoveWork work, bool success)
    {
        if (success)
        {
            P_UnsetThingPosition(work.Thing);
            work.Thing.FloorZ = work.FloorZ;
            work.Thing.CeilingZ = work.CeilingZ;
            work.Thing.X = work.XPosition;
            work.Thing.Y = work.YPosition;
            P_SetThingPosition(work.Thing);
        }

        if (work.Cube is not null)
            P_RemoveMapObject(work.Cube);
    }
    ...
    public void P_MapObjectThinker(ActionParams actionParams)
    {
        ...
        if (mobj.MomX != Fixed.Zero || mobj.MomY != Fixed.Zero || (mobj.Flags & MapObjectFlag.MF_SKULLFLY) != 0)
        {
            ...
            if (mobj.Action == ThinkerAction.None)
            {
    ...
    private void P_NightmareRespawn(MapObject mobj)
    {
        var x = Fixed.FromRaw(mobj.Spawnpoint.X << Constants.FracBits);
        var y = Fixed.FromRaw(mobj.Spawnpoint.Y << Constants.FracBits);
        ...
        mo = P_SpawnMapObject(x, y, ss.Sector!.FloorHeight, MapObjectType.MT_TFOG);
        ...
        // spawn it
        Fixed z;
        if ((mobj.Info.Flags & MapObjectFlag.MF_SPAWNCEILING) != 0)
        {
            z = Constants.OnCeilingZ;
        }
        else
        ...
        mo = P_SpawnMapObject(x, y, z, mobj.Type);
        mo.Spawnpoint = mobj.Spawnpoint;
        mo.Angle = Angle.Angle45 * (uint)(mthing.Angle / 45);

        if ((mthing.Options & (int)MapThingFlag.MTF_AMBUSH) != 0)
        {
            mo.Flags |= MapObjectFlag.MF_AMBUSH;
        }
    ...
    public MapObject P_SpawnMapObject(Fixed x, Fixed y, Fixed z, MapObjectType type)
    {
        ...
        {
            mobj.Z = Fixed.FromRaw(Fixed.RawValue(mobj.CeilingZ) - mobj.Info.Height);
        }
        else
        {
            mobj.Z = z;
        ...
        mobj.Action = ThinkerAction.MapObjectThinker;
    ...
    public void P_RemoveMapObject(MapObject mobj)
    {
        ...
        {
            _itemRespawnQueue[_iqueueHead] = mobj.Spawnpoint;
            _itemRespawnTime[_iqueueHead] = LevelTime;
            _iqueueHead = (_iqueueHead + 1) & (ItemQueueSize - 1);
    ...
    private void P_LoadLineDefs(int lump)
    {
        ...
        for (var i = 0; i < _numLines; i++)
        {
            ...
            if (ld.V1.X < ld.V2.X)
            {
                ld.BoundingBox[BoundingBox.BoxLeft] = Fixed.RawValue(ld.V1.X);
                ld.BoundingBox[BoundingBox.BoxRight] = Fixed.RawValue(ld.V2.X);
            }
            else
            {
                ld.BoundingBox[BoundingBox.BoxLeft] = Fixed.RawValue(ld.V2.X);
                ld.BoundingBox[BoundingBox.BoxRight] = Fixed.RawValue(ld.V1.X);
            }

            if (ld.V1.Y < ld.V2.Y)
            {
                ld.BoundingBox[BoundingBox.BoxBottom] = Fixed.RawValue(ld.V1.Y);
                ld.BoundingBox[BoundingBox.BoxTop] = Fixed.RawValue(ld.V2.Y);
            }
            else
            {
                ld.BoundingBox[BoundingBox.BoxBottom] = Fixed.RawValue(ld.V2.Y);
                ld.BoundingBox[BoundingBox.BoxTop] = Fixed.RawValue(ld.V1.Y);
            }
    ...
    private void P_GroupLines()
    {
        ...
        var boundingBox = new int[4];
        var blockMapOriginX = Fixed.RawValue(_blockMapOriginX);
        var blockMapOriginY = Fixed.RawValue(_blockMapOriginY);
        var maxRadius = Fixed.RawValue(Constants.MaxRadius);
        for (var i = 0; i < _numSectors; i++)
        {
            BoundingBox.ClearRawBox(boundingBox);
            ...
            for (var j = 0; j < _numLines; j++)
            {
                ...
                if (line.FrontSector == sector || line.BackSector == sector)
                {
                    ...
                    BoundingBox.AddRawToBox(boundingBox, Fixed.RawValue(line.V1.X), Fixed.RawValue(line.V1.Y));
                    BoundingBox.AddRawToBox(boundingBox, Fixed.RawValue(line.V2.X), Fixed.RawValue(line.V2.Y));
                }
            }
            ...
            var block = (boundingBox[BoundingBox.BoxTop] - blockMapOriginY + maxRadius) >> Constants.MapBlockShift;
            block = block >= _blockMapHeight ? _blockMapHeight - 1 : block;
            sector.BlockBox[BoundingBox.BoxTop] = block;

            block = (boundingBox[BoundingBox.BoxBottom] - blockMapOriginY - maxRadius) >> Constants.MapBlockShift;
            block = block < 0 ? 0 : block;
            sector.BlockBox[BoundingBox.BoxBottom] = block;

            block = (boundingBox[BoundingBox.BoxRight] - blockMapOriginX + maxRadius) >> Constants.MapBlockShift;
            block = block >= _blockMapWidth ? _blockMapWidth - 1 : block;
            sector.BlockBox[BoundingBox.BoxRight] = block;

            block = (boundingBox[BoundingBox.BoxLeft] - blockMapOriginX - maxRadius) >> Constants.MapBlockShift;
            block = block < 0 ? 0 : block;
    ...
    private void SetupLevel(int episode, int map, int playerMask, SkillLevel skillLevel)
    {
        ...
        // find map name
        var lumpName = DoomString.MapLumpName(DoomGame.Instance.GameMode == GameMode.Commercial, episode, map);
    ...
    private void P_InitPicAnims()
    {
        ...
        for (var i = 0; i < AnimationDefinition.Definitions.Length; i++)
        {
            ...
            {
                DoomGame.Error(string.Concat(
                    string.Concat("P_InitPicAnims: bad cycle from ", animDef.StartName, " to "),
                    animDef.EndName,
                    ""));
                return;
            }
    ...
    public void P_ActivateInStasisCeiling(Line line)
    {
        for (var i = 0; i < Ceiling.MaxCeilings; i++)
        {
            ...
            ceiling.Action = ThinkerAction.MoveCeiling;
            return;
        }
    }
    ...
    public int CeilingCrushStopEvent(Line line)
    {
        ...
        for (var i = 0; i < Ceiling.MaxCeilings; i++)
        {
                ...
                ceiling.Direction != 0)
            {
                ...
                ceiling.Action = ThinkerAction.None;
    ...
    {
        for (var i = 0; i < _activePlats.Length; i++)
        {
            var platform = _activePlats[i];
            if (platform == null || platform.Status == PlatformState.InStatis || platform.Tag != line.Tag)
            {
                continue;
            }
            ...
            platform.Action = ThinkerAction.None;
        }
    }
    ...
    public void P_ActivateInStasis(int tag)
    {
        for (var i = 0; i < Platform.MaxPlats; i++)
        {
            ...
            platform.Action = ThinkerAction.PlatformRaise;
            return;
        }
    }
    ...
    public int SectorIndex(Sector target)
    {
        for (var i = 0; i < _sectors.Length; i++)
        {
            if (ReferenceEquals(_sectors[i], target))
                return i;
        }
        return -1;
    }
    ...
    {
        var heightList = new int[MaxAdjoiningSectors];
        var h = 0;
        ...
        for (var i = 0; i < sec.LineCount; i++)
        {
            ...
            {
                heightList[h++] = Fixed.RawValue(other.FloorHeight);
            }
        ...
        var lowest = heightList[0];
        for (var i = 1; i < h; i++)
        {
            if (heightList[i] < lowest)
                lowest = heightList[i];
        }
        return Fixed.FromRaw(lowest);
    }
    ...
    /// <summary>
    /// Handles the restricted set of line specials that a moving missile can
    /// reach without routing the AOT call graph through every Trigger.Handle
    /// case. TeleportEvent is a no-op for MF_MISSILE, but one-shot teleport
    /// lines still have to be consumed exactly as Trigger.Handle consumes them.
    /// </summary>
    private static void P_CrossMissileSpecialLine(Line line, MapObject missile)
    {
        switch (missile.Type)
        {
            case MapObjectType.MT_ROCKET:
        ...
        {
            case 4:
                Trigger.DoorEvent(line, DoorType.Normal);
                line.Special = 0;
                break;

            case 10:
                Trigger.PlatformEvent(line, PlatformType.DownWaitUpStay, 0);
                line.Special = 0;
                break;

            case 39:
            case 125:
                // Trigger.TeleportEvent returns immediately for MF_MISSILE.
                line.Special = 0;
                break;
            ...
            case 97:
            case 126:
                // Repeatable teleport specials are no-ops for missiles.
                break;
        }
    }
    ...
    /// <summary>
    /// F_StartFinale - minimal implementation for demo playback.
    /// Sets the game into finale state so that P_Ticker stops
    /// and the demo can end cleanly.
    /// </summary>
    ...
    private bool PTR_ShootTraverse(Intercept intercept)
    {
        ...
        if (intercept.IsLine)
        {
            ...
            if ((li.Flags & Constants.Line.TwoSided) == 0)
            {
                return HitShootLine(intercept, li);
            }
    ...
    private bool PTR_ShootTraverseForAction(
        ActionParams actionParams,
        Intercept intercept,
        MapObjectActionContinuation continuation,
        HitscanAttackWork? continuationWork)
    {
        ...
        if (intercept.IsLine)
        {
            var line = intercept.Line!;
            if (line.Special != 0)
                P_ShootSpecialLine(_shootThing!, line);
            if ((line.Flags & Constants.Line.TwoSided) == 0)
                return HitShootLine(intercept, line);
        ...
        var target = intercept.Thing!;
        if (target == _shootThing || (target.Flags & MapObjectFlag.MF_SHOOTABLE) == 0)
            return true;

        dist = _attackRange * intercept.Frac;
        var thingTopSlope = (target.Z + target.Height - _shootZ) / dist;
        if (thingTopSlope < _aimSlope)
            return true;
        var thingBottomSlope = (target.Z - _shootZ) / dist;
        if (thingBottomSlope > _aimSlope)
            return true;
        ...
        var y = _trace.Y + _trace.Dy * frac;
        var z = _shootZ + _aimSlope * (frac * _attackRange);
        if ((target.Flags & MapObjectFlag.MF_NOBLOOD) != 0)
            P_SpawnPuff(x, y, z);
        else
            P_SpawnBlood(x, y, z, _laDamage);
        ...
        if (_laDamage != 0)
        {
            var transition = MapObject.DamageMapObjectCore(target, _shootThing, _shootThing, _laDamage);
            var transitionState = MapObject.DamageTransitionState(transition);
            if (transitionState != StateNum.NUMSTATES)
            {
                actionParams.ScheduleStateTransition(
                    ...
                    MapObject.DamageTransitionCompletion(transition),
                    continuation,
                    continuationHitscanAttackWork: continuationWork);
            }
        }
        return false;
    }

    private bool HitShootLine(Intercept intercept, Line li)
    {
        // position a bit closer
    ...
    public Fixed P_AimLineAttack(MapObject thing, Angle angle, Fixed distance)
    {
        ...
        var distanceRaw = Fixed.RawValue(distance);
        var x2 = thing.X + (distanceRaw >> Constants.FracBits) * DoomMath.Cos(angle);
        var y2 = thing.Y + (distanceRaw >> Constants.FracBits) * DoomMath.Sin(angle);
        _shootZ = Fixed.FromRaw(Fixed.RawValue(thing.Z) + (Fixed.RawValue(thing.Height) >> 1) + 8 * Constants.FracUnit);
        ...
        P_PathTraverseAim(thing.X, thing.Y, x2, y2, PT_AddLines | PT_AddThings);
    ...
    public void P_LineAttack(MapObject thing, Angle angle, Fixed distance, Fixed slope, int damage)
    {
        ...
        P_PathTraverseShoot(thing.X, thing.Y, x2, y2, PT_AddLines | PT_AddThings);
    }

    internal void P_LineAttackForAction(
        ActionParams actionParams,
        MapObject thing,
        Angle angle,
        Fixed distance,
        ...
        int damage)
    {
        P_LineAttackForActionCore(actionParams, thing, angle, distance, slope, damage, null);
    }
    ...
    internal void P_LineAttackForAction(
        ...
        HitscanAttackWork continuationWork)
    {
        P_LineAttackForActionCore(actionParams, thing, angle, distance, slope, damage, continuationWork);
    }

    private void P_LineAttackForActionCore(
        ActionParams actionParams,
        MapObject thing,
        Angle angle,
        Fixed distance,
        ...
        HitscanAttackWork? continuationWork)
    {
        ...
        _attackRange = distance;
        _aimSlope = slope;
        P_PathTraverseShootForAction(
            actionParams,
            x2,
            y2,
            continuationWork is null ? MapObjectActionContinuation.None : MapObjectActionContinuation.SPosAttack,
            continuationWork);
    }
    ...
    public void P_SpawnPuff(Fixed x, Fixed y, Fixed z)
    {
        ...
        {
            th.SetStateWithoutAction(StateNum.S_PUFF3);
        }
    }
    ...
    private void P_SpawnBlood(Fixed x, Fixed y, Fixed z, int damage)
    {
        ...
        {
            th.SetStateWithoutAction(StateNum.S_BLOOD2);
        }
        else if (damage < 9)
        {
            th.SetStateWithoutAction(StateNum.S_BLOOD3);
        }
    }
    ...
    {
        if (!P_CheckMissileSpawnPlacement(th))
        {
            P_ExplodeMissile(th);
        }
    }

    internal void P_RadiusAttackForAction(
        ActionParams actionParams,
        MapObject spot,
        MapObject source,
        int damage)
    {
        var dist = Fixed.FromInt(damage + Fixed.RawValue(Constants.MaxRadius));
        var work = new RadiusAttackWork
        {
            Spot = spot,
            Source = source,
            Damage = damage,
            YHigh = Fixed.RawValue(spot.Y + dist - _blockMapOriginY) >> Constants.MapBlockShift,
            YLow = Fixed.RawValue(spot.Y - dist - _blockMapOriginY) >> Constants.MapBlockShift,
            XHigh = Fixed.RawValue(spot.X + dist - _blockMapOriginX) >> Constants.MapBlockShift,
            XLow = Fixed.RawValue(spot.X - dist - _blockMapOriginX) >> Constants.MapBlockShift
        };
        work.X = work.XLow;
        work.Y = work.YLow;
        ContinueRadiusAttackForAction(actionParams, work, false);
    }

    internal void ContinueRadiusAttackForAction(
        ActionParams actionParams,
        RadiusAttackWork work,
        bool advanceCurrentThing)
    {
        _bombSpot = work.Spot;
        _bombSource = work.Source;
        _bombDamage = work.Damage;
        if (advanceCurrentThing)
        {
            work.CurrentThing = work.NextThing;
            work.NextThing = null;
        ...
        while (work.Y <= work.YHigh)
        {
            while (work.X <= work.XHigh)
            {
                ...
                while (work.CurrentThing is not null)
                {
                    var thing = work.CurrentThing;
                    work.NextThing = thing.BlockNext;
                    var damage = P_RadiusAttackDamage(thing);
                    if (damage != 0)
                    {
                        var transition = MapObject.DamageMapObjectCore(
                        ...
                        if (transitionState != StateNum.NUMSTATES)
                        {
                            actionParams.ScheduleStateTransition(
                                ...
                                transitionState,
                                MapObject.DamageTransitionCompletion(transition),
                                MapObjectActionContinuation.RadiusAttack,
                                continuationRadiusWork: work);
                            return;
                        }
                    }
                    work.CurrentThing = work.NextThing;
    ...
    private bool P_CheckMissileSpawnPlacement(MapObject th)
    {
        th.Tics -= DoomRandom.P_Random() & 3;
        if (th.Tics < 1)
        {
        ...
        return P_TryMove(th, th.X, th.Y);
    }

    private void P_CheckMissileSpawnPlacementForAction(
        ActionParams actionParams,
        MapObject missile,
        MapObjectActionContinuation completion,
        int completionIndex,
        MapObject? completionMapObject2)
    {
        missile.Tics -= DoomRandom.P_Random() & 3;
        if (missile.Tics < 1)
            missile.Tics = 1;
        ...
        _floatOk = false;
        _tmThing = missile;
        _tmFlags = missile.Flags;
        _tmX = missile.X;
        _tmY = missile.Y;
        _tmBoundingBox[BoundingBox.BoxTop] = Fixed.RawValue(_tmY + missile.Radius);
        _tmBoundingBox[BoundingBox.BoxBottom] = Fixed.RawValue(_tmY - missile.Radius);
        _tmBoundingBox[BoundingBox.BoxRight] = Fixed.RawValue(_tmX + missile.Radius);
        _tmBoundingBox[BoundingBox.BoxLeft] = Fixed.RawValue(_tmX - missile.Radius);
        var subSector = DoomGame.Instance.Renderer.PointInSubSector(_tmX, _tmY);
        _ceilingLine = null;
        _tmFloorZ = _tmDropOffZ = subSector.Sector!.FloorHeight;
        _tmCeilingZ = subSector.Sector.CeilingHeight;
        DoomGame.Instance.Renderer.ValidCount++;
        _numSpecHit = 0;

        var work = new MissilePlacementWork
        {
            Missile = missile,
            XPosition = _tmX,
            YPosition = _tmY,
            ...
            CompletionMapObject2 = completionMapObject2
        };
        work.X = work.XLow;
        work.Y = work.YLow;
        ContinueMissilePlacementForAction(actionParams, work, false);
    }

    internal void ContinueMissilePlacementForAction(
        ActionParams actionParams,
        MissilePlacementWork work,
        bool damageCompleted)
    {
        RestoreMissilePlacementContext(work);
        if (damageCompleted)
        {
            CompleteMissilePlacementForAction(actionParams, work, false);
            return;
        }
        ...
        while (work.X <= work.XHigh)
        {
            while (work.Y <= work.YHigh)
            {
                ...
                while (work.CurrentThing is not null)
                {
                    ...
                    if ((candidate.Flags & (MapObjectFlag.MF_SOLID | MapObjectFlag.MF_SPECIAL | MapObjectFlag.MF_SHOOTABLE)) != 0)
                    {
                        var blockDist = candidate.Radius + work.Missile.Radius;
                        if (Fixed.Abs(candidate.X - work.XPosition) < blockDist &&
                            Fixed.Abs(candidate.Y - work.YPosition) < blockDist &&
                            candidate != work.Missile &&
                            work.Missile.Z <= candidate.Z + candidate.Height &&
                            work.Missile.Z + work.Missile.Height >= candidate.Z)
                        {
                            var source = work.Missile.Target;
                            if (source != null &&
                                (source.Type == candidate.Type ||
                                 source.Type == MapObjectType.MT_KNIGHT && candidate.Type == MapObjectType.MT_BRUISER ||
                                 source.Type == MapObjectType.MT_BRUISER && candidate.Type == MapObjectType.MT_KNIGHT))
                            {
                                if (candidate == source)
                                {
                                    work.CurrentThing = work.NextThing;
                                ...
                                if (candidate.Type != MapObjectType.MT_PLAYER)
                                {
                                    CompleteMissilePlacementForAction(actionParams, work, false);
                                    return;
                                }
                            ...
                            {
                                var damage = ((DoomRandom.P_Random() % 8) + 1) * work.Missile.Info.Damage;
                                var transition = MapObject.DamageMapObjectCore(candidate, work.Missile, source, damage);
                                var transitionState = MapObject.DamageTransitionState(transition);
                                if (transitionState != StateNum.NUMSTATES)
                                {
                                    actionParams.ScheduleStateTransition(
                                        ...
                                        transitionState,
                                        MapObject.DamageTransitionCompletion(transition),
                                        MapObjectActionContinuation.MissilePlacement,
                                        continuationMissilePlacementWork: work);
                                    return;
                                }
                                CompleteMissilePlacementForAction(actionParams, work, false);
                                return;
        ...
        var xl = unchecked(_tmBoundingBox[BoundingBox.BoxLeft] - Fixed.RawValue(_blockMapOriginX)) >> Constants.MapBlockShift;
        var xh = unchecked(_tmBoundingBox[BoundingBox.BoxRight] - Fixed.RawValue(_blockMapOriginX)) >> Constants.MapBlockShift;
        var yl = unchecked(_tmBoundingBox[BoundingBox.BoxBottom] - Fixed.RawValue(_blockMapOriginY)) >> Constants.MapBlockShift;
        var yh = unchecked(_tmBoundingBox[BoundingBox.BoxTop] - Fixed.RawValue(_blockMapOriginY)) >> Constants.MapBlockShift;
        for (var bx = xl; bx <= xh; bx++)
        {
            for (var by = yl; by <= yh; by++)
            {
        ...
        if ((_tmCeilingZ - _tmFloorZ < work.Missile.Height) ||
            ((work.Missile.Flags & MapObjectFlag.MF_TELEPORT) == 0 && _tmCeilingZ - work.Missile.Z < work.Missile.Height) ||
            ((work.Missile.Flags & MapObjectFlag.MF_TELEPORT) == 0 && _tmFloorZ - work.Missile.Z > Fixed.FromInt(24)) ||
            ((work.Missile.Flags & (MapObjectFlag.MF_DROPOFF | MapObjectFlag.MF_FLOAT)) == 0 &&
             _tmFloorZ - _tmDropOffZ > Fixed.FromInt(24)))
        {
            CompleteMissilePlacementForAction(actionParams, work, false);
            return;
        }
        CompleteMissilePlacementForAction(actionParams, work, true);
    }

    private void RestoreMissilePlacementContext(MissilePlacementWork work)
    {
        _tmThing = work.Missile;
        _tmFlags = work.Missile.Flags;
        _tmX = work.XPosition;
        _tmY = work.YPosition;
        _tmBoundingBox[BoundingBox.BoxTop] = Fixed.RawValue(_tmY + work.Missile.Radius);
        _tmBoundingBox[BoundingBox.BoxBottom] = Fixed.RawValue(_tmY - work.Missile.Radius);
        _tmBoundingBox[BoundingBox.BoxRight] = Fixed.RawValue(_tmX + work.Missile.Radius);
        _tmBoundingBox[BoundingBox.BoxLeft] = Fixed.RawValue(_tmX - work.Missile.Radius);
        _tmFloorZ = work.FloorZ;
        _tmCeilingZ = work.CeilingZ;
        _tmDropOffZ = work.DropOffZ;
    }

    private void CompleteMissilePlacementForAction(ActionParams actionParams, MissilePlacementWork work, bool success)
    {
        if (!success)
        {
            actionParams.ScheduleStateTransition(
                work.Missile,
                BeginExplodeMissile(work.Missile),
                NestedStateCompletion.MissileExplosion,
                work.Completion,
                work.CompletionIndex,
                work.Missile,
                work.CompletionMapObject2);
            return;
        }

        P_UnsetThingPosition(work.Missile);
        var oldX = work.Missile.X;
        var oldY = work.Missile.Y;
        work.Missile.FloorZ = _tmFloorZ;
        work.Missile.CeilingZ = _tmCeilingZ;
        work.Missile.X = work.XPosition;
        work.Missile.Y = work.YPosition;
        P_SetThingPosition(work.Missile);
        if ((work.Missile.Flags & (MapObjectFlag.MF_TELEPORT | MapObjectFlag.MF_NOCLIP)) == 0)
        {
            while (_numSpecHit-- != 0)
            {
                var line = _specHit[_numSpecHit];
                var side = P_PointOnLineSide(work.Missile.X, work.Missile.Y, line);
                var oldSide = P_PointOnLineSide(oldX, oldY, line);
                if (side != oldSide && line.Special != 0)
                    P_CrossMissileSpecialLine(line, work.Missile);
            }
        }
        actionParams.ScheduleContinuation(
            work.Completion,
            work.CompletionIndex,
            work.Missile,
            work.CompletionMapObject2);
    ...
    internal MapObject P_SpawnMissileForAction(
        MapObject source,
        MapObject dest,
        MapObjectType type,
        ActionParams actionParams,
    ...
    {
        var th = P_CreateMissile(source, dest, type);
        P_CheckMissileSpawnPlacementForAction(
            actionParams,
            th,
            continuation,
            continuationIndex,
            continuationMapObject2);
        return th;
    }
    ...
    private MapObject P_CreateMissile(MapObject source, MapObject dest, MapObjectType type)
    {
        ...
        {
            an = an + new Angle((DoomRandom.P_Random() - DoomRandom.P_Random()) << 20);
        }
        ...
        if (dist < Fixed.FromRaw(1))
        {
            dist = Fixed.FromRaw(1);
        }

        th.MomZ = (dest.Z - source.Z) / Fixed.RawValue(dist);
        return th;
    }
    ...
    private int P_DivlineSide(Fixed x, Fixed y, DividerLine node)
    {
        ...
        var left = Fixed.FromRaw(Fixed.RawValue(node.Dy) >> Constants.FracBits) * (Fixed.RawValue(dx) >> Constants.FracBits);
        var right = Fixed.FromRaw(Fixed.RawValue(dy) >> Constants.FracBits) * (Fixed.RawValue(node.Dx) >> Constants.FracBits);
    ...
    private bool P_CrossBSPNode(int bspnum)
    {
        ...
        while (true)
        {
            // Descend through the starting side. Each frame is resumed only after
            // that subtree succeeds, matching the recursive front-first traversal.
            while ((bspnum & Constants.NodeLeafSubSector) == 0)
            {
                ...
                if (stackCount >= _sightNodeStack.Length)
                {
                    DoomGame.Error("P_CrossBSPNode: cyclic or over-deep BSP");
                }

                _sightNodeStack[stackCount] = bspnum;
                _sightSideStack[stackCount] = side;
                stackCount++;
                bspnum = bsp.Children[side];
            }

            var subSector = bspnum == -1 ? 0 : bspnum & (~Constants.NodeLeafSubSector);
            if (!P_CrossSubSector(subSector))
            {
                return false;
            }

            // Resume parents until the trace actually crosses a partition. The
            // ending-side test intentionally stays after the front subtree.
            var foundBackSide = false;
            while (stackCount != 0)
            {
                stackCount--;
                var nodeIndex = _sightNodeStack[stackCount];
                var side = _sightSideStack[stackCount];
                var bsp = Nodes[nodeIndex];
    ...
    public bool P_CheckSight(MapObject t1, MapObject t2)
    {
        ...
        var s1 = SectorIndex(t1.SubSector!.Sector!);
        var s2 = SectorIndex(t2.SubSector!.Sector!);
        ...
        {
            _sightCounts[0] = _sightCounts[0] + 1;
        ...
        _sightCounts[1] = _sightCounts[1] + 1;
    ...
    public void P_NewChaseDir(MapObject actor)
    {
        ...
        {
            var firstDirection = directions[1];
            directions[1] = directions[2];
            directions[2] = firstDirection;
        }
    ...
    public static readonly int[] XSpeed =
    {
        Constants.FracUnit, 47000, 0, -47000, -Constants.FracUnit, -47000, 0, 47000
    };

    public static readonly int[] YSpeed =
    {
        0, 47000, Constants.FracUnit, 47000, 0, -47000, -Constants.FracUnit, -47000
    };
    ...
    public bool P_Move(MapObject actor)
    {
        ...
        var tryX = actor.X + actor.Info.Speed * Fixed.FromRaw(XSpeed[(int)actor.MoveDir]);
        var tryY = actor.Y + actor.Info.Speed * Fixed.FromRaw(YSpeed[(int)actor.MoveDir]);

        // P_Move is the ordinary AI-walk path: the mover is neither a missile,
        // a skull in MF_SKULLFLY, nor a pickup-capable player. Its thing checks
        // therefore have no state-transition side effects.
        var tryOk = P_TryMovePassive(actor, tryX, tryY);
    ...
    public bool P_CheckMeleeRange(MapObject actor)
    {
        ...
        var meleeRangeWithTargetRadius = Fixed.FromRaw(Fixed.RawValue(meleeRange) + player.Info.Radius);
        if (dist >= meleeRangeWithTargetRadius)
        {
            return false;
        }
    ...
    public bool P_CheckMissileRange(MapObject actor)
    {
        ...
        {
            dist = dist - Fixed.FromInt(128); // no melee attack, so fire more
        }

        var distValue = Fixed.RawValue(dist) >> 16;

...
public class Player
{
    ...
    public GameController.TicCommand Command { get; set; } = new();
    ...
    public void Think()
    {
        ...
        {
            var index = (int)PowerUpType.Strength;
            Powers[index] = Powers[index] + 1;
        }

        if (Powers[(int)PowerUpType.Invulnerability] > 0)
        {
            var index = (int)PowerUpType.Invulnerability;
            Powers[index] = Powers[index] - 1;
        }

        if (Powers[(int)PowerUpType.Invisibility] != 0)
        {
            var index = (int)PowerUpType.Invisibility;
            Powers[index] = Powers[index] - 1;
            if (Powers[index] == 0)
            {
        ...
        {
            var index = (int)PowerUpType.InfraRed;
            Powers[index] = Powers[index] - 1;
        }

        if (Powers[(int)PowerUpType.IronFeet] > 0)
        {
            var index = (int)PowerUpType.IronFeet;
            Powers[index] = Powers[index] - 1;
        }
    ...
    private void UseLines()
    {
        ...
        var x1 = MapObject.X;
        var y1 = MapObject.Y;
        var useRange = Fixed.RawValue(Constants.UseRange) >> Constants.FracBits;
        var x2 = x1 + useRange * DoomMath.Cos(angle);
        var y2 = y1 + useRange * DoomMath.Sin(angle);

        game.P_PathTraverse(x1, y1, x2, y2, GameController.PT_AddLines, GameController.TraversalAction.Use);
    }
    ...
    public void Move()
    {
        ...
        {
            MapObject.SetStateWithoutAction(StateNum.S_PLAY_RUN1);
        }
    }
        ...
        switch (sector.Special)
        {
            ...
            default:
                DoomGame.Error(string.Concat(
                    ...
                    DoomString.DecimalInt(sector.Special)));
                break;
        }
    }
    ...
    public bool GiveAmmo(AmmoType ammo, int num)
    {
        ...
        var ammoIndex = (int)ammo;
        if (Ammo[ammoIndex] == MaxAmmo[ammoIndex])
        {
            return false;
        }
        ...
        if (num != 0)
        {
            num *= GameController.ClipAmmo[ammoIndex];
        }
        else
        {
            num = GameController.ClipAmmo[ammoIndex] / 2;
        }
        ...
        var oldAmmo = Ammo[ammoIndex];
        Ammo[ammoIndex] = oldAmmo + num;

        if (Ammo[ammoIndex] > MaxAmmo[ammoIndex])
        {
            Ammo[ammoIndex] = MaxAmmo[ammoIndex];
        }

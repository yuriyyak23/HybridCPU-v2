...
public static class Trigger
{
    ...
    public static void ExecuteAction(ThinkerAction action, ActionParams parameters)
    {
        switch (action)
        {
            case ThinkerAction.VerticalDoor: VerticalDoor(parameters); break;
            case ThinkerAction.MoveFloor: MoveFloor(parameters); break;
            case ThinkerAction.MoveCeiling: MoveCeiling(parameters); break;
            case ThinkerAction.PlatformRaise: PlatformRaise(parameters); break;
            case ThinkerAction.StrobeFlash: PerformStrobeFlash(parameters); break;
            case ThinkerAction.LightFlash: PerformLightFlash(parameters); break;
            case ThinkerAction.GlowingLight: PerformGlowingLight(parameters); break;
            case ThinkerAction.FireFlicker: PerformFireFlicker(parameters); break;
            case ThinkerAction.None: break;
            default: DoomGame.Error("Invalid trigger action"); break;
        }
    }
    ...
    public static bool DoorEvent(Line line, DoorType type)
    {
        ...
        while ((secnum = game.P_FindSectorFromLineTag(line, secnum)) >= 0)
        {
            ...
            door.Action = ThinkerAction.VerticalDoor;
            door.Type = type;
    ...
    public static void VerticalDoorEvent(Line line, MapObject thing)
    {
        ...
        var secnum = DoomGame.Instance.Game.SectorIndex(sec);
        ...
        newDoor.Action = ThinkerAction.VerticalDoor;
        newDoor.Direction = 1;
    ...
    public static bool FloorEvent(Line line, FloorType type)
    {
        ...
        while ((secnum = game.P_FindSectorFromLineTag(line, secnum)) >= 0)
        {
            ...
            floor.Action = ThinkerAction.MoveFloor;
            floor.Type = type;
            floor.Crush = false;
                ...
                case FloorType.RaiseToTexture:
                    {
                        ...
                        for (var i = 0; i < sec.LineCount; i++)
                        {
                            if (TwoSided(secnum, i))
                            {
                                ...
                                {
                                    if (renderer.GetTextureHeight(side.BottomTexture) < minSize)
                                    {
                                        minSize = renderer.GetTextureHeight(side.BottomTexture);
                                    }
                                }
        ...
        while ((secnum = game.P_FindSectorFromLineTag(line, secnum)) >= 0)
        {
            ...
            do
            {
                ...
                for (var i = 0; i < sec.LineCount; i++)
                {
                    ...
                    if (game.SectorIndex(tsec) != secnum)
                    {
                        continue;
                    }

                    tsec = secLine.BackSector!;
                    var newSecNum = game.SectorIndex(tsec);
        ...
        while ((secnum = DoomGame.Instance.Game.P_FindSectorFromLineTag(line, secnum)) >= 0)
        {
            ...
            switch (type)
            {
                ...
                case CeilingType.CrushAndRaise:
                    ceiling.Crush = true;
                    ceiling.TopHeight = sec.CeilingHeight;
                    goto case CeilingType.LowerAndCrush; // fall through
    ...
    public static bool PlatformEvent(Line line, PlatformType type, int amount)
    {
        ...
        while ((secnum = game.P_FindSectorFromLineTag(line, secnum)) >= 0)
        {
            ...
            plat.Action = ThinkerAction.PlatformRaise;
            plat.Crush = false;
            plat.Tag = line.Tag;
    ...
    public static int TeleportEvent(Line line, int side, MapObject thing)
    {
        ...
        var tag = line.Tag;
        var game = DoomGame.Instance.Game;
        for (var sectorIndex = 0; sectorIndex < game.Sectors.Length; sectorIndex++)
        {
            var sector = game.Sectors[sectorIndex];
            if (sector.Tag != tag)
            {
                continue;
            }

            for (var thinkerNode = game.Thinkers; thinkerNode != null; thinkerNode = thinkerNode.Next)
            {
                if (thinkerNode is not MapObject m)
                {
                    continue;
                }
    ...
    public static void SpawnStrobeFlash(Sector sec, int fastOrSlow, int inSync)
    {
        ...
        flash.Action = ThinkerAction.StrobeFlash;
    ...
    public static void SpawnLightFlash(Sector sector)
    {
        ...
        flash.Action = ThinkerAction.LightFlash;
    ...
    public static void SpawnGlowingLight(Sector sector)
    {
        ...
        glow.Action = ThinkerAction.GlowingLight;
        glow.Direction = -1;
    ...
    public static void SpawnFireFlicker(Sector sector)
    {
        ...
        flicker.Action = ThinkerAction.FireFlicker;
    ...
    public static bool IsSideSectorCurrent(int currentSector, int lineIdx, int sideIdx)
    {
        ...
        return game.SectorIndex(side.Sector) == currentSector;
    }
}

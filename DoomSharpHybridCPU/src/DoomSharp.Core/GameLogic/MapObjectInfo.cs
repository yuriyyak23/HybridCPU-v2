...
public sealed class MapObjectInfo
{
    private static readonly MapObjectInfo[] _predefinedTypes = new MapObjectInfo[(int)MapObjectType.NUMMOBJTYPES];
    ...
    {
        return _predefinedTypes[(int)type].Copy();
    }

    private static void AddPredefinedType(MapObjectInfo info)
    {
        _predefinedTypes[(int)info.Type] = info;
    }

    public static void SetSpeed(MapObjectType type, int speed)
    {
        _predefinedTypes[(int)type].Speed = speed;
    }

    public static MapObjectInfo? FindByDoomedNum(int doomedNum)
    {
        for (var i = 0; i < _predefinedTypes.Length; i++)
        {
            if (_predefinedTypes[i].DoomedNum == doomedNum)
            {
                return _predefinedTypes[i].Copy();
            }
        }
    ...
    private MapObjectInfo Copy() => new()
    {
        Type = Type,
        DoomedNum = DoomedNum,
    ...
    {
        AddPredefinedType(new MapObjectInfo
        {

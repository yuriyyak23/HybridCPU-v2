...
public class RenderEngine
{
    ...
    // -1 is the inactive state; valid color-map offsets are non-negative multiples of 256.
    private int _fixedColorMapIdx = -1;
    ...
    private int[] _renderBspNodeStack = Array.Empty<int>();
    private int[] _renderBspSideStack = Array.Empty<int>();
    ...
    private Patch?[] _patchCache = Array.Empty<Patch?>();
    ...
    private ColumnMode _baseColumnMode;
    private bool _lowDetailSpan;
    ...
    private void DrawCurrentColumn()
    {
        switch (_columnMode)
        {
            case ColumnMode.Normal:
                DrawColumn();
                break;
            case ColumnMode.LowDetail:
                DrawColumnLow();
                break;
            case ColumnMode.Fuzz:
                DrawFuzzColumn();
                break;
            case ColumnMode.Translated:
                DrawTranslatedColumn();
                break;
        }
    }

    private void DrawCurrentSpan()
    {
        if (_lowDetailSpan)
        {
            DrawSpanLow();
    ...
    // Angle is an immutable single-UInt32 value. Keep this hot lookup table in its
    // exact scalar representation so AOT array access does not require a general
    // value-type ldelem/stelem ABI or introduce copies with different semantics.
    private uint[] _xToViewAngle = new uint[Constants.ScreenWidth + 1];
    ...
    private int[] _textureHeight = Array.Empty<int>();

    public Fixed GetTextureHeight(int texture) => Fixed.FromRaw(_textureHeight[texture]);
    private int[] _textureCompositeSize = Array.Empty<int>();
    private int[][] _textureColumnLump = Array.Empty<int[]>();
    private int[][] _textureColumnOfs = Array.Empty<int[]>();
    ...
    private int[] _ySlope = new int[Constants.ScreenHeight];
    private int[] _distScale = new int[Constants.ScreenWidth];
    private Fixed _baseXScale;
    private Fixed _baseYScale;

    private int[] _cachedHeight = new int[Constants.ScreenHeight];
    private int[] _cachedDistance = new int[Constants.ScreenHeight];
    private int[] _cachedXStep = new int[Constants.ScreenHeight];
    private int[] _cachedYStep = new int[Constants.ScreenHeight];
    ...
    private int _dcSourceOffset;
    private int _dcSourceLength;
    ...
    {
        var denominator = (uint)Fixed.RawValue(den);
        if (denominator < 512)
        {
            return SlopeRange;
        }

        var ans = ((uint)Fixed.RawValue(num) << 3) / (denominator >> 8);
    ...
    {
        InitTextures();
        DoomGame.Console.WriteLine("InitTextures");

        InitFlats();
        DoomGame.Console.WriteLine("InitFlats");

        InitSpriteLumps();
        DoomGame.Console.WriteLine("InitSpriteLumps");

        InitColorMaps();
        DoomGame.Console.WriteLine("InitColormaps");
    }

    private Patch GetPatch(int lump)
    {
        var patch = _patchCache[lump];
        if (patch is null)
        {
            var data = DoomGame.Instance.WadData.GetLumpNum(lump, PurgeTag.Cache)!;
            patch = Patch.FromBytes(data);
            _patchCache[lump] = patch;
        }
    ...
    private Column GetColumn(int tex, int col)
    {
        ...
        if (lump > 0)
        {
            var patch = GetPatch(lump);
            return patch.GetColumnByOffset((uint)ofs) ?? new Column(0xff, 0, Array.Empty<byte>());
        }
        ...
        return CreateCompositeColumn(_textureComposite[tex]!, ofs, _textures[tex].Height);
    }

    internal static Column CreateCompositeColumn(byte[] composite, int offset, int length)
    {
        return new Column(0, length, composite, offset);
    }

    private void SetColumnSource(Column column)
    {
        _dcSource = column.Pixels;
        _dcSourceOffset = column.PixelOffset;
        _dcSourceLength = column.Length;
    }
    ...
    private void InitTextures()
    {
        ...
        var numMapPatches = DoomConvert.ToInt32(names, 0);
        var patchLookup = new int[numMapPatches];

        for (var i = 0; i < numMapPatches; i++)
        {
            var name = new ByteReader(names, 4 + (i * 8)).ReadName(8);
        ...
        var numTextures1 = DoomConvert.ToInt32(mapTex, 0);
        ...
        if (DoomGame.Instance.WadData.CheckNumForName("TEXTURE2") != -1)
        {
            ...
            numTextures2 = DoomConvert.ToInt32(mapTex2, 0);
        ...
        for (var i = 0; i < _numTextures; i++, directory += 4)
        {
            ...
            var offset = DoomConvert.ToInt32(mapTex, directory);
            if (offset > maxOff)
            {
            ...
            var mapTexture = MapTexture.FromBytes(mapTex, offset);
            mapTexture.ReadMapPatches(mapTex, offset + MapTexture.BaseSize);
            ...
            _textureColumnLump[i] = new int[texture.Width];
            _textureColumnOfs[i] = new int[texture.Width];
            ...
            _textureHeight[i] = texture.Height << Constants.FracBits;
    ...
    /// </summary>
    private void DrawColumnInCache(Column? patch, byte[] cache, int cacheOffset, int originY, int cacheHeight)
    {
        if (patch is null) 
        {
            return; 
    ...
    private void GenerateLookup(int texNum)
    {
        ...
        for (var i = 0; i < texture.PatchCount; i++)
        {
            ...
            for (; x < x2; x++)
            {
                patchCount[x] = (byte)(patchCount[x] + 1);
                colLump[x] = patch.Patch;
                colOfs[x] = (int)realPatch.ColumnOffsets[x - x1];
            }
        }
        ...
        for (x = 0; x < texture.Width; x++)
        {
            if (patchCount[x] == 0)
            {
                DoomGame.Console.WriteLine(string.Concat("R_GenerateLookup: column without a patch (", texture.Name, ")"));
                return;
            }
            ...
            if (patchCount[x] > 1)
            {
                ...
                colLump[x] = -1;
                colOfs[x] = _textureCompositeSize[texNum];

                if (_textureCompositeSize[texNum] > 0x10000 - texture.Height)
                {
                    DoomGame.Error(string.Concat("R_GenerateLookup: texture ", DoomString.DecimalInt(texNum), " is >64k"));
                    return;
                }

                _textureCompositeSize[texNum] = _textureCompositeSize[texNum] + texture.Height;
            }
        }
    }
    ...
    public int CheckTextureNumForName(string name)
    {
        ...
        for (var i = 0; i < _numTextures; i++)
        {
            if (WadFileCollection.NameEquals(_textures[i].Name, name))
            {
                return i;
            }
        }
    ...
    public void ExecuteSetViewSize()
    {
        ...
        if (_detailShift == 0)
        {
            _columnMode = _baseColumnMode = ColumnMode.Normal;
            _lowDetailSpan = false;
        }
        else
        {
            _columnMode = _baseColumnMode = ColumnMode.LowDetail;
            _lowDetailSpan = true;
        }
        ...
        for (var i = 0; i < ViewHeight; i++)
        {
            ...
            dy = Fixed.Abs(dy);
            _ySlope[i] = Fixed.RawValue(Fixed.FromInt((ViewWidth << _detailShift) / 2) / dy);
        }

        for (var i = 0; i < ViewWidth; i++)
        {
            var cosAdj = Fixed.Abs(DoomMath.Cos(new Angle(_xToViewAngle[i])));
            _distScale[i] = Fixed.RawValue(Fixed.Unit / cosAdj);
        }
    ...
    private void InitLightTables()
    {
        ...
        for (var i = 0; i < LightLevels; i++)
        {
            ...
            for (var j = 0; j < MaxLightZ; j++)
            {
                ...
                // Keep the single-field value on the evaluation stack. Calling the instance
                // property would force a general address-taken local solely for a readonly read.
                var level = startMap - Fixed.RawValue(scale) / DistMap;
    ...
    {
        _patchCache = new Patch?[DoomGame.Instance.WadData.LumpCount];
        InitData();
        DoomGame.Console.Write($"{DoomGame.NewLine}R_InitData");
        DoomMath.InitPointToAngle();
        DoomGame.Console.Write($"{DoomGame.NewLine}R_InitPointToAngle");
        DoomMath.InitTables();
        // viewwidth / viewheight / detailLevel are set by the defaults
        DoomGame.Console.Write($"{DoomGame.NewLine}R_InitTables");

        SetViewSize(DoomGame.Instance.Menu.ScreenBlocks, DoomGame.Instance.Menu.DetailLevel);
        // InitPlanes(); // Doesn't do anything
        DoomGame.Console.Write($"{DoomGame.NewLine}R_InitPlanes");
        InitLightTables();
        DoomGame.Console.Write($"{DoomGame.NewLine}R_InitLightTables");
        Sky.InitSkyMap();
        DoomGame.Console.Write($"{DoomGame.NewLine}R_InitSkyMap");
        InitTranslationTables();
        DoomGame.Console.Write($"{DoomGame.NewLine}R_InitTranslationTables");
    ...
    private void InstallSpriteLump(int lump, int frame, int rotation, bool flipped)
    {
        ...
        {
            DoomGame.Error("R_InstallSpriteLump: bad frame characters");
            return;
        }
        ...
        if (rotation == 0)
        {
            ...
            if (_spriteTemp[frame].RotationMode == SpriteRotationMode.Single)
            {
                DoomGame.Error("R_InitSprites: sprite frame has multiple rot=0 lumps");
                return;
            }

            if (_spriteTemp[frame].RotationMode == SpriteRotationMode.Rotating)
            {
                DoomGame.Error("R_InitSprites: sprite frame has rotations and a rot=0 lump");
                return;
            }

            _spriteTemp[frame].RotationMode = SpriteRotationMode.Single;
            for (var r = 0; r < 8; r++)
            {
        ...
        _spriteTemp[frame].RotationMode = SpriteRotationMode.Rotating;

        // make 0 based
        rotation--;
        if (_spriteTemp[frame].Lumps[rotation] != -1)
        {
            DoomGame.Error("R_InitSprites: sprite rotation has two lumps mapped to it");
            return;
        }
    ...
    private void InitSpriteDefs(string[] spriteNames)
    {
        ...
        for (var i = 0; i < _numSprites; i++)
        {
            ...
            for (var l = start + 1; l < end; l++)
            {
                ...
                if (lumpName.Length >= 4 && _spriteName.Length >= 4 &&
                    lumpName[0] == _spriteName[0] && lumpName[1] == _spriteName[1] &&
                    lumpName[2] == _spriteName[2] && lumpName[3] == _spriteName[3])
                {
            ...
            {
                switch (_spriteTemp[frame].RotationMode)
                {
                    case SpriteRotationMode.Unknown:
                        // no rotations were found for that frame at all
                        DoomGame.Error("R_InitSprites: no patches found for sprite frame");
                        break;

                    case SpriteRotationMode.Single:
                        // only the first rotation is needed
                        break;

                    case SpriteRotationMode.Rotating:
                        // must have all 8 frames
                        for (var rotation = 0; rotation < 8; rotation++)
                        {
                            if (_spriteTemp[frame].Lumps[rotation] == -1)
                            {
                                DoomGame.Error("R_InitSprites: sprite frame is missing rotations");
                            }
                        }
            ...
            for (var frame = 0; frame < _maxFrame; frame++)
            {
                _sprites[i].Frames[frame] = _spriteTemp[frame];
            }
        }
    }
    ...
    private void InitTextureMapping()
    {
        ...
        for (i = 0; i < DoomMath.FineAngleCount / 2; i++)
        {
            ...
            {
                t = (Fixed.RawValue(_centerXFrac - DoomMath.Tan(i) * focalLength) + Constants.FracUnit - 1) >> Constants.FracBits;
        ...
        for (x = 0; x <= ViewWidth; x++)
        {
            ...
            _xToViewAngle[x] = Angle.RawValue(new Angle((uint)(i << DoomMath.AngleToFineShift)) - Angle.Angle90);
        }
        ...
        _clipAngle = new Angle(_xToViewAngle[0]);
    }
    ...
    private void DrawColumn()
    {
        ...
        if (_dcSource is null || _dcSourceLength == 0)
        {
            return;
        }
        ...
        do
        {
            ...
            var sourceIdx = (Fixed.RawValue(frac) >> Constants.FracBits) & 127;
            if (sourceIdx == _dcSourceLength)
            {
                sourceIdx--; // simulate patches repeating the last pixel byte
            }

            DoomGame.Instance.Video.Screens[0][dest] = _dcColorMap[_dcSource[_dcSourceOffset + (sourceIdx % _dcSourceLength)]];
    ...
    private void DrawTranslatedColumn()
    {
        ...
        do
        {
            ...
            var sourceIdx = Fixed.RawValue(frac) >> Constants.FracBits;
            if (sourceIdx == _dcSourceLength)
            {
                sourceIdx--; // simulate patches repeating the last pixel byte
            }

            DoomGame.Instance.Video.Screens[0][dest] = _dcColorMap[_dcTranslation[_dcSource[_dcSourceOffset + sourceIdx]]];
            dest += Constants.ScreenWidth;
    ...
    private void DrawSpan()
    {
        ...
        do
        {
            ...
            var spot = ((Fixed.RawValue(yFrac) >> (16 - 6)) & (63 * 64)) + ((Fixed.RawValue(xFrac) >> 16) & 63);
            ...
            // Next step in u,v.
            xFrac = xFrac + _dsXStep;
            yFrac = yFrac + _dsYStep;
        } while (count-- != 0);
    }
    ...
    private void SetupFrame(Player player)
    {
        ...
        _viewAngle = new Angle(Angle.RawValue(player.MapObject.Angle) + unchecked((uint)_viewAngleOffset));
        ...
        if (player.FixedColorMap != 0)
        {
            ...
            {
                _scaleLightFixed[i] = _fixedColorMapIdx;
            }
        }
        else
        {
    ...
    private void RenderMaskedSegmentRange(DrawSegment ds, int x1, int x2)
    {
        ...
        if ((_currentLine.LineDef.Flags & Constants.Line.DontPegBottom) != 0)
        {
            ...
            _dcTextureMid = _dcTextureMid + Fixed.FromRaw(_textureHeight[texNum]) - _viewZ;
        }
        else
        {
        ...
        _dcTextureMid = _dcTextureMid + _currentLine.SideDef.RowOffset;

        if (_fixedColorMapIdx >= 0)
        {
            Array.Copy(_colorMaps, _fixedColorMapIdx, _dcColorMap, 0, 256);
        ...
        {
            // calculate lighting
            if (MaskedTextureCol![MaskedTextureColIdx + _dcX] != short.MaxValue)
            {
                if (_fixedColorMapIdx < 0)
                {
                    var index = Fixed.RawValue(_sprYScale) >> LightScaleShift;
                ...
                _dcIScale = Fixed.FromRaw((int)(0xffffffffu / (uint)Fixed.RawValue(_sprYScale)));

                // draw the texture
                var col = GetColumn(texNum, MaskedTextureCol[MaskedTextureColIdx + _dcX]);
                DrawMaskedColumn(col);
                MaskedTextureCol[MaskedTextureColIdx + _dcX] = short.MaxValue;
            }
            _sprYScale = _sprYScale + _rwScaleStep;
        }
    }
    ...
    private void RenderSegLoop()
    {
        ...
        for (; _rwX < _rwStopX; _rwX++)
        {
            ...
            var yl = (Fixed.RawValue(_topFrac) + HeightUnit - 1) >> HeightBits;
            ...
            var yh = Fixed.RawValue(_bottomFrac) >> HeightBits;
            ...
            if (_segTextured)
            {
                ...
                var angle = _rwCenterAngle + new Angle(_xToViewAngle[_rwX]);
                textureColumn = Fixed.RawValue(_rwOffset - (DoomMath.Tan(angle) * _rwDistance)) >> Constants.FracBits;
                
                // calculate lighting
                var index = (uint)(Fixed.RawValue(_rwScale) >> LightScaleShift);
                ...
                Array.Copy(_colorMaps, _wallLights![index], _dcColorMap, 0, 256);
                _dcX = _rwX;
                _dcIScale = Fixed.FromRaw((int)(0xffffffffu / (uint)Fixed.RawValue(_rwScale)));
            }
            ...
            else
            {
                ...
                {
                    // top wall
                    mid = Fixed.RawValue(_pixHigh) >> HeightBits;
                    _pixHigh = _pixHigh + _pixHighStep;
                ...
                {
                    // bottom wall
                    mid = (Fixed.RawValue(_pixLow) + HeightUnit - 1) >> HeightBits;
                    _pixLow = _pixLow + _pixLowStep;
                ...
                if (_maskedTexture)
                {
                    ...
                    MaskedTextureCol![MaskedTextureColIdx + _rwX] = (short)textureColumn;
                }
            }

            _rwScale = _rwScale + _rwScaleStep;
            _topFrac = _topFrac + _topStep;
            _bottomFrac = _bottomFrac + _bottomStep;
        }
    }
    ...
    private void StoreWallRange(int start, int stop)
    {
        ...
        ds.Scale1 = _rwScale = ScaleFromGlobalAngle(_viewAngle + new Angle(_xToViewAngle[start]));

        if (stop > start)
        {
            ds.Scale2 = ScaleFromGlobalAngle(_viewAngle + new Angle(_xToViewAngle[stop]));
        ...
        if (_backSector == null)
        {
            ...
            {
                vTop = _frontSector.FloorHeight + Fixed.FromRaw(_textureHeight[_sideDef.MidTexture]);
            ...
            _rwMidTextureMid = _rwMidTextureMid + _sideDef.RowOffset;
        ...
        else
        {
            ...
            if (_worldHigh < _worldTop)
            {
                ...
                {
                    vTop = _backSector.CeilingHeight + Fixed.FromRaw(_textureHeight[_sideDef.TopTexture]);
            ...
            _rwTopTextureMid = _rwTopTextureMid + _sideDef.RowOffset;
            _rwBottomTextureMid = _rwBottomTextureMid + _sideDef.RowOffset;
        ...
        if (_segTextured)
        {
            ...
            _rwOffset = _rwOffset + _sideDef.TextureOffset + _currentLine.Offset;
        ...
        _worldTop = _worldTop >> 4;
        _worldBottom = _worldBottom >> 4;
        ...
        if (_backSector != null)
        {
            _worldHigh = _worldHigh >> 4;
            _worldLow = _worldLow >> 4;
    ...
    private void ClipSolidWallSegment(int first, int last)
    {
        ...
        if (first < _solidSegments[start].First)
        {
            if (last < (_solidSegments[start].First - 1))
            {
                ...
                while (next != start)
                {
                    _solidSegments[next].CopyFrom(_solidSegments[next - 1]);
                    next--;
                }
        ...
        while (last >= _solidSegments[next + 1].First - 1)
        {
            ...
            if (last <= _solidSegments[next].Last)
            {
                ...
                {
                    // Remove a post.
                    _solidSegments[++start].CopyFrom(_solidSegments[next]);
                }
    ...
    private void AddLine(Segment line)
    {
        ...
        var x1 = _viewAngleToX[Angle.RawValue(angle1) >> DoomMath.AngleToFineShift];
        var x2 = _viewAngleToX[Angle.RawValue(angle2) >> DoomMath.AngleToFineShift];
    ...
    private bool CheckBBox(int[] bspcoord)
    {
        ...
        if (Fixed.RawValue(_viewX) <= bspcoord[BoundingBox.BoxLeft])
        {
            boxx = 0;
        }
        else if (Fixed.RawValue(_viewX) < bspcoord[BoundingBox.BoxRight])
        {
            boxx = 1;
        }
        else
        ...
        if (Fixed.RawValue(_viewY) >= bspcoord[BoundingBox.BoxTop])
        {
            boxy = 0;
        }
        else if (Fixed.RawValue(_viewY) > bspcoord[BoundingBox.BoxBottom])
        {
            boxy = 1;
        }
        else
        ...
        var angle1 = PointToAngle(Fixed.FromRaw(x1), Fixed.FromRaw(y1)) - _viewAngle;
        var angle2 = PointToAngle(Fixed.FromRaw(x2), Fixed.FromRaw(y2)) - _viewAngle;
        ...
        angle2 += Angle.Angle90;
        // TODO possible bug here
        var sx1 = _viewAngleToX[Angle.RawValue(angle1) >> DoomMath.AngleToFineShift];
        var sx2 = _viewAngleToX[Angle.RawValue(angle2) >> DoomMath.AngleToFineShift];
    ...
    private void Subsector(int num)
    {
        if (num >= DoomGame.Instance.Game.NumSubSectors)
        {
            DoomGame.Error(
                string.Concat(
                    string.Concat("R_Subsector: ss ", DoomString.DecimalInt(num)),
                    string.Concat(" with numss = ", DoomString.DecimalInt(DoomGame.Instance.Game.NumSubSectors))));
            return;
        }
    ...
    private int PointOnSide(Fixed x, Fixed y, Node node)
    {
        ...
        if (((Fixed.RawValue(node.Dy) ^ Fixed.RawValue(node.Dx) ^ Fixed.RawValue(dx) ^ Fixed.RawValue(dy)) & int.MinValue) != 0)
        {
            if (((Fixed.RawValue(node.Dy) ^ Fixed.RawValue(dx)) & int.MinValue) != 0)
            {
                // (left is negative)
                return 1;
            }
        ...
        var left = Fixed.FromRaw(Fixed.RawValue(node.Dy) >> Constants.FracBits) * dx;
        var right = dy * Fixed.FromRaw(Fixed.RawValue(node.Dx) >> Constants.FracBits);
    ...
    private int PointOnSegSide(Fixed x, Fixed y, Segment line)
    {
        ...
        // Try to quickly decide by looking at sign bits.
        if (((Fixed.RawValue(ldy) ^ Fixed.RawValue(ldx) ^ Fixed.RawValue(dx) ^ Fixed.RawValue(dy)) & int.MinValue) != 0)
        {
            if (((Fixed.RawValue(ldy) ^ Fixed.RawValue(dx)) & int.MinValue) != 0)
            {
                // (left is negative)
                return 1;
            }
        ...
        var left = Fixed.FromRaw(Fixed.RawValue(ldy) >> Constants.FracBits) * dx;
        var right = dy * Fixed.FromRaw(Fixed.RawValue(ldx) >> Constants.FracBits);
    ...
    ///  traversing the subtree front-first.
    /// Just call with BSP root.
    /// </summary>
    private void RenderBSPNode(int bspNum)
    {
        var nodeCount = DoomGame.Instance.Game.NumNodes;
        if (_renderBspNodeStack.Length < nodeCount)
        {
            _renderBspNodeStack = new int[nodeCount];
            _renderBspSideStack = new int[nodeCount];
        }

        var stackCount = 0;
        while (true)
        {
            // Descend through front space first, retaining the parent frames.
            while ((bspNum & Constants.NodeLeafSubSector) == 0)
            {
                ...
                if (stackCount >= _renderBspNodeStack.Length)
                {
                    DoomGame.Error("RenderBSPNode: cyclic or over-deep BSP");
                }

                _renderBspNodeStack[stackCount] = bspNum;
                _renderBspSideStack[stackCount] = side;
                stackCount++;
                bspNum = bsp.Children[side];
            }

            Subsector(bspNum == -1 ? 0 : bspNum & (~Constants.NodeLeafSubSector));

            // Check the back box only after front rendering has updated clipping.
            var foundVisibleBackSide = false;
            while (stackCount != 0)
            {
                stackCount--;
                var nodeIndex = _renderBspNodeStack[stackCount];
                var side = _renderBspSideStack[stackCount];
                var bsp = DoomGame.Instance.Game.Nodes[nodeIndex];
                if (CheckBBox(bsp.BoundingBox[side ^ 1]))
                {
                    bspNum = bsp.Children[side ^ 1];
                    foundVisibleBackSide = true;
                    break;
                }
            }

            if (!foundVisibleBackSide)
            {
                return;
            }
        }
    ...
    private void MapPlane(int y, int x1, int x2)
    {
        ...
        var planeHeightRaw = Fixed.RawValue(_planeHeight);
        if (planeHeightRaw != _cachedHeight[y])
        {
            _cachedHeight[y] = planeHeightRaw;
            distance = _planeHeight * Fixed.FromRaw(_ySlope[y]);
            _cachedDistance[y] = Fixed.RawValue(distance);
            _dsXStep = distance * _baseXScale;
            _cachedXStep[y] = Fixed.RawValue(_dsXStep);
            _dsYStep = distance * _baseYScale;
            _cachedYStep[y] = Fixed.RawValue(_dsYStep);
        }
        else
        {
            distance = Fixed.FromRaw(_cachedDistance[y]);
            _dsXStep = Fixed.FromRaw(_cachedXStep[y]);
            _dsYStep = Fixed.FromRaw(_cachedYStep[y]);
        }

        var length = distance * Fixed.FromRaw(_distScale[x1]);
        var angle = _viewAngle + new Angle(_xToViewAngle[x1]);
        _dsXFrac = _viewX + (DoomMath.Cos(angle) * length);
        _dsYFrac = -_viewY - (DoomMath.Sin(angle) * length);

        if (_fixedColorMapIdx >= 0)
        {
            Array.Copy(_colorMaps, _fixedColorMapIdx, _dsColorMap, 0, 256);
        }
        else
        {
            var index = (uint)(Fixed.RawValue(distance) >> LightZShift);
        ...
        for (var i = 0; i < _lastVisPlaneIdx; i++)
        {
            ...
            if (pl.PicNum == Sky.FlatNum)
            {
                ...
                for (var x = pl.MinX; x <= pl.MaxX; x++)
                {
                    ...
                    if (_dcYl <= _dcYh)
                    {
                        var angle = Angle.RawValue(_viewAngle + new Angle(_xToViewAngle[x])) >> Sky.AngleToSkyShift;
                        _dcX = x;
    ...
    private Angle PointToAngle(Fixed x, Fixed y)
    {
        ...
        if (x >= Fixed.Zero)
        {
            ...
            if (y >= Fixed.Zero)
            {
                ...
                {
                    // octant 0
                    return DoomMath.GetTanToAngle(SlopeDiv(y, x));
                }
             
                // octant 1
                return new Angle(Angle.RawValue(Angle.Angle90) - 1) - DoomMath.GetTanToAngle(SlopeDiv(x, y));
            }
            ...
            // octant 7
            return Angle.Angle270 + DoomMath.GetTanToAngle(SlopeDiv(x, y));
        }
        ...
        if (y >= Fixed.Zero)
        {
            ...
            {
                // octant 3
                return new Angle(Angle.RawValue(Angle.Angle180) - 1) - DoomMath.GetTanToAngle(SlopeDiv(y, x));
            }

            // octant 2
            return Angle.Angle90 + DoomMath.GetTanToAngle(SlopeDiv(x, y));
        }
        ...
        {
            // octant 4
            return Angle.Angle180 + DoomMath.GetTanToAngle(SlopeDiv(y, x));
        }
     
        // octant 5
        return new Angle(Angle.RawValue(Angle.Angle270) - 1) - DoomMath.GetTanToAngle(SlopeDiv(x, y));
    }
    ...
    private Fixed PointToDist(Fixed x, Fixed y)
    {
        ...
        var angle = DoomMath.GetTanToAngle((uint)Fixed.RawValue(dy / dx) >> DBits) + Angle.Angle90;
    ...
    private Fixed ScaleFromGlobalAngle(Angle visAngle)
    {
        ...
        if (den > (num >> 16))
        {
            ...
            else if (Fixed.RawValue(scale) < 256)
            {
                scale = Fixed.FromRaw(256);
            }
        }
        else
        {
    ...
    private void DrawMaskedColumn(Column? column)
    {
        ...
        for (; column is not null && column.TopDelta != 0xff; )
        {
            ...
            _dcYl = (Fixed.RawValue(topscreen) + Constants.FracUnit - 1) >> Constants.FracBits;
            _dcYh = (Fixed.RawValue(bottomscreen) - 1) >> Constants.FracBits;

            if (_dcYh >= _mFloorClip[_mFloorClipIdx + _dcX])
            {
                _dcYh = _mFloorClip[_mFloorClipIdx + _dcX] - 1;
            }

            if (_dcYl <= _mCeilingClip[_mCeilingClipIdx + _dcX])
            {
                _dcYl = _mCeilingClip[_mCeilingClipIdx + _dcX] + 1;
            }
    ...
    {
        var patch = GetPatch(vis.Patch + _firstSpriteLump);

        if (vis.ColormapIndex >= 0)
        {
            Array.Copy(_colorMaps, vis.ColormapIndex, _dcColorMap, 0, 256);
        }

        if (vis.ColormapIndex < 0)
        {
            // NULL colormap = shadow draw
            _columnMode = ColumnMode.Fuzz;
        }
        else if ((vis.MapObjectFlags & MapObjectFlag.MF_TRANSLATION) != 0)
        {
            _columnMode = ColumnMode.Translated;
        ...
        for (_dcX = vis.X1; _dcX <= vis.X2; _dcX++, frac = frac + vis.XiScale)
        {
            var textureColumn = Fixed.RawValue(frac) >> Constants.FracBits;
            //# ifdef RANGECHECK
    ...
    private void ProjectSprite(MapObject thing)
    {
        ...
        int lump;
        bool flip;
        if (sprframe.RotationMode == SpriteRotationMode.Rotating)
        {
            // choose a different rotation based on player view
            var ang = PointToAngle(thing.X, thing.Y);
            var rot = (Angle.RawValue(ang) - Angle.RawValue(thing.Angle) + (Angle.RawValue(Angle.Angle45) / 2) * 9) >> 29;
        ...
        tx = tx - Fixed.FromInt(_spriteOffset[lump]);
        var x1 = Fixed.RawValue(_centerXFrac + (tx * xscale)) >> Constants.FracBits;
        ...
        tx = tx + Fixed.FromInt(_spriteWidth[lump]);
        var x2 = (Fixed.RawValue(_centerXFrac + (tx * xscale)) >> Constants.FracBits) - 1;
        ...
        if (flip)
        {
            vis.StartFrac = Fixed.FromRaw(Fixed.RawValue(Fixed.FromInt(_spriteWidth[lump])) - 1);
            vis.XiScale = -iscale;
        }
        else
        {
        ...
        else if (_fixedColorMapIdx >= 0)
        {
            // fixed map
            vis.ColormapIndex = _fixedColorMapIdx;
        }
        ...
        {
            // diminished light
            var index = Fixed.RawValue(xscale) >> (LightScaleShift - _detailShift);
            ...
            vis.ColormapIndex = _spriteLights[index];
        }
    }

    /// <summary>
    /// During BSP traversal
    /// </summary>
    ...
    private void DrawPlayerSprite(PlayerSprite psp)
    {
        ...
        var x1 = Fixed.RawValue(_centerXFrac + (tx * _pSpriteScale)) >> Constants.FracBits;
        ...
        tx = tx + Fixed.FromInt(_spriteWidth[lump]);
        var x2 = (Fixed.RawValue(_centerXFrac + (tx * _pSpriteScale)) >> Constants.FracBits) - 1;
        ...
        {
            // local light
            vis.ColormapIndex = _spriteLights[MaxLightScale - 1];
        }
    ...
    {
        var clipBottom = new int[Constants.ScreenWidth];
        ...
        for (var i = _drawSegIdx - 1; i >= 0; i--)
        {
            ...
            if (silhouette == 1)
            {
                ...
                for (var x = r1; x <= r2; x++)
                {
                    if (clipBottom[x] == -2)
                    {
                        clipBottom[x] = ds.SpriteBottomClip[ds.SpriteBottomClipIdx + x];
                    }
                }
            }
            else if (silhouette == 2)
            {
                ...
                for (var x = r1; x <= r2; x++)
                {
                    if (clipTop[x] == -2)
                    {
                        clipTop[x] = ds.SpriteTopClip[ds.SpriteTopClipIdx + x];
                    }
                }
            }

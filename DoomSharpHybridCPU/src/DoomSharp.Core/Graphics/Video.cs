...
public class Video
{
    ...
    private readonly int[] _dirtyBox = new int[4];

    private int _gamma;
    private static readonly byte[][] GammaTable = new byte[5][];

    private static int _wipeGo;
    ...
    public void Initialize()
    {
        ...
        BoundingBox.ClearRawBox(_dirtyBox);
    }
    ...
    public void DrawPatch(int x, int y, int screen, Patch patch)
    {
        ...
        if (x < 0 ||
            ...
            screen > 4)
        {
            DoomGame.Console.WriteLine(
                string.Concat(
                    string.Concat("Patch at ", DoomString.DecimalInt(x)),
                    string.Concat(string.Concat(", ", DoomString.DecimalInt(y)), " exceeds LFB")));
    ...
    public bool WipeScreenEffect(WipeMethod wipeMethod, int x, int y, int width, int height, int tics)
    {
        ...
        if (_wipeGo == 0)
        {
            ...
            ExecuteWipePhase(wipeNo * 3, width, height, tics);
        }

        // do a piece of wipe-in
        MarkRectangle(0, 0, width, height);
        var rc = ExecuteWipePhase(wipeNo * 3 + 1, width, height, tics);
        ...
        {
            _wipeGo = 0;
            ExecuteWipePhase(wipeNo * 3 + 2, width, height, tics);
        }

        return _wipeGo == 0;
    }

    private bool ExecuteWipePhase(int phase, int width, int height, int tics)
    {
        return phase switch
        {
            0 => InitializeColorTransform(width, height, tics),
            1 => DoColorTransform(width, height, tics),
            2 => ExitColorTransform(width, height, tics),
            3 => InitializeMelt(width, height, tics),
            4 => DoMelt(width, height, tics),
            5 => ExitMelt(width, height, tics),
            _ => InvalidWipePhase()
        };
    }

    private static bool InvalidWipePhase()
    {
        DoomGame.Error("Invalid wipe phase");
        return false;
    }
    ...
    private bool DoMelt(int width, int height, int tics)
    {
        ...
        while (tics-- > 0)
        {
            for (var i = 0; i < width; i++)
            {
                if (_wipeMeltPos[i] < 0)
                {
                    _wipeMeltPos[i] = _wipeMeltPos[i] + 1;
                    done = false;
                }
                else if (_wipeMeltPos[i] < height)
                {
                    ...
                    _wipeMeltPos[i] = _wipeMeltPos[i] + dy;
                    sourceIdx = i * height;
    ...
    {
        BoundingBox.AddRawToBox(_dirtyBox, x << Constants.FracBits, y << Constants.FracBits);
        BoundingBox.AddRawToBox(_dirtyBox, (x + width - 1) << Constants.FracBits,
            (y + height - 1) << Constants.FracBits);
    }

    public void CopyStatusBarRectangle(int srcX, int srcY, int width, int height, int destX, int destY)
    {
        ...
        for (; height > 0; height--)
        {
            Array.Copy(_screens[4], src, _screens[0], dest, width);

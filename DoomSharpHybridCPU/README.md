...
HybridCPU AOT GUI workbench: see
[DoomSharp.HybridCpu.Windows](src/DoomSharp.HybridCpu.Windows/README.md) and
`src/DoomSharp.HybridCpu.slnx`. It links the real ISE/runtime DLLs and validates
`.hcexe`/WAD inputs. Native Doom execution remains blocked on the managed loader
and ECALL bridge; the existing `DoomSharp.Windows` is the CoreCLR reference host.
Use `tools/Build-HybridCpuGui.ps1` to keep all GUI build artifacts under `TempEnv`.

...
{
    /// <summary>
    /// Deterministic Stage 6 placement policy for bounded whole-block placement search without rescheduling.
    /// </summary>
    internal static class HybridCpuBasicBlockPlacementPolicy
    {
        public static bool Dominates(IrBasicBlockPlacementCandidate candidate, IrBasicBlockPlacementCandidate other)
        {
            ...
            return CompareBundleSlots(candidate.BundleInstructionSlots, other.BundleInstructionSlots) < 0;
        }

        public static bool IsBetterPlacement(
            IrBasicBlockPlacementCandidate candidate,
            IrBasicBlockPlacementCandidate current,
            HybridCpuBackendPlacementTieBreakContext tieBreakContext = default)
        {
            ...
            return CompareBundleSlots(candidate.BundleInstructionSlots, current.BundleInstructionSlots) < 0;
        }
        ...
        private static int CompareBundleSlots(IReadOnlyList<IReadOnlyList<int>> candidateBundles, IReadOnlyList<IReadOnlyList<int>> currentBundles)
        {
            int bundleCount = Math.Min(candidateBundles.Count, currentBundles.Count);
            for (int bundleIndex = 0; bundleIndex < bundleCount; bundleIndex++)
            {
                int comparison = CompareAssignedSlots(candidateBundles[bundleIndex], currentBundles[bundleIndex]);
                if (comparison != 0)
                {
                    return comparison;
                }
            ...
            return candidateBundles.Count.CompareTo(currentBundles.Count);
        }

...
internal static class CompilerMatrixTileEmissionLowerer
{
        ...
        out CompilerMatrixTileEmissionPlan? plan)
    {
        ...
        CompilerMatrixTileEmissionRequest request = RecoverRequest(opcode, in instruction);
        ...
        in VLIW_Instruction instruction)
    {
        ...
        VectorInstructionPayload payload = CreatePayload(in instruction);
    ...
    private static void ValidateMaccRequest(CompilerMatrixTileEmissionRequest request)
    {
        ...
        MatrixTileMaccSemanticContract contract =
            MatrixTileAccumulatorAndTransposePolicyAbi.CreateMaccContract(
                ...
                accumulatorPolicy.ElementKind);

        if (contract.AccumulatorPolicy != accumulatorPolicy.AccumulatorPolicy)
        {
            throw new ArgumentException(
                "MTILE_MACC compiler helper requires the runtime widening accumulator policy.",
                nameof(request));
        }
        ...
        if (!DataTypeUtils.IsInteger(request.Descriptor.ElementType))
        {
            throw new ArgumentException(
                "MTILE_MACC compiler helper requires an integer source element type.",
                nameof(request));
        }
    }
    ...
    private static VectorInstructionPayload CreatePayload(in VLIW_Instruction instruction)
    {

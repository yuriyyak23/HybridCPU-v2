...
/// <summary>RF-12.6bg decision-only inventory for the final AX raw-token witness.</summary>
public sealed class Rf126bgInvalidCancellationForgedTokenMigrationInventoryTests
{
    [Fact]
    public void AxForgedWitnessIsSeparateFromInvalidBindingFailClosedCoverage()
    {
        string source = File.ReadAllText(Path.Combine(Root(), "HybridCPU_ISE.Tests",
            "Architecture", "Rf126axLegacyCancellationStoredBindingInvalidBehaviorTests.cs"));
        Assert.DoesNotContain("new MemorySubsystem.MemoryRequestToken(", source,
            StringComparison.Ordinal);
        Assert.Contains("AssertBindingFailurePreservesAcceptedRequest", source,
            StringComparison.Ordinal);
        Assert.Contains("SetBinding(token, replacement)", source,
            StringComparison.Ordinal);
    ...
    [Fact]
    public void PaperSeparatesRemovalCallersFromTokenAuthority()
    {

...
/// <summary>RF-12.7r ResumeTransfer post-validation channel projection.</summary>
public sealed class Rf127rDmaResumeTransferValidInputCutoverTests
{
    [Fact]
    public void ValidPausedChannelResumesThroughCheckedProjection()
    {
        ...
        Assert.True(dma.ResumeTransfer(7));
        Assert.Equal(DMAController.ChannelState.Active, dma.GetChannelState(7));
    }

    [Fact]
    public void InvalidAndNonPausedRawOutcomesRemainFalse()
    {
        Processor processor = default;
        var dma = new DMAController(ref processor);
    ...
    public void SourceProjectsOnlyAfterExistingRangeGate()
    {
        ...
        int start = source.IndexOf("public bool ResumeTransfer(byte channelID)", StringComparison.Ordinal);

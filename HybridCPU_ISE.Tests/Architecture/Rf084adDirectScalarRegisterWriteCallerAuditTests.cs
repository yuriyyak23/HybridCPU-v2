...
public sealed class Rf084adDirectScalarRegisterWriteCallerAuditTests
{
    ...
    [Fact]
    public void PublicCommittedWriteIsLimitedToStateOwnerAndManagedStartupSeeding()
    {
        string root = FindRepositoryRoot();
        string coreRoot = Path.Combine(root, "HybridCPU_ISE", "CloseToHSL", "Core");
        ...
        Assert.Equal(
            [
                "Architecture/State/Architectural/CPU_Core.StateData.cs",
                "Runtime/Managed/HybridCpuIseManagedGuestExecutionV1.cs",
            ],
            FindCallerFiles(coreRoot, "WriteCommittedArch("));
        string managedStartup = Read(root, "HybridCPU_ISE", "CloseToHSL", "Core", "Runtime", "Managed",
            "HybridCpuIseManagedGuestExecutionV1.cs");
        Assert.Contains("SeedStartupState(core, request);", managedStartup, StringComparison.Ordinal);
        Assert.Contains("core.WriteCommittedArch(request.VirtualThreadId, request.StackPointerRegister, request.StackPointer);", managedStartup, StringComparison.Ordinal);
    }

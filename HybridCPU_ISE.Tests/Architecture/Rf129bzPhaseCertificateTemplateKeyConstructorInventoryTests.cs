...
public sealed class Rf129bzPhaseCertificateTemplateKeyConstructorInventoryTests
{
    [Fact]
    public void ConstructorsRemainInternalFamilySeamsWithOnlyFrozenCallers()
    {
        ...
        string guardTest = Read("HybridCPU_ISE.Tests", "tests", "Phase09SafetyVerifierGuardMatrixProofTests.cs");
        ...
        Assert.Contains("new PhaseCertificateTemplateKey4Way(", guardTest, StringComparison.Ordinal);
        Assert.DoesNotContain("PhaseCertificateTemplateKey", testSupport, StringComparison.Ordinal);
        Assert.Contains("RF-12.9bz | closed inventory/freeze", ledger, StringComparison.Ordinal);
    }

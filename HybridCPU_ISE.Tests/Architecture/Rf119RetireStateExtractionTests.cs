...
public sealed class Rf119RetireStateExtractionTests
{
    ...
    public void RuntimeContainsExactRetireAuthorityAndCertificates()
    {
        ...
        string[] fieldNames = retire.GetFields(Flags).Select(static field => field.Name).ToArray();
        Assert.Contains("Coordinator", fieldNames);
        Assert.Contains("CompletionCommitOwner", fieldNames);
        Assert.Contains("CanonicalPipelineCompletionProducer", fieldNames);
        Assert.Contains("CanonicalCpuTranslationFaultCompletionProducer", fieldNames);
        Assert.Contains("CanonicalCpuSecondStageTranslationFaultCompletionProducer", fieldNames);
        Assert.Contains("DecodePublicationCertificate", fieldNames);
        Assert.Contains("ExecuteCompletionCertificate", fieldNames);
        Assert.Contains("RetireVisibilityCertificate", fieldNames);
    }

...
public sealed class Rf124VtPipelineScoreboardIdentityInventoryTests
{
    ...
    public void CheckedSurfaceAndTypedCallerManifestRemainFrozen()
    {
        ...
        {
            ["Compilers/HybridCPU_Compiler/API/Runtime/NativeTransportRuntimeAdapter.cs"] = 1,
            ["Compilers/HybridCPU_Compiler/API/Threading/HybridCpuThreadCompilerContext.cs"] = 1,
            ...
            ["Diagnostics/TestAssemblerConsoleApps/SimpleAsmApp.cs"] = 1,
            ["Diagnostics/TestAssemblerConsoleApps/SimpleAsmApp.Emit.cs"] = 1
        }, createCallers);
        Assert.Equal(new Dictionary<string, int>(StringComparer.Ordinal)
        {
            ...
            ["HybridCPU_ISE/CloseToHSL/Core/Pipeline/Safety/SafetyVerifier.VirtualizationAdmission.cs"] = 1
    ...
    public void RawStorageWireAndAbsenceSeamsRemainFrozen()
    {
        ...
        string ir = Read(root, "Compilers", "HybridCPU_Compiler", "Core", "IR", "Model",
            "IrSlotMetadata.cs");
        string former = Read(root, "Compilers", "HybridCPU_Compiler", "Core", "IR",
        ...
        string diagnostic = Read(root, "Diagnostics", "TestAssemblerConsoleApps",
    ...
    private static Dictionary<string, int> FindCallers(string root, string pattern)
    {
        ...
        foreach (string sourceRoot in new[]
                 {
                     "HybridCPU_ISE", "Compilers/HybridCPU_Compiler",

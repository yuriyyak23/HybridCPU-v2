...
public sealed class Rf124nCsrRetireOwnerVtValidInputCutoverTests
{
    [Fact]
        ...
        int owner)
    {
        ...
        operation.CapturePrimaryWriteBackResult(0xC500UL + (ulong)owner);
        ...
        Assert.Equal(owner, records[0].VtId);
        Assert.Equal(9, records[0].ArchReg);
        Assert.Equal(0xC500UL + (ulong)owner, records[0].Value);
        Assert.NotEqual(operation.VirtualThreadId, records[0].VtId);
    }
    ...
    public void AllPresenceGatesRetainInvalidOwnerSuppression(int rawOwner)
    {
        Processor.CPU_Core core = null!;
    ...
    [InlineData(int.MinValue)]
    [InlineData(int.MaxValue)]
    public void ActiveInvalidOwnerRetainsExactFaultBeforeCapacity(int rawOwner)
    {
        var operation = new CsrReadCounterMicroOp
        {
            OwnerThreadId = rawOwner,
    ...
    [Fact]
    public void DefaultPayloadRawDestinationAndCapacityBehaviorRemainExact()
    {
        Processor.CPU_Core core = null!;
        var defaultPayload = new CsrReadCounterMicroOp
        {
            OwnerThreadId = 0,
            WritesRegister = true,
            DestRegID = 32
        ...
        RetireRecord[] records = new RetireRecord[1];
        int count = 0;
        defaultPayload.EmitWriteBackRetireRecords(
            ref core, records, ref count);
        Assert.Equal(1, count);
    ...
    [Fact]
    public void TypedMainlineAndExecutionSourceOwnerRemainUnchanged()
    {
    ...
    [Fact]
    public void ProductionCallerInventoryAddsOnlyTheCsrCompatibilityEmitter()
    {
        string root = FindRepositoryRoot();
        Assert.Equal(new Dictionary<string, int>(StringComparer.Ordinal)
        {
            ...
            ["HybridCPU_ISE/CloseToHSL/Core/Pipeline/MicroOps/Memory/MicroOp.LoadStore.cs"] = 2,
        ...
        Assert.Empty(FindOccurrences(root, "Compilers/HybridCPU_Compiler",
            "TryGetArchitecturalOwnerVtId"));
        Assert.Empty(FindOccurrences(root, "Diagnostics/TestAssemblerConsoleApps",
            "TryGetArchitecturalOwnerVtId"));
    }

...
public sealed class Rf112CoreIdentityAndCopySeamHardeningTests
{
    ...
    public void ProductionAndDiagnosticConsoleDoNotIndexCoreTableDirectly()
    {
        ...
        string diagnostics = ReadTree(Path.Combine(root, "Diagnostics", "TestAssemblerConsoleApps"));
    ...
    public void ObservationIsSnapshotOnlyAndDiagnosticMutationIsLiveRefOnly()
    {
        ...
        string session = Read(root, "Diagnostics", "TestAssemblerConsoleApps", "DiagnosticRuntimeSession.cs");
        string showcase = Read(root, "Diagnostics", "TestAssemblerConsoleApps", "SimpleAsmApp.Showcase.cs");
        string matrix = Read(root, "Diagnostics", "TestAssemblerConsoleApps", "MatrixTileSpecSuite.cs");
    ...
    public void ExistingLaneCopiesRemainCallLocalExplicitGetModifySetProtocols()
    {
        ...
        string[] runtimeDomains = runtimeFields.Select(static field => field.FieldType.Name).ToArray();
        string[] requiredDomains =
        [
            "AdmissionState", "ArchitecturalState", "AssistState", "BackendState", "CacheState",
            "CompatibilityControlPolicyState", "CoreBindingState", "CpuInstructionTranslationState",
            "DecodeState", "ExecutionState", "ExtensionState", "FrontendState",
            "LegacyCompatibilityState", "MemoryPipelineState", "ReplayState", "ResourceState",
            "RetireState", "SchedulingState", "ScratchState", "TelemetryState",
            "VirtualThreadControlState"
        ];
        Assert.All(requiredDomains, domain => Assert.Single(runtimeDomains, actual => actual == domain));
    }

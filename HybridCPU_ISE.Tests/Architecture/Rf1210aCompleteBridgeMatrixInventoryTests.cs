...
public sealed class Rf1210aCompleteBridgeMatrixInventoryTests
{
    ...
    public void SourceShapesFreezeDistinctJsonBinaryCompilerAndObservationBridges()
    {
        ...
        string compiler = Read("Compilers", "HybridCPU_Compiler", "Core", "IR", "Bundling", "HybridCpuBundleLowerer.cs");
        string compilerRuntimeAdapter = Read("Compilers", "HybridCPU_Compiler", "API", "Runtime", "NativeTransportRuntimeAdapter.cs");
        ...
        Assert.Contains("VtId.Create(metadata.VirtualThreadId)", compilerRuntimeAdapter, StringComparison.Ordinal);
        Assert.Contains("ResolvePinnedLaneId(", compiler, StringComparison.Ordinal);

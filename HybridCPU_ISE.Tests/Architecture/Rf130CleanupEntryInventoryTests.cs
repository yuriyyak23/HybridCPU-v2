...
public sealed class Rf130CleanupEntryInventoryTests
{
    ...
    [
        ...
        "Compilers/CpuInterfaceBridge",
        "Diagnostics/TestAssemblerConsoleApps",
        "Diagnostics/HybridCPU.IsaGen",
    ];
    ...
    public void StaticIsaCandidates_RecordGeneratedStorageAfterRawRowRemoval()
    {
        ...
        string generator = Read("Diagnostics", "HybridCPU.IsaGen", "Program.cs");
    ...
    public void CompatibilityCandidates_HaveClosedWorldRuntimeTestSupportAndReflectionDisposition()
    {
        Assert.Equal(
            new[]
            {
                "Diagnostics/TestAssemblerConsoleApps/MatrixTileSpecSuite.cs",
                "Diagnostics/TestAssemblerConsoleApps/StreamVectorSpecSuite.cs",
        ...
        Assert.Equal(
            new[] { "Diagnostics/TestAssemblerConsoleApps/SimpleAsmApp.Showcase.cs" },
    ...
    public void ProtectedFallbacksAndExcludedProofTests_RemainVisibleInsteadOfBeingCalledLegacyFree()
    {
        ...
        string quarantineManifest = Read("HybridCPU_ISE.Tests", "VmxRefactoring",
            "VmxQuarantineEvidenceManifest.cs");
        Assert.Contains("VmxQuarantineEvidenceEntry[] EntryTable", quarantineManifest,
            StringComparison.Ordinal);
        Assert.Contains("IsRecognizedLegacyRisk", quarantineManifest,
            StringComparison.Ordinal);
        Assert.Contains("CanReturnToCore", quarantineManifest,
            StringComparison.Ordinal);
    ...
    private static string[] ExternalCallers(string token) =>
        new[] { "Compilers/HybridCPU_Compiler", "Compilers/CpuInterfaceBridge", "Diagnostics/TestAssemblerConsoleApps" }
            .SelectMany(SourceFiles)
            .Where(path => File.ReadAllText(path).Contains(token, StringComparison.Ordinal))
            .Select(Relative)
            .OrderBy(path => path, StringComparer.Ordinal)

...
public sealed class Rf084cPcWriteCompatibilitySourceCertificationTests
{
    ...
    public void ForegroundControlLaneIsFilteredBeforeDirectExecuteLatchMaterialization()
    {
        ...
        Assert.Contains("if (!CanVirtualThreadIssueInForeground(issueLane.OwnerThreadId))", materialization, StringComparison.Ordinal);
        Assert.Contains("return Core.IssuePacketLane.CreateEmpty(laneIndex);", materialization, StringComparison.Ordinal);
        Assert.Contains("pipeEX.Lane7 = CreateExecuteLaneState(", materialization, StringComparison.Ordinal);
        Assert.True(
            materialization.IndexOf("ApplyIssueLaneExecutionSurfaceContract(issueLane, issuePacket.PC)", StringComparison.Ordinal) <
            materialization.IndexOf("if (!CanVirtualThreadIssueInForeground(issueLane.OwnerThreadId))", StringComparison.Ordinal));
        Assert.True(
            materialization.IndexOf("if (!CanVirtualThreadIssueInForeground(issueLane.OwnerThreadId))", StringComparison.Ordinal) <
            materialization.IndexOf("pipeEX.Lane7 = CreateExecuteLaneState(", StringComparison.Ordinal));

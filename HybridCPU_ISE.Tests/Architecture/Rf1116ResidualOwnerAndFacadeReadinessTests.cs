...
public sealed class Rf1116ResidualOwnerAndFacadeReadinessTests
{
    ...
    public void ResidualDirectFieldSetIsOnlyStableRuntimeReference()
    {
        ...
        Assert.Empty(core.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly));
    ...
    public void RuntimeRootRemainsAuthorityPreservingDomains()
    {
        ...
        string[] domains = runtime.GetFields(Fields).Select(static field => field.FieldType.Name).ToArray();
        Assert.Contains("CoreBindingState", domains);
        Assert.Contains("CpuInstructionTranslationState", domains);
        Assert.Contains("CompatibilityControlPolicyState", domains);
        Assert.Contains("RetireState", domains);
    ...
    public void StructCopyReflectionAndTestMutationSeamsRemainExplicit()
    {
        ...
        MatchCollection byRefParameters = Regex.Matches(production,
            @"\b(?:ref|in|out)\s+(?:Processor\.)?CPU_Core(?!\.)\b");
        Assert.NotEmpty(byRefParameters);
        Assert.DoesNotContain(byRefParameters.Cast<Match>(), static match => match.Value.StartsWith("out ", StringComparison.Ordinal));
    ...
    public void PartialAndInPlaceCrossStageTopologyIsFrozenWithoutCurrentNextRewrite()
    {
        ...
        Assert.Contains(sources, source => source.Contains("sealed partial class CPU_Core", StringComparison.Ordinal));

        string production = string.Join("\n", sources);
        Assert.True(AssignmentCount(production, "IF") > 0);
        Assert.True(AssignmentCount(production, "ID") > 0);
        Assert.True(AssignmentCount(production, "EX") > 0);
        Assert.True(AssignmentCount(production, "MEM") > 0);
        Assert.True(AssignmentCount(production, "WB") > 0);

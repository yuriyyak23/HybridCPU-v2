...
public sealed class Rf110StateOwnerEntryInventoryFreezeTests
{
    ...
    public void CpuCore_RemainsAValueFacadeOverTheFrozenMixedOwnerGraph()
    {
        ...
        string[] runtimeDomains = runtimeRoot.GetFields(InstanceFields)
            ...
            .ToArray();
        Assert.Contains("CpuInstructionTranslationState", runtimeDomains);
        Assert.Contains("CompatibilityControlPolicyState", runtimeDomains);
        Assert.Contains("RetireState", runtimeDomains);
        Assert.Equal(runtimeDomains.Length, runtimeDomains.Distinct(StringComparer.Ordinal).Count());
    ...
    public void PartialGraphAndCrossStageWriters_RemainAtTheInventoriedTopology()
    {
        ...
        Assert.NotEmpty(partialFiles);
        Assert.All(partialFiles, path =>
            Assert.DoesNotContain("partial struct CPU_Core", File.ReadAllText(path), StringComparison.Ordinal));
    ...
    public void DangerousCopyReflectionAndTestMutationSeams_RemainExplicit()
    {
        ...
        string diagnosticSession = Read(root, "Diagnostics", "TestAssemblerConsoleApps", "DiagnosticRuntimeSession.cs");
        string simpleAsm = Read(root, "Diagnostics", "TestAssemblerConsoleApps", "SimpleAsmApp.cs");

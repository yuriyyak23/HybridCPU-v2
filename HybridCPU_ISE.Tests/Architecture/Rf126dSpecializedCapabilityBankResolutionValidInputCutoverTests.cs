...
public sealed class Rf126dSpecializedCapabilityBankResolutionValidInputCutoverTests
{
    ...
    public void SourceKeepsRawResolverAndHasOneAuthorizedResultCaller()
    {
        ...
        Assert.Empty(CaptureCallSites(root, "Compilers/HybridCPU_Compiler",
            resultCallPattern));
        Assert.Empty(CaptureCallSites(root, "Compilers/CpuInterfaceBridge",
            resultCallPattern));
        Assert.Empty(CaptureCallSites(root, "Diagnostics/TestAssemblerConsoleApps",
            resultCallPattern));
    }

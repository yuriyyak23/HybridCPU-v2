...
public sealed class Rf125SlotPhysicalLanePinningInventoryTests
{
    ...
    {
        string root = FindRepositoryRoot();
        string irSlot = Read(root, "Compilers", "HybridCPU_Compiler", "Core", "IR", "Model",
            "IrMaterializedBundleSlot.cs");
        string lowerer = Read(root, "Compilers", "HybridCPU_Compiler", "Core", "IR",
        ...
        Assert.Contains("slotMetadata[slot.SlotIndex]", lowerer,
            StringComparison.Ordinal);
        Assert.Contains("new IrInstructionSlotMetadata", lowerer,
            StringComparison.Ordinal);

...
public sealed class Rf125bSourceOperationProvenanceSlotInventoryTests
{
    ...
    public void SoleWriterAndReaderProjectionContoursRemainClosed()
    {
        ...
        string vmxRetire = Read(root, "HybridCPU_ISE", "CloseToHSL", "Core",
        ...
        Assert.Contains("workingSlotId != source.SourceSlotId",
            postStageB, StringComparison.Ordinal);
        Assert.Contains("admission.SourceProvenance.SourceSlotIndex,",
            vmxRetire, StringComparison.Ordinal);
        Assert.DoesNotContain("new SourceOperationProvenance", vmxRetire,
            StringComparison.Ordinal);

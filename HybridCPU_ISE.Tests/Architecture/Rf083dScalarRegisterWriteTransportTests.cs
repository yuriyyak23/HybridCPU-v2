...
public sealed class Rf083dScalarRegisterWriteTransportTests
{
    ...
    public void ProductionTransportUsesOnlyTypedStageBAndLeavesBackendAuthorityUnchanged()
    {
        ...
        Assert.Equal(1, Count(scheduler, "PostStageBIssuedAttempt.CreateAfterSuccessfulStageB(template, LaneId.Create(lane))"));
        Assert.DoesNotContain("MaterializePostStageBIssuedAttempt(candidate, slot)", scheduler, StringComparison.Ordinal);
        int typedSmtCommit = scheduler.IndexOf("result[lane] = candidate;", StringComparison.Ordinal);
        int typedSmtMaterialization = scheduler.IndexOf(
            "MaterializePostStageBIssuedAttempt(",
            typedSmtCommit + 1,
            StringComparison.Ordinal);
        Assert.True(typedSmtCommit >= 0 && typedSmtCommit < typedSmtMaterialization);

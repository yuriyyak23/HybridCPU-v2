...
{
 [Fact] public void CpuCoreIsOneSealedReferenceFacade(){Type t=typeof(Processor.CPU_Core);Assert.False(t.IsValueType);Assert.True(t.IsClass);Assert.True(t.IsSealed);FieldInfo f=Assert.Single(t.GetFields(BindingFlags.Instance|BindingFlags.Public|BindingFlags.NonPublic|BindingFlags.DeclaredOnly),static field=>field.Name=="_runtime");Assert.True(f.IsInitOnly);Assert.Equal("CoreRuntimeState",f.FieldType.Name);Assert.Empty(t.GetFields(BindingFlags.Instance|BindingFlags.Public|BindingFlags.DeclaredOnly));}
 [Fact] public void AllPartialDeclarationsConvertedAtomically(){string p=Sources(Path.Combine(Root(),"HybridCPU_ISE"));Assert.Empty(Regex.Matches(p,@"partial\s+struct\s+CPU_Core"));Assert.NotEmpty(Regex.Matches(p,@"sealed\s+partial\s+class\s+CPU_Core"));Assert.Empty(Regex.Matches(p,@"\bref\s+this(?:\s*[,\)]|\.)"));Assert.DoesNotContain("private readonly ref",p);}
 [Fact]

...
public sealed class Rf1149AtomicReferenceFacadeConversionTests
{
 ...
 [Fact] public void AllPartialDeclarationsConvertedAtomically(){string p=Sources(Path.Combine(Root(),"HybridCPU_ISE"));Assert.Empty(Regex.Matches(p,@"partial\s+struct\s+CPU_Core"));Assert.Equal(67,Regex.Matches(p,@"sealed\s+partial\s+class\s+CPU_Core").Count);Assert.Empty(Regex.Matches(p,@"\bref\s+this(?:\s*[,\)]|\.)"));Assert.DoesNotContain("private readonly ref",p);}
 [Fact]

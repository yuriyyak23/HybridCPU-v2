...
public sealed class Rf1152FinalClosedWorldExitAuditTests
{
    ...
    public void FacadeHasOneStableRuntimeIdentityAndNoCompetingStorage()
    {
        ...
        FieldInfo runtime = Assert.Single(facade.GetFields(Flags | BindingFlags.DeclaredOnly),
            static field => field.Name == "_runtime");
    ...
    public void RuntimeRootContainsEveryReviewedDomainExactlyOnce()
    {
        ...
        [
            ...
            "Retire", "Architectural", "Scheduling", "Execution", "MemoryPipeline",
            "CpuInstructionTranslation", "CompatibilityControlPolicy", "Backend", "Extensions"
        ];
    ...
    public void LifecycleCopyReflectionSerializationAndTestSeamsAreClosed()
    {
        ...
        Match assignment = Assert.Single(Regex.Matches(production, @"(?m)^\s*core\s*=(?!=)").Cast<Match>());
        Assert.Contains("core = new Processor.CPU_Core", production.Substring(assignment.Index), StringComparison.Ordinal);

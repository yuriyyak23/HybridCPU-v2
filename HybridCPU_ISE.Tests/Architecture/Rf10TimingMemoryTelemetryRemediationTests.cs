...
public sealed class Rf10TimingMemoryTelemetryRemediationTests
{
    ...
    public void TestAssemblerPerformanceConsumersHaveClosedWorldAggregationPolicies()
    {
        ...
        string consumerSource = Read(root, "Diagnostics/TestAssemblerConsoleApps/SimpleAsmApp.Metrics.cs");
        string aggregationSource = Read(root, "Diagnostics/TestAssemblerConsoleApps/SimpleAsmApp.cs");
    ...
    public void PerformanceAndConsoleTransportPreserveZeroVersusUnavailableAndManifestCompatibility()
    {
        ...
        string consoleReport = Read(root,
            "Diagnostics/TestAssemblerConsoleApps/TimingMemoryReport.cs");
        string program = Read(root, "Diagnostics/TestAssemblerConsoleApps/Program.cs");
        string controller = Read(root,
            "Diagnostics/TestAssemblerConsoleApps/DiagnosticRunController.cs");
        string manifest = Read(root,
            "Diagnostics/TestAssemblerConsoleApps/DiagnosticArtifactManifest.cs");
        ...
        Assert.Contains("CreateAdditiveDiagnosticIntervalSince(previousPerformanceSnapshot)",
            Read(root, "Diagnostics/TestAssemblerConsoleApps/SimpleAsmApp.cs"), StringComparison.Ordinal);
    ...
    private static string FindRepositoryRoot()
    {
        ...
        while (current != null)
        {
            if (Directory.Exists(Path.Combine(current.FullName, "HybridCPU_ISE")) &&
                Directory.Exists(Path.Combine(current.FullName, "Diagnostics", "TestAssemblerConsoleApps")))
            {

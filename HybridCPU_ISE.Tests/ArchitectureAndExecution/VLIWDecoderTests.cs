...
namespace HybridCPU_ISE.Tests
{
    ...
    public class VLIWDecoderTests
    {
        ...
        [Fact]
        public void CanonicalAddiWithoutDestination_PublishesNoRegisterWrite()
        {
            var rawSlots = new VLIW_Instruction[BundleMetadata.BundleSlotCount];
            rawSlots[2] = new VLIW_Instruction
            {
            ...
            MicroOp?[] carrierBundle =
                DecodedBundleTransportProjector.BuildCanonicalCarrierBundleForTesting(
                    rawSlots,
                    bundle);
            ScalarALUMicroOp microOp = Assert.IsType<ScalarALUMicroOp>(carrierBundle[2]);

            Assert.Equal(VLIW_Instruction.NoArchReg, bundle.GetDecodedSlot(2).Instruction!.Rd);
            Assert.Equal(0UL, legality.DependencySummary!.Value.WriteRegisterMask);

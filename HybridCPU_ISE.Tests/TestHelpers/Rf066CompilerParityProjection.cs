        ...
        for (int bundleIndex = 0; bundleIndex < bundleCount; bundleIndex++)
        {
            ...
            VliwBundleAnnotations? annotations = package.Sideband is { } sideband &&
                sideband.BundleAnnotations.Count > bundleIndex
                    ? NativeTransportRuntimeAdapter.ToRuntime(sideband.BundleAnnotations[bundleIndex])
                    : null;
            ...
            IrTypedSlotBundleFacts compilerFacts = typedFacts.Facts[bundleIndex];
            for (int slotIndex = 0; slotIndex < BundleMetadata.BundleSlotCount; slotIndex++)
            {
                ...
                IrSlotClass placementClass = compilerFacts.GetSlotClass(slotIndex);
                SlotClass runtimePlacementClass = NativeTransportRuntimeAdapter.ToRuntime(placementClass);
                ...
                IrOpcodeExecutionProfile compilerProfile =
                    HybridCpuHazardModel.GetExecutionProfile(
                        NativeTransportRuntimeAdapter.ToCore((InstructionsEnum)canonical.Opcode));
                ...
                if (annotations is not null &&
                    ...
                    (metadata.SlotMetadata.AdmissionMetadata.Placement.RequiredSlotClass != runtimePlacementClass ||
                     metadata.SlotMetadata.AdmissionMetadata.Placement.PinningKind != placementPinning))
                {
                    throw Fail(
                        "annotation-placement-drift",
                        $"Compiler annotation and typed placement differ at {bundleIndex}:{slotIndex}: " +
                        $"annotation-class={metadata.SlotMetadata.AdmissionMetadata.Placement.RequiredSlotClass}, " +
                        $"facts-class={runtimePlacementClass}, " +
                        $"annotation-pinning={metadata.SlotMetadata.AdmissionMetadata.Placement.PinningKind}, " +
                        $"facts-pinning={placementPinning}.");
                }

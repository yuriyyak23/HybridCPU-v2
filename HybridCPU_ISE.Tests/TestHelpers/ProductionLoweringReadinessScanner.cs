...
internal static class ProductionLoweringReadinessScanner
{
        ...
        string source)
    {
        ...
        bool isTransportVocabularyAdapter = normalizedPath.EndsWith(
            "/API/Runtime/NativeTransportRuntimeAdapter.cs",
            StringComparison.OrdinalIgnoreCase);
        ...
        for (int lineIndex = 0; lineIndex < codeLines.Length; lineIndex++)
        {
            ...
            if (!isTransportVocabularyAdapter && ContainsBothContourDescriptors(codeLine))
            {
        ...
        if (!isTransportVocabularyAdapter)
        {
    ...
    private static bool IsHiddenFallbackShape(string codeLine, string normalizedPath)
    {
        ...
        if (codeLine.Contains("record struct", StringComparison.Ordinal))
        {
            return false;
        }

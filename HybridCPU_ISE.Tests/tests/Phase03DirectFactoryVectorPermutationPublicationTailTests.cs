...
public sealed class Phase03DirectFactoryVectorPermutationPublicationTailTests
{
    [Theory]
    [InlineData(InstructionsEnum.VPERMUTE)]
    [InlineData(InstructionsEnum.VRGATHER)]
    public void DirectFactoryVectorPermutation_ProjectsIndexedMemoryRangesBeforeManualPublication(
        InstructionsEnum opcode)
    {
        var core = new Processor.CPU_Core(0);
        core.PrepareExecutionStart(0xAA00);

        VectorPermutationMicroOp microOp = CreateDirectFactoryVectorPermutationMicroOp(opcode);
    ...
    private static VectorPermutationMicroOp CreateDirectFactoryVectorPermutationMicroOp(
        InstructionsEnum opcode)
    {
        ...
        return Assert.IsType<VectorPermutationMicroOp>(

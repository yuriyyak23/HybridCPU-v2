...
//
// Covers:
//   [T4-01] ECALL: generates EcallEvent → FSM handles trap → mepc/mcause updated → PC redirected
//   [T4-02] EBREAK: generates EbreakEvent → FSM handles debug trap → mepc/mcause(3) → PC redirected
//   [T4-03] MRET: generates MretEvent → privilege restored → mepc restored to PC
//   [T4-04] SRET: generates SretEvent → privilege restored → sepc restored to PC
//   [T4-05] FENCE: memory queue drained for affected VT before continuing (FSM state unchanged)
//   [T4-06] FENCE.I: instruction cache flush event → FSM state unchanged
//   [T4-07] WFI: VT enters Halted FSM state → resumes on interrupt
//   [T4-08] VMXON: explicit VMX executor enables VMX CSR state; requires M-mode
//   [T4-09] VMLAUNCH: FSM transitions Task→VmEntry→GuestExecution; guest state loaded
...
//   [T4-15] Regression: WFE → VT suspended; SEV → all VTs woken
...
namespace HybridCPU_ISE.Tests.IsaV5
{
    ...
    internal sealed class P4CpuState : ICanonicalCpuState
    {
        ...
        // ── Unused ICanonicalCpuState members ──────────────────────────────────────────
    ...
    public sealed class EcallHandlerTests
    {
        ...
        public void T4_01_Ecall_MachineMode_UpdatesMepcMcauseAndRedirectsPC()
        {
            ...
            Assert.Equal(trapHandlerAddr, cpuState.GetInstructionPointer()); // PC → mtvec
        }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // T4-05 — FENCE (data memory ordering)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // T4-09 — VMLAUNCH (typed VM-entry path)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // T4-10 — VM_EXIT (typed VM-exit path)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public sealed class VmExitHandlerTests
    {
        ...
        public void T4_10_VmExit_FromGuestExecution_SavesGuestRestoresHost_AndRecordsDiagnostics()
        {
            ...
            // FSM: GuestExecution → VmExit → Task
        }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // T4-11 — VMREAD / VMWRITE are InternalOpKind, not PipelineEvent subtypes
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // T4-12 / T4-13 — FspAdmissionPolicy
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public sealed class FspAdmissionPolicyTests
    {
        ...
        public void T4_12_FspAdmission_NotStealableSlot_NotAdmitted()
        {
            ...
            var slotMetas = new[]
            {
                ...
                stealable, // slot 1: occupied — irrelevant
            ...
            // Slot 2 IS free and stealable — must be admitted.
        ...
        // ── T4-14: FspAdmissionPolicy method signature uses only metadata ─────
        [Fact]
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // T4-15 — WFE / SEV — regression for FSM-owned wait/wake transitions
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public sealed class WfeSevHandlerTests
    {
        ...
        public void T4_15_Wfe_TransitionsToWaitForEvent_SevResumesWaitingState()
        {
            ...
            // VT 0 issues SEV — wakes the waiting FSM state.

...
public sealed class MatrixTileCompilerHandoffTests
{
    ...
    public void CurrentCompilerSurfacesRemainVisibleAndPositiveHandoffIsOpenAfterPhase19SidebandGate()
    {
        ...
        {
            Assert.Equal(
                (HybridCpuOpcode)(uint)MatrixTileCompilerEmissionHandoffPackage.GetRow(row.Mnemonic).Opcode,
                row.Opcode);
            ...
            Assert.Contains($"HybridCpuOpcode.{row.Opcode}", compilerEmissionSurface, StringComparison.Ordinal);

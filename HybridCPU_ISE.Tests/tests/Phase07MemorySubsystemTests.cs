// Phase 07: Memory Subsystem — Typed Loads/Stores and Full 64-bit Atomic Plane
// Covers:
...
namespace HybridCPU_ISE.Tests
{
    ...
    internal sealed class Mem07FakeCpuState : ICanonicalCpuState
    {
        ...
        // ── Unused state surface (required by interface) ──────────────────
    ...
    public sealed class IAtomicMemoryUnitInterfaceTests
    {
        [Fact]
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 2. MemoryUnit — signed load tests (LB, LH, LW with negative values)
    // ═══════════════════════════════════════════════════════════════════════════

    public sealed class SignedLoadTests
    {
        private readonly FakeMemoryBus _bus = new();
        private readonly MemoryUnit _unit;
        private readonly Mem07FakeCpuState _state = new();

        public SignedLoadTests()
        {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 3. MemoryUnit — unsigned load tests (LBU, LHU, LWU — zero extension)
    // ═══════════════════════════════════════════════════════════════════════════

    public sealed class UnsignedLoadTests
    {
        private readonly FakeMemoryBus _bus = new();
        private readonly MemoryUnit _unit;
        private readonly Mem07FakeCpuState _state = new();

        public UnsignedLoadTests()
        {
            _unit = new MemoryUnit(_bus);
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 4. MemoryUnit — doubleword load test
    // ═══════════════════════════════════════════════════════════════════════════

    public sealed class DoublewordLoadTests
    {
        [Fact]
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 5. MemoryUnit — typed store tests (SB, SH, SW, SD)
    // ═══════════════════════════════════════════════════════════════════════════

    public sealed class TypedStoreTests
    {
        private readonly FakeMemoryBus _bus = new();
        private readonly MemoryUnit _unit;
        private readonly Mem07FakeCpuState _state = new();

        public TypedStoreTests()
        {
            _unit = new MemoryUnit(_bus);
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 6. MemoryUnit — load/store round-trip tests
    // ═══════════════════════════════════════════════════════════════════════════

    public sealed class LoadStoreRoundTripTests
    {
        [Fact]
        ...
        public void SW_LW_SignedRoundTrip()
        {
            ...
            // LW sign-extends: 0xDEADBEEF → 0xFFFFFFFFDEADBEEF
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 7. MemoryUnit — alignment trap tests
    // ═══════════════════════════════════════════════════════════════════════════

    public sealed class AlignmentTrapTests
    {
        private readonly FakeMemoryBus _bus = new();
        private readonly MemoryUnit _unit;
        private readonly Mem07FakeCpuState _state = new();

        public AlignmentTrapTests()
        {
        ...
        {
            _state.SetReg(1, 0x101); // odd address — OK for byte
        ...
        {
            _state.SetReg(1, 0x103); // odd address — OK for byte
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 8. MemoryUnit — alignment trap disabled mode
    // ═══════════════════════════════════════════════════════════════════════════

    public sealed class AlignmentDisabledTests
    {
        [Fact]
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 9. MemoryUnit — x0 write protection (writes to rd=0 are discarded)
    // ═══════════════════════════════════════════════════════════════════════════

    public sealed class X0WriteProtectionTests
    {
        [Fact]
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 10. ExecutionDispatcherV4 — memory unit integration
    // ═══════════════════════════════════════════════════════════════════════════

    public sealed class DispatcherMemoryIntegrationTests
    {
        [Fact]
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 11. Classification audit — memory ops have correct class and serialization
    // ═══════════════════════════════════════════════════════════════════════════

    public sealed class ClassificationAuditTests
    {
        [Theory]
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 12. MemoryAlignmentException — properties
    // ═══════════════════════════════════════════════════════════════════════════

    public sealed class MemoryAlignmentExceptionTests
    {
        [Fact]
    ...
    public sealed class AmoD_EnumCompletenessTests
    {
        [Theory]
    ...
    public sealed class IMemoryBusInterfaceTests
    {
        [Fact]
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // 15. MemoryUnit — constructor validation
    // ═══════════════════════════════════════════════════════════════════════════

    public sealed class MemoryUnitConstructorTests
    {
        [Fact]
    ...
    public sealed class SignExtensionBoundaryTests
    {
        private readonly FakeMemoryBus _bus = new();
        private readonly MemoryUnit _unit;
        private readonly Mem07FakeCpuState _state = new();

        public SignExtensionBoundaryTests()
        {
            _unit = new MemoryUnit(_bus);
            _state.SetReg(1, 0x100);
        }

// Phase 12: Final ISA Integration — ISA v4 Freeze and End-to-End Trace Wiring
// Covers:
...
namespace HybridCPU_ISE.Tests
{
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // ExecutionDispatcherV4 — backward compatibility (no trace sink)
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public sealed class DispatcherBackwardCompatTests
    {
        ...
        public void Execute_WithoutTraceSink_DoesNotThrow()
        {
            ...
            var result = dispatcher.Execute(P12Ir.Make(InstructionsEnum.ADD), state);
        ...
        public void Execute_WithoutBundleSerial_UsesZeroSerial()
        {
            ...
            dispatcher.Execute(P12Ir.Make(InstructionsEnum.ADD), state);
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // ExecutionDispatcherV4 — trace event stamping
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public sealed class DispatcherTraceStampingTests
    {
        ...
        public void Execute_WithBundleSerial_StampsEvent()
        {
            ...
            _dispatcher.Execute(P12Ir.Make(InstructionsEnum.ADD), _state, bundleSerial: 42, vtId: 1);
        ...
        public void Execute_StampsPipelineState()
        {
            ...
            _dispatcher.Execute(P12Ir.Make(InstructionsEnum.ADD), _state, bundleSerial: 1, vtId: 0);
        ...
        public void Execute_MultipleInstructions_EmitsOneEventEach()
        {
            ...
            _dispatcher.Execute(P12Ir.Make(InstructionsEnum.ADD), _state, 1, 0);
            _dispatcher.Execute(P12Ir.Make(InstructionsEnum.SUB), _state, 2, 0);
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // ExecutionDispatcherV4 — TraceEventKind classification per instruction class
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public sealed class DispatcherTraceKindClassificationTests
    {
        ...
        [Theory]
        [InlineData(InstructionsEnum.ADD)]       // ADD
        [InlineData(InstructionsEnum.SUB)]    // SUB
        [InlineData(InstructionsEnum.MUL)] // MUL
        [InlineData(InstructionsEnum.MULH)]
        [InlineData(InstructionsEnum.MULHU)]
        [InlineData(InstructionsEnum.MULHSU)]
        [InlineData(InstructionsEnum.DIV)]       // DIV
        ...
        public void Memory_Load_EmitsLoadExecuted(InstructionsEnum opcode)
        {
            ...
            // No real memory bus — dispatcher will throw MemoryAccessException on actual access.
        ...
        [Theory]
        [InlineData(InstructionsEnum.BEQ, true,  0, 0)]  // 0 == 0 → BEQ taken  → BranchTaken
        [InlineData(InstructionsEnum.BNE, false, 5, 5)]  // 5 == 5 → BNE not taken → BranchNotTaken
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // ExecutionDispatcherV4 — TelemetryCounters integration
    // ─────────────────────────────────────────────────────────────────────────
    ...
    public sealed class DispatcherTelemetryIntegrationTests
    {
        ...
        public void Execute_WithTelemetry_IncrementsAluInstrCount()
        {
            ...
            dispatcher.Execute(P12Ir.Make(InstructionsEnum.ADD), state, 1, vtId: 0);
        ...
        public void Execute_WithTelemetry_SinkAndTelemetryBothReceiveEvent()
        {
            ...
            dispatcher.Execute(P12Ir.Make(InstructionsEnum.ADD), state, 7, vtId: 2);
        ...
        public void Execute_WithoutTelemetry_DoesNotThrow()
        {
            ...
            // Should not throw — telemetry is optional
            var result = dispatcher.Execute(P12Ir.Make(InstructionsEnum.ADD), state);
    ...
    public sealed class IsaV4InstructionCoverageTests
    {
        ...
        public void MandatoryCoreOpcodes_ContainsAllScalarAluOpcodes()
        {
            ...
            // for legacy opcodes (ADD, SUB, ...) and v4 names for new ones.
            var expected = new[]
            {

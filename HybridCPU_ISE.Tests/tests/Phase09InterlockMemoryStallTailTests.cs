...
public sealed class InterlockMemoryStallTailTests
{
    [Theory]
    [InlineData(false)]
    [InlineData(true)]
    public void CheckPipelineHazards_WhenOlderFullSerialCarrierIsInFlight_ThenYoungerDecodeIsBlocked(
        bool carrierInMemory)
    {
        var core = new Processor.CPU_Core(0);

        bool shouldStall = core.TestCheckPipelineHazardForOlderSerializingCarrier(carrierInMemory);
        var pipeState = core.GetPipelineControl();

        Assert.True(shouldStall);
        Assert.Equal(Processor.CPU_Core.PipelineStallKind.ControlHazard, pipeState.StallReason);
    }

    [Theory]
    [InlineData(false)]
    [InlineData(true)]
    public void CheckPipelineHazards_WhenDecodedFullSerialCarrierHasOlderWork_ThenItWaitsForRetirement(
        bool olderInMemory)
    {
        var core = new Processor.CPU_Core(0);

        bool shouldStall = core.TestCheckPipelineHazardForDecodedSerializingCarrier(olderInMemory);
        var pipeState = core.GetPipelineControl();

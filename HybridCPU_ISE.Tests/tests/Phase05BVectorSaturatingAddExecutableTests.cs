...
namespace HybridCPU_ISE.Tests.VectorSaturation;

public sealed class BVectorSaturatingAddExecutableTests
{
    [Fact]
    public void VaddSat_StatusDecoderProjectionAndMaterializer_CloseSelectedPolicyBitChain()
    {
        ...
        Assert.Equal(InstructionClass.ScalarAlu, (InstructionClass)(byte)sat.InstructionClass);
        Assert.Equal(SerializationClass.Free, (SerializationClass)(byte)sat.SerializationClass);
        Assert.Equal(SlotClass.AluClass, (SlotClass)(byte)sat.Placement.RequiredSlotClass);

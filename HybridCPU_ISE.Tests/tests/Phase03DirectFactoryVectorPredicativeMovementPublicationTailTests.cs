...
public sealed class Phase03DirectFactoryVectorPredicativeMovementPublicationTailTests
{
    [Theory]
    [InlineData(InstructionsEnum.VCOMPRESS)]
    [InlineData(InstructionsEnum.VEXPAND)]
    public void DirectFactoryVectorPredicativeMovement_ProjectsSingleSurfaceCarrierBeforeManualPublication(
        InstructionsEnum opcode)
    {
        var core = new Processor.CPU_Core(0);
        core.PrepareExecutionStart(0xAE00);

        VectorPredicativeMovementMicroOp microOp = CreateDirectFactoryVectorPredicativeMovementMicroOp(opcode);
    ...
    private static VectorPredicativeMovementMicroOp CreateDirectFactoryVectorPredicativeMovementMicroOp(
        InstructionsEnum opcode)
    {
        ...
        return Assert.IsType<VectorPredicativeMovementMicroOp>(

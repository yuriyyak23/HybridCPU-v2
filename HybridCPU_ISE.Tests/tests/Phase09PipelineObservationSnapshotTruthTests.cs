...
public sealed class PipelineObservationSnapshotTruthTests
{
    [Fact]
    ...
    public void WriteBackTrace_WhenReplayObservationIsPublished_EmitsObservationSnapshotFields()
    {
        ...
        try
        {
            ...
            // The phase sample is a real CPU-owned full-state publication. Keep
            // the architectural register payload observable so this regression
            // can serve as the minimal trace control for safepoint triage.
            Assert.NotNull(evt.RegisterFile);
            Assert.Equal(YAKSys_Hybrid_CPU.Core.Registers.RenameMap.ArchRegs, evt.RegisterFile!.Length);
            Assert.Equal(core.ReadArch(0, 10), evt.RegisterFile[10]);

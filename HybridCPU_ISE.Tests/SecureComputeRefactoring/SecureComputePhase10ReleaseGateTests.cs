...
public sealed class SecureComputePhase10ReleaseGateTests
{
    ...
    [Fact]
    public void Plans_RecordReleaseGateStatusAndHardeningInvariants()
    {
    ...
    public void SecureComputeActivationPlan_RecordsPhase15MigrationOutputManifestClassificationClosureProof()
    {
        ...
        string index = ReadSecureComputeActivationPlan("00_securecompute_activation_refactoring_index.md");
        string matrix = ReadSecureComputeActivationPlan("01_current_state_and_gap_matrix.md");
        string phase15 = ReadSecureComputeActivationPlan("15_secure_migration_checkpoint_restore_plan.md");
        string conformance = ReadSecureComputeActivationPlan("21_conformance_negative_positive_test_matrix.md");
        string releaseGate = ReadSecureComputeActivationPlan("22_limited_securecompute_release_gate.md");
        string backlog = ReadSecureComputeActivationPlan("23_open_decision_backlog.md");

        Assert.Contains("SecureOutputManifestEntryKind", manifestSource);
        Assert.Contains("InternalBackendResult", manifestSource);
        Assert.Contains("InternalCompletionRecord", manifestSource);
        Assert.Contains("GuestVisibleOutput", manifestSource);
        Assert.Contains("RetireVisibleState", manifestSource);
        Assert.Contains("RecomputedAfterRestoreState", manifestSource);
        Assert.Contains("DeniedHostOwnedEvidence", manifestSource);
        Assert.Contains("DeniedVmcsProjectionAuthority", manifestSource);
        Assert.Contains("DeniedCompatibilityMetadataAuthority", manifestSource);
        Assert.Contains("DeniedRawSecret", manifestSource);
        Assert.Contains("DeniedActiveHostPointer", manifestSource);
        Assert.Contains("CreatesRuntimeAuthority: false", manifestSource);
        Assert.Contains("CreatesCompletionPublicationAuthority: false", manifestSource);
        Assert.Contains("CreatesRetirePublicationAuthority: false", manifestSource);

        Assert.Contains("DeniedPrivateMemoryWithoutSealedEncryptedContract", migrationSource);
        Assert.Contains("DeniedRawSealingKey", migrationSource);
        Assert.Contains("DeniedCompatibilityProjectionAuthority", migrationSource);

        Assert.Contains("OutputManifestClassification_RejectsForbiddenAuthorityPayloadClasses", phase15Tests);
        Assert.Contains("InternalBackendResultAndCompletionRecord_AreManifestOnlyNotCheckpointOrRestoreAuthority", phase15Tests);
        Assert.Contains("CompleteOutputManifest_IsClassificationEvidenceOnlyAndCreatesNoPublicationAuthority", phase15Tests);
        Assert.Contains("RecomputedAfterRestoreState_RequiresRestoreValidationProof", phase15Tests);
        ...
        Assert.DoesNotContain("IVmcsManager", manifestSource);
        Assert.DoesNotContain("VmxCaps", manifestSource);
        Assert.DoesNotContain("VmxExecutionUnit", manifestSource);
        Assert.DoesNotContain("VMREAD", manifestSource);
        Assert.DoesNotContain("VMWRITE", manifestSource);
        Assert.DoesNotContain("BackendExecutionAuthorized: true", manifestSource);
        Assert.DoesNotContain("CompletionPublicationAuthorized: true", manifestSource);
        Assert.DoesNotContain("RetirePublicationAuthorized: true", manifestSource);
        Assert.DoesNotContain("SecureBackendExecutionRequest", manifestSource);
        Assert.DoesNotContain("SecureBackendExecutionDecision", manifestSource);
        Assert.DoesNotContain("TrapCompletionRouteDescriptor", manifestSource);
        Assert.DoesNotContain("TrapCompletionRouteService", manifestSource);
        Assert.DoesNotContain("SecureComputeControlledEmission", manifestSource);
    }
    ...
    public void SecureComputeActivationPlan_RecordsPhase16DebugAttestationVisibilityClosureProof()
    {
        ...
        string index = ReadSecureComputeActivationPlan("00_securecompute_activation_refactoring_index.md");
        string matrix = ReadSecureComputeActivationPlan("01_current_state_and_gap_matrix.md");
        string phase16 = ReadSecureComputeActivationPlan("16_secure_debug_attestation_api_plan.md");
        string phase20 = ReadSecureComputeActivationPlan("20_positive_secure_runtime_execution_activation_plan.md");
        string conformance = ReadSecureComputeActivationPlan("21_conformance_negative_positive_test_matrix.md");
        string releaseGate = ReadSecureComputeActivationPlan("22_limited_securecompute_release_gate.md");
        string backlog = ReadSecureComputeActivationPlan("23_open_decision_backlog.md");

        Assert.Contains("SecureDebugAttestationQueryKind", visibilitySource);
        Assert.Contains("DebugTrace", visibilitySource);
        Assert.Contains("AttestationReport", visibilitySource);
        Assert.Contains("TelemetrySnapshot", visibilitySource);
        Assert.Contains("HostInspectionMetadata", visibilitySource);
        Assert.Contains("CompatibilityAliasEvidence", visibilitySource);
        Assert.Contains("DeniedMigrationAuthority", visibilitySource);
        Assert.Contains("DeniedVmreadAuthority", visibilitySource);
        Assert.Contains("DeniedActivationEvidence", visibilitySource);
        Assert.Contains("DeniedBackendOwnerProof", visibilitySource);
        Assert.Contains("DeniedPrivateMemoryInspection", visibilitySource);
        Assert.Contains("DeniedCompletionPublication", visibilitySource);
        Assert.Contains("DeniedRetirePublication", visibilitySource);
        Assert.Contains("CreatesRuntimeAuthority: false", visibilitySource);
        Assert.Contains("CreatesVmreadAuthority: false", visibilitySource);
        Assert.Contains("CreatesMigrationAuthority: false", visibilitySource);
        Assert.Contains("CreatesActivationEvidence: false", visibilitySource);
        Assert.Contains("CreatesBackendOwnerProof: false", visibilitySource);
        Assert.Contains("CompletionPublicationAuthorized: false", visibilitySource);
        Assert.Contains("RetirePublicationAuthorized: false", visibilitySource);

        Assert.Contains("DebugTraceVisibility_IsDebugOnlyAndCreatesNoAuthority", visibilityTests);
        Assert.Contains("DebugTrace_CannotBecomeMigrationPayloadGuestStateCompletionOrRetire", visibilityTests);
        Assert.Contains("AttestationReport_CanBeGuestVisibleOnlyUnderEvidencePolicyAndNeverVmreadOrActivationAuthority", visibilityTests);
        Assert.Contains("TelemetrySnapshot_IsHostOnlyAndCannotSatisfyBackendOwnerProof", visibilityTests);
        Assert.Contains("HostInspectionMetadata_CannotBypassPrivateMemoryPolicy", visibilityTests);
        ...
        Assert.DoesNotContain("IVmcsManager", visibilitySource);
        Assert.DoesNotContain("VmxCaps", visibilitySource);
        Assert.DoesNotContain("VmxExecutionUnit", visibilitySource);
        Assert.DoesNotContain("VMREAD", visibilitySource);
        Assert.DoesNotContain("VMWRITE", visibilitySource);
        Assert.DoesNotContain("BackendExecutionAuthorized: true", visibilitySource);
        Assert.DoesNotContain("CompletionPublicationAuthorized: true", visibilitySource);
        Assert.DoesNotContain("RetirePublicationAuthorized: true", visibilitySource);
        Assert.DoesNotContain("SecureBackendExecutionRequest", visibilitySource);
        Assert.DoesNotContain("SecureBackendExecutionDecision", visibilitySource);
        Assert.DoesNotContain("SecureComputeControlledEmission", visibilitySource);
    }
    ...
    [Fact]
    public void SecureComputeActivationPlan_RecordsExternalAuditRisksWithLocalSourceProof()
    {
        string toolchain = ReadProjectSource(
            "global.json",
        ...
        string ownerProcess = ReadSecureComputeActivationPlan("03_owner_specific_rfc_adr_process.md");
        string phase13 = ReadSecureComputeActivationPlan("13_secure_hypercall_backend_owner_rfc.md");
        string phase14 = ReadSecureComputeActivationPlan("14_secure_completion_retire_publication_plan.md");
        string phase15 = ReadSecureComputeActivationPlan("15_secure_migration_checkpoint_restore_plan.md");
        string phase17 = ReadSecureComputeActivationPlan("17_secure_vmx_boundary_zero_authority_plan.md");
        string phase19 = ReadSecureComputeActivationPlan("19_compiler_no_emission_to_controlled_emission_gate.md");
        string phase20 = ReadSecureComputeActivationPlan("20_positive_secure_runtime_execution_activation_plan.md");
        string conformance = ReadSecureComputeActivationPlan("21_conformance_negative_positive_test_matrix.md");
        string release = ReadSecureComputeActivationPlan("22_limited_securecompute_release_gate.md");
        string backlog = ReadSecureComputeActivationPlan("23_open_decision_backlog.md");

        Assert.Contains("\"version\": \"10.0.201\"", toolchain);
        Assert.Contains("DefineTestSupport", toolchain);
        Assert.Contains("Assert.Equal((ushort)259, IsaOpcodeValues.VMCALL)", vmcallReadiness);
        Assert.Contains("Assert.DoesNotContain(\"0x10\", secureHypercallDescriptorSource)", vmcallReadiness);
        Assert.DoesNotContain("0x10", hypercallDescriptor);
        Assert.Contains("AllowedProofOnlyNoExecution", secureBackend);
        Assert.Contains("BackendExecutionAuthorized: false", secureBackend);
        Assert.Contains("AllowedAdmittedDenied", secureBackend);
        Assert.Contains("DeniedBackendSuccessClosed", secureBackend);
        Assert.Contains("CompletionPublicationAuthorized: true", neutralPublication);
        Assert.Contains("RetirePublicationAuthorized: true", neutralPublication);

        Assert.Contains("External architecture audit integration, 2026-06-12", index);
        Assert.Contains("production SecureCompute activation not supported; readiness/policy/projection only", index);
        Assert.Contains("Current P0 production-activation blockers", matrix);
        Assert.Contains("Scans must be owner/path scoped", guards);
        Assert.Contains("Audit-Derived Rollback Gate", guards);
        Assert.Contains("transport opcode", ownerProcess);
        Assert.Contains("Test fixture IDs are not ABI assignments", ownerProcess);
        ...
        Assert.Contains("one-path output manifest coverage", phase15);
        Assert.Contains("trap projection and compatibility completion do not become SecureCompute service identity", phase17);
        Assert.Contains("explicit no-compiler-change decision", phase19);
    ...
    {
        string legacyPath = Path.Combine(SecureComputeRoot(), "Plan", fileName);

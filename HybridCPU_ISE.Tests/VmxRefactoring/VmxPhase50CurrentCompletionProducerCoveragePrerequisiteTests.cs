...
public sealed class VmxPhase50CurrentCompletionProducerCoveragePrerequisiteTests
{
    ...
    [Fact]
    public void CpuTranslationProducers_AreSeparateFromRetireTrapAndCarryDistinctExactSemantics()
    {
        ...
        string cpuTranslation = ActiveVmxConformanceHelpers.ReadProjectSource(
            "CloseToHSL/Core/Pipeline/ExecutionFlow/Faults/CPU_Core.PipelineExecution.CpuTranslation.cs");

        Assert.Equal(3, Count(state, ".RegisterProducer("));
        Assert.Contains("CanonicalPipelineTrapEntryProducer", state);
        Assert.Contains("CanonicalCpuInstructionTranslationFaultProducer", state);
        Assert.Contains("CanonicalCpuSecondStageTranslationFaultProducer", state);
        Assert.Contains("CommitAtCanonicalPreciseFaultBoundary", cpuTranslation);
        Assert.Contains("NeutralFaultAddressSemantic.VirtualAddress", cpuTranslation);
        Assert.Contains("NeutralFaultAddressSemantic.GuestPhysicalAddress", cpuTranslation);
        Assert.Contains("NeutralFaultAuxiliarySemantic.SecondStageTranslationViolation", cpuTranslation);
        Assert.DoesNotContain("NestedTranslationResult", cpuTranslation);
        Assert.DoesNotContain("TranslationViolationInfo", cpuTranslation);

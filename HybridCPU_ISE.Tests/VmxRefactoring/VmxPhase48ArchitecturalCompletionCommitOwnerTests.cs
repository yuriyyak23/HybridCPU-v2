...
public sealed class VmxPhase48ArchitecturalCompletionCommitOwnerTests
{
    ...
    [Fact]
    public void ProductionSeam_PrevalidatesBeforeLateEffectsAndCommitsBeforeVisibilityCertificate()
    {
        string repositoryRoot =
            VmxDocumentationMigrationClaimHygieneTests.FindRepositoryRoot();
        ...
        int certificate = retire.IndexOf(
            ...
            StringComparison.Ordinal);
        Assert.True(
            finalize >= 0 && finalize < prepare && prepare < late &&
            late < commit && commit < certificate);

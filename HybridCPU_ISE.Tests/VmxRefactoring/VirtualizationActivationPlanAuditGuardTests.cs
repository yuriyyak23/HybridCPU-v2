...
public sealed class VirtualizationActivationPlanAuditGuardTests
{
    ...
    public void ActivationPlanDirectory_ContainsRequiredDocsAndStaysReadinessOnly()
    {
        ...
        [
            ...
            "47_current_completion_vmread_scalar_delivery_e0_blocked.md",
    ...
    public void CurrentHeadReconciliation_UsesManifestAndStrictAuthorityAndGovernanceOrder()
    {
        ...
        string currentEvidence = File.ReadAllText(Path.Combine(
            planRoot,
            "evidence",
            "2026-08-11-pr-j-worktree-readiness-evidence.json"));
        Assert.Contains("\"file_count\": 39", currentEvidence);
        Assert.Contains("\"aggregate_sha256\": \"a97ea6f9cb3fa84f1058ab93465bfcb5c2bf9d8297b5ce10da34357fa58ac7a3\"", currentEvidence);
    ...
    public void CurrentActivationStatus_IsCanonicalAndCurrentSectionsCannotDrift()
    {
        ...
        Assert.Equal("ClosedExactProbeControlledEmissionDefaultDisabled",
            gates.GetProperty("CompilerGate").GetString());
        Assert.Equal("ClosedProductionExactProbeProfile", gates.GetProperty("ReleaseGate").GetString());
        Assert.Equal("Denied", gates.GetProperty("BroadActivation").GetString());
        Assert.Equal("ClosedExactProfileProductionReachableDefaultDisabled", root.GetProperty("ExactActivation").GetString());
        Assert.Equal("LimitedProductionReleaseOfProbeNoStateV1Only", root.GetProperty("ReleaseClaim").GetString());
        Assert.Equal("None", root.GetProperty("NextOpenPool").GetString());
        Assert.Equal(
        ...
        Assert.Contains("compiler gate remains closed by default", next, StringComparison.OrdinalIgnoreCase);

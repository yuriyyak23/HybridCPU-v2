...
public sealed class VmxPhase49CurrentCompletionVmReadScalarDeliveryE0Tests
{
    ...
    [Fact]
    public void ProductionRegistrations_RemainExactAndDoNotProveFourFieldCoverage()
    {
        ...
        Assert.Contains("NeutralArchitecturalCompletionClass.TranslationFault", state);
        Assert.Contains("AllowsQualification: true", state);
        Assert.Contains("NeutralFaultAuxiliarySemantic.TranslationFault", state);
        Assert.Contains("\"CanonicalCpuSecondStageTranslationFaultProducer\"", state);
        Assert.Contains("NeutralFaultAddressSemantic.GuestPhysicalAddress", state);
        Assert.Contains("NeutralFaultAuxiliarySemantic.SecondStageTranslationViolation", state);

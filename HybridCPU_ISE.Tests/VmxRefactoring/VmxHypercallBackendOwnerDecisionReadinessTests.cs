...
public sealed class VmxHypercallBackendOwnerDecisionReadinessTests
{
    ...
    [Fact]
    public void VmCallAbiInventory_ProvesOpcodeAndRegisterFormButDoesNotOwnSecureComputeIdentifiers()
    {
        Assert.Equal((ushort)259, IsaOpcodeValues.VMCALL);

        bool foundVmCallAlias = false;
        foreach (VmxOpcodeAlias alias in VmxOpcodeAliasSet.FrozenAliases)
        {
            if (alias.Opcode != IsaOpcodeValues.VMCALL)
            {
                continue;
            }

            foundVmCallAlias = true;
            Assert.Equal("VMCALL", alias.Name);
            Assert.Equal(VmxOpcodeAliasKind.PointerAndCall, alias.Kind);
        }

        Assert.True(foundVmCallAlias);

        VmxInstructionPayload payload = VmxInstructionPayload.FromDecodedRegisters(
            IsaOpcodeValues.VMCALL,
            rd: 7,
            rs1: 2,
            rs2: 3);

        Assert.Equal(VmxOperandForm.HypercallLeafAndDescriptor, payload.OperandForm);
        Assert.Equal((ushort)2, payload.ExitQualification.Leaf);
        Assert.Equal(3UL, payload.ExitQualification.Descriptor);
        Assert.Equal(VmxInvalidationScope.None, payload.ExitQualification.Scope);
        Assert.NotEqual(
            (ulong)payload.ExitQualification.Leaf,
            SecureHypercallBackendOwnerAbiRegistry.DecodedLeaf.Value);
        Assert.NotEqual(
            (ulong)payload.ExitQualification.Leaf,
            SecureHypercallBackendOwnerAbiRegistry.ServiceId.Value);
        Assert.NotEqual(
            (ulong)payload.ExitQualification.Leaf,
            SecureHypercallBackendOwnerAbiRegistry.OwnerId.Value);

        Assert.False(YAKSys_Hybrid_CPU.Arch.VmxSpecTable.VmxV2DraftsAreCompatibilityTargets);
        Assert.DoesNotContain(
            YAKSys_Hybrid_CPU.Arch.VmxSpecTable.Vmx8Opcodes,
            spec => spec.Opcode == IsaOpcodeValues.VMCALL);

        Assert.Equal(1, (ushort)VmxFunctionLeaf.CapabilityQuery);
        Assert.Equal(7, (ushort)VmxFunctionLeaf.Lane7QueryCaps);
        ...
        Assert.DoesNotContain("DecodeHypercallLeaf", vmCallAbiSource);
        Assert.Contains("IReadOnlyList<ulong> AllowedHypercallIds", secureHypercallDescriptorSource);
        Assert.DoesNotContain("0x10", secureHypercallDescriptorSource);
        Assert.Contains("ADR-SC-HYP-BACKEND-OWNER", secureHypercallRegistrySource);
        Assert.DoesNotContain("VmExitReason", secureHypercallRegistrySource);
        Assert.DoesNotContain("VmxCaps", secureHypercallRegistrySource);
        Assert.DoesNotContain("Vmcs", secureHypercallRegistrySource);
        Assert.DoesNotContain("VMREAD", secureHypercallRegistrySource);
        Assert.DoesNotContain("VMWRITE", secureHypercallRegistrySource);
    }

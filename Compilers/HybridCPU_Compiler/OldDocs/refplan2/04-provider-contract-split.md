...
Complete. `IContourProductionLoweringProvider` is separate from the shell
contract. The registry resolves only explicitly enabled exact-contour providers
for scalar, load/store, branch/control, scoped vector transfer, DSC lane6, and
L7-SDC lane7; shell resolution remains the compatibility/fail-closed path.

...
Complete. `NativeVliwScalarProductionProvider` is registered only through the
explicit profile path and is covered by `CompilerPhase06...Tests`; its package
result remains runtime-authority-pending and no caller migration was performed.
...
- `ProductionProfile.Mode == ExplicitlyEnabled`.

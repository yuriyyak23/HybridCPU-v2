...
Complete for the bounded direct-transfer subset only.
`StreamEngineVectorDirectTransferProductionProvider` accepts contiguous
`VLOAD`/`VSTORE` packages under its dedicated gate. General vector/stream and
MatrixTile helper surfaces remain outside this provider.

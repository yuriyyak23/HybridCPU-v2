...
| 02 authority taxonomy | `HybridCPU_ISE/CloseToHSL/Core/Contracts/CompilerContract.cs` | Runtime bridge already defines typed-slot policy modes, ingress actions, contract version and fail-closed mismatch check. | Phase 02/06 must reuse runtime-owned contract semantics without importing runtime authority into compiler. | Medium: duplicated policy vocabulary can diverge. |

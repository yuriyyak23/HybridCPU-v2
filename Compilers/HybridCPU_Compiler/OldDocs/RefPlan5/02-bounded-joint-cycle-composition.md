...
- Exact-oracle metrics may be absent until Phase 15, but all frozen small cases must retain enough trace for later optimality-gap comparison; production never requires that backend.

## Audit correction: downstream ownership

- Phase 12, not the schema/core phases, is where scheduling regions become EBB/guarded/hyperblock-capable after Phase 07 pressure information exists.
- Phase 15 is the exact-oracle/optimality-gap owner; Phase 02 remains production-only and must not acquire an SMT dependency.
- Any future consumer of the Phase 02 placement witness must validate the same W=8 membership/assignment; a downstream phase may reject or reschedule at its own explicit boundary, but `BundleFormer` cannot silently repair the chosen cycle.
...
Phase 03 enriches feasibility/cost with register/PRF/bank/channel/certificate resources. Phase 12 enlarges the scheduling region only after Phase 07 pressure modelling and the Phase 08A target contract are available.

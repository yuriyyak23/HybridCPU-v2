...
namespace YAKSys_Hybrid_CPU
{
    public partial struct Processor
    {
        public sealed partial class CPU_Core
        {
            ...
            /// </summary>
            public class LiveCpuStateAdapter : ICanonicalCpuState, IVectorExceptionCounterState
            {
                ...
                private readonly CommitMap            _commitMap;
                    ...
                    ulong instructionsRetired = 0)
                {
                    ...
                    _commitMap = commitMap ?? throw new ArgumentNullException(nameof(commitMap));
                ...
                public void SetVectorExceptionCounters(
                    ulong overflowCount,
                    ...
                    ulong inexactCount)
                {
                    _exceptionStatus.OverflowCount = checked((uint)overflowCount);
                    _exceptionStatus.UnderflowCount = checked((uint)underflowCount);
                    _exceptionStatus.DivByZeroCount = checked((uint)divByZeroCount);
                    _exceptionStatus.InvalidOpCount = checked((uint)invalidOpCount);
                    _exceptionStatus.InexactCount = checked((uint)inexactCount);
                }
                ...
                private ulong ReadArchitecturalRegister(int vtId, int regId)
                {
                    ...
                    int committedPhysReg = _commitMap.Lookup(normalizedVtId, regId);

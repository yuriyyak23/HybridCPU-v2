    ...
    public partial struct Processor
    {
        public sealed partial class CPU_Core
        {
            ...
            private static Core.DecodedBundleRuntimeState BuildCanonicalDecodedBundleState(
                ...
                Core.Decoder.CanonicalDecodeContext? decodeContext = null)
            {
                ...
                Core.Decoder.DecodedInstructionBundle canonicalBundle =
                    decoderFrontend.DecodeInstructionBundle(
                        ...
                        bundleSerial,
                        decodeContext);
                return BuildCanonicalDecodedBundleState(rawSlots, canonicalBundle);
            }
            ...
            /// <summary>
            /// Rehydrates fresh compatibility carriers from one validated
            /// immutable semantic entry without invoking the decoder. Every
            /// call creates a new transport/materialization surface.
            /// </summary>
            private static Core.DecodedBundleRuntimeState BuildReplayDecodedBundleState(
                Core.Decoder.ReplayEntry replayEntry,
                ulong requestedPc)
            {
                ...
                if (!replayEntry.HasValidFrozenContent())
                {
                    throw new InvalidOperationException(
                        "Replay serving requires a valid frozen semantic entry.");
                }

                Core.Decoder.CanonicalBundle canonicalBundle =
                    replayEntry.CanonicalBundle;
                if (canonicalBundle.BundleAddress != requestedPc)
                {
                    throw new InvalidOperationException(
                        "The bounded loop replay nomination must match the frozen bundle placement.");
                }

                Core.BundleMetadata bundleMetadata =
                    canonicalBundle.BundleSideband.Deserialize<Core.BundleMetadata>() ??
                    throw new InvalidOperationException(
                        "Replay serving could not restore frozen bundle sideband.");
                var rawSlots = new VLIW_Instruction[Core.BundleMetadata.BundleSlotCount];
                var decodedSlots =
                    new List<Core.Decoder.DecodedInstruction>(
                        Core.BundleMetadata.BundleSlotCount);
                for (int slotIndex = 0;
                     slotIndex < Core.BundleMetadata.BundleSlotCount;
                     slotIndex++)
                {
                    Core.Decoder.CanonicalDecodedInstruction canonicalSlot =
                        canonicalBundle.GetSlot(slotIndex);
                    if (!rawSlots[slotIndex].TryReadBytes(
                            canonicalSlot.RawSlot.Content.Span))
                    {
                        throw new InvalidOperationException(
                            $"Replay raw slot {slotIndex} is not a complete VLIW carrier.");
                    }

                    InstructionSlotMetadata slotMetadata =
                        canonicalSlot.SlotSideband.Deserialize<InstructionSlotMetadata>();
                    if (!canonicalSlot.IsOccupied)
                    {
                        decodedSlots.Add(
                            Core.Decoder.DecodedInstruction.CreateEmpty(
                                slotIndex,
                                slotMetadata));
                        continue;
                    }

                    Core.Pipeline.MicroOps.InstructionIR instruction =
                        Core.Decoder.CanonicalInstructionIrCompatibilityProjector
                            .ProjectReplayInstruction(
                                in rawSlots[slotIndex],
                                canonicalSlot,
                                slotMetadata);
                    decodedSlots.Add(
                        Core.Decoder.DecodedInstruction.CreateOccupied(
                            slotIndex,
                            instruction,
                            slotMetadata));
                }

                var replayBundle = new Core.Decoder.DecodedInstructionBundle(
                    requestedPc,
                    canonicalBundle.BundleSerial,
                    decodedSlots,
                    bundleMetadata,
                    canonicalBundle: canonicalBundle);
                return BuildCanonicalDecodedBundleState(rawSlots, replayBundle);
            }
            ...
            private void DecodeFullBundle()
            {
                ...
                if (pipeIF.DecodeContext == null ||
                    !pipeIF.DecodeContext.IsReplayEligible)
                {
                    ResetCurrentDecodedBundleSlotCarrierState(pipeIF.PC);
                    return;
                }
                ...
                try
                {
                    var canonicalState =
                        BuildCanonicalDecodedBundleState(
                            ...
                            pipeIF.PC,
                            bundleSerial: 0,
                            decodeContext: pipeIF.DecodeContext);
                    ObserveReplaySemanticShadow(
                        canonicalState.CanonicalDecode.CanonicalBundle);
                ...
                out VLIW_Instruction instruction)
            {
                ...
                instruction = new VLIW_Instruction
                {
                    ...
                    Word0 = System.Buffers.Binary.BinaryPrimitives.ReadUInt64LittleEndian(source.Slice(offset, 8)),
                    Word1 = System.Buffers.Binary.BinaryPrimitives.ReadUInt64LittleEndian(source.Slice(offset + 8, 8)),
                    Word2 = System.Buffers.Binary.BinaryPrimitives.ReadUInt64LittleEndian(source.Slice(offset + 16, 8)),
                    Word3 = System.Buffers.Binary.BinaryPrimitives.ReadUInt64LittleEndian(source.Slice(offset + 24, 8))
                };
                return true;
            }

...
namespace YAKSys_Hybrid_CPU.Core
{
    public static partial class InstructionRegistry
    {
        ...
        private static void RejectScalarUnaryRegisterFormAlias(
            in DecoderContext ctx)
        {
            if (unchecked((ushort)ctx.OpCode) is not (
                    OpcodeValues.SEXT_W or
                    OpcodeValues.ZEXT_W or
                    OpcodeValues.CLZ or
                    OpcodeValues.CTZ or
                    OpcodeValues.CPOP or
                    OpcodeValues.SEXT_B or
                    OpcodeValues.SEXT_H or
                    OpcodeValues.ZEXT_H or
                    OpcodeValues.REV8 or
                    OpcodeValues.BREV8))
            {
                return;
            }
        ...
        {
            if (unchecked((ushort)ctx.OpCode) is not (
                    OpcodeValues.SH1ADD or
                    OpcodeValues.SH2ADD or
                    OpcodeValues.SH3ADD or
                    OpcodeValues.ADD_UW or
                    OpcodeValues.SH1ADD_UW or
                    OpcodeValues.SH2ADD_UW or
                    OpcodeValues.SH3ADD_UW))
            {
                return;
            }
            ...
            {
                throw new DecodeProjectionFaultException(
                    $"Scalar address-generation opcode {OpcodeRegistry.GetMnemonicOrHex(ctx.OpCode)} requires canonical Immediate=0. " +
                    "Immediate-form aliasing is not accepted.");
            }
        }

        private static ushort GetScalarAddressGenerationImmediate(
            in DecoderContext ctx)
        {
            if (unchecked((ushort)ctx.OpCode) is not OpcodeValues.SLLI_UW)
            {
                return 0;
            }

            string opcodeName = OpcodeRegistry.GetMnemonicOrHex(ctx.OpCode);
            if (ctx.Reg3ID != 0)
            {
                throw new DecodeProjectionFaultException(
                    $"Scalar address-generation immediate opcode {opcodeName} requires canonical Word1=(rd, rs1, x0). " +
                    "Register-form aliasing through rs2 is not accepted.");
            }

            ushort immediate = GetRequiredDecoderImmediate(
                in ctx,
                $"Scalar address-generation immediate opcode {opcodeName}");

            if (immediate > 0x3F)
            {
                throw new DecodeProjectionFaultException(
                    $"Scalar address-generation immediate opcode {opcodeName} requires imm6 Immediate in [0, 63]. " +
        ...
        {
            if (unchecked((ushort)ctx.OpCode) is not (
                    OpcodeValues.CLMUL or
                    OpcodeValues.CLMULH or
                    OpcodeValues.CLMULR))
            {
                return;
            }
            ...
            {
                throw new DecodeProjectionFaultException(
                    $"Scalar carry-less opcode {OpcodeRegistry.GetMnemonicOrHex(ctx.OpCode)} requires canonical Immediate=0. " +
                    "Immediate-form aliasing is not accepted.");
            }
        }
        ...
        private static void RejectScalarRotateRegisterFormAlias(
            in DecoderContext ctx)
        {
            if (unchecked((ushort)ctx.OpCode) is not (OpcodeValues.ROL or OpcodeValues.ROR))
            {
                return;
            }
            ...
            {
                throw new DecodeProjectionFaultException(
                    $"Scalar rotate opcode {OpcodeRegistry.GetMnemonicOrHex(ctx.OpCode)} requires canonical Immediate=0. " +
                    "Immediate-form aliasing is not accepted.");
            }
        }

        private static ushort GetScalarRotateImmediate(
            in DecoderContext ctx)
        {
            if (unchecked((ushort)ctx.OpCode) is not (OpcodeValues.ROLI or OpcodeValues.RORI))
            {
                return 0;
            }
            ...
            {
                throw new DecodeProjectionFaultException(
                    $"Scalar rotate-immediate opcode {opcodeName} requires canonical Word1=(rd, rs1, x0). " +
                    "Register-form aliasing through rs2 is not accepted.");
            }

            ushort immediate = GetRequiredDecoderImmediate(
                in ctx,
                $"Scalar rotate-immediate opcode {opcodeName}");

            if (immediate > 0x3F)
            {
                throw new DecodeProjectionFaultException(
                    $"Scalar rotate-immediate opcode {opcodeName} requires imm6 Immediate in [0, 63]. " +
                    "Out-of-range immediate payloads are not accepted.");
            }
            ...
            in DecoderContext ctx)
        {
            ...
            {
                throw new DecodeProjectionFaultException(
                    $"Scalar boolean-invert opcode {OpcodeRegistry.GetMnemonicOrHex(ctx.OpCode)} requires canonical Immediate=0. " +
                "Immediate-form aliasing is not accepted.");
            }
        }
        ...
        {
            if (unchecked((ushort)ctx.OpCode) is not (
                    OpcodeValues.BSET or
                    OpcodeValues.BCLR or
                    OpcodeValues.BINV or
                    OpcodeValues.BEXT))
            {
                return;
            }
            ...
            {
                throw new DecodeProjectionFaultException(
                    $"Scalar bitfield opcode {OpcodeRegistry.GetMnemonicOrHex(ctx.OpCode)} requires canonical Immediate=0. " +
                    "Immediate-form aliasing is not accepted.");
            }
        }

        private static ushort GetScalarBitfieldImmediate(
            in DecoderContext ctx)
        {
            if (unchecked((ushort)ctx.OpCode) is not (
                    OpcodeValues.BSETI or
                    OpcodeValues.BCLRI or
                    OpcodeValues.BINVI or
                    OpcodeValues.BEXTI))
            {
                return 0;
            }
            ...
            {
                throw new DecodeProjectionFaultException(
                    $"Scalar bitfield-immediate opcode {opcodeName} requires canonical Word1=(rd, rs1, x0). " +
                    "Register-form aliasing through rs2 is not accepted.");
            }

            ushort immediate = GetRequiredDecoderImmediate(
                in ctx,
                $"Scalar bitfield-immediate opcode {opcodeName}");

            if (immediate > 0x3F)
            {
                throw new DecodeProjectionFaultException(
                    $"Scalar bitfield-immediate opcode {opcodeName} requires imm6 Immediate in [0, 63]. " +
                    "Out-of-range immediate payloads are not accepted.");
            }
            ...
            in DecoderContext ctx)
        {
            ...
            {
                throw new DecodeProjectionFaultException(
                    $"Scalar min/max opcode {OpcodeRegistry.GetMnemonicOrHex(ctx.OpCode)} requires canonical Immediate=0. " +
                    "Immediate-form aliasing is not accepted.");
            }
        }
        ...
        private static void RejectScalarZeroingSelectRegisterFormAlias(
            in DecoderContext ctx)
        {
            if (unchecked((ushort)ctx.OpCode) is not OpcodeValues.CZERO_NEZ)
            {
                return;
            }
            ...
            {
                throw new DecodeProjectionFaultException(
                    $"Scalar zeroing-select opcode {OpcodeRegistry.GetMnemonicOrHex(ctx.OpCode)} requires canonical Immediate=0. " +
                    "Immediate-form aliasing is not accepted.");
            }
        }
            ...
            out IReadOnlyList<int> readRegisters)
        {
            switch (opcode)
            {
                ...
                case OpcodeValues.ACCEL_QUERY_CAPS:
                case OpcodeValues.ACCEL_SUBMIT:
                    readRegisters = Array.Empty<int>();
                    return true;
                case OpcodeValues.ACCEL_POLL:
                case OpcodeValues.ACCEL_STATUS:
                case OpcodeValues.ACCEL_WAIT:
                case OpcodeValues.ACCEL_CANCEL:
                case OpcodeValues.ACCEL_FENCE:
                    readRegisters = BuildArchitecturalReadRegisterList(rs1);
                    return true;
                default:
                    readRegisters = Array.Empty<int>();
            ...
            out VmxOperationKind operationKind)
        {
            operationKind = opcode switch
            {
                ...
                OpcodeValues.VMPTRST => VmxOperationKind.VmPtrSt,
                OpcodeValues.VMCALL => VmxOperationKind.VmCall,
                OpcodeValues.INVEPT => VmxOperationKind.Invept,
                OpcodeValues.INVVPID => VmxOperationKind.Invvpid,
                OpcodeValues.VMFUNC => VmxOperationKind.VmFunc,
                OpcodeValues.VMSAVEX => VmxOperationKind.VmSaveX,
                OpcodeValues.VMRESTX => VmxOperationKind.VmRestX,
        ...
        private static void RegisterScalarAddressGenerationRegisterOp(uint opCode)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                RejectScalarAddressGenerationRegisterFormAlias(in ctx);
        ...
        private static void RegisterScalarAddressGenerationImmediateOp(uint opCode)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                ushort immediate = GetScalarAddressGenerationImmediate(in ctx);
        ...
        private static void RegisterScalarCarryLessRegisterOp(uint opCode)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                RejectScalarCarryLessRegisterFormAlias(in ctx);
        ...
        private static void RegisterScalarRotateRegisterOp(uint opCode)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                RejectScalarRotateRegisterFormAlias(in ctx);
        ...
        private static void RegisterScalarRotateImmediateOp(uint opCode)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                ushort immediate = GetScalarRotateImmediate(in ctx);
        ...
        private static void RegisterScalarBooleanInvertRegisterOp(uint opCode)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                RejectScalarBooleanInvertRegisterFormAlias(in ctx);
        ...
        private static void RegisterScalarBitfieldRegisterOp(uint opCode)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                RejectScalarBitfieldRegisterFormAlias(in ctx);
        ...
        private static void RegisterScalarBitfieldImmediateOp(uint opCode)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                ushort immediate = GetScalarBitfieldImmediate(in ctx);
        ...
        private static void RegisterScalarMinMaxRegisterOp(uint opCode)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                RejectScalarMinMaxRegisterFormAlias(in ctx);
        ...
        private static void RegisterScalarZeroingSelectRegisterOp(uint opCode)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                RejectScalarZeroingSelectRegisterFormAlias(in ctx);
        ...
        private static void RegisterTypedLoadOp(uint opCode)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                var op = new LoadMicroOp
                {
                    ...
                    UsesBaseRegisterAddressing = ctx.Reg2ID != VLIW_Instruction.NoReg && !ctx.HasMemoryAddress,
                    AddressDisplacement = unchecked((short)ctx.Immediate),
        ...
        /// Machine-mode privilege. Compatibility publication is routed through typed fail-closed effects
        ...
        private static void RegisterVmxOp(uint opCode)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                ...
                var op = new VmxMicroOp
                {
                    ...
                    Rs1 = rs1,
                    Rs2 = rs2,
                    WritesRegister = IsVmxRegisterWritebackOpcode(opcode) &&
                                     rd != 0 &&
                                     rd != ArchRegisterTripletEncoding.NoArchReg,
                };
        ...
        private static bool IsVmxRegisterWritebackOpcode(ushort opcode) =>
            opcode is OpcodeValues.VMREAD or OpcodeValues.VMPTRST or OpcodeValues.VMFUNC;
    }
}

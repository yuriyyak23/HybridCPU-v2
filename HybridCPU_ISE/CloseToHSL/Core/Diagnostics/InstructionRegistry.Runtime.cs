        ...
        public static MicroOp CreateMicroOp(uint opCode, DecoderContext context)
        {
            ...
            if (_factories.TryGetValue(opCode, out MicroOpFactory factory))
            {
                ...
                if (microOp != null && _descriptors.TryGetValue(opCode, out var desc))
                {
                    ...
                    {
                        bool writesRegister = desc.WritesRegister.Value;
                        if (writesRegister &&
                            microOp is ScalarALUMicroOp &&
                            microOp.DestRegID == VLIW_Instruction.NoReg)
                        {
                            // Scalar descriptors publish the opcode-level capability to write.
                            // The decoded instruction instance remains non-writing when its exact
                            // canonical destination is absent. Promoting this contour would invent
                            // a register effect that is absent from the canonical IR and leave the
                            // authoritative WriteRegisters set empty.
                            writesRegister = false;
                        }

                        microOp.WritesRegister = writesRegister;
                    }

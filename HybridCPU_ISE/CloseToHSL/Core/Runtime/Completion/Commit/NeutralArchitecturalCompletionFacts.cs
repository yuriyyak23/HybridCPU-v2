...
internal readonly record struct NeutralTranslationProvenance(
    bool IsPresent,
    ulong MemoryOperationId,
    ulong TranslationOwnerEpoch,
    ulong MemoryDomainOwnerEpoch,
    ulong AddressSpaceGeneration,
    ulong AddressSpaceIdentity)
{
    internal static NeutralTranslationProvenance Present(
        ulong memoryOperationId,
        ulong translationOwnerEpoch,
        ...
        new(
            true,
            memoryOperationId,
            translationOwnerEpoch,
            memoryDomainOwnerEpoch,
            addressSpaceGeneration,
            addressSpaceIdentity);

    internal bool HasStageOneIdentity =>
        IsPresent && MemoryOperationId != 0 && TranslationOwnerEpoch != 0;

    internal bool HasSecondStageIdentity =>
        HasStageOneIdentity && MemoryDomainOwnerEpoch != 0 &&
...
internal readonly record struct ArchitecturalCompletionCandidate(
    ...
    NeutralTranslationProvenance TranslationProvenance = default);

...
internal static class CompletionReasonQualificationVmReadDecisionValidatorV2
{
    ...
    internal const string ExpectedDecisionId =
        "D2-HV-VMREAD-COMPLETION-REASON-QUALIFICATION-0004";
    internal const string ExpectedOperationNamespace = "HybridCPU.VMREAD.ScalarDelivery.v1";
    internal const string ExpectedOperationId = "DELIVER_COMPLETION_REASON_QUALIFICATION_SCALAR_V1";
    internal const ulong ExpectedSourceOwnerId = 0x434F_4D50_4F42_5356UL;
    internal const string ExpectedDependencyContract =
        "Source=DomainCompletionObservationOwner.ExactCommittedSnapshot;Producer=CanonicalCpuInstructionTranslationFaultProducer;Reasons=AccessDenied+OwnerScopeMismatch;ReasonMapping=FrozenSecurityPolicyViolation;Qualification=ExactReasonAccessKindAccessSizeEncodingUnchanged;PairBinding=Required;Fields=ExitReason+ExitQualification;Denied=GuestPhysicalAddress+EptViolationQualification;Freshness=CompletionGeneration+RestoreGeneration+OwnerScope+Attempt+Event+Digest;Transport=ExistingE1CarrierScalarReceiptCanonicalPRFRenameWritebackRetire;Activation=DefaultDisabled;NoInferenceNoZeroFallbackNoIommuNoNestedNoGenericExceptionNoVmcsBackingNoCompletionCreationNoVmxAuthority";

    internal static VmReadScalarDeliveryDecisionValidationResultV2 ValidateSpecShape(
        VirtualizationDecisionSpecV2? spec)
    {
        if (spec is null)
            return Deny(VmReadScalarDeliveryDecisionValidationDecisionV2.DeniedMissingSpec, "Completion projection SpecV2 is missing.");
        ...
        if (!ValidateProfile(spec))
            return Deny(VmReadScalarDeliveryDecisionValidationDecisionV2.DeniedProfile, "Exact completion projection profile mismatched.");
        if (!ValidateOwnerMap(spec.OwnerMap))
            return Deny(VmReadScalarDeliveryDecisionValidationDecisionV2.DeniedOwnerMap, "Exact completion owner/value map mismatched.");
        ...
        return new(VmReadScalarDeliveryDecisionValidationDecisionV2.ExactPolicyShape,
            "Exact reason/qualification mapping is canonical governance policy; runtime composition remains unauthorized.");
    }
        ...
        VirtualizationDecisionValidationEvidenceV2? evidence)
    {
        ...
        return new(VmReadScalarDeliveryDecisionValidationDecisionV2.AcceptedPolicyObject,
            "Exact completion reason/qualification D2 accepted as compatibility-only policy.",
    ...
    private static bool ValidateProfile(VirtualizationDecisionSpecV2 spec) =>
        ...
        spec.RequiresConformanceProof && spec.ExactFieldIds is { } fields &&
        fields.SequenceEqual(Phase55CompletionReasonQualificationVmReadE0Contract.ExactFieldIds);
    ...
    private static bool ValidateOwnerMap(ImmutableArray<VirtualizationDecisionOwnerMapEntryV2> map) =>
        map.Length == 2 && map.All(entry =>
            ...
            entry.EvidenceClass.Contains("ExactCpuTranslationProducerSemanticTuple", StringComparison.Ordinal)) &&
        map.Any(entry => entry.FieldOrOperation == "VmcsField.ExitReason") &&
        map.Any(entry => entry.FieldOrOperation == "VmcsField.ExitQualification");

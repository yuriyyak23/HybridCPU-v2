...
namespace YAKSys_Hybrid_CPU.Core.Execution.DmaStreamCompute
{
    public sealed record DmaStreamComputeExecutionResult
    {
        public DmaStreamComputeExecutionResult(
            ...
            DmaStreamComputeIssueAdmissionResult? issueAdmission = null)
        {
        ...
        public DmaStreamComputeIssueAdmissionResult? IssueAdmission { get; }

        public DmaStreamComputeTokenHandle TokenHandle =>
            IssueAdmission?.Handle ?? default;

        public bool IsStoreTracked =>
            IssueAdmission?.HasAllocatedToken == true;
    ...
    {
        // Explicit helper surface retained for direct runtime tests. Production
        // MicroOp execution enters through ExecuteMaterializedMicroOpToCommitPending
        // so issue/admission owns token allocation.
            ...
            DmaStreamComputeTelemetryCounters? telemetry = null)
        {
            ...
            var token = new DmaStreamComputeToken(
                ...
                telemetry);
            DmaStreamComputeCommitResult completion =
                ExecuteTokenToCommitPending(descriptor, token, backend);
            return CreateResult(token, completion, backend);
        }

        public static DmaStreamComputeExecutionResult ExecuteMaterializedMicroOpToCommitPending(
            DmaStreamComputeMicroOp microOp,
            DmaStreamComputeTokenStore tokenStore,
            DmaStreamComputeIssueAdmissionMetadata issueMetadata,
            DmaStreamAcceleratorBackend? backend = null,
            DmaStreamComputeTelemetryCounters? telemetry = null)
        {
            ArgumentNullException.ThrowIfNull(microOp);
            ArgumentNullException.ThrowIfNull(tokenStore);

            backend ??= new DmaStreamAcceleratorBackend(Processor.MainMemory, telemetry);
            backend.AttachTelemetry(telemetry);

            DmaStreamComputeIssueAdmissionResult admission =
                tokenStore.TryAllocateAtIssueAdmission(
                    DmaStreamComputeIssueAdmissionRequest.ForLane6(
                        microOp,
                        issueMetadata,
                        telemetry));

            if (!admission.IsAccepted || admission.Token is null)
            {
                throw new InvalidOperationException(
                    $"DmaStreamCompute materialized lane6 execution rejected before token-owned runtime entry: {admission.Message}");
            }

            DmaStreamComputeCommitResult completion =
                ExecuteTokenToCommitPending(microOp.Descriptor, admission.Token, backend);
            return CreateResult(admission.Token, completion, backend, admission);
        }

        private static DmaStreamComputeCommitResult ExecuteTokenToCommitPending(
            DmaStreamComputeDescriptor descriptor,
            DmaStreamComputeToken token,
            DmaStreamAcceleratorBackend backend)
        {
            ...
            return token.MarkComputeComplete();
        }
        ...
        private static DmaStreamComputeExecutionResult CreateResult(
            ...
            DmaStreamAcceleratorBackend backend,
            DmaStreamComputeIssueAdmissionResult? issueAdmission = null) =>
            new(token, completion, backend.SnapshotTelemetry(), issueAdmission);
            ...
            out DmaStreamComputeCommitResult fault)
        {
            ...
            if (descriptor.RangeEncoding != DmaStreamComputeRangeEncoding.InlineContiguous)
            {
                fault = token.PublishFault(
                    ...
                    "DmaStreamCompute runtime only supports inline contiguous range encoding in Phase 06.",
            ...
            if (descriptor.PartialCompletionPolicy != DmaStreamComputePartialCompletionPolicy.AllOrNone)
            {
                fault = token.PublishFault(
                    DmaStreamComputeTokenFaultKind.UnsupportedAbiOrOperation,
                    "DmaStreamCompute runtime only supports all-or-none partial completion in Phase 06.",
                    descriptor.DescriptorReference.DescriptorAddress,
                    isWrite: false);
                return false;
            }
                ...
                descriptor.WriteMemoryRanges.Count == 0)
            {
                fault = token.PublishFault(
                    ...
                    "DmaStreamCompute runtime descriptor range counts do not match the Phase 06 execution contract.",
                    descriptor.DescriptorReference.DescriptorAddress,
                    isWrite: false);
                return false;
            }

            if (!IsSupportedRuntimeShape(descriptor.Operation, descriptor.Shape))
            {
                fault = token.PublishFault(
                    DmaStreamComputeTokenFaultKind.UnsupportedAbiOrOperation,
                    "DmaStreamCompute runtime reached an unsupported Phase 06 DSC1 shape.",
                    descriptor.DescriptorReference.DescriptorAddress,
                    isWrite: false);
                return false;
            }
        ...
        private static bool IsSupportedRuntimeShape(

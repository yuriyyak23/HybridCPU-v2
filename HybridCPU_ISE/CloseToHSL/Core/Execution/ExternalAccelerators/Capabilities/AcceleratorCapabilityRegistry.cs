...
public sealed class AcceleratorCapabilityRegistry
{
    ...
    private static void ValidateMetadataOnlyDescriptor(AcceleratorCapabilityDescriptor descriptor)
    {
        ...
        if (!AcceleratorDescriptorTaxonomyCatalog.TryGetEntry(
            ...
            taxonomyEntry.Status != descriptor.TaxonomyStatus)
        {
            throw new ArgumentException(
                "Accelerator capability metadata must reference a known Lane7 descriptor taxonomy entry with the catalog-pinned status.",
                nameof(descriptor));
        }
    }

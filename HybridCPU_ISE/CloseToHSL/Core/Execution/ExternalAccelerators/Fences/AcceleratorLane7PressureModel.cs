...
public sealed class AcceleratorLane7PressureThrottle
{
    ...
    public AcceleratorLane7PressureResult TryAdmit(
        SystemDeviceCommandKind commandKind)
    {
        if (commandKind is not (SystemDeviceCommandKind.Submit
            or SystemDeviceCommandKind.Poll
        ...
        if (SubmitPollCount >= MaxSubmitPollPerWindow)
        {
            ...
            AcceleratorLane7PressureResult result = AcceleratorLane7PressureResult.Reject(
                ...
                SubmitPollCount,
                ThrottleRejectCount,
                "L7-SDC submit/poll/status storm exceeded the bounded lane7 pressure budget; command was rejected before authority-sensitive state changed.");
        ...
        return AcceleratorLane7PressureResult.Allow(
            ...
            ThrottleRejectCount,
            branchOrSystemProgressPreserved: true,
            "L7-SDC submit/poll/status command admitted within bounded lane7 pressure budget.");
    }

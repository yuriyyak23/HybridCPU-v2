...
namespace YAKSys_Hybrid_CPU.Execution
{
    internal static partial class VectorALU
    {
        ...
        /// <summary>
        /// Apply VMSBF set-before-first semantics within the active vector length.
        /// Bits before the first set source bit are one; the first set bit and all
        /// later active bits are zero. Tail bits outside VL are always zero.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ulong ApplyMaskSetBeforeFirst(ulong mask, ulong vl)
        {
            ulong activeMask = CreateActiveLaneMask(vl);
            ulong activeSource = mask & activeMask;
            if (activeSource == 0)
            {
                return activeMask;
            }

            int firstSetLane = System.Numerics.BitOperations.TrailingZeroCount(activeSource);
            return firstSetLane == 0
                ? 0UL
                : (1UL << firstSetLane) - 1UL;
        }
        ...
        public static ulong MaskPopCount(ulong mask, ulong vl)
        {
            ...
            ulong maskedValue = mask & CreateActiveLaneMask(vl);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static ulong CreateActiveLaneMask(ulong vl)
        {
            ulong effectiveVL = Math.Min(vl, 64);
            if (effectiveVL == 0)
            {
            ...
            return effectiveVL == 64
                ? ulong.MaxValue
                : (1UL << (int)effectiveVL) - 1UL;
        }

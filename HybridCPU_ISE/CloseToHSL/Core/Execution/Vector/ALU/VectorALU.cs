...
namespace YAKSys_Hybrid_CPU.Execution
{
    ...
    internal static partial class VectorALU
    {
            ...
            ref Processor.CPU_Core core)
        {
            ...
            bool isWideHalfOrBFloat = op == (uint)Processor.CPU_Core.InstructionsEnum.VDOT_WIDE &&
                (dt == DataTypeEnum.FLOAT16 || dt == DataTypeEnum.BFLOAT16);
            bool isWideSignedByteInteger = op == (uint)Processor.CPU_Core.InstructionsEnum.VDOT_WIDE &&
                dt == DataTypeEnum.INT8;
            bool isWideUnsignedByteInteger = op == (uint)Processor.CPU_Core.InstructionsEnum.VDOT_WIDE &&
                dt == DataTypeEnum.UINT8;
            ...
            else if (isWideHalfOrBFloat)
            {
                ...
                ElementCodec.StoreF(dst, 0, DataTypeEnum.FLOAT32, accumulator);
            }
            else if (isWideSignedByteInteger)
            {
                long accumulator = 0;
                ...
                ElementCodec.StoreI(dst, 0, DataTypeEnum.INT32, accumulator);
            }
            else if (isWideUnsignedByteInteger)
            {
                ulong accumulator = 0;
                ...
                ElementCodec.StoreU(dst, 0, DataTypeEnum.UINT32, accumulator);
            }

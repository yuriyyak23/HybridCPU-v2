...
namespace YAKSys_Hybrid_CPU.Execution
{
    ...
    internal static partial class StreamEngine
    {
            ...
            int ownerThreadId = -1)
        {
            ...
            ThrowIfUnsupportedRawSlideOneContour(in request, opCode);
            ThrowIfUnsupportedRawWideDotContour(in request, opCode);
            ThrowIfUnsupportedRawPerm2Contour(in request, opCode);
            ThrowIfUnsupportedRawTransposeContour(in request, opCode);
            ...
            ThrowIfUnsupportedRawWidenNarrowConvertContour(in request, opCode);
            ThrowIfUnsupportedRawScanContour(in request, opCode);
            ThrowIfUnsupportedRawSaturatingPolicyContour(in request, opCode);
            ...
            int executionVtId)
        {
            ...
            if (core.GetPipelineControl().Enabled &&
                ...
                !OpcodeRegistry.IsWidenNarrowConvertOp(opCode))
            {
            ...
            uint opCode)
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Stream/vector opcode 0x{opCode:X} reached StreamEngine.Execute(...) with unsupported {addressingContour} vector-scan addressing. " +
                    "Phase 05A scan execution only follows through packed 1D staged-writeback truth and must fail closed instead of hidden compat success.");
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void ThrowIfUnsupportedRawWidenNarrowConvertContour(
            in StreamExecutionRequest request,
            uint opCode)
        {
            ...
            throw new InvalidOperationException(
                $"Stream/vector opcode 0x{opCode:X} reached raw StreamEngine.Execute(...) on a widen/narrow/convert contour. " +
                "Phase 05 VZEXT is executable only through the typed VectorZeroExtendMicroOp staging/writeback path; raw stream fallback must fail closed instead of publishing early memory writes.");
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void ThrowIfUnsupportedRawScanContour(
            in StreamExecutionRequest request,
            uint opCode)
        {
            ...
            throw new InvalidOperationException(
                $"Stream/vector opcode 0x{opCode:X} reached raw StreamEngine.Execute(...) on a vector scan contour. " +
                "Phase 05A VSCAN.SUM is executable only through the typed VectorScanSumMicroOp staging/writeback path; raw stream fallback must fail closed instead of publishing early memory writes.");
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void ThrowIfUnsupportedRawSlideOneContour(
            in StreamExecutionRequest request,
            uint opCode)
        {
            ...
            string mnemonic = opCode == (uint)Processor.CPU_Core.InstructionsEnum.VSLIDE1DOWN
                ? "VSLIDE1DOWN"
                : "VSLIDE1UP";
            string microOpName = opCode == (uint)Processor.CPU_Core.InstructionsEnum.VSLIDE1DOWN
                ? "VectorSlideOneDownMicroOp"
                : "VectorSlideOneUpMicroOp";
            throw new InvalidOperationException(
                $"Stream/vector opcode 0x{opCode:X} reached raw StreamEngine.Execute(...) on a Phase 09 slide-one contour. " +
                $"{mnemonic} is executable only through the typed {microOpName} staging/writeback path; raw stream fallback must fail closed instead of publishing early memory writes.");
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void ThrowIfUnsupportedRawWideDotContour(
            in StreamExecutionRequest request,
            uint opCode)
        {
            ...
            throw new InvalidOperationException(
                $"Stream/vector opcode 0x{opCode:X} reached raw StreamEngine.Execute(...) on a Phase 09 wide-dot contour. " +
                "VDOT.WIDE is executable only through the typed VectorDotWideMicroOp staging/writeback path; raw stream fallback must fail closed instead of publishing early memory writes.");
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void ThrowIfUnsupportedRawPerm2Contour(
            in StreamExecutionRequest request,
            uint opCode)
        {
            ...
            throw new InvalidOperationException(
                $"Stream/vector opcode 0x{opCode:X} reached raw StreamEngine.Execute(...) on a Phase 09 permute2 contour. " +
                "VPERM2 is executable only through the typed VectorPermute2MicroOp staging/writeback path; raw stream fallback must fail closed instead of publishing early memory writes.");
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void ThrowIfUnsupportedRawTransposeContour(
            in StreamExecutionRequest request,
            uint opCode)
        {
            ...
            throw new InvalidOperationException(
                $"Stream/vector opcode 0x{opCode:X} reached raw StreamEngine.Execute(...) on a Phase 09 transpose contour. " +
                "VTRANSPOSE is executable only through the typed VectorTransposeMicroOp staging/writeback path; raw stream fallback must fail closed instead of publishing early memory writes.");
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static void ThrowIfUnsupportedRawSaturatingPolicyContour(
            in StreamExecutionRequest request,
            uint opCode)
        {
            if (!request.Saturating)
            {
                return;
            }

            throw new InvalidOperationException(
                $"Stream/vector opcode 0x{opCode:X} reached raw StreamEngine.Execute(...) with the saturating policy bit set. " +
                "Phase 05B VADD.SAT is executable only through the typed VectorSaturatingAddMicroOp staging/writeback path; raw stream fallback must fail closed instead of silently publishing non-saturating writes.");
        }

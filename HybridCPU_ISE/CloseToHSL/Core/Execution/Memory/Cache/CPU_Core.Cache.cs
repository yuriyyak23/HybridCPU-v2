...
namespace YAKSys_Hybrid_CPU
{
    public partial struct Processor
    {
        public sealed partial class CPU_Core
        {
            ...
            private void InvalidateAllVliwFetchState(
                Core.ReplayPhaseInvalidationReason invalidationReason,
                bool advanceCodeGenerationEpoch)
            {
                if (advanceCodeGenerationEpoch)
                {
                    AdvanceReplayCodeGenerationEpoch();
                }
            ...
            public Cache_VLIWBundle_Object GetL2_VLIWBundleByPointer(ulong MemoryAddress)
            {
                ...
                ReadBoundMainMemoryExact(
                    L2_VLIWBundles[Current_VLIWBundle_Position].VLIWCache_MemoryAddress,
                    L2_VLIWBundles[Current_VLIWBundle_Position].VLIWCache_VLIWBundle,
                    "Canonical CPU instruction fetch");
                GetBoundMemorySubsystem()?.CycleController.RecordInstructionFetchReadBytes(256);
                if (GetBoundMainMemory().TryReadVliwBundleAnnotations(MemoryAddress, out VliwBundleAnnotations? bundleAnnotations))
                {

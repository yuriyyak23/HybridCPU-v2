...
namespace YAKSys_Hybrid_CPU.Core.Execution
{
    public sealed partial class ExecutionDispatcherV4
    {
        ...
        private CsrRetireEffect ResolveCsrEffect(InstructionIR instr, ICanonicalCpuState state, byte vtId)
        {
            ...
            ValidateCounterReadCsrInstruction(instr, opcode);

            ushort csrAddr = opcode == IsaOpcodeValues.RDCYCLE
                ? CsrAddresses.Cycle
                : (ushort)(instr.Imm & 0xFFF);
            ...
            {
                IsaOpcodeValues.RDCYCLE => CsrRetireEffect.Create(
        ...
        private static void ValidateCounterReadCsrInstruction(
            InstructionIR instr,
            ushort opcode)
        {
            if (opcode != IsaOpcodeValues.RDCYCLE)
            {
                return;
            }
            ...
            if (instr.Rs1 != 0 ||
                instr.Rs2 != 0 ||
                instr.Imm != 0 ||
                (instr.CsrAddress.HasValue && instr.CsrAddress.Value != CsrAddresses.Cycle))
            {
                throw new InvalidOpcodeException(
                    "RDCYCLE requires canonical rd-only CSR counter-read payload: rs1=x0, rs2=x0, Immediate=0, CsrAddress=0xC00.",
                    FormatOpcode(opcode),
                    -1,
                    false);
            }

...
namespace YAKSys_Hybrid_CPU.Core.Legality
{
    ...
    public sealed class BundleLegalityAnalyzer
    {
            ...
            ulong domainTag = 0)
        {
            ...
            return BuildCanonicalPlacement(slotClass, domainTag);
        }
            ...
            SlotClass slotClass)
        {
            ...
            bool isMemoryOp = IsMemoryLikeClass(instruction.Class) &&
                !IsCurrentLane6DmaStreamRegisterResultContour(instruction);
            bool isControlFlow = instruction.Class == InstructionClass.ControlFlow;
            SlotPlacementMetadata placement = BuildCanonicalPlacement(slotClass, domainTag: 0);
            ...
            ref ResourceBitset aggregateResourceMask)
        {
            for (int i = 0; i < slot.ReadRegisters.Count; i++)
            {
                ...
                aggregateResourceMask |= ArchRegId.TryCreate(
                    registerId,
                    out ArchRegId architecturalRegisterId)
        ...
        private static bool IsPinnedSlotClass(SlotClass slotClass)
        {
            return slotClass == SlotClass.BranchControl
                ...
                || slotClass == SlotClass.DmaStreamClass;
        }
        ...
        internal static bool MayWriteArchitecturalRegister(InstructionIR instruction)
        {
            ...
            if (IsCurrentLane6DmaStreamRegisterResultContour(instruction))
                return true;
            ...
            if (opcodeInfo.HasValue)
            {
                return instruction.Class switch
                {
                    ...
                    InstructionClass.System => IsCurrentL7SdcRegisterResultContour(instruction) ||
        ...
        private static bool IsCurrentLane6DmaStreamContour(InstructionIR instruction)
        {
            return unchecked((ushort)instruction.CanonicalOpcode) is
                Processor.CPU_Core.IsaOpcodeValues.DmaStreamCompute or
                Processor.CPU_Core.IsaOpcodeValues.DSC_STATUS or
                Processor.CPU_Core.IsaOpcodeValues.DSC_QUERY_CAPS;
        }

        private static bool IsCurrentLane6DmaStreamRegisterResultContour(InstructionIR instruction)
        {
            return unchecked((ushort)instruction.CanonicalOpcode) is
                Processor.CPU_Core.IsaOpcodeValues.DSC_STATUS or
                Processor.CPU_Core.IsaOpcodeValues.DSC_QUERY_CAPS;
        ...
        private static bool IsCurrentL7SdcRegisterResultContour(InstructionIR instruction)
        {
            if (instruction.Class != InstructionClass.System)
            {
                return false;
        ...
        private static ResourceBitset BuildStructuralResourceMask(in DecodedBundleSlotDescriptor slot)
        {
            ...
            if (MatrixTileResourceContour.IsMatrixTileMemoryOpcode(slot.OpCode))
            {
                ResourceBitset mask =
                    ...
                    ResourceMaskBuilder.ForStreamEngine(StreamEngineId.Zero) |

        ...
        public sealed partial class CPU_Core
        {
                ...
                out Core.MicroOp[] packedBundle)
            {
                ...
                for (int entryIndex = 0; entryIndex < entries.Length && nominatedCount < injectableGapCount; entryIndex++)
                {
                    ...
                    AttachRf08PostStageBIdentityTemplate(
                        currentBundlePc,
                        entry,
                        candidate);
            ...
            private void AttachRf08PostStageBIdentityTemplate(
                ulong currentBundlePc,
                in Core.ScalarClusterIssueEntry entry,
                Core.MicroOp candidate)
            {
                ...
                if (!runtimeState.HasCanonicalDecode ||
                    runtimeState.BundlePc != currentBundlePc ||
                    runtimeState.CanonicalDecode.CanonicalBundle is not Core.Decoder.CanonicalBundle canonicalBundle)
                    return;

                Core.Decoder.CanonicalDecodedInstruction canonical;
                try
                {
                    canonical = canonicalBundle.GetSlot(entry.SlotIndex);
                    if (canonical.Opcode != candidate.OpCode)
                        return;

                    Core.Decoder.ExecutionContract contract =
                        Core.Decoder.Rf06ScalarLegacyProjection.CreateContract(canonical);
                    Core.Decoder.SourceOperationProvenance provenance = new(
                        canonicalBundle.SemanticKey,
                        entry.VirtualThreadId,
                        runtimeState.BundleSerial,
                        Core.SlotId.Create(entry.SlotIndex),
                        runtimeState.StateEpoch);
                    Core.Decoder.AdmissionRecord admission = Core.Decoder.AdmissionRecord.Create(
                        provenance,
                        contract,
                        entry.VirtualThreadId,
                ...
                {
                    // The live candidate is outside the frozen first scalar family;
                    // retain the existing identity-less behavior for that family.
                    candidate.PostStageBIdentityTemplate = null;
                }
            }

...
namespace YAKSys_Hybrid_CPU.Core
{
    public partial class MicroOpScheduler
    {
        ...
        {
            // RF-12.9bb: a raw zero epoch cannot denote an active replay
            // phase. Canonicalize it before any replay telemetry, sampling or
            // reuse mutation, then consume the established inactive contour.
            if (phase.IsActive && phase.EpochId == 0)
            {
                ...
                phase = new ReplayPhaseContext(
                    isActive: false,
                    epochId: 0,
                    cachedPc: phase.CachedPc,
                    epochLength: phase.EpochLength,
                    completedReplays: phase.CompletedReplays,
                    validSlotCount: phase.ValidSlotCount,
                    stableDonorMask: phase.StableDonorMask,
        ...
        private void RecordTypedSlotReject(TypedSlotRejectReason reason)
        {
            switch (reason)
            {
                ...
                case TypedSlotRejectReason.LateBindingConflict:
                    ...
                    break;
                case TypedSlotRejectReason.InvalidPinnedLane:
                    InvalidPinnedLaneRejects++;
                    SmtRejectionsCount++;
                    break;

...
public partial class MicroOpScheduler
{
    ...
    internal bool TryPrepareCompletionReasonQualificationVmReadAfterCanonicalValueRead(
        CompletionReasonQualificationVmReadScalarDeliveryCanonicalComposition composition,
        IssuePacketLane issueLane,
        ulong fieldSelector,
        ulong restoreGeneration)
    {
        ArgumentNullException.ThrowIfNull(composition);
        if (issueLane.MicroOp is not VmxMicroOp carrier ||
            carrier.VirtualizationAdmission is null || !issueLane.IsOccupied ||
            issueLane.PhysicalLaneIndex != 7)
            return false;
        LastVmReadScalarDeliveryResult = composition.Prepare(
            _currentReplayPhase,
            carrier,
            new VmReadScalarAttemptBinding(carrier.VirtualizationAdmission),
            fieldSelector,
            restoreGeneration);
        if (LastVmReadScalarDeliveryResult is not { IsPrepared: true, Receipt: not null } result)
            return false;
    ...
    internal bool TryPrepareCompletionSecondStageVmReadAfterCanonicalValueRead(
        CompletionSecondStageVmReadScalarDeliveryCanonicalComposition composition,
        IssuePacketLane issueLane,
        ulong fieldSelector,
        ulong restoreGeneration)
    {
    ...
    internal bool TryPreparePinBasedControlsVmReadAfterCanonicalValueRead(
        PinBasedControlsVmReadScalarDeliveryCanonicalComposition? composition,
        IssuePacketLane issueLane,
        ulong fieldSelector,
        ulong restoreGeneration)
    {
        if (composition is null || issueLane.MicroOp is not VmxMicroOp carrier ||
            carrier.VirtualizationAdmission is null || !issueLane.IsOccupied ||
            issueLane.PhysicalLaneIndex != 7)
            return false;
        LastVmReadScalarDeliveryResult = composition.Prepare(

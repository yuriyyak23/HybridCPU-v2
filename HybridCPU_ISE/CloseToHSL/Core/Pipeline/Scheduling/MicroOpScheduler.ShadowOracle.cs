...
namespace YAKSys_Hybrid_CPU.Core
{
    ...
    public partial class MicroOpScheduler
    {
        ...
        /// <summary>
        /// RF-06.4b opt-in differential assertion. When enabled, the existing
        /// ShadowOracle memory predicate is evaluated once from the legacy
        /// carrier and once from the immutable memory contract projection.
        /// The flag is diagnostic-only and defaults to false; it never drives
        /// production admission or packing.
        /// </summary>
        public bool EnableRf06MemoryContractDifferential { get; set; }
            ...
            int ownerVirtualThreadId)
        {
            ...
            for (int vt = 0; vt < SMT_WAYS; vt++)
            {
                ...
                AssertRf06MemoryContractDifferential(
                    candidate,
                    candidateVirtualThreadId,
                    localOutstandingStoreBankMask,
                    remainingHardwareMemoryIssueBudget,
                    consumedHardwareMemoryBudgetByBank,
                    remainingHardwareLoadIssueBudget,
                    consumedHardwareLoadBudgetByBank,
                    ...
                    consumedHardwareStoreBudgetByBank,
                    projectedOutstandingMemoryCount,
                    projectedOutstandingMemoryCapacity);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void AssertRf06MemoryContractDifferential(
            MicroOp candidate,
            int candidateVirtualThreadId,
            ushort localOutstandingStoreBankMask,
            ...
            IReadOnlyList<byte> projectedOutstandingMemoryCount,
            IReadOnlyList<byte> projectedOutstandingMemoryCapacity)
        {
            if (!EnableRf06MemoryContractDifferential || candidate is not LoadStoreMicroOp carrier)
                return;
            if (!Rf06MemoryShadowOracle.TryProjectLegacyCarrier(carrier, out MemoryCapability capability))
                return;

            byte[] memoryBudgets = new byte[MemoryBankId.BankCount];
            byte[] readBudgets = new byte[MemoryBankId.BankCount];
            byte[] writeBudgets = new byte[MemoryBankId.BankCount];
            byte[] consumedMemory = new byte[MemoryBankId.BankCount];
            byte[] consumedReads = new byte[MemoryBankId.BankCount];
            byte[] consumedWrites = new byte[MemoryBankId.BankCount];
            for (int bank = 0; bank < MemoryBankId.BankCount; bank++)
            {
                memoryBudgets[bank] = _hardwareOccupancySnapshot.GetMemoryBudgetForBank(bank);
                readBudgets[bank] = _hardwareOccupancySnapshot.GetReadBudgetForBank(bank);
                writeBudgets[bank] = _hardwareOccupancySnapshot.GetWriteBudgetForBank(bank);
                consumedMemory[bank] = GetPackedConsumedHardwareBudget(consumedHardwareMemoryBudgetByBank, bank);
                consumedReads[bank] = GetPackedConsumedHardwareBudget(consumedHardwareLoadBudgetByBank, bank);
                consumedWrites[bank] = GetPackedConsumedHardwareBudget(consumedHardwareStoreBudgetByBank, bank);
            }

            ushort pendingBankMask = 0;
            if (IsBankPendingForVT(capability.Bank!.Value.Value, candidateVirtualThreadId))
                pendingBankMask = (ushort)(1 << capability.Bank.Value.Value);
            ...
            Rf06MemoryShadowState state = Rf06MemoryShadowState.Create(
                ...
                pendingBankMask,
                localOutstandingStoreBankMask);
            Rf06MemoryShadowDecision legacy = Rf06MemoryShadowOracle.EvaluateLegacyCarrier(carrier, state);
            ...
            if (legacy != contract)
            {
                throw new InvalidOperationException(
                    $"RF-06.4b memory ShadowOracle differential mismatch for VT {candidateVirtualThreadId}: " +
                    $"legacy={legacy.RejectReason}, contract={contract.RejectReason}.");
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void AssertRf06MemoryContractProjection(MicroOp candidate)
        {
            if (!EnableRf06MemoryContractDifferential || candidate is not LoadStoreMicroOp carrier)
                return;
            ...
            {
                throw new InvalidOperationException(
                    $"RF-06.4b FSP memory carrier for VT {candidate.VirtualThreadId} has no representable immutable bank/footprint contract.");
            }
        }

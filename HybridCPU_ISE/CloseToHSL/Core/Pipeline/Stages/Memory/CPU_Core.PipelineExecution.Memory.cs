        ...
        public sealed partial class CPU_Core
        {
            ...
            private void ExecuteExplicitPacketMemoryWork()
            {
                ...
                for (byte laneIndex = 4; laneIndex < 8; laneIndex++)
                {
                    ...
                    if (lane.MicroOp is Core.LoadStoreMicroOp scalarMemoryOperation)
                    {
                        try
                        {
                            CPU_Core stableCoreIdentity = this;
                            address = scalarMemoryOperation.ResolveCpuTranslatedAddressForPipeline(
                                ref stableCoreIdentity,
                                lane.IsLoad
                                    ? Core.Memory.CpuInstructionTranslationAccessKind.ScalarLoad
                                    : Core.Memory.CpuInstructionTranslationAccessKind.ScalarStore,
                                accessSize);
                        ...
                        {
                            if (scalarMemoryOperation.IsSpeculative)
                            {
                                scalarMemoryOperation.MarkFaulted();
                                ReleaseScalarLaneBookkeeping(lane);
                                ScalarMemoryLaneState cleared = new();
                                cleared.Clear(laneIndex);
                                pipeMEM.SetLane(laneIndex, cleared);
                                pipeCtrl.ExceptionYoungerSuppressCount++;
                                continue;
                            }

                            DeliverStageAwareMemoryCpuTranslationFault(
                                translationFaultException.Fault);
                            throw new InvalidOperationException(
                                "Stage-aware CPU translation fault delivery must terminate the current cycle.");
                        }
                    }
                ...
                ref int executedPhysicalLaneCount)
            {
                ...
                {
                    if (loadOp.UsesBaseRegisterAddressing)
                    {
                        int consumerThreadId = NormalizePipelineStateVtId(lane.OwnerThreadId);
                        ulong baseValue = GetRegisterValueWithForwardingExcludingExecuteLane(
                            consumerThreadId, loadOp.BaseRegID, laneIndex);
                        loadOp.ResolveBaseRelativeAddressForPipeline(baseValue);
                    }
                ...
                {
                    int consumerThreadId = NormalizePipelineStateVtId(lane.OwnerThreadId);
                    if (storeOp.UsesBaseRegisterAddressing)
                    {
                        ulong baseValue = GetRegisterValueWithForwardingExcludingExecuteLane(
                            consumerThreadId, storeOp.BaseRegID, laneIndex);
                        storeOp.ResolveBaseRelativeAddressForPipeline(baseValue);
                    }
                    lane.IsMemoryOp = true;
                    lane.IsLoad = false;
                    lane.MemoryAccessSize = storeOp.Size;
                    lane.MemoryAddress = storeOp.Address;
                    lane.MemoryData = GetRegisterValueWithForwardingExcludingExecuteLane(
                        consumerThreadId, storeOp.SrcRegID, laneIndex);
                    lane.ResultValue = lane.MemoryData;
                    lane.ResultReady = true;
                    pipeEX.SetLane(laneIndex, lane);
                    executedPhysicalLaneCount++;

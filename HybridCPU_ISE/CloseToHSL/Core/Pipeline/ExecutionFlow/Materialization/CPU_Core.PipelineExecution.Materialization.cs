        ...
        public sealed partial class CPU_Core
        {
                ...
                byte executableNonScalarPhysicalLaneMask)
            {
                ...
                if (e1Attached && issueLane.MicroOp is Core.VmxMicroOp vmx)
                {
                    ...
                    if (!operandAttached)
                    {
                        Core.VmcsField? selectedField = rs1Value <= ushort.MaxValue
                            ? unchecked((Core.VmcsField)(ushort)rs1Value)
                            : null;
                        if (selectedField is Core.VmcsField.GuestPhysicalAddress or
                            Core.VmcsField.EptViolationQualification)
                        {
                            _fspScheduler.TryPrepareCompletionSecondStageVmReadAfterCanonicalValueRead(
                                _completionSecondStageVmRead,
                                issueLane,
                                rs1Value,
                                _virtualizationRestoreGenerationOwner.CurrentGeneration);
                        }
                        else if (selectedField is Core.VmcsField.ExitReason or
                            Core.VmcsField.ExitQualification)
                        {
                            _fspScheduler.TryPrepareCompletionReasonQualificationVmReadAfterCanonicalValueRead(
                                _completionReasonQualificationVmRead,
                                issueLane,
                                rs1Value,
                                _virtualizationRestoreGenerationOwner.CurrentGeneration);
                        }
                        else if (selectedField == Core.VmcsField.PinBasedControls)
                        {
                            _fspScheduler.TryPreparePinBasedControlsVmReadAfterCanonicalValueRead(
                ...
                Core.IssuePacketLane issueLane)
            {
                ...
                lane.PostStageBIssuedAttempt = issueLane.MicroOp?.PostStageBIssuedAttempt;
            ...
            private static Processor.CPU_Core.ScalarMemoryLaneState CreateMemoryLaneState(ScalarExecuteLaneState executeLane)
            {
                ...
                lane.PostStageBIssuedAttempt = executeLane.PostStageBIssuedAttempt;
                ...
                lane.CpuTranslationFault = executeLane.CpuTranslationFault;
            ...
            private static Processor.CPU_Core.ScalarWriteBackLaneState CreateWriteBackLaneState(ScalarMemoryLaneState memoryLane)
            {
                ...
                lane.PostStageBIssuedAttempt = memoryLane.PostStageBIssuedAttempt;
                ...
                lane.CpuTranslationFault = memoryLane.CpuTranslationFault;

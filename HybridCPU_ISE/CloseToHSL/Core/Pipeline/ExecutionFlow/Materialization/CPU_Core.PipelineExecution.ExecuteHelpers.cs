        ...
        public sealed partial class CPU_Core
        {
            ...
            private bool TryExecuteSingleLaneMicroOp()
            {
                ...
                try
                {
                    ...
                    ApplyResolvedCpuTranslationToExecuteLane(pipeEX.ActiveLaneIndex);
                ...
                ref int executedScalarLaneCount)
            {
                ...
                bool success;
                if (lane.MicroOp is Core.ScalarALUMicroOp scalarAlu)
                {
                    int consumerThreadId = NormalizePipelineStateVtId(lane.OwnerThreadId);
                    ulong source1 = GetRegisterValueWithForwardingExcludingExecuteLane(
                        consumerThreadId, scalarAlu.Src1RegID, laneIndex);
                    ulong source2 = scalarAlu.UsesImmediate
                        ? scalarAlu.Immediate
                        : GetRegisterValueWithForwardingExcludingExecuteLane(
                            consumerThreadId, scalarAlu.Src2RegID, laneIndex);
                    success = scalarAlu.ExecuteWithResolvedPipelineOperands(source1, source2, lane.PC);
                }
                else
                {
                    // MicroOp.Execute() remains the execution path for non-scalar-ALU carriers.
                    success = ExecuteMicroOpWithStableCoreIdentity(lane.MicroOp!);
                }
                ApplyResolvedCpuTranslationToExecuteLane(laneIndex);
                lane = pipeEX.GetLane(laneIndex);
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private void ApplyResolvedCpuTranslationToExecuteLane(byte laneIndex)
            {
                ScalarExecuteLaneState lane = pipeEX.GetLane(laneIndex);
                if (lane.MicroOp is not Core.LoadStoreMicroOp memoryOperation ||
                    !memoryOperation.TryGetCpuTranslatedAddress(out ulong physicalAddress, out _))
                {
                    return;
                }

                lane.MemoryAddress = physicalAddress;
                pipeEX.SetLane(laneIndex, lane);
            }

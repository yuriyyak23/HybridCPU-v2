            ...
            private ulong GetRegisterValueWithForwarding(int consumerThreadId, ushort regID)
            {
                ...
                if (TryResolveLaneAwareProducerMatch(consumerThreadId, regID, out LaneAwareForwardingMatch laneAwareMatch))
                {
                    ...
                    throw new InvalidOperationException(
                        $"Register x{regID} for VT{consumerThreadId} was consumed while its lane-aware producer remained not-ready. " +
                        $"Producer stage={laneAwareMatch.SourceStage}, lane={laneAwareMatch.SourceLaneIndex}, " +
                        $"value=0x{laneAwareMatch.ResultValue:x16}. " +
                        "The pipeline must interlock this consumer instead of exposing stale architectural state.");
                }
                ...
                // With no live producer, a self-destination consumer may already own a new
                // rename mapping whose physical value is not initialized yet. Its source
                // operand is the last committed architectural version, not that destination.
                return ReadCommittedArch(consumerThreadId, regID);
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private ulong GetRegisterValueWithForwardingExcludingExecuteLane(
                int consumerThreadId,
                ushort regID,
                byte consumerLaneIndex)
            {
                if (regID == 0 || regID >= YAKSys_Hybrid_CPU.Core.Registers.RenameMap.ArchRegs)
                    return 0;

                LaneAwareForwardingMatch match;
                for (byte laneIndex = 0; laneIndex < 8; laneIndex++)
                {
                    if (laneIndex == consumerLaneIndex)
                        continue;
                    if (pipeEX.Valid && TryResolveLaneAwareProducerMatch(
                        pipeEX.GetLane(laneIndex), consumerThreadId, regID, out match))
                    {
                        if (!match.ResultReady)
                            throw new InvalidOperationException(
                                $"Register x{regID} for VT{consumerThreadId} depends on not-ready EX lane {laneIndex}.");
                        return match.ResultValue;
                    }
                }

                for (byte laneIndex = 0; laneIndex < 8; laneIndex++)
                {
                    if (pipeMEM.Valid && TryResolveLaneAwareProducerMatch(
                        pipeMEM.GetLane(laneIndex), consumerThreadId, regID, out match))
                    {
                        if (!match.ResultReady)
                            throw new InvalidOperationException(
                                $"Register x{regID} for VT{consumerThreadId} depends on not-ready MEM lane {laneIndex}.");
                        return match.ResultValue;
                    }
                    if (pipeWB.Valid && TryResolveLaneAwareProducerMatch(
                        pipeWB.GetLane(laneIndex), consumerThreadId, regID, out match))
                        return match.ResultValue;
                }

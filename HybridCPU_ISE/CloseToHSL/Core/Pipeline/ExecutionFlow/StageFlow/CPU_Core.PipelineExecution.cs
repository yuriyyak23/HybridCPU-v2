...
namespace YAKSys_Hybrid_CPU
{
    public partial struct Processor
    {
        public sealed partial class CPU_Core
        {
            ...
            /// An unknown non-fault MicroOp exception clears transient lane state and
            /// propagates as a categorized fail-closed execution fault.
            /// </summary>
            ...
            private void ExecuteExplicitPacketLanes()
            {
                ...
                for (byte laneIndex = 0; laneIndex < 8; laneIndex++)
                {
                    ...
                    catch (Core.Memory.CpuInstructionTranslationFaultException translationFaultException)
                    {
                        DeliverStageAwareExecuteCpuTranslationFault(translationFaultException.Fault);
                    }
                    ...
                    {
                        // An unknown exception is not a retry/stall outcome.  Clear the
                        // transient carrier before making the failure observable to the
                        // caller, so no occupied ResultReady=false lane can survive it.
                        FailCloseExplicitPacketLaneAfterNonFaultExecutionException(ref lane, exception);

                        Core.ExecutionFaultCategory category =
                            Core.ExecutionFaultContract.TryGetCategory(exception, out Core.ExecutionFaultCategory propagatedCategory)
                                ? propagatedCategory
                                : Core.ExecutionFaultCategory.InvalidInternalOp;

                        throw Core.ExecutionFaultContract.CreateWrappedException(
                            category,
                            $"Explicit packet lane {laneIndex} opcode 0x{lane.OpCode:X} threw a non-fault exception inside the authoritative execution contour.",
                            exception);
                    }
                }
            ...
            private void PipelineStage_Memory()
            {
                ...
                if (!pipeMEM.Valid && pipeEX.Valid && canMoveToMemory)
                {
                    ...
                    if (pipeMEM.UsesExplicitPacketLanes)
                    {
                        ...
                        catch (Core.PageFaultException)
                        {
                            // Existing stage-aware memory fault delivery owns the
                            // architectural carrier, fault winner and flush.
                            throw;
                        }
                        catch (Exception exception)
                        {
                            // RF-07.2aj: an unexpected explicit-MEM failure is
                            // neither a pending request nor an eligible not-ready
                            // carrier. Project FatalInvariantViolation before the
                            // existing full flush/cleanup and public compatibility
                            // wrapper; do not leave an occupied MEM lane behind.
                            Core.Execution.ExecutionOutcome outcome =
                                ProjectExplicitPacketMemoryNonFaultExceptionOutcome(exception);
                            ...
                            throw Core.ExecutionFaultContract.CreateWrappedException(
                                category,
                                "Explicit-packet memory stage threw a non-fault exception inside the authoritative memory contour.",
                                exception);
                        }
                    }
            ...
            private void PipelineStage_WriteBack()
            {
                ...
                if (pipeWB.Valid)
                {
                    ...
                    Core.VectorTransferRetireEffect?[] vectorTransferEffects =
                        new Core.VectorTransferRetireEffect?[RetireWindowEffectCapacity];
                    ...
                    // Materialize effects only from the already selected older prefix.
                    // No retire-visible forwarding, counter, resource or speculation-budget
                    // mutation occurs until the complete bounded effect union validates.

        ...
        public sealed partial class CPU_Core
        {
            ...
            public static byte ResolveRetireEligibleWriteBackLanes(in WriteBackStage writeBackStage)
            {
                ...
                if (!writeBackStage.UsesExplicitPacketLanes && writeBackStage.RetainsReferenceSequentialPath)
                {
                    ...
                    if (retireLaneIndex >= 4 &&
                        !IsMatrixTileRetireLane(writeBackStage, retireLaneIndex))
                        return 0;
            ...
            private static bool IsRetireAuthoritativeWriteBackLane(in WriteBackStage writeBackStage, byte laneIndex)
            {
                if (!IsRetireAuthoritativeWriteBackLane(laneIndex) &&
                    !IsMatrixTileRetireLane(writeBackStage, laneIndex))
                    return false;
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private static bool IsMatrixTileRetireLane(
                in WriteBackStage writeBackStage,
                byte laneIndex)
            {
                return laneIndex ==
                    CloseToHSL.Core.ISA.Instructions.NonVmx.Lanes00_03Vector.MatrixTile.MatrixTileResourceContour.TileStreamLaneId &&
                    writeBackStage.Lane6.IsOccupied &&
            ...
            private static byte GetOccupiedRetireEligibleWriteBackLaneMask(in WriteBackStage writeBackStage)
            {
                ...
                if (writeBackStage.Lane6.IsOccupied && IsRetireAuthoritativeWriteBackLane(writeBackStage, 6))
                    occupiedMask |= 1 << 6;
            ...
            public readonly struct StageAwareExceptionWinnerMetadata
            {
                ...
                public ulong PC { get; }
                public Core.Memory.CpuInstructionTranslationFault? CpuTranslationFault { get; }
                ...
                public StageAwareExceptionWinnerMetadata(
                    ...
                    bool faultIsWrite,
                    ulong pc,
                    Core.Memory.CpuInstructionTranslationFault? cpuTranslationFault = null)
                {
                    ...
                    PC = pc;
                    CpuTranslationFault = cpuTranslationFault;
                }
                ...
                in WriteBackStage writeBackStage, byte laneIndex)
            {
                ...
                return new StageAwareExceptionWinnerMetadata(
                    ...
                    lane.FaultAddress, lane.FaultIsWrite, lane.PC, lane.CpuTranslationFault);
            }

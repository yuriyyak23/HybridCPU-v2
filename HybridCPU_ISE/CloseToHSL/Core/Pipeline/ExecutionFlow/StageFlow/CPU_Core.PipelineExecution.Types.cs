        ...
        public sealed partial class CPU_Core
        {
                ...
                internal readonly struct RetireWindowEffect
                {
                    ...
                    private RetireWindowEffect(
                        ...
                        byte vectorTransferEffectSlot = NoPipelineEventSlot)
                    {
                        Kind = kind;
                        ...
                        VectorTransferEffectSlot = vectorTransferEffectSlot;
                    }
                    ...
                    public int VectorStreamDirtyVtId => SystemEventVtId;
                    ...
                    public byte VectorTransferEffectSlot { get; }
                    ...
                    public static RetireWindowEffect VectorStreamDirty(int virtualThreadId) =>
                        new(
                            RetireWindowEffectKind.VectorStreamDirty,
                            0,
                            default,
                            default,
                            default,
                    ...
                    public static RetireWindowEffect VectorTransfer(byte vectorTransferEffectSlot) =>
                        new(
                            RetireWindowEffectKind.VectorTransfer,
                            0,
                            default,
                            default,
                            default,
                ...
                internal ref struct RetireWindowBatch
                {
                    ...
                    private readonly Core.VectorTransferRetireEffect?[] _vectorTransferEffects;
                    private int _retireRecordCount;
                    private int _effectCount;
                    private int _pipelineEventCount;
                    private int _vectorTransferEffectCount;
                    ...
                    public RetireWindowBatch(
                        ...
                        Core.VectorTransferRetireEffect?[]? vectorTransferEffects = null)
                    {
                        _retireRecords = retireRecords;
                        _effects = effects;
                        _pipelineEvents = pipelineEvents ?? throw new ArgumentNullException(nameof(pipelineEvents));
                        _vectorTransferEffects = vectorTransferEffects ??
                            Array.Empty<Core.VectorTransferRetireEffect?>();
                        _retireRecordCount = 0;
                        _effectCount = 0;
                        _pipelineEventCount = 0;
                        _vectorTransferEffectCount = 0;
                    ...
                    public Core.VectorTransferRetireEffect GetVectorTransferEffectPayload(
                        byte vectorTransferEffectSlot)
                    {
                        if (vectorTransferEffectSlot == byte.MaxValue ||
                            vectorTransferEffectSlot >= _vectorTransferEffectCount ||
                            vectorTransferEffectSlot >= _vectorTransferEffects.Length ||
                            _vectorTransferEffects[vectorTransferEffectSlot] == null)
                        {
                            throw new InvalidOperationException(
                                "Canonical vector-transfer retire payload is missing.");
                        }

                        return _vectorTransferEffects[vectorTransferEffectSlot]!;
                    }
                    ...
                    public void CaptureVectorTransferEffect(
                        Core.VectorTransferRetireEffect vectorTransferEffect)
                    {
                        ArgumentNullException.ThrowIfNull(vectorTransferEffect);
                        if (_vectorTransferEffectCount >= _vectorTransferEffects.Length ||
                            _vectorTransferEffectCount >= byte.MaxValue)
                        {
                            throw new InvalidOperationException(
                                "Retire batch canonical vector-transfer payload capacity exceeded.");
                        }

                        byte vectorTransferEffectSlot = checked((byte)_vectorTransferEffectCount++);
                        _vectorTransferEffects[vectorTransferEffectSlot] = vectorTransferEffect;
                        AppendEffect(RetireWindowEffect.VectorTransfer(vectorTransferEffectSlot));
                    }
                    ...
                    public void CaptureRetireWindowVectorStreamDirty(int virtualThreadId)
                    {
                        if ((uint)virtualThreadId < 32)
                        {
                            uint vtMask = 1u << virtualThreadId;
                            if ((_vectorStreamDirtyVtMask & vtMask) != 0)
                            {
                                return;
                            }

                            _vectorStreamDirtyVtMask |= vtMask;
                        }

                        AppendEffect(RetireWindowEffect.VectorStreamDirty(virtualThreadId));
                    }

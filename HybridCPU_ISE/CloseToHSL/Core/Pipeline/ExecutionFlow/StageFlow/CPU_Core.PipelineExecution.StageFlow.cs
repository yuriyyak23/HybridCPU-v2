    ...
    public partial struct Processor
    {
        public sealed partial class CPU_Core
        {
            ...
            public void ExecutePipelineCycle()
            {
                ...
                pipeCtrl.CycleCount++;
                Csr.HardwareWrite(CsrAddresses.Cycle, pipeCtrl.CycleCount);

                var memorySubsystem = GetBoundMemorySubsystem();
                if (memorySubsystem != null)
                {
                    MemoryCyclePlatformOrchestrator.AdvanceCoreObservedPlatformEdge(
                        memorySubsystem.CycleController,
                        CoreID,
                        pipeCtrl.CycleCount);
                }
                ...
                // Older pipeline work must be allowed to drain before an ID-side RAW/WAW
                // interlock is evaluated. Returning on a load-use hazard while its producer
                // remains in EX/MEM permanently deadlocks the producer and consumer.
            ...
            {
                if (TryDeliverPendingCpuFetchTranslationFault())
                    return;

                if (pipeIF.Valid)
                    return;

                SynchronizeReplayRelevantMemoryEpoch();
                ulong fetchPC = ReadActiveLivePc();

                Core.Memory.CpuInstructionTranslationResult translation =
                    TranslateInstructionFetch(fetchPC);
                if (!translation.Succeeded)
                {
                    if (translation.Fault is not Core.Memory.CpuInstructionTranslationFault fault)
                        throw new InvalidOperationException("Failed CPU instruction translation omitted typed fault facts.");

                    PendingCpuFetchTranslationFault = fault;
                    _ = TryDeliverPendingCpuFetchTranslationFault();
                    return;
                }

                ulong physicalFetchAddress = translation.PhysicalAddress;

                if (physicalFetchAddress >= GetBoundMainMemoryLength())
                {
                    pipeIF.Valid = false;
                    return;
                }
                ...
                if (_loopBuffer.TryGetReplayEntry(
                        fetchPC,
                        decodeContext,
                        out Core.Decoder.ReplayEntry? replayEntry))
                {
                    try
                    {
                        Core.DecodedBundleRuntimeState replayRuntimeState =
                            BuildReplayDecodedBundleState(
                                replayEntry!,
                                fetchPC);
                        PublishCurrentCanonicalDecodedBundleState(replayRuntimeState);
                        pipeIF.PC = fetchPC;
                        pipeIF.VLIWBundle = null;
                        pipeIF.BundleAnnotations = null;
                        pipeIF.HasBundleAnnotations = false;
                        pipeIF.DecodeContext = decodeContext;
                        pipeIF.Valid = true;
                    ...
                    catch (Exception replayProjectionFailure)
                        when (replayProjectionFailure is InvalidOperationException or
                              ArgumentException or
                              System.Text.Json.JsonException or
                              Arch.InvalidOpcodeException)
                    {
                ...
                Cache_VLIWBundle_Object fetchedBundle = GetVLIWBundleByPointer(physicalFetchAddress);
                ...
                if (CpuInstructionTranslationOwner.IsIdentity &&
                    nextPC < GetBoundMainMemoryLength())
                {
            ...
            private DecodeStageResult PipelineStage_Decode()
            {
                ...
                {
                    if (!IsFetchedDecodeContextCurrent())
                    {
                        ulong stalePc = pipeIF.PC;
                        InvalidateAllVliwFetchState(
                            Core.ReplayPhaseInvalidationReason.CertificateMutation);
                        _observedReplayRelevantMemoryEpoch =
                            GetBoundMainMemory().ReplayRelevantMutationEpoch;
                        pipeIF.Clear();
                        bundleDecodedAndPacked = false;
                        ResetCurrentDecodedBundleSlotCarrierState(stalePc);
                        ...
                        if (decodedBundleSlots.Length == 8)
                        {
                            ...
                            if (maxIterations > 1)
                            {
                                Core.Decoder.CanonicalBundle? canonicalBundle =
                                    GetCurrentDecodedInstructionBundle().CanonicalBundle;
                                if (canonicalBundle is null ||
                                    pipeIF.DecodeContext is null)
                                {
                                    throw new InvalidOperationException(
                                        "Semantic loop replay requires the live canonical bundle and captured context.");
                                }
                                var liveCarriers = new Core.MicroOp?[Core.BundleMetadata.BundleSlotCount];
                                for (int i = 0; i < liveCarriers.Length; i++)
                                    liveCarriers[i] = decodedBundleSlots[i].MicroOp;
                                _loopBuffer.BeginSemanticLoad(
                                    pipeIF.PC,
                                    maxIterations,
                                    Core.Decoder.ReplayEntry.Create(canonicalBundle),
                                    liveCarriers,
                                    pipeIF.DecodeContext);
                    ...
                    if (HasOlderPipelineRegisterProducer(
                        currentSlotDescriptor.GetRuntimeAdmissionReadRegisters()))
                    {
                        pipeCtrl.DataHazards++;
                        return DecodeStageResult.Stall(
                            currentBundlePc,
                            PipelineStallKind.DataHazard,
                            bankConflict: false,
                            rejectedSlots: 0,
                            DecodeStageDiagnosticsKind.DataDependency);
                    }
            ...
            private static bool IsExecutableNonScalarIssueLane(Core.IssuePacketLane issueLane)
            {
                ...
                return issueLane.PhysicalLaneIndex switch
                {
                    ...
                    6 => issueLane.MicroOp is Core.MatrixTileMicroOp,

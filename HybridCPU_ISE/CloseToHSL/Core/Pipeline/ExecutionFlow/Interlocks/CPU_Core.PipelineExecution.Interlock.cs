    ...
    public partial struct Processor
    {
        public sealed partial class CPU_Core
        {
            ...
            private PipelineCycleStallDecision ResolvePipelineHazardStallDecision()
            {
                ...
                if (pipeID.Valid)
                {
                    // FullSerial is a two-sided boundary. The serializing carrier itself
                    // cannot enter EX until every older EX/MEM producer has retired;
                    // otherwise an ECALL can capture stale argument registers.
                    if (HasDecodedSerializingCarrierWaitingForOlderWork())
                    {
                        pipeCtrl.StallReason = PipelineStallKind.ControlHazard;
                        return PipelineCycleStallDecision.ForKind(pipeCtrl.StallReason);
                    }

                    // FullSerial/VmxSerial is an admission boundary, not merely a register
                    // dependency.  Once an older serializing carrier has reached EX/MEM,
                    // no younger decoded bundle may enter execution before that carrier
                    // retires and its existing retire path flushes speculative state and
                    // publishes the architectural event outcome.
                    if (HasOlderInFlightSerializingCarrier())
                    {
                        pipeCtrl.StallReason = PipelineStallKind.ControlHazard;
                        return PipelineCycleStallDecision.ForKind(pipeCtrl.StallReason);
                    }

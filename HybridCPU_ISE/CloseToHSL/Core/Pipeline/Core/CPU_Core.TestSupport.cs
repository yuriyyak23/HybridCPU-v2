...
namespace YAKSys_Hybrid_CPU
{
    public partial struct Processor
    {
        ...
        public sealed partial class CPU_Core
        {
            ...
            private void CaptureTestStreamPublicationsWithStableCoreIdentity(
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private void ExecuteTestStreamRequestWithStableCoreIdentity(
                in StreamExecutionRequest request,
                int ownerThreadId)
            {
                CPU_Core stableCoreIdentity = this;
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private bool ExecuteTestAtomicWithStableCoreIdentity(AtomicMicroOp atomicMicroOp)
            {
                CPU_Core stableCoreIdentity = this;
                return atomicMicroOp.Execute(ref stableCoreIdentity);
            }
            ...
            /// <summary>
            /// TEST-ONLY: observes the already materialized fetch-cache carriers for one PC.
            /// This is observational only; it never manufactures an ingress carrier or fetches
            /// a bundle on the caller's behalf.
            /// </summary>
            internal (bool L1Present, bool L2Present, VliwBundleAnnotations? L1Annotations, VliwBundleAnnotations? L2Annotations)
                TestReadVliwFetchCacheCarriers(ulong memoryAddress)
            {
                Cache_VLIWBundle_Object? l1 = FindVliwBundleCacheLine(L1_VLIWBundles, memoryAddress);
                Cache_VLIWBundle_Object? l2 = FindVliwBundleCacheLine(L2_VLIWBundles, memoryAddress);
                return (
                    l1.HasValue && l1.Value.VLIWCache_HasAnnotationCarrier,
                    l2.HasValue && l2.Value.VLIWCache_HasAnnotationCarrier,
                    l1?.VLIWCache_BundleAnnotations,
                    l2?.VLIWCache_BundleAnnotations);
            }

            private static Cache_VLIWBundle_Object? FindVliwBundleCacheLine(
                Cache_VLIWBundle_Object[]? cache,
                ulong memoryAddress)
            {
                if (cache == null)
                {
            ...
            {
                return Runtime.Telemetry.TestReferenceRawFallbackCount;
            }
            ...
            /// <summary>
            /// TEST-ONLY: prime the production immutable semantic replay contour from one
            /// canonical bundle and its exact decode context. This is distinct from the
            /// legacy mutable-carrier replay helper used by retirement-only tests.
            /// </summary>
            internal void TestPrimeSemanticReplayPhase(
                ulong pc,
                ...
                VliwBundleAnnotations? annotations = null)
            {
                ...
                Core.Decoder.CanonicalDecodeContext decodeContext =
                    CaptureReplayDecodeContext();
                Core.Decoder.IDecoderFrontend decoder =
                    Core.Decoder.DecoderFeatureFlags.CreateDecoder();
                Core.Decoder.DecodedInstructionBundle decodedBundle =
                    decoder.DecodeInstructionBundle(
                        ...
                        bundleSerial: 0,
                        decodeContext);
                Core.Decoder.CanonicalBundle canonicalBundle =
                    decodedBundle.CanonicalBundle ??
                    throw new InvalidOperationException(
                        "Semantic replay test setup requires a canonical bundle.");
                Core.MicroOp?[] liveCarriers =
                    Core.Decoder.DecodedBundleTransportProjector
                        .BuildCanonicalCarrierBundleForTesting(rawSlots, decodedBundle);

                _loopBuffer.Initialize();
                _loopBuffer.BeginSemanticLoad(
                    pc,
                    totalIterations,
                    Core.Decoder.ReplayEntry.Create(canonicalBundle),
                    liveCarriers,
                    decodeContext);
                ...
                if (_fspScheduler == null)
                {
                    _fspScheduler = new Core.MicroOpScheduler();
                }
                ...
                byte vtId = 0)
            {
                ...
                VectorTransferRetireEffect?[] vectorTransferEffects = new VectorTransferRetireEffect?[1];
                RetireWindowBatch retireBatch = new(
                    retireRecords,
                    retireEffects,
                    pipelineEvents,
                    vectorTransferEffects);
                ...
                int ownerThreadId = -1)
            {
                    ...
                    request.RequiresPredicateStateCarrier)
                {
                    ...
                    Span<RetireWindowEffect> retireEffects = stackalloc RetireWindowEffect[2];
                    PipelineEvent?[] pipelineEvents = Array.Empty<PipelineEvent?>();
                    VectorTransferRetireEffect?[] vectorTransferEffects = Array.Empty<VectorTransferRetireEffect?>();
                    RetireWindowBatch retireBatch = new(
                        retireRecords,
                        retireEffects,
                        pipelineEvents,
                    ...
                    CaptureTestStreamPublicationsWithStableCoreIdentity(
                        in request,
                        ref retireBatch,
                ...
                ExecuteTestStreamRequestWithStableCoreIdentity(
                    in request,
                    executionOwnerThreadId);
            }
                ...
                int vtId = 0)
            {
                ...
                lane.PC = pc;
                lane.OpCode = (uint)InstructionsEnum.SD;
                ...
                InstructionsEnum? opcode = null)
            {
                ...
                if (storeMemSub != null)
                {
                    // RF-10.4 controller causality: one tick stages readiness;
                    // the next tick publishes it to the MEM consumer.
                    storeMemSub.AdvanceCycles(2);
                    ExecuteExplicitPacketMemoryWork();
                }

                ScalarMemoryLaneState readyLane = pipeMEM.GetLane(laneIndex);
                if (!readyLane.ResultReady ||
                    readyLane.PendingMemoryRequest != null ||
                    readyLane.PendingMemoryControllerRequestId.HasValue ||
                    !readyLane.DefersStoreCommitToWriteBack)
                {
                    throw new InvalidOperationException(
                ...
                InstructionsEnum? opcode = null)
            {
                ...
                if (loadMemSub != null)
                {
                    // RF-10.2 controller causality: one tick stages the result;
                    // the next tick publishes it to the consumer.
                    loadMemSub.AdvanceCycles(2);
                    ExecuteExplicitPacketMemoryWork();
                }
                ...
                int vtId = 0)
            {
                ...
                if (!ExecuteTestAtomicWithStableCoreIdentity(atomicMicroOp))
                {
                    throw new InvalidOperationException(
            ...
            private void TestExecuteSingleLaneReferenceRawFallback(
                ...
                ulong operand2)
            {
                Runtime.Telemetry.TestReferenceRawFallbackCount++;
            ...
            private static bool IsTestReferenceRawFallbackAllowed(uint opCode)
            {
                return (InstructionsEnum)opCode switch
                {
                    ...
                    InstructionsEnum.SUB => true,
                    InstructionsEnum.MUL => true,
                    InstructionsEnum.DIV => true,

                    InstructionsEnum.AND => true,
                    InstructionsEnum.OR => true,
                    InstructionsEnum.XOR => true,
                    InstructionsEnum.SLL => true,
                    InstructionsEnum.SRL => true,
            ...
            internal static YAKSys_Hybrid_CPU.Core.DecodedBundleRuntimeState
                TestBuildReplayDecodedBundleState(
                    YAKSys_Hybrid_CPU.Core.Decoder.ReplayEntry replayEntry,
                    ulong requestedPc) =>
                BuildReplayDecodedBundleState(replayEntry, requestedPc);
                ...
                VliwBundleAnnotations? annotations = null)
            {
                ...
                pipeIF.DecodeContext = CaptureReplayDecodeContext();
            ...
            internal (bool Occupied, bool ResultReady, bool VectorComplete, string? MicroOpType)
                TestReadExecuteLaneStatus(byte laneIndex)
            {
                ScalarExecuteLaneState lane = pipeEX.GetLane(laneIndex);
                return (lane.IsOccupied, lane.ResultReady, lane.VectorComplete, lane.MicroOp?.GetType().Name);
            }
            ...
            /// <summary>
            /// TEST-ONLY: prove that a decoded younger bundle cannot cross an older
            /// FullSerial carrier in either EX or MEM, independently of register masks.
            /// </summary>
            internal bool TestCheckPipelineHazardForOlderSerializingCarrier(bool carrierInMemory)
            {
                pipeID.Clear();
                pipeEX.Clear();
                pipeMEM.Clear();
                ...
                Core.SysEventMicroOp serializing = Core.SysEventMicroOp.ForEcall();
                if (carrierInMemory)
                {
                    ScalarMemoryLaneState lane = new();
                    lane.Clear(7);
                    lane.IsOccupied = true;
                    lane.ResultReady = true;
                    lane.MicroOp = serializing;
                    pipeMEM.SetLane(7, lane);
                    pipeMEM.Valid = true;
                    pipeMEM.UsesExplicitPacketLanes = true;
                    pipeMEM.MaterializedPhysicalLaneCount = 1;
                }
            ...
            /// <summary>
            /// TEST-ONLY: prove that a decoded FullSerial carrier waits for all older
            /// ordinary work, even when the older result is already marked ready.
            /// </summary>
            internal bool TestCheckPipelineHazardForDecodedSerializingCarrier(bool olderInMemory)
            {
                pipeID.Clear();
                pipeEX.Clear();
                pipeMEM.Clear();
                ...
                pipeID.Valid = true;
                pipeID.PC = 0x6700;
                pipeID.MicroOp = Core.SysEventMicroOp.ForEcall();
                ...
                if (olderInMemory)
                {
                    ...
                    lane.IsOccupied = true;
                    lane.ResultReady = true;
                    lane.MicroOp = new Core.ScalarALUMicroOp();
                    pipeMEM.SetLane(0, lane);
                    pipeMEM.Valid = true;
                    pipeMEM.UsesExplicitPacketLanes = true;
                    pipeMEM.MaterializedPhysicalLaneCount = 1;

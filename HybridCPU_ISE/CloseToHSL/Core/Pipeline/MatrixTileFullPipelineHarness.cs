...
namespace YAKSys_Hybrid_CPU.Core
{
    ...
    public static class MatrixTileFullPipelineHarness
    {
            ...
            MatrixTileFullPipelineHarnessOptions? options = null)
        {
            ...
            for (int bundleIndex = 0; bundleIndex < loweredBundles.Count; bundleIndex++)
            {
                ...
                DecodedInstructionBundle decodedBundle = decoder.DecodeInstructionBundle(
                    ...
                    bundleAddress,
                    bundleSerial);
                MicroOp?[] carrierBundle = BuildCanonicalMatrixTileCarrierBundle(decodedBundle);
        ...
        internal static MicroOp?[] BuildCanonicalMatrixTileCarrierBundle(
            DecodedInstructionBundle decodedBundle)
        {
            ArgumentNullException.ThrowIfNull(decodedBundle);
            CanonicalBundle canonicalBundle = decodedBundle.CanonicalBundle
                ?? throw new InvalidOperationException(
                    "MatrixTile harness requires the public canonical decoder handoff.");
            var carriers = new MicroOp?[BundleMetadata.BundleSlotCount];
            for (int slotIndex = 0; slotIndex < carriers.Length; slotIndex++)
            {
                DecodedInstruction decodedSlot = decodedBundle.GetDecodedSlot(slotIndex);
                CanonicalDecodedInstruction canonicalSlot = canonicalBundle.GetSlot(slotIndex);
                if (!decodedSlot.IsOccupied)
                {
                    if (canonicalSlot.IsOccupied || canonicalSlot.StaticBinding is not null)
                    {
                        throw new InvalidOperationException(
                            $"MatrixTile canonical handoff disagrees about empty slot {slotIndex}.");
                    }

                    continue;
                }

                if (!MatrixTileRuntimeOwnedVlmRows.IsMatrixTileOpcode(
                        decodedSlot.CanonicalOpcode.ToInstructionsEnum()))
                {
                    throw new InvalidOperationException(
                        $"MatrixTile-only harness received non-MatrixTile occupied slot {slotIndex}.");
                }
                ...
                {
                    // The harness records decoder-published projection faults in
                    // its per-slot evidence below. No carrier crosses admission.
                    carriers[slotIndex] = null;
                }
            }

...
namespace YAKSys_Hybrid_CPU
{
    public partial struct Processor
    {
        public sealed partial class CPU_Core
        {
            ...
            private Core.DomainHypercallExactRuntimeProfile? _configuredExactProbeRuntimeProfile;
            private Core.MicroOpScheduler? _configuredExactProbeScheduler;
            ...
            internal bool HasActiveConfiguredExactProbeRuntimeProfile =>
                _configuredExactProbeRuntimeProfile?.LifecycleGate.Observe().State ==
                    Core.DomainHypercallLifecycleState.ActiveExactProfile;

            private void EnsureConfiguredExactProbeRuntimeProfile(
                Core.MicroOpScheduler scheduler)
            {
                CpuExactProbeRuntimeConstructionProfile? request =
                    _platformContext.ExactProbeRuntimeProfile;
                if (request is null)
                    return;
                if (!request.Value.IsConfigured ||
                    !string.Equals(
                        request.Value.ProvenContourCommitSha,
                        CpuExactProbeRuntimeConstructionProfile.ProvenContourSubjectCommitSha,
                        StringComparison.Ordinal))
                {
                    throw new InvalidOperationException(
                        "Exact probe construction lost its reviewed contour SHA binding.");
                }
                if (_configuredExactProbeRuntimeProfile is not null)
                {
                    if (!ReferenceEquals(scheduler, _configuredExactProbeScheduler) ||
                        !HasActiveConfiguredExactProbeRuntimeProfile)
                    {
                        throw new InvalidOperationException(
                            "Exact probe construction cannot silently replace or reactivate its canonical binding.");
                    }
                    return;
                }

                var profile = new Core.DomainHypercallExactRuntimeProfile(
                    request.Value.DomainTag,
                    scheduler,
                    _domainHypercallRetireOwner);
                Core.DomainHypercallExactActivationResult activation =
                    profile.Activate(Core.DomainHypercallExactActivationRequest.Phase38Exact);
                if (!activation.IsActivated)
                {
                    _ = profile.KillSwitch(TimeSpan.Zero);
                    throw new InvalidOperationException(
                        $"Exact probe production construction failed closed: {activation.Decision}: {activation.Reason}");
                }

                _configuredExactProbeRuntimeProfile = profile;
                _configuredExactProbeScheduler = scheduler;
            }

            /// <summary>
            /// Operational rollback for the explicitly constructed exact probe
            /// profile. It closes new admission, drains live authorities and
            /// restores the deterministic fault-only compatibility fallback.
            /// </summary>
            public bool DisableConfiguredExactProbeRuntimeProfile(TimeSpan transitionTimeout)
            {
                if (transitionTimeout < TimeSpan.Zero)
                    throw new ArgumentOutOfRangeException(nameof(transitionTimeout));
                Core.DomainHypercallExactRuntimeProfile? profile =
                    _configuredExactProbeRuntimeProfile;
                if (profile is null)
                    return true;

                Core.DomainHypercallKillSwitchResult result =
                    profile.KillSwitch(transitionTimeout);
                return result.IsDeterministicFaultOnly;
            }

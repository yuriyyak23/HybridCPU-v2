...
namespace YAKSys_Hybrid_CPU
{
    public partial struct Processor
    {
        public sealed partial class CPU_Core
        {
            ...
            private void ApplyLiveCpuStateWithStableCoreIdentity(LiveCpuStateAdapter liveState)
            {
                // RF-11.33: keep the legacy ref ABI local while preserving the sole
                // reference-owned runtime identity and ApplyTo statement order.
                CPU_Core stableCoreIdentity = this;
                liveState.ApplyTo(ref stableCoreIdentity);
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private Core.Pipeline.PipelineEvent? MaterializeSystemEventWithStableCoreIdentity(
                Core.SysEventMicroOp systemEventMicroOp)
            {
                if (systemEventMicroOp.EventKind == Core.SystemEventKind.Ecall)
                {
                    int vtId = NormalizePipelineStateVtId(systemEventMicroOp.OwnerThreadId);
                    ulong ecallCode = GetRegisterValueWithForwarding(
                        vtId,
                        HybridCPU.Platform.Contracts.HybridCpuExternalServiceEcallContractV1.EcallNumberRegister);
                    return new Core.Pipeline.EcallEvent
                    {
                        VtId = (byte)vtId,
                        BundleSerial = 0,
                        EcallCode = unchecked((long)ecallCode)
                    };
                }

                // RF-11.34: retain ECALL's architectural a7 read and the legacy
                // ref ABI without passing the value-type facade instance itself.
                CPU_Core stableCoreIdentity = this;
                return systemEventMicroOp.CreatePipelineEvent(ref stableCoreIdentity);
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private Core.CsrRetireEffect MaterializeCsrEffectWithStableCoreIdentity(
                Core.CSRMicroOp csrMicroOp)
            {
                // RF-11.35: retain CSR surface resolution/read timing while
                // localizing the legacy ref facade ABI.
                CPU_Core stableCoreIdentity = this;
                return csrMicroOp.CreateRetireEffect(ref stableCoreIdentity);
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private void WriteRetiredCsrWithStableCoreIdentity(
                Core.CsrStorageSurface storageSurface,
                ulong address,
                ulong value)
            {
                // RF-11.36: retain selected-retire publication and exact CSR
                // storage routing while localizing the legacy ref facade ABI.
                CPU_Core stableCoreIdentity = this;
                Core.CSRMicroOp.WriteCsr(ref stableCoreIdentity, storageSurface, address, value);
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private void EmitBatchMicroOpRetireRecordsWithStableCoreIdentity(
                ref RetireWindowBatch retireBatch,
                in ScalarWriteBackLaneState lane)
            {
                // RF-11.37: preserve bounded-batch capture order and the legacy
                // ref ABI without passing the value-type facade instance itself.
                CPU_Core stableCoreIdentity = this;
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private void EmitExclusionProbeRetireRecordsWithStableCoreIdentity(
                Core.MicroOp microOp,
                Span<RetireRecord> probeRetireRecords,
                ref int probeRetireRecordCount)
            {
                // RF-11.38: preserve the fail-closed exclusion probe while
                // localizing its legacy ref facade ABI.
                CPU_Core stableCoreIdentity = this;
                microOp.EmitWriteBackRetireRecords(
                    ref stableCoreIdentity,
                    probeRetireRecords,
                ...
                in ScalarWriteBackLaneState lane)
            {
                ...
                else if (lane.MicroOp is Core.VConfigMicroOp vectorConfigMicroOp)
                {
                    ...
                    {
                        EmitBatchMicroOpRetireRecordsWithStableCoreIdentity(
                            ref retireBatch,
                            lane);
                    }
                }
                ...
                else if (lane.MicroOp is Core.VectorTransferMicroOp vectorTransferMicroOp &&
                    vectorTransferMicroOp.HasControllerRetireEffect)
                {
                    retireBatch.CaptureVectorTransferEffect(
                        vectorTransferMicroOp.CreateRetireEffect());
                }
                else
                {
                    EmitBatchMicroOpRetireRecordsWithStableCoreIdentity(
                ...
                if (lane.GeneratedEvent != null)
                {
                    ...
                    if (lane.GeneratedEvent is Core.Pipeline.EcallEvent)
                    {
                        // ECALL carries the exact architectural a7/x17 code captured at EX.
                        // Reducing it to SystemEventKind would reconstruct a zero-code event
                        // at retire and silently bypass the platform ECALL bridge.
                    ...
                    else if (lane.MicroOp is Core.SysEventMicroOp typedSystemEventMicroOp)
                    {
                ...
                {
                    retireBatch.CaptureRetireWindowVectorStreamDirty(lane.VirtualThreadId);
                }
            ...
            private void FinalizeRetiredWriteBackLane(
                ...
                in ScalarWriteBackLaneState lane)
            {
                Runtime.Retire.LastRetiredLaneIndex = laneIndex;
                Runtime.Retire.LastRetiredOpcode = lane.OpCode;
                Runtime.Retire.LastRetiredWritesRegister = lane.WritesRegister;
                Runtime.Retire.LastRetiredDestinationRegister = lane.DestRegID;
                Runtime.Retire.LastRetiredResultValue = lane.ResultValue;
                Runtime.Retire.LastRetiredMicroOpType = lane.MicroOp?.GetType().Name ?? string.Empty;
                Runtime.Retire.LastRetiredMicroOpDescription = lane.MicroOp?.GetDescription() ?? string.Empty;
            ...
            /// <summary>
            /// Validates the complete materialized selected-prefix batch before any
            /// retire counter, bookkeeping, speculation-budget or architectural
            /// publication mutation.  This is deliberately a validation boundary,
            /// not a second scheduler, retire selector, fault resolver or publisher.
            /// </summary>
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
                ...
                for (int i = 0; i < capturedRetireEffects.Length; i++)
                {
                    ...
                    switch (retireEffect.Kind)
                    {
                        case RetireWindowEffectKind.DeferredStoreCommit:
                            PrevalidateDeferredStoreEffect(retireEffect.DeferredStoreLaneIndex);
                            break;

                        case RetireWindowEffectKind.Csr:
                            PrevalidateCsrEffect(retireEffect.CsrEffect);
                            break;

                        case RetireWindowEffectKind.VectorConfig:
                            PrevalidateVectorConfigEffect(retireEffect.VectorConfigEffect);
                            break;
                        ...
                        case RetireWindowEffectKind.System:
                            PrevalidateSystemEffect(
                                retireEffect.SystemEventKind,
                                retireEffect.SystemEventOrderGuarantee,
                                retireEffect.SystemEventVtId);
                            break;

                        case RetireWindowEffectKind.PipelineEvent:
                            _ = retireBatch.GetPipelineEventPayload(retireEffect.PipelineEventSlot);
                            PrevalidateSystemOrderAndVt(
                                retireEffect.SystemEventOrderGuarantee,
                                retireEffect.SystemEventVtId,
                                "pipeline-event retire effect");
                            break;

                        case RetireWindowEffectKind.Vmx:
                            PrevalidateVmxEffect(retireEffect.VmxEffect, retireEffect.VmxEffectVtId);
                            break;

                        case RetireWindowEffectKind.ScalarMemoryStore:
                            PrevalidateScalarStoreEffect(
                        ...
                        case RetireWindowEffectKind.PredicateState:
                            if (retireEffect.PredicateRegisterId > 15)
                            {
                                throw new InvalidOperationException(
                                    $"Retire batch predicate effect targets invalid predicate register {retireEffect.PredicateRegisterId}.");
                            }
                            break;

                        case RetireWindowEffectKind.VectorStreamDirty:
                            PrevalidateVirtualThreadId(
                                retireEffect.VectorStreamDirtyVtId,
                                "vector-stream-dirty retire effect");
                            break;

                        case RetireWindowEffectKind.VectorTransfer:
                            PrevalidateVectorTransferEffect(
                                retireBatch.GetVectorTransferEffectPayload(
                                    retireEffect.VectorTransferEffectSlot));
                            break;

                        case RetireWindowEffectKind.SerializingBoundary:
                            break;

                        default:
                            throw new InvalidOperationException(
                                $"Retire batch carries unsupported effect kind {retireEffect.Kind}.");
                    }
                }
            }
            ...
            private void PrevalidateDeferredStoreEffect(byte laneIndex)
            {
                ...
                {
                    throw new InvalidOperationException(
                        $"Retire batch deferred store names invalid WB lane {laneIndex}.");
                }

                ScalarWriteBackLaneState lane = pipeWB.GetLane(laneIndex);
                if (!lane.IsOccupied || !lane.DefersStoreCommitToWriteBack ||
                    !lane.IsMemoryOp || lane.IsLoad)
                {
                    throw new InvalidOperationException(
                        $"Retire batch deferred store lane {laneIndex} is not a live deferred scalar store carrier.");
                }
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private void PrevalidateCsrEffect(in Core.CsrRetireEffect csrEffect)
            {
                if (!csrEffect.ClearsArchitecturalExceptionState &&
                    !csrEffect.HasCsrWrite &&
                    !csrEffect.HasRegisterWriteback)
                {
                    throw new InvalidOperationException("Retire batch carries an empty CSR effect.");
                }

                if (csrEffect.HasRegisterWriteback &&
                    (uint)csrEffect.DestRegId >= (uint)RenameMap.ArchRegs)
                {
                    throw new InvalidOperationException(
                        $"Retire batch CSR effect targets invalid architectural register {csrEffect.DestRegId}.");
                }

                if (csrEffect.HasCsrWrite &&
                    csrEffect.StorageSurface is not (Core.CsrStorageSurface.VectorPodPlane or Core.CsrStorageSurface.WiredCsrFile))
                {
                    throw new InvalidOperationException(
                        "Retire batch CSR write has no supported storage surface.");
                }
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private static void PrevalidateVectorConfigEffect(in Core.VectorConfigRetireEffect vectorConfigEffect)
            {
                if (!vectorConfigEffect.IsValid ||
                    vectorConfigEffect.TailAgnostic > 1 ||
                    vectorConfigEffect.MaskAgnostic > 1 ||
                    (vectorConfigEffect.HasRegisterWriteback &&
                     vectorConfigEffect.DestinationRegister >= RenameMap.ArchRegs))
                {
                    throw new InvalidOperationException("Retire batch carries an invalid vector-config effect.");
                }
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private void PrevalidateAtomicEffect(in Core.AtomicRetireEffect atomicEffect)
            {
                if (!atomicEffect.IsValid ||
                    atomicEffect.CoreId != unchecked((ushort)CoreID) ||
                    atomicEffect.AccessSize != Core.Memory.MainMemoryAtomicMemoryUnit.ResolveAccessSize(atomicEffect.Opcode) ||
                    (atomicEffect.HasRegisterDestination &&
                     atomicEffect.DestinationRegister >= RenameMap.ArchRegs))
                {
                    throw new InvalidOperationException("Retire batch carries an invalid atomic effect.");
                }

                PrevalidateVirtualThreadId(atomicEffect.VirtualThreadId, "atomic retire effect");
                PrevalidateScalarStoreEffect(
                    atomicEffect.Address,
                    atomicEffect.AccessSize,
                    "Atomic retire effect");
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private void PrevalidateVmxEffect(in Core.VmxRetireEffect vmxEffect, int virtualThreadId)
            {
                if (!vmxEffect.IsValid ||
                    (vmxEffect.HasRegisterDestination &&
                     vmxEffect.RegisterDestination >= RenameMap.ArchRegs))
                {
                    throw new InvalidOperationException("Retire batch carries an invalid VMX effect.");
                }

                PrevalidateVirtualThreadId(virtualThreadId, "VMX retire effect");
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private void PrevalidateSystemEffect(
                Core.SystemEventKind systemEventKind,
                Core.SystemEventOrderGuarantee orderGuarantee,
                int virtualThreadId)
            {
                if (!Enum.IsDefined(systemEventKind))
                {
                    throw new InvalidOperationException(
                        $"Retire batch carries unsupported system-event kind {systemEventKind}.");
                }

                PrevalidateSystemOrderAndVt(orderGuarantee, virtualThreadId, "system retire effect");
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private void PrevalidateSystemOrderAndVt(
                Core.SystemEventOrderGuarantee orderGuarantee,
                int virtualThreadId,
                string effectDescription)
            {
                if (!Enum.IsDefined(orderGuarantee))
                {
                    throw new InvalidOperationException(
                        $"Retire batch {effectDescription} carries unsupported ordering guarantee {orderGuarantee}.");
                }

                PrevalidateVirtualThreadId(virtualThreadId, effectDescription);
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private void PrevalidateScalarStoreEffect(
                ulong memoryAddress,
                byte memoryAccessSize,
                string effectDescription)
            {
                byte accessSize = NormalizeScalarMemoryAccessSize(memoryAccessSize);
                if (accessSize is not (1 or 2 or 4 or 8) ||
                    !IsScalarMemoryAccessInBounds(memoryAddress, accessSize))
                {
                    throw new InvalidOperationException(
                        $"{effectDescription} carries invalid memory publication payload.");
                }
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private static void PrevalidateVirtualThreadId(int virtualThreadId, string effectDescription)
            {
                if ((uint)virtualThreadId >= SmtWays)
                {
                    throw new InvalidOperationException(
                        $"Retire batch {effectDescription} targets invalid VT{virtualThreadId}.");
                }
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private void PrevalidateVectorTransferEffect(
                Core.VectorTransferRetireEffect vectorTransferEffect)
            {
                if (vectorTransferEffect.Opcode is not (
                        IsaOpcodeValues.VLOAD or IsaOpcodeValues.VSTORE) ||
                    vectorTransferEffect.ElementCount == 0 ||
                    vectorTransferEffect.ElementSize <= 0 ||
                    vectorTransferEffect.Stride == 0)
                {
                    throw new InvalidOperationException(
                        "Retire batch carries an invalid canonical vector-transfer effect.");
                }
                ...
                try
                {
                    expectedBytes = checked(
                        vectorTransferEffect.ElementCount *
                        (ulong)vectorTransferEffect.ElementSize);
                }
                catch (OverflowException exception)
                {
                    throw new InvalidOperationException(
                        "Retire batch vector-transfer byte count overflowed.",
                        exception);
                }

                if (expectedBytes > int.MaxValue ||
                    vectorTransferEffect.PackedData.Length != (int)expectedBytes)
                {
                    throw new InvalidOperationException(
                        "Retire batch vector-transfer payload length is invalid.");
                }

                for (ulong element = 0;
                     element < vectorTransferEffect.ElementCount;
                     element++)
                {
                    ulong destinationAddress;
                    try
                    {
                        destinationAddress = checked(
                            vectorTransferEffect.DestinationAddress +
                            checked(element * vectorTransferEffect.Stride));
                    }
                    catch (OverflowException exception)
                    {
                        throw new InvalidOperationException(
                            "Retire batch vector-transfer destination address overflowed.",
                            exception);
                    }

                    ThrowIfBoundMainMemoryRangeUnavailable(
                        destinationAddress,
                        vectorTransferEffect.ElementSize,
                        "Canonical vector-transfer retire prevalidation");
                }
            }
                ...
                out int retiredPcWriteVtId)
            {
                ...
                for (int i = 0; i < capturedRetireEffects.Length; i++)
                {
                    ...
                    switch (retireEffect.Kind)
                    {
                        ...
                        case RetireWindowEffectKind.VectorTransfer:
                            ApplyRetiredVectorTransferEffect(
                                retireBatch.GetVectorTransferEffectPayload(
                                    retireEffect.VectorTransferEffectSlot));
                            break;
                    }
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private void ApplyRetiredVectorTransferEffect(
                Core.VectorTransferRetireEffect vectorTransferEffect)
            {
                byte[] packedData = vectorTransferEffect.CopyPackedData();
                for (ulong element = 0;
                     element < vectorTransferEffect.ElementCount;
                     element++)
                {
                    ulong destinationAddress = checked(
                        ...
                        checked(element * vectorTransferEffect.Stride));
                    int offset = checked(
                        (int)(element * (ulong)vectorTransferEffect.ElementSize));
                    byte[] elementData = packedData
                        .AsSpan(offset, vectorTransferEffect.ElementSize)
                        .ToArray();
                    WriteBoundMainMemoryExact(
                        destinationAddress,
                        elementData,
                        "Canonical vector-transfer selected-retire publication");
                    GetBoundMemorySubsystem()?.CycleController.RecordCommittedDataWriteBytes(
                        vectorTransferEffect.ElementSize);
                }
            }
                ...
                ReadOnlySpan<RetireWindowEffect> capturedRetireEffects)
            {
                ...
                using Core.ArchitecturalCompletionCommitOwner.CanonicalRetireWindowCommitLease?
                    completionLease = PrepareArchitecturalCompletionAtCanonicalRetireBoundary(
                        ref retireBatch);
                ApplyRetireBatchLateEffectsAndRedirect(
                    ...
                    retiredPcWriteVtId);
                CompleteArchitecturalCompletionAtCanonicalRetireBoundary(
                    completionLease,
                    retireBatch.ArchitecturalCompletionCandidates.Length);
                ...
                if (retireLaneCount != 0)
                {
                    Runtime.Retire.LastRetiredBundlePc = retireContourPc;
                    Runtime.Retire.RetiredBundleSequence = checked(Runtime.Retire.RetiredBundleSequence + 1);
                }

                if (retireLaneCount != 0 && !hasRetireWindowExceptionDecision)
                {
                    TryDeliverPendingVmxEventAtProductionSafeBoundary(ReadActiveVirtualThreadId());
                }
            }
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private Core.ArchitecturalCompletionCommitOwner.CanonicalRetireWindowCommitLease?
                PrepareArchitecturalCompletionAtCanonicalRetireBoundary(
                ref RetireWindowBatch retireBatch)
            {
                ...
                Core.ArchitecturalCompletionBatchPreparationResult preparation =
                    ArchitecturalCompletionCommitOwner.PrepareCanonicalRetireWindow(
                        CanonicalPipelineCompletionProducer,
                        candidates);
                if (!preparation.IsPrepared)
                {
                    throw new InvalidOperationException(
                        $"Mandatory architectural completion was denied before retire-visible effects: {preparation.Decision}: {preparation.Reason}");
                }
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private static void CompleteArchitecturalCompletionAtCanonicalRetireBoundary(
                Core.ArchitecturalCompletionCommitOwner.CanonicalRetireWindowCommitLease? lease,
                int expectedCommitCount)
            {
                if (lease is null)
                {
                    if (expectedCommitCount != 0)
                        throw new InvalidOperationException("Mandatory completion lease was absent for a non-empty retire candidate set.");
                    return;
                }

                Core.ArchitecturalCompletionCommitResult[] results = lease.Complete();
                if (results.Length != expectedCommitCount)
                {
                    throw new InvalidOperationException(
                        "A prevalidated mandatory completion failed before retire visibility publication.");
                }
                for (int index = 0; index < results.Length; index++)
                {
                    if (!results[index].IsCommitted)
                    {
                        throw new InvalidOperationException(
                            "A prevalidated mandatory completion failed before retire visibility publication.");
                    }
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            private static bool IsVectorStreamDirtyRetireOpcode(uint opcode)
            {
                if (opcode is
                    IsaOpcodeValues.VSTORE or
                    IsaOpcodeValues.VSCATTER or
                    IsaOpcodeValues.VPOPC or
                ...
                if (opcode is
                    IsaOpcodeValues.VSETVEXCPMASK or
                    IsaOpcodeValues.VSETVEXCPPRI or
                ...
                return OpcodeRegistry.IsVectorOp(opcode);
            }
                ...
                ulong? retiredPc)
            {
                ...
                ApplyLiveCpuStateWithStableCoreIdentity(liveState);
            }
            ...
            private Core.Pipeline.PipelineEvent? MaterializeLaneGeneratedEvent(Core.MicroOp microOp)
            {
                ...
                {
                    return MaterializeSystemEventWithStableCoreIdentity(systemEventMicroOp);
                }
            ...
            private Core.CsrRetireEffect? MaterializeLaneCsrEffect(Core.MicroOp microOp)
            {
                ...
                {
                    Core.CsrRetireEffect effect =
                        MaterializeCsrEffectWithStableCoreIdentity(csrMicroOp);
                    return effect.ClearsArchitecturalExceptionState ||
                           effect.HasCsrWrite ||
                ...
                in Core.CsrRetireEffect csrEffect)
            {
                ...
                if (csrEffect.HasCsrWrite)
                {
                    WriteRetiredCsrWithStableCoreIdentity(
                ...
                in Core.AtomicRetireEffect atomicEffect)
            {
                ...
                Core.AtomicRetireOutcome outcome = AtomicMemoryUnit.ApplyRetireEffect(atomicEffect);
                return outcome;
            }
                ...
                in ScalarWriteBackLaneState lane)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (lane.PostStageBIssuedAttempt is not null &&
                        retireRecord.Kind == RetireRecordKind.RegisterWrite)
                    {
                        // RF-08.3d consumes the exact Stage-B carrier only to
                        // freeze one scalar effect identity. Existing selected
                        // RetireWindowBatch/RetireCoordinator publication remains

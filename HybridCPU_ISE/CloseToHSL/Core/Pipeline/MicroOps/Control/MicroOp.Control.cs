...
namespace YAKSys_Hybrid_CPU.Core
{
    ...
    public sealed class BranchMicroOp : MicroOp
    {
        ...
        public void InitializeMetadata()
        {
            ...
            if (IsConditional)
            {
                ResourceMask |= ArchRegId.TryCreate(Reg1ID, out ArchRegId source1RegisterId)
                    ? ResourceMaskBuilder.ForArchitecturalRegisterRead(source1RegisterId)
                    : ResourceMaskBuilder.ForRegisterRead(Reg1ID);
                ResourceMask |= ArchRegId.TryCreate(Reg2ID, out ArchRegId source2RegisterId)
                    ? ResourceMaskBuilder.ForArchitecturalRegisterRead(source2RegisterId)
                    : ResourceMaskBuilder.ForRegisterRead(Reg2ID);
            }
            else if (opcode == Processor.CPU_Core.IsaOpcodeValues.JALR &&
                     Reg1ID != VLIW_Instruction.NoReg)
            {
                ResourceMask |= ArchRegId.TryCreate(Reg1ID, out ArchRegId baseRegisterId)
            ...
            if (publishesLinkRegister)
            {
                ResourceMask |= ArchRegId.TryCreate(
                        DestRegID,
                        out ArchRegId linkRegisterId)
                    ? ResourceMaskBuilder.ForArchitecturalRegisterWrite(linkRegisterId)
    ...
    public abstract class CSRMicroOp : MicroOp
    {
        ...
        public void InitializeMetadata()
        {
            ...
            if (publishesSourceRegisterRead)
            {
                ResourceMask |= ArchRegId.TryCreate(
                        SrcRegID,
                        out ArchRegId sourceRegisterId)
    ...
    public sealed class CsrReadCounterMicroOp : CSRMicroOp
    {
        protected override string DescriptionVerb => "CSR_READ_COUNTER";
    }

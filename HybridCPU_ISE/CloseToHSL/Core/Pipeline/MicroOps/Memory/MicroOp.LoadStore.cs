...
namespace YAKSys_Hybrid_CPU.Core
{
    ...
    {
        private bool _cpuTranslationResolved;
        private ulong _cpuTranslatedAddress;
        private ulong _cpuTranslationOwnerEpoch;
        private bool _baseRelativeAddressResolved;
        private ulong _baseRelativeAddress;

        /// <summary>
        /// Runtime-owned scalar base-register plus signed-displacement contour.
        /// Decode publishes the shape; execution resolves the effective address.
        /// </summary>
        public bool UsesBaseRegisterAddressing { get; set; }
        public long AddressDisplacement { get; set; }
        ...
        internal virtual void ResolveBaseRelativeAddressForPipeline(Processor.CPU_Core core) { }
        internal virtual void ResolveBaseRelativeAddressForPipeline(ulong baseValue) { }

        protected ulong ResolveBaseRelativeAddress(
            Processor.CPU_Core core,
            ushort baseRegister)
        {
            if (!UsesBaseRegisterAddressing)
                return MemoryAddress;
            if (_baseRelativeAddressResolved)
                return _baseRelativeAddress;
            if (baseRegister == VLIW_Instruction.NoReg)
                throw new InvalidOperationException(
                    "Base-relative scalar memory execution requires an architectural base register.");

            int vtId = TryGetArchitecturalOwnerVtId(out VtId checkedOwner)
                ? checkedOwner.Value
                : NormalizeExecutionVtId(OwnerThreadId);
            ulong baseValue = core.ReadArch(vtId, baseRegister);
            _baseRelativeAddress = unchecked(baseValue + (ulong)AddressDisplacement);
            _baseRelativeAddressResolved = true;
            return _baseRelativeAddress;
        }

        protected ulong ResolveCpuTranslatedAddress(
            ref Processor.CPU_Core core,
            CpuInstructionTranslationAccessKind accessKind,
            byte accessSize)
        {
            if (_cpuTranslationResolved)
                return _cpuTranslatedAddress;

            CpuInstructionTranslationResult result =
                core.TranslateScalarInstructionMemoryAddress(this, accessKind, accessSize);
            if (!result.Succeeded)
            {
                if (result.Fault is not CpuInstructionTranslationFault fault)
                    throw new InvalidOperationException("Failed CPU scalar translation omitted typed fault facts.");
                throw new CpuInstructionTranslationFaultException(fault);
            }

            _cpuTranslatedAddress = result.PhysicalAddress;
            _cpuTranslationOwnerEpoch = result.OwnerEpoch;
            _cpuTranslationResolved = true;
            return _cpuTranslatedAddress;
        }

        internal bool TryGetCpuTranslatedAddress(out ulong physicalAddress, out ulong ownerEpoch)
        {
            physicalAddress = _cpuTranslatedAddress;
            ownerEpoch = _cpuTranslationOwnerEpoch;
            return _cpuTranslationResolved;
        }

        internal ulong ResolveCpuTranslatedAddressForPipeline(
            ref Processor.CPU_Core core,
            CpuInstructionTranslationAccessKind accessKind,
            byte accessSize) =>
            ResolveCpuTranslatedAddress(ref core, accessKind, accessSize);
    ...
    public class LoadMicroOp : LoadStoreMicroOp
    {
        ...
        internal override void ResolveBaseRelativeAddressForPipeline(Processor.CPU_Core core)
        {
            if (!UsesBaseRegisterAddressing) return;
            Address = ResolveBaseRelativeAddress(core, BaseRegID);
            ReadMemoryRanges = new[] { (Address, (ulong)Size) };
        }

        internal override void ResolveBaseRelativeAddressForPipeline(ulong baseValue)
        {
            if (!UsesBaseRegisterAddressing) return;
            Address = unchecked(baseValue + (ulong)AddressDisplacement);
            ReadMemoryRanges = new[] { (Address, (ulong)Size) };
        }
        ...
        public override bool Execute(ref Processor.CPU_Core core)
        {
            ...
            try
            {
                ...
                ulong translatedAddress = ResolveCpuTranslatedAddress(
                    ref core,
                ...
                if (HasExactMainMemoryRange(ref core, translatedAddress, Size))
                {
                    byte[] bytes = new byte[Size];
                    ReadMainMemoryExact(ref core, translatedAddress, bytes, "LoadMicroOp.Execute()");
                    _loadedValue = DecodeLoadValue(
                        OpCode,
                        bytes,
                        Size,
            ...
            catch (CpuInstructionTranslationFaultException)
            {
                if (this.IsSpeculative)
                {
                    this.MarkFaulted();
        ...
        public override bool Execute(ref Processor.CPU_Core core)
        {
            ...
            try
            {
                ...
                if (HasExactMainMemoryRange(ref core, translatedAddress, Size))
                {
                    ...
                    core.WriteBoundMainMemoryExact(translatedAddress, buffer, "StoreMicroOp.Execute()");
                    return true;
                }

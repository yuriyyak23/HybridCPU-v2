...
namespace YAKSys_Hybrid_CPU.Core
{
    ...
    public class CustomAcceleratorMicroOp : MicroOp
    {
        ...
        public void InitializeMetadata(int acceleratorId, int[] inputRegIds, int outputRegId)
        {
            ...
            {
                ResourceMask |= ArchRegId.TryCreate(regId, out ArchRegId inputRegister)
            ...
            if (WritesRegister)
            {
                ResourceMask |= ArchRegId.TryCreate(outputRegId, out ArchRegId outputRegister)
    ...
    public class MoveMicroOp : MicroOp
    {
        ...
        private void InitializeMetadata()
        {
            ...
            for (int i = 0; i < readRegs.Count; i++)
                ResourceMask |= ArchRegId.TryCreate(readRegs[i], out ArchRegId readRegister)
            ...
            for (int i = 0; i < writeRegs.Count; i++)
                ResourceMask |= ArchRegId.TryCreate(writeRegs[i], out ArchRegId writeRegister)
        ...
        {
            byte dataType = ResolveMoveDataType();
            bool isStoreShape = dataType == 2;
            bool isLoadShape = dataType == 3;

...
namespace YAKSys_Hybrid_CPU.Core
{
    ...
    public static class ResourceMaskBuilder
    {
        ...
        private static int RequireResourceId(
            int resourceId,
            int resourceCount,
            string parameterName,
            string resourceName)
        {
            if ((uint)resourceId >= (uint)resourceCount)
            {
                throw new ArgumentOutOfRangeException(
                    parameterName,
                    resourceId,
                    $"Invalid {resourceName} id; expected [0, {resourceCount - 1}]. Resource 0 is valid and must not be used as a fallback.");
            }
        ...
        {
            bankId = RequireResourceId(bankId, 16, nameof(bankId), "memory-bank");
        ...
        {
            vtId = RequireResourceId(vtId, Processor.CPU_Core.SmtWays, nameof(vtId), "virtual-thread");
        ...
        /// <summary>
        /// Build the existing per-VT register-read mask from a checked
        /// architectural-register identity while retaining the raw VT boundary.
        /// </summary>
        public static ResourceBitset ForArchitecturalRegisterRead(ArchRegId regId, int vtId) =>
            ForRegisterRead(regId.Value, vtId);

        /// <summary>
        /// Build the existing per-VT register-write mask from a checked
        /// architectural-register identity while retaining the raw VT boundary.
        /// </summary>
        public static ResourceBitset ForArchitecturalRegisterWrite(ArchRegId regId, int vtId) =>
            ForRegisterWrite(regId.Value, vtId);
        ...
        /// <summary>
        /// Build the existing register-read mask from a checked architectural
        /// register identity.
        /// </summary>
        public static ResourceBitset ForArchitecturalRegisterRead(ArchRegId regId) =>
            ForRegisterRead(regId.Value);

        /// <summary>
        /// Build the existing register-write mask from a checked architectural
        /// register identity.
        /// </summary>
        public static ResourceBitset ForArchitecturalRegisterWrite(ArchRegId regId) =>
            ForRegisterWrite(regId.Value);
        ...
        {
            domainId = RequireResourceId(domainId, 16, nameof(domainId), "memory-domain");
        ...
        {
            channelId = RequireResourceId(channelId, 4, nameof(channelId), "DMA-channel");
        ...
        {
            engineId = RequireResourceId(engineId, 4, nameof(engineId), "stream-engine");
            return new ResourceBitset(1UL << (STREAM_ENGINE_BASE + engineId), 0);
        }

        /// <summary>
        /// Builds the existing stream-engine resource mask from a checked
        /// selector. Representation does not grant scheduling authority.
        /// </summary>
        public static ResourceBitset ForStreamEngine(StreamEngineId engineId) =>
            ForStreamEngine(engineId.Value);
        ...
        {
            accelId = RequireResourceId(accelId, 4, nameof(accelId), "accelerator");
        ...
        /// <summary>
        /// Build the existing 128-bit register-read safety mask from a checked
        /// architectural-register identity.
        /// </summary>
        public static SafetyMask128 ForArchitecturalRegisterRead128(ArchRegId regId)
        {
            ResourceBitset res = ForArchitecturalRegisterRead(regId);
            return new SafetyMask128(res.Low, res.High);
        }

        /// <summary>
        /// Build the existing 128-bit register-write safety mask from a checked
        /// architectural-register identity.
        /// </summary>
        public static SafetyMask128 ForArchitecturalRegisterWrite128(ArchRegId regId)
        {
            ResourceBitset res = ForArchitecturalRegisterWrite(regId);
            return new SafetyMask128(res.Low, res.High);
        }
        ...
        /// <summary>
        /// Builds the existing 128-bit stream-engine safety mask from a
        /// checked selector. Representation does not grant scheduling authority.
        /// </summary>
        public static SafetyMask128 ForStreamEngine128(StreamEngineId engineId)
        {
            ResourceBitset res = ForStreamEngine(engineId);
            return new SafetyMask128(res.Low, res.High);
        }
        ...
        {
            channelId = RequireResourceId(channelId, 32, nameof(channelId), "extended-GRLB-channel");
        ...
        {
            domainId = RequireResourceId(domainId, 16, nameof(domainId), "extended-memory-domain");
    ...
    public abstract class MicroOp
    {
        ...
        // RF-08.3d: optional exact issued-attempt sideband. It is assigned only
        // by the successful existing Stage-B commit arm and follows this same
        // live MicroOp reference through FSP-derived packet/latch transport.
        internal PostStageBIdentityTemplate? PostStageBIdentityTemplate { get; set; }
        internal PostStageBIssuedAttempt? PostStageBIssuedAttempt { get; set; }
        ...
        /// <summary>
        /// Attempts to project the raw architectural/effect-owner thread
        /// identity into the sole checked SMT virtual-thread representation.
        /// This projection provides representational safety only; it does not
        /// grant legality, ownership, admission, execution, replay, completion
        /// or publication authority.
        /// </summary>
        public bool TryGetArchitecturalOwnerVtId(out VtId ownerVtId) =>
            VtId.TryCreate(OwnerThreadId, out ownerVtId);
        ...
        /// <summary>
        /// Validates an owner-thread identifier at execution ingress. An invalid
        /// identifier is not permitted to alias valid VT0.
        /// </summary>
        protected static int NormalizeExecutionVtId(int ownerThreadId)
        {
            if ((uint)ownerThreadId >= (uint)Processor.CPU_Core.SmtWays)
            {
                throw new ArgumentOutOfRangeException(
                    ...
                    ownerThreadId,
                    $"Execution owner thread id is invalid; expected [0, {Processor.CPU_Core.SmtWays - 1}].");
            }
            ...
            out ulong value)
        {
            ...
            if (core is null ||
                core.ArchRenameMap is null ||
                core.PhysicalRegisters is null ||
                core.ArchContexts is null)
                return false;

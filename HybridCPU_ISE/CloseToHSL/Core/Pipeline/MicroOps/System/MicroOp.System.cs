    ...
    public sealed class SysEventMicroOp : MicroOp
    {
        ...
        public void InitializeMetadata()
        {
            ...
            WriteRegisters = EventKind == SystemEventKind.Ecall
                ? new[] { 10, 11, 12 }
                : Array.Empty<int>();
            WritesRegister = WriteRegisters.Count != 0;
            ...
            if (EventKind == SystemEventKind.Ecall)
            {
                foreach (int register in ReadRegisters)
                    ResourceMask |= ResourceMaskBuilder.ForArchitecturalRegisterRead(ArchRegId.Create(register));
                foreach (int register in WriteRegisters)
                    ResourceMask |= ResourceMaskBuilder.ForArchitecturalRegisterWrite(ArchRegId.Create(register));
            }

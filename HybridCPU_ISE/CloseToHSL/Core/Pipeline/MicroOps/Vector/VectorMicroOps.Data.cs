...
namespace YAKSys_Hybrid_CPU.Core
{
    ...
    public sealed class VectorTransferMicroOp : VectorMicroOp
    {
        ...
        private bool _completedThroughController;
        private byte[]? _transferBuffer;
        private VectorTransferRetireEffect? _retireEffect;
        private uint _acceptedOpcode;
        private ulong _acceptedSourceAddress;
        private ulong _acceptedDestinationAddress;
        private ulong _acceptedElementCount;
        private int _acceptedElementSize;
        private ushort _acceptedStride;

        internal bool OwnsPendingMemoryCompletion =>
            _state == ExecutionState.LoadingOperands &&
            _requestController != null &&
            _controllerRequestId.HasValue &&
            _requestController.OwnsOutstandingCanonicalVectorTransfer(
                _controllerRequestId.Value,
                _acceptedOpcode,
                0,
                _acceptedSourceAddress,
                _acceptedDestinationAddress,
                _acceptedElementCount,
                _acceptedElementSize,
                _acceptedStride);

        internal bool HasControllerAdmissionBackpressure =>
            _controllerAdmissionBackpressured &&
            _state == ExecutionState.NotStarted &&
            _requestController == null &&
            !_controllerRequestId.HasValue;

        internal bool HasControllerRetireEffect =>
            _completedThroughController &&
            _state == ExecutionState.Complete &&
            _retireEffect != null;
        ...
        public override bool Execute(ref Processor.CPU_Core core)
        {
            ...
            if (_state == ExecutionState.LoadingOperands &&
                (OpCode != _acceptedOpcode ||
                 readPointer != _acceptedSourceAddress ||
                 writePointer != _acceptedDestinationAddress ||
                 Instruction.StreamLength != _acceptedElementCount ||
                 elemSize != _acceptedElementSize ||
                 stride != _acceptedStride))
            {
                if (_requestController is not null && _controllerRequestId.HasValue)
                {
                    _requestController.TryCancel(_controllerRequestId.Value);
                }
                ClearControllerRequestState(resetExecutionState: true);
                throw new InvalidOperationException(
                    "VectorTransferMicroOp transfer shape changed after controller admission.");
            }
            ...
            if (_transferBuffer.Length != transferByteCount)
            {
                ...
                {
                    throw new InvalidOperationException(
                        "VectorTransferMicroOp packed byte count changed after controller admission.");
                }
                _transferBuffer = new byte[transferByteCount];
            }
            ...
            if (_state == ExecutionState.NotStarted)
            {
                MemoryCycleController? controller =
                    core.GetBoundMemorySubsystem()?.CycleController;
                if (controller != null)
                {
                    ...
                    if (admission.Status == MemoryAdmissionStatus.Rejected)
                    {
                        throw new InvalidOperationException(
                            admission.Reason ??
                            "MemoryCycleController rejected a canonical vector transfer without a reason.");
                    }

                    _requestController = controller;
                    _controllerRequestId = admission.RequestId;
                    _acceptedOpcode = OpCode;
                    _acceptedSourceAddress = readPointer;
                    _acceptedDestinationAddress = writePointer;
                    _acceptedElementCount = Instruction.StreamLength;
                    _acceptedElementSize = elemSize;
                    _acceptedStride = checked((ushort)stride);
                    _state = ExecutionState.LoadingOperands;
                    return false;
                }

                // No-bound-subsystem compatibility remains the eager functional
                // contour explicitly excluded by RF-10.10.
            ...
            if (_state == ExecutionState.LoadingOperands)
            {
                MemoryCycleController? currentController =
                    core.GetBoundMemorySubsystem()?.CycleController;
                if (_requestController == null || !_controllerRequestId.HasValue)
                {
                    throw new InvalidOperationException(
                        "VectorTransferMicroOp lost its accepted controller identity.");
                }
                if (!ReferenceEquals(_requestController, currentController))
                {
                    _requestController.TryCancel(_controllerRequestId.Value);
                    ClearControllerRequestState(resetExecutionState: true);
                    throw new InvalidOperationException(
                        "VectorTransferMicroOp controller binding changed while a request was outstanding.");
                }
                ...
                if (completion == null || !completion.Succeeded)
                {
                    throw new PageFaultException(
                        $"VectorTransferMicroOp observed failed completed controller source read. " +
                ...
                if (_transferBuffer == null ||
                    completion.Data.Length != _transferBuffer.Length)
                {
                    throw new InvalidOperationException(
                        "VectorTransferMicroOp controller completion payload does not match the accepted packed byte count.");
                }
                ...
                _retireEffect = new VectorTransferRetireEffect(
                    ...
                    _acceptedStride,
                    _transferBuffer);
                _completedThroughController = true;
                _state = ExecutionState.Complete;
                return true;
            }
        ...
        internal VectorTransferRetireEffect CreateRetireEffect()
        {
            if (!HasControllerRetireEffect)
            {
                throw new InvalidOperationException(
                    "VectorTransferMicroOp has no completed controller effect for selected retirement.");
            }

            return _retireEffect!;
        }
        ...
        private void ClearControllerRequestState(bool resetExecutionState)
        {
            _requestController = null;
            _controllerRequestId = null;
            _controllerAdmissionBackpressured = false;
            if (resetExecutionState && _state == ExecutionState.LoadingOperands)
            {
                _state = ExecutionState.NotStarted;
                _transferBuffer = null;
                _retireEffect = null;
                _completedThroughController = false;
            }
        }
    ...
    public class VConfigMicroOp : MicroOp
    {
        ...
        public void InitializeMetadata()
        {
            ...
            {
                ResourceMask |= ArchRegId.TryCreate(registerId, out ArchRegId register)

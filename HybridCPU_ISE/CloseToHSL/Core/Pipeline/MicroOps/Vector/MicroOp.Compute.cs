...
namespace YAKSys_Hybrid_CPU.Core
{
    ...
    public class ScalarALUMicroOp : MicroOp
    {
        ...
        public void InitializeMetadata()
        {
            ...
            if (Src1RegID != noReg)
            {
                ResourceMask |= ArchRegId.TryCreate(Src1RegID, out ArchRegId source1Register)
                    ? ResourceMaskBuilder.ForArchitecturalRegisterRead(source1Register)
                    : ResourceMaskBuilder.ForRegisterRead(Src1RegID);
            }
            if (!UsesImmediate && Src2RegID != noReg)
            {
                ResourceMask |= ArchRegId.TryCreate(Src2RegID, out ArchRegId source2Register)
                    ? ResourceMaskBuilder.ForArchitecturalRegisterRead(source2Register)
        ...
        internal bool ExecuteWithResolvedPipelineOperands(
            ulong source1,
            ulong source2,
            ulong executionPc)
        {
            _result = ExecuteScalarOp(OpCode, source1, source2, executionPc);
            return true;
        }
    ...
    public class VectorALUMicroOp : MicroOp
    {
        ...
        public void InitializeMetadata()
        {
            ...
            StreamEngineId streamEngine = StreamEngineId.Zero;
            ResourceMask = ResourceMaskBuilder.ForStreamEngine(streamEngine);

...
namespace YAKSys_Hybrid_CPU.Core
{
    public class LoadSegmentMicroOp : VectorMicroOp
    {
        ...
        // RF-07.2f/RF-10.6: exact accepted-request evidence for the false wait.
        // Controller/address/size qualification prevents unrelated vector work
        // from borrowing this retry disposition.
        internal bool OwnsPendingMemoryCompletion =>
            ...
            _loadBuffer != null &&
            _requestController.OwnsOutstandingVectorSegmentLoad(
        ...
        public override bool Execute(ref Processor.CPU_Core core)
        {
            ...
            if (_state == ExecutionState.NotStarted)
            {
                ...
                int totalBytes = checked((int)(_totalElements * (ulong)elemSize));
                _loadBuffer = new byte[totalBytes];

                // A bound subsystem uses the RF-10.6 controller request and
                // completion boundary. The functional fallback remains below.
                ...
                if (memSub != null)
                {
                    ...
                    MemoryAdmissionResult admission =
                        controller.TryAcceptVectorSegmentLoad(
                        0,
                        Instruction.DestSrc1Pointer,
                        _loadBuffer.Length);
                    ...
                    if (admission.Status == MemoryAdmissionStatus.Rejected)
                    {
                        throw new InvalidOperationException(
                            admission.Reason ??
                            "MemoryCycleController rejected a vector-segment load without a reason.");
                    }
            ...
            if (_state == ExecutionState.LoadingOperands)
            {
                ...
                {
                    throw new InvalidOperationException(
                        "LoadSegmentMicroOp lost its accepted controller identity while loading operands.");
                }
                if (!ReferenceEquals(_requestController, currentController))
                {
                    ...
                    throw new InvalidOperationException(
                        "LoadSegmentMicroOp controller binding changed while an accepted request was outstanding.");
                }
                ...
                if (_loadBuffer == null || completion.Data.Length != _loadBuffer.Length)
                {
                    throw new InvalidOperationException(
                        "LoadSegmentMicroOp controller completion payload does not match the accepted segment byte count.");
                }

                completion.Data.Span.CopyTo(_loadBuffer);
                _state = ExecutionState.Complete;
                return true;
            }
    ...
    public class StoreSegmentMicroOp : VectorMicroOp
    {
        ...
        public override bool Execute(ref Processor.CPU_Core core)
        {
            ...
            if (_state == ExecutionState.StoringResults)
            {
                ...
                {
                    // RF-07.2x: an explicit terminal failure from this exact
                    // owned vector-store request is an architectural write
                    // fault, not a retry and not an unknown vector exception.
                    // The existing single-lane PageFault catch keeps all
                    // stage-aware delivery and cleanup authority.
                    if (!_requestToken.Succeeded)
                    {
                        throw new PageFaultException(
                            $"StoreSegmentMicroOp observed failed completed write token. " +
                            (_requestToken.FailureReason ??
                             "MemorySubsystem did not provide a failure reason."),
                            Instruction.DestSrc1Pointer,
                            isWrite: true);
                    }

...
namespace YAKSys_Hybrid_CPU.Core
{
    ...
    public class VectorMaskOpMicroOp : VectorMicroOp
    {
        ...
        private void ExecutePredicateMaskManipulation(ref Processor.CPU_Core core)
        {
            ...
            ulong resultMask = OpCode switch
            {
                ...
                Processor.CPU_Core.IsaOpcodeValues.VMSBF =>
                    ResolveMaskSetBeforeFirstResult(mask1),
        ...
        private ulong ResolveMaskSetBeforeFirstResult(ulong mask)
        {
            byte reservedSrcPred2 = (byte)((Instruction.Immediate >> 4) & 0x0F);
            if (reservedSrcPred2 != 0)
            {
                throw new InvalidOperationException(
                    "VectorMaskOpMicroOp.Execute() rejected VMSBF with a non-zero reserved src2 predicate nibble. " +
                    "The Phase 04 contour is the canonical unary mask-prefix ABI only.");
            }

            ulong vl = Math.Min(_totalElements, Processor.CPU_Core.RVV_Config.VLMAX);
            return VectorALU.ApplyMaskSetBeforeFirst(mask, vl);
        }
    }
    ...
    public class VectorZeroExtendMicroOp : VectorMicroOp
    {
        ...
        public override void InitializeMetadata()
        {
            ...
            if (Instruction.StreamLength > 0)
            {
                int sourceElementSize = GetElementSize();
                DataTypeEnum destinationType = ResolveDestinationDataType(Instruction.DataTypeValue);
                int destinationElementSize = DataTypeUtils.SizeOf(destinationType);

                ReadMemoryRanges =
                [
                    (Instruction.Src2Pointer, checked(Instruction.StreamLength * (ulong)sourceElementSize))
                ];
                WriteMemoryRanges =
                [
                    (Instruction.DestSrc1Pointer, checked(Instruction.StreamLength * (ulong)destinationElementSize))
                ];
            }
        ...
        public override bool Execute(ref Processor.CPU_Core core)
        {
            ...
            if (_state == ExecutionState.NotStarted)
            {
                ...
                ThrowIfZeroLengthVectorComputeContour("VectorZeroExtendMicroOp.Execute()");

                DataTypeEnum sourceType = Instruction.DataTypeValue;
                DataTypeEnum destinationType = ResolveDestinationDataType(sourceType);
                int sourceElementSize = DataTypeUtils.SizeOf(sourceType);
                int destinationElementSize = DataTypeUtils.SizeOf(destinationType);
                ...
                {
                    if (!IsLaneActiveForZeroExtend(ref core, element))
                    {
                        continue;
                    }

                    ulong sourceAddress = checked(
                        Instruction.Src2Pointer + (element * (ulong)sourceElementSize));
                    ulong destinationAddress = checked(
                        Instruction.DestSrc1Pointer + (element * (ulong)destinationElementSize));

                    byte[] sourceBuffer = new byte[sourceElementSize];
                    core.ReadBoundMainMemoryExact(
                        sourceAddress,
                        sourceBuffer,
                        "VectorZeroExtendMicroOp.Execute()");

                    byte[] destinationBuffer = new byte[destinationElementSize];
                    ulong value = ElementCodec.LoadU(sourceBuffer, 0, sourceType);
                    ElementCodec.StoreU(destinationBuffer, 0, destinationType, value);

                    core.ThrowIfBoundMainMemoryRangeUnavailable(
                        destinationAddress,
                        destinationElementSize,
                        "VectorZeroExtendMicroOp.Execute()");
        ...
        private void PublishStagedWritesAtWriteBack(ref Processor.CPU_Core core)
        {
            ...
            {
                throw new InvalidOperationException(
                    "VectorZeroExtendMicroOp WB publication reached retire without a completed staged VZEXT result.");
            }
            ...
            for (int i = 0; i < _stagedWriteCount; i++)
            {
                ...
                core.ThrowIfBoundMainMemoryRangeUnavailable(
                    write.Address,
                    write.Data.Length,
                    "VectorZeroExtendMicroOp.EmitWriteBackRetireRecords()");
            }
        ...
        private void ThrowIfUnsupportedPublicationContour()
        {
            if (OpCode != (uint)Processor.CPU_Core.InstructionsEnum.VZEXT ||
                Instruction.OpCode != (uint)Processor.CPU_Core.InstructionsEnum.VZEXT)
            {
                throw new DecodeProjectionFaultException(
                    "VectorZeroExtendMicroOp.InitializeMetadata() can only publish the Phase 05 VZEXT contour.");
            }
            ...
            if (Instruction.Indexed || Instruction.Is2D)
            {
                string addressingContour = Instruction.Indexed && Instruction.Is2D
                    ...
                    : Instruction.Indexed
                        ...
                        : "2D";
                throw new DecodeProjectionFaultException(
                    $"VectorZeroExtendMicroOp.InitializeMetadata() rejected unsupported {addressingContour} VZEXT publication. " +
                    "Phase 05 only opens the packed 1D unsigned zero-extension contour.");
            }

            if (Instruction.Immediate != 0)
            {
                throw new DecodeProjectionFaultException(
                    "VectorZeroExtendMicroOp.InitializeMetadata() rejected non-zero VZEXT immediate sideband.");
            }

            if (Instruction.Stride != 0)
            {
                throw new DecodeProjectionFaultException(
                    "VectorZeroExtendMicroOp.InitializeMetadata() rejected non-zero VZEXT stride. " +
                    "The selected Phase 05 contour is packed 1D only because one carrier stride cannot describe both source and widened destination element widths.");
            }

            _ = ResolveDestinationDataType(Instruction.DataTypeValue);
        }
        ...
        private void ThrowIfUnsupportedRuntimeContour()
        {
            ...
            throw new InvalidOperationException(
                "VectorZeroExtendMicroOp.Execute() rejected a non-canonical VZEXT runtime contour. " +
                "Only packed 1D unsigned source types UINT8/UINT16/UINT32 are executable.");
        }

        private static DataTypeEnum ResolveDestinationDataType(DataTypeEnum sourceType) =>
            sourceType switch
            {
                DataTypeEnum.UINT8 => DataTypeEnum.UINT16,
                DataTypeEnum.UINT16 => DataTypeEnum.UINT32,
                DataTypeEnum.UINT32 => DataTypeEnum.UINT64,
                _ => throw new DecodeProjectionFaultException(
                    $"VZEXT supports only unsigned source DataType UINT8, UINT16, or UINT32; got {sourceType}.")
            };

        private bool IsLaneActiveForZeroExtend(ref Processor.CPU_Core core, ulong element)
        {

...
namespace YAKSys_Hybrid_CPU.Core
{
    ...
    public abstract class VectorMicroOp : MicroOp
    {
        ...
        protected void RefreshVectorAdmissionMetadata(bool readsMemory, bool writesMemory)
        {
            ...
            {
                int rawRegister = readRegisters[i];
                ResourceMask |= ArchRegId.TryCreate(rawRegister, out ArchRegId register)
                    ? ResourceMaskBuilder.ForArchitecturalRegisterRead(register)
                    : ResourceMaskBuilder.ForRegisterRead(rawRegister);
            }

            IReadOnlyList<int> writeRegisters = WriteRegisters ?? Array.Empty<int>();
            for (int i = 0; i < writeRegisters.Count; i++)
            {
                int rawRegister = writeRegisters[i];
                ResourceMask |= ArchRegId.TryCreate(rawRegister, out ArchRegId register)
                    ? ResourceMaskBuilder.ForArchitecturalRegisterWrite(register)
                    : ResourceMaskBuilder.ForRegisterWrite(rawRegister);
            }

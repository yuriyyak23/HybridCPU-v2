...
public readonly record struct MatrixTileMaterializedInstruction(
    ...
    MatrixTileCanonicalDescriptorAbi SecondaryTileDescriptor,
    MatrixTileCanonicalDescriptorAbi ResultTileDescriptor,
    bool IsTypedCloseToHSLRuntimeObject,
    ...
    bool UsesFallbackPath)
{
    public bool IsRuntimeLegal =>
        ...
        IsTypedCloseToHSLRuntimeObject &&
...
public static class MatrixTileIrProjectionAndMaterializer
{
    ...
    public const string TypedObjectDecision = "ClosedMaterializedTypedCloseToHSLInstructionObjects";
    ...
    /// <summary>
    /// Materializes the existing typed MatrixTile carrier from the immutable
    /// canonical decode handoff. The generated binding is consumed from the
    /// exact decoded slot; no opcode/provider/materializer lookup is performed.
    /// </summary>
    public static MatrixTileMicroOp MaterializeCanonicalCarrier(
        CanonicalDecodedInstruction canonical,
        DecodedInstruction compatibilitySlot)
    {
        ArgumentNullException.ThrowIfNull(canonical);
        ArgumentNullException.ThrowIfNull(compatibilitySlot);

        GeneratedStaticBinding binding = canonical.StaticBinding
            ?? throw new InvalidOperationException(
                "Canonical MatrixTile materialization requires the generated static binding carried by decode.");
        InstructionIR instruction = compatibilitySlot.RequireInstruction();
        if (!canonical.IsOccupied ||
            compatibilitySlot.SlotIndex != canonical.SlotIndex ||
            binding.Opcode != canonical.Opcode ||
            (uint)instruction.CanonicalOpcode != canonical.Opcode ||
            instruction.Class != canonical.InstructionClass ||
            instruction.SerializationClass != canonical.SerializationClass)
        {
            throw new InvalidOperationException(
                "Canonical MatrixTile materialization received mismatched decoded slot identity or binding.");
        }

        MatrixTileInstructionIrProjection projection = instruction.MatrixTileProjection
            ?? throw new DecodeProjectionFaultException(
                "Canonical MatrixTile materialization requires the decoder-published typed projection.");
        if ((uint)projection.Opcode != canonical.Opcode)
        {
            throw new DecodeProjectionFaultException(
                "Canonical MatrixTile materialization received a projection for a different opcode.");
        }
        ...
        {
            throw new DecodeProjectionFaultException(
                $"Canonical MatrixTile materialization rejected the decoded projection: {faultKind}.");
        }

        MatrixTileMicroOp carrier = CreateTypedMicroOp(materializedInstruction);
        carrier.InitializeMetadata();
        return carrier;
    }

    /// <summary>
    /// Single typed MatrixTile carrier factory shared by the registry adapter
    /// and canonical handoff. Selection is based on the already-decoded typed
    /// operation kind, never on a second opcode registry.
    /// </summary>
    public static MatrixTileMicroOp CreateTypedMicroOp(
        MatrixTileMaterializedInstruction materializedInstruction)
    {
        return materializedInstruction.OperationKind switch
        {
            ...
            _ => throw new DecodeProjectionFaultException(
                $"Unsupported MTILE materialized operation kind {materializedInstruction.OperationKind}.")
        };
    }
        ...
        out MatrixTileIrProjectionFaultKind faultKind)
    {
        ...
        materializedInstruction = new MatrixTileMaterializedInstruction(
            ...
            IsTypedCloseToHSLRuntimeObject: true,

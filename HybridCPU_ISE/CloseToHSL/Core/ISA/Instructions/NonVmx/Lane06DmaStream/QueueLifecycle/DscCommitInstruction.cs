// DSC_COMMIT metadata anchor: Lane6 queue commit remains deferred until commit
// authority, staged publication, retire side effects, and rollback evidence close.
// No VMX frontend path is added; future execution requires virtualization-boundary policy.
namespace YAKSys_Hybrid_CPU.CloseToHSL.Core.ISA.Instructions.NonVmx.Lane06DmaStream.QueueLifecycle;

public sealed partial class DscCommitInstruction
{
    public const string Mnemonic = "DSC_COMMIT";
    public const string OperandShape = "QueueTokenOrHandle, commit scope sideband";
    public const string ParameterDescriptor = "TokenOrQueueHandle, commit scope, staged descriptor identity sideband";
    public const string MicroOpShape = "Lane6QueueControl.Commit, queue-authority gated, retire-owned staged commit";
    ...
    public const bool RequiresStagedCommitAuthority = true;

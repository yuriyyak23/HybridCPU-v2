// DSC_QUERY_SHAPE metadata anchor: Lane6 shape capability query remains
// deferred until shape ABI, bounded publication, and replay evidence close.
// It must never publish hidden host-owned evidence as guest architectural state.
namespace YAKSys_Hybrid_CPU.CloseToHSL.Core.ISA.Instructions.NonVmx.Lane06DmaStream.Queries;

public sealed partial class DscQueryShapeInstruction
{
    public const string Mnemonic = "DSC_QUERY_SHAPE";
    public const string OperandShape = "Shape query sideband, scalar/status destination sideband";
    public const string ParameterDescriptor = "Shape query selector, optional type/range scope, retire-visible bounded capability result";
    public const string MicroOpShape = "Lane6CapabilityQuery.Shape, read-only capability query, retire-owned scalar publication";
    ...
    public const bool RequiresShapeQueryAbi = true;

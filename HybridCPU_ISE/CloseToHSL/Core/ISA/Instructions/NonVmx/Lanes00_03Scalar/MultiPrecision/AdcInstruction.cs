// ADC is a multi-precision negative-decision gate: no opcode/materializer/helper
// authority until carry-in and carry-out publication ABI is retire-owned. VMX stays generic.
namespace YAKSys_Hybrid_CPU.CloseToHSL.Core.ISA.Instructions.NonVmx.Lanes00_03Scalar.MultiPrecision;

public sealed partial class AdcInstruction
{
    public const string Mnemonic = "ADC";
    public const string OperandShape = "explicit carry-in/result ABI TBD";
    public const string ParameterDescriptor = "rd, rs1, rs2, explicit carry-in sideband, explicit carry-out/result sideband";
    ...
    public const string CarryInputPolicy = "Carry-in must be explicit in the instruction carrier; no hidden global carry state.";
    public const string CarryOutputPolicy = "Carry-out must be replayable and rollback-safe before execution opens.";
    public const bool RequiresCarryInAbi = true;
    public const bool RequiresCarryOutAbi = true;
    public const bool RequiresCarryBorrowPublicationAbi = true;
    public const bool RequiresExplicitCarryInputTransportAbi = true;

// SBC is a multi-precision negative-decision gate: no opcode/materializer/helper
// authority until borrow-in and borrow-out publication ABI is retire-owned. VMX stays generic.
namespace YAKSys_Hybrid_CPU.CloseToHSL.Core.ISA.Instructions.NonVmx.Lanes00_03Scalar.MultiPrecision;

public sealed partial class SbcInstruction
{
    public const string Mnemonic = "SBC";
    public const string OperandShape = "explicit borrow-in/result ABI TBD";
    public const string ParameterDescriptor = "rd, rs1, rs2, explicit borrow-in sideband, explicit borrow-out/result sideband";
    ...
    public const string BorrowInputPolicy = "Borrow-in must be explicit in the instruction carrier; no hidden global borrow state.";
    public const string BorrowOutputPolicy = "Borrow-out must be replayable and rollback-safe before execution opens.";
    public const bool RequiresBorrowInAbi = true;
    public const bool RequiresBorrowOutAbi = true;
    public const bool RequiresCarryBorrowPublicationAbi = true;
    public const bool RequiresExplicitBorrowInputTransportAbi = true;

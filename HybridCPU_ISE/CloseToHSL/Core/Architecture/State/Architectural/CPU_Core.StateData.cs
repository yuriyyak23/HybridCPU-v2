    ...
    public partial struct Processor
    {
        public sealed partial class CPU_Core
        {
            ...
            public CPU_Core(ushort CoreID, CpuCorePlatformContext platformContext)
            {
                ...
                this._runtime = new CoreRuntimeState();
                this.pipeCtrl = new PipelineControl(this._runtime.Telemetry);
                this.CoreID = CoreID;
                ...
                Runtime.CpuInstructionTranslation.Owner =
                    new Core.Memory.CpuInstructionTranslationOwner(
                        platformContext.CpuInstructionTranslationPolicy,
                        platformContext.MainMemory);
                if (platformContext.CompatibilityControlPolicyProfile is { IsConfigured: true } compatibilityPolicyProfile)
                {
                    Runtime.CompatibilityControlPolicy.Owner =
                        new CompatibilityControlPolicyOwner(compatibilityPolicyProfile);
                }
                ...
                Runtime.Retire.CanonicalCpuTranslationFaultCompletionProducer =
                    Runtime.Retire.CompletionCommitOwner.RegisterProducer(
                        new ArchitecturalCompletionProducerPolicy(
                            ...
                            NeutralFaultAddressSemantic.VirtualAddress,
                            NeutralFaultAuxiliarySemantic.TranslationFault,
                            RequiresTranslationProvenance: true));
                Runtime.Retire.CanonicalCpuSecondStageTranslationFaultCompletionProducer =
                    Runtime.Retire.CompletionCommitOwner.RegisterProducer(
                        new ArchitecturalCompletionProducerPolicy(
                            "CanonicalCpuSecondStageTranslationFaultProducer",
                            NeutralArchitecturalCompletionClass.TranslationFault,
                            RequiresReason: true,
                            AllowsQualification: true,
                            ...
                            RequiresTranslationProvenance: true,
                            RequiresSecondStageProvenance: true));
                Runtime.CpuInstructionTranslation.Owner.BindCompletionLifecycle(
                    Runtime.Retire.CompletionCommitOwner);
                _completionReasonQualificationVmRead =
                    new CompletionReasonQualificationVmReadScalarDeliveryCanonicalComposition(
                        Runtime.Retire.CompletionCommitOwner,
                        Runtime.Retire.CanonicalCpuTranslationFaultCompletionProducer,
                        platformContext.EnableCompletionReasonQualificationVmRead);
                _completionSecondStageVmRead =
                    new CompletionSecondStageVmReadScalarDeliveryCanonicalComposition(
                        Runtime.Retire.CompletionCommitOwner,
                        Runtime.Retire.CanonicalCpuSecondStageTranslationFaultCompletionProducer,
                        platformContext.EnableCompletionSecondStageVmRead);
                if (platformContext.EnablePinBasedControlsVmRead &&
                    Runtime.CompatibilityControlPolicy.Owner is null)
                {
                    throw new InvalidOperationException(
                        "Exact PinBasedControls VMREAD requires the canonical compatibility-control policy owner.");
                }
                if (Runtime.CompatibilityControlPolicy.Owner is { } compatibilityControlOwner)
                {
                    _pinBasedControlsVmRead =
                        new PinBasedControlsVmReadScalarDeliveryCanonicalComposition(
                            compatibilityControlOwner,
                            platformContext.EnablePinBasedControlsVmRead);
                }
            ...
            /// Reads a speculative value from the Physical Register File only while the rename
            /// map differs from the committed map. Committed mappings read the per-VT
            /// architectural mirror because reset identity mappings are shared bootstrap aliases,
            /// not per-VT physical storage.
            ...
            public ulong ReadArch(int vtId, int archReg)
            {
                ...
                int committedPhysReg = ArchCommitMap.Lookup(vtId, archReg);
                if (physReg == 0 || physReg == committedPhysReg)
            ...
            public ulong ReadCommittedArch(int vtId, int archReg)
            {
                vtId = ResolveLiveStateVtIdOrThrow(vtId, nameof(ReadCommittedArch));
                if (archReg == 0) return 0;
                if ((uint)archReg >= (uint)RenameMap.ArchRegs) return 0;
                return ArchContexts[vtId].CommittedRegs[archReg];
            }
            ...
            public void SetExecutionPrivilegeForStartup(int vtId, PrivilegeLevel privilege)
            {
                vtId = ResolveLiveStateVtIdOrThrow(vtId, nameof(SetExecutionPrivilegeForStartup));
                if (!Enum.IsDefined(privilege))
                    throw new ArgumentOutOfRangeException(nameof(privilege));
                if (pipeCtrl.CycleCount != 0)
                    throw new InvalidOperationException(
                        "Execution privilege may be assigned only before the first pipeline cycle.");
                ArchContexts[vtId].CurrentPrivilege = privilege;
            }

            /// <summary>
            /// Reads the last successfully published write-back retire contour. The sequence
            /// changes only after a non-empty retire window has passed canonical publication.
            /// </summary>
            public bool TryReadLastRetiredBundle(out ulong bundlePc, out ulong sequence)
            {
                bundlePc = Runtime.Retire.LastRetiredBundlePc;
                sequence = Runtime.Retire.RetiredBundleSequence;
                return sequence != 0;
            }

            public bool TryReadLastRetiredLaneDiagnostic(
                out byte laneIndex,
                out uint opcode,
                out bool writesRegister,
                out ushort destinationRegister,
                out ulong resultValue,
                out string microOpType,
                out string microOpDescription)
            {
                laneIndex = Runtime.Retire.LastRetiredLaneIndex;
                opcode = Runtime.Retire.LastRetiredOpcode;
                writesRegister = Runtime.Retire.LastRetiredWritesRegister;
                destinationRegister = Runtime.Retire.LastRetiredDestinationRegister;
                resultValue = Runtime.Retire.LastRetiredResultValue;
                microOpType = Runtime.Retire.LastRetiredMicroOpType;
                microOpDescription = Runtime.Retire.LastRetiredMicroOpDescription;
                return Runtime.Retire.RetiredBundleSequence != 0;
            }
            ...
            public void ResetExecutionStartPcState(ulong pc, int activeVtId = 0)
            {
                ...
                Runtime.CompatibilityControlPolicy.Owner?
                    .ReplaceAfterArchitecturalStateReplacement();
                ArchitecturalCompletionCommitOwner

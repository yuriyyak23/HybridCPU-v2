...
namespace YAKSys_Hybrid_CPU
{
    public partial struct Processor
    {
        public sealed partial class CPU_Core
        {
            ...
            internal Func<ulong, int, byte[]?> CaptureMatrixTileMemoryReader()
            {
                Processor.MainMemoryArea boundMemory = GetBoundMainMemory();
                return (address, length) =>
                {
                    ...
                    ulong memoryLength = (ulong)boundMemory.Length;
                    ulong requestSize = (ulong)length;
                    if (requestSize > memoryLength || address > memoryLength - requestSize)
                    {
                        return null;
                    }

                    byte[] buffer = new byte[length];
                    return boundMemory.TryReadPhysicalRange(address, buffer)
                        ? buffer
                        : null;
                };
            }

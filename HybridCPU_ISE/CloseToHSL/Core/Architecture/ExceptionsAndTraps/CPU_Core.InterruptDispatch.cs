...
namespace YAKSys_Hybrid_CPU
{
    public partial struct Processor
    {
        public sealed partial class CPU_Core
        {
            ...
            {
                Core.CompatibilityControlPolicyOwner? policyOwner =
                    Runtime.CompatibilityControlPolicy.Owner;
                if (policyOwner is not null)
                {
                    Core.CompatibilityControlPolicySnapshot snapshot =
                        policyOwner.CaptureCurrent();
                    Core.NeutralInterruptRoutingPolicyResult policy =
                        Runtime.CompatibilityControlPolicy.InterruptRoutingConsumer.Validate(
                            policyOwner,
                            snapshot,
                            policyOwner.CurrentDomainIdentity);
                    if (!policy.IsAllowed)

...
namespace YAKSys_Hybrid_CPU.Core.Decoder
{
    ...
    public sealed class DecodedInstructionBundle : IAbstractBundle
    {
        ...
        public DecodedInstructionBundle(
            ...
            CanonicalBundle? canonicalBundle = null)
        {
            ...
            if (canonicalBundle is not null &&
                (canonicalBundle.BundleAddress != bundleAddress ||
                 canonicalBundle.BundleSerial != bundleSerial ||
                 canonicalBundle.Slots.Length != BundleMetadata.BundleSlotCount))
            {
                throw new ArgumentException("Canonical bundle identity must match its legacy adapter.", nameof(canonicalBundle));
            }
        ...
        /// <summary>
        /// RF-04 immutable semantic projection. It is populated by the live legacy
        /// decoder in shadow mode; legacy adapter consumers remain unchanged until
        /// RF-06 migration closes.
        /// </summary>

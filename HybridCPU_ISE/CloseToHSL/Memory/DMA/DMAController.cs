...
namespace HybridCPU_ISE.CloseToHSL.Memory.DMA
{
    ...
    /// Models persistent memory-to-memory channel transfers. Production
    /// progress is supplied only by the bound MemoryCycleController edge.
    ///
    /// Features:
    ...
    ///
    /// StreamEngine BurstIO remains a functional adapter and does not submit or
    /// advance this agent. This class-level API should not be read as a
    /// DmaStreamCompute request protocol or a complete overlap model.
    /// </summary>
    public class DMAController
    {
            ...
            MemoryCoherencyObserver? coherencyObserver = null)
        {
            ...
            {
                DmaChannelId channel = DmaChannelId.Create((byte)i);
                channels[channel].State = ChannelState.Idle;
                channels[channel].BytesTransferred = 0;
                channels[channel].TotalBytes = 0;
                channels[channel].ErrorCode = 0;
            }
        ...
        public bool ConfigureTransfer(TransferDescriptor desc, TransferCompletionCallback? callback = null)
        {
            ...
            DmaChannelId channel = DmaChannelId.Create(desc.ChannelID);

            // Check if channel is available
            if (channels[channel].State != ChannelState.Idle &&
                channels[channel].State != ChannelState.Completed)
            {
            ...
            // Configure channel
            channels[channel].CurrentDesc = desc;
            channels[channel].BytesTransferred = 0;
            channels[channel].TotalBytes = desc.TransferSize;
            channels[channel].State = ChannelState.Configured;
            channels[channel].ErrorCode = 0;
        ...
        public bool StartTransfer(byte channelID)
        {
            ...
            DmaChannelId channel = DmaChannelId.Create(channelID);

            if (channels[channel].State != ChannelState.Configured)
                return false;
        ...
        public void ExecuteCycle()
        {
            ...
            {
                DmaChannelId channel = DmaChannelId.Create((byte)ch);
                if (channels[channel].State == ChannelState.Active)
                {
                    byte channelPriority = channels[channel].CurrentDesc.Priority;
            ...
            {
                DmaChannelId channel = DmaChannelId.Create((byte)selectedChannel);
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void ProcessChannel(DmaChannelId channel)
        {
            byte channelID = channel;
            ref ChannelControl ch = ref channels[channel];
            ...
            else
            {
                ...
                RaiseInterrupt(channel, isError: true);
            }
        }
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void CompleteChannel(DmaChannelId channel)
        {
            byte channelID = channel;
            ref ChannelControl ch = ref channels[channel];
            ...
            RaiseInterrupt(channel, isError: false);
        }
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void RaiseInterrupt(DmaChannelId channel, bool isError)
        {
            byte channelID = channel;
        ...
        public (uint transferred, uint total) GetChannelProgress(byte channelID)
        {
            ...
            DmaChannelId channel = DmaChannelId.Create(channelID);
            return (channels[channel].BytesTransferred, channels[channel].TotalBytes);
        }
        ...
        public (ulong srcAddr, ulong dstAddr) GetChannelAddresses(byte channelID)
        {
            ...
            DmaChannelId channel = DmaChannelId.Create(channelID);
            return (channels[channel].CurrentDesc.SourceAddress, channels[channel].CurrentDesc.DestAddress);
        }

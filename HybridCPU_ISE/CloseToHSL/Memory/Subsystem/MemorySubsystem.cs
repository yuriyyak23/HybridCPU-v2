using HybridCPU_ISE.CloseToHSL.Memory.Cache;
...
namespace YAKSys_Hybrid_CPU.Memory
{
    public partial class MemorySubsystem : IBurstBackend, IHardwareOccupancyInput
    {
        ...
        // RF-12.6ac: sole owner-local serialization boundary for the split
        // physical-bank geometry and every positional state derived from it.
        // Controller-owned paths may enter this gate only while already
        // holding MemoryCycleController._gate.
        ...
        /// <summary>
        /// Current authoritative physical-bank geometry snapshot. Compatibility
        /// setters do not publish or advance this snapshot.
        /// </summary>
        public PhysicalMemoryBankGeometry PublishedPhysicalBankGeometry
        {
            get
            {
                lock (geometryLifecycleGate)
                {
                    return _publishedPhysicalBankGeometry;
                }
            }
        }

        /// <summary>
        /// Attempts one atomic authoritative replacement of the physical-bank
        /// geometry. The controller coordinates its request stores first; the
        /// memory owner then selects the exact lifecycle result.
        /// </summary>
        public MemoryBankGeometryUpdateResult TryReplacePhysicalMemoryBankGeometry(
            int bankCount,
            int bankWidthBytes) =>
            CycleController.TryReplacePhysicalMemoryBankGeometry(
                bankCount,
                bankWidthBytes);

        internal PhysicalMemoryBankBinding
                ...
                ulong address)
        {
            lock (geometryLifecycleGate)
            {
                ...
                ulong rawBankIndex =
                    (address / (ulong)geometry.BankWidthBytes) %
                    (ulong)geometry.BankCount;
                PhysicalMemoryBankIndex bankIndex =
                    PhysicalMemoryBankIndex.Create((int)rawBankIndex);
                return PhysicalMemoryBankBinding.Create(
                    bankIndex,
                    geometry.Generation);
            }
        }
                ...
                ushort stride)
        {
            lock (geometryLifecycleGate)
            {
                PhysicalMemoryBankGeometry geometry =
                    _publishedPhysicalBankGeometry;
                int capturedElementCount = checked((int)elementCount);
                PhysicalMemoryBankIndex[] sourceBankIndexes =
                    new PhysicalMemoryBankIndex[capturedElementCount];

                for (int elementIndex = 0;
                     elementIndex < capturedElementCount;
                     elementIndex++)
                {
                    ...
                    ulong rawBankIndex =
                        ...
                        (ulong)geometry.BankCount;
                    sourceBankIndexes[elementIndex] =
                        PhysicalMemoryBankIndex.Create((int)rawBankIndex);
                }

                return CanonicalVectorPhysicalBankEnvelope.Create(
                    geometry.Generation,
                    sourceBankIndexes);
            }
        }
        ...
        /// <summary>
        /// Retained compatibility threshold setting. RF-10.13 removes hidden
        /// size-based DMA routing from synchronous memory APIs.
        /// </summary>
        ...
        /// <summary>
        /// Sole RF-10 owner of the shared memory-controller cycle and migrated
        /// request/completion latches.
        /// </summary>
        public MemoryCycleController CycleController { get; }
        ...
        public class MemoryRequestToken
        {
            ...
            // Owner-only physical-location evidence captured at legacy request
            // acceptance. The public compatibility constructor leaves it absent.
            ...
            internal MemoryRequestToken(
                ...
                bool isRead,
                long enqueueCycle,
                bool defersPhysicalWriteUntilRetire,
                PhysicalMemoryBankBinding physicalBankBinding)
            {
                ...
                CompleteCycle = 0;
                FailureReason = null;
                this.physicalBankBinding = physicalBankBinding;
            }
            ...
            // Internal owner-only read for the legacy cancellation location
            // consumer. This does not expose a public token location carrier.
            internal PhysicalMemoryBankBinding GetPhysicalBankBindingForOwner() =>
                physicalBankBinding;
        ...
        {
            public PhysicalBankTopologyCandidate(
                bool[] bankOccupied,
                long[] bankLastAccessCycle,
                Queue<BankRequest>[] bankQueues,
                PortState[] portStates)
            {
                BankOccupied = bankOccupied;
                BankLastAccessCycle = bankLastAccessCycle;
                BankQueues = bankQueues;
                PortStates = portStates;
            }

            public bool[] BankOccupied { get; }
            public long[] BankLastAccessCycle { get; }
            public Queue<BankRequest>[] BankQueues { get; }
            public PortState[] PortStates { get; }
        }
        ...
        internal MemoryBankGeometryUpdateResult
                ...
                bool controllerIsQuiescent)
        {
            lock (geometryLifecycleGate)
            {
                if (!PhysicalMemoryBankGeometry.IsBankCountRepresentable(bankCount))
                {
                    return MemoryBankGeometryUpdateResult.Rejected(
                        MemoryBankGeometryUpdateRejectReason.InvalidBankCount);
                }

                if (!PhysicalMemoryBankGeometry.IsBankWidthRepresentable(bankWidthBytes))
                {
                    return MemoryBankGeometryUpdateResult.Rejected(
                        MemoryBankGeometryUpdateRejectReason.InvalidBankWidth);
                }

                if (!controllerIsQuiescent || !IsPhysicalBankGeometryOwnerQuiescent())
                {
                    return MemoryBankGeometryUpdateResult.Rejected(
                        MemoryBankGeometryUpdateRejectReason.Busy);
                }

                if (_lastIssuedPhysicalBankGeometryGeneration ==
                ...
                if (!TryPreparePhysicalBankTopologyCandidate(
                        bankCount,
                        out PhysicalBankTopologyCandidate candidate))
                {
                    return MemoryBankGeometryUpdateResult.Rejected(
                ...
                ulong nextGenerationRaw =
                    _lastIssuedPhysicalBankGeometryGeneration + 1UL;
                MemoryBankGeometryGeneration nextGeneration =
                ...
                _numBanks = bankCount;
                _bankWidthBytes = bankWidthBytes;
                bankOccupied = candidate.BankOccupied;
                bankLastAccessCycle = candidate.BankLastAccessCycle;
                bankQueues = candidate.BankQueues;
                _portStates = candidate.PortStates;
                roundRobinIndex = 0;
        ...
        private bool IsPhysicalBankGeometryOwnerQuiescent()
        {
            ...
            if (TRB.OutstandingCount != 0)
            {
                return false;
            }

            foreach (bool active in bankOccupied)
            {
                if (active)
                {
                    return false;
            ...
            foreach (PortState port in _portStates)
            {
                if (port.Busy)
                {
                    return false;
            ...
            out PhysicalBankTopologyCandidate candidate)
        {
            ...
            try
            {
                bool[] candidateBankOccupied = new bool[bankCount];
                long[] candidateBankLastAccessCycle = new long[bankCount];
                Queue<BankRequest>[] candidateBankQueues =
                    new Queue<BankRequest>[bankCount];
                for (int bankIndex = 0; bankIndex < bankCount; bankIndex++)
                {
                    candidateBankQueues[bankIndex] = new Queue<BankRequest>();
                }

                PortState[] candidatePortStates = new PortState[_numMemoryPorts];
                for (int portIndex = 0;
                     portIndex < candidatePortStates.Length;
                     portIndex++)
                {
                    candidatePortStates[portIndex] = new PortState
                    {
                        PortId = portIndex,
                        CurrentBankId = -1,
                        LastSwitchCycle = 0,
                ...
                candidate = new PhysicalBankTopologyCandidate(
                    candidateBankOccupied,
                    candidateBankLastAccessCycle,
                    candidateBankQueues,
                    candidatePortStates);
                return true;
            }
        ...
        public MemorySubsystem(ref Processor proc, DMAController? dmaController = null)
        {
            ...
            nextRequestID = 1;
            CycleController = new MemoryCycleController(this);
            ...
            MemoryBankGeometryGeneration initialGeneration =
        ...
        public bool Read(ulong deviceID, ulong address, Span<byte> buffer)
        {
            lock (geometryLifecycleGate)
            {
                ...
                OnBurstStarted(address, length, true, bankIndex.Value);

                // Check for bank conflict with adaptive stall calculation
                if (bankOccupied[bankIndex.Value])
                {
                    BankConflicts++;

                    // Calculate adaptive stall based on queue depth and transfer size
                    int queueDepth = bankQueues[bankIndex.Value].Count;
                ...
                bankOccupied[bankIndex.Value] = true;
                bankLastAccessCycle[bankIndex.Value] = currentCycle;

                // Use IOMMU for direct access
                bool success = IOMMU.ReadBurst(deviceID, address, buffer);

                // Release bank
                bankOccupied[bankIndex.Value] = false;
                ...
                OnBurstCompleted(address, length, true, bankIndex.Value);
        ...
        public bool Write(ulong deviceID, ulong address, ReadOnlySpan<byte> buffer)
        {
            lock (geometryLifecycleGate)
            {
                ...
                // Fire BurstStarted event
                OnBurstStarted(address, length, false, bankIndex.Value);
                ...
                // Fire BurstCompleted event
                OnBurstCompleted(address, length, false, bankIndex.Value);

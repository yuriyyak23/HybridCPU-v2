...
namespace YAKSys_Hybrid_CPU.Memory
{
    public partial class MemorySubsystem
    {
        ...
        {
            return PhysicalMemoryBankIndex.FromRawValue(
                Core.Memory.MemoryBankRouting.ResolveBankId(
                    address,
                    BankWidthBytes,
                    NumBanks));
        }
        ...
        /// <summary>
        /// Advance the explicitly bound persistent DMA agent once. The sole
        /// caller is MemoryCycleController's normalized shared-domain edge.
        /// </summary>
        ...
        {
            // Telemetry reset is observation-only. It deliberately does not
            // reset MemoryCycle or any request/completion state.
            CycleController.ResetTelemetry();
        ...
        {
            CycleController.AdvanceCompatibilityCycles(cycles);
        }

        /// <summary>
        /// One compatibility service edge for request families not yet migrated
        /// to typed controller admission. Only MemoryCycleController may call it.
        /// </summary>
        internal void AdvanceLegacyAgentOneCycle()
        {
            lock (geometryLifecycleGate)
            {
                currentCycle = checked(currentCycle + 1);
        ...
        /// read classes.
        /// Translation remains on its existing IOMMU owner; no PTW/cache/DMA
        /// timing authority is inferred by this narrow cutover.
        /// </summary>
        ...
        /// <summary>
        /// Canonical vector-transfer source adapter retained independently:
        /// one accepted source binding cannot represent every address reached
        /// by a strided transfer.
        /// </summary>
        ...
        /// <summary>
        /// Functional source-read adapter for one controller-native canonical
        /// vector transfer. One invocation is one controller service decision;
        /// element iteration is not a new byte-port or multi-beat timing model.
        /// </summary>
        internal bool ExecuteControllerVectorTransferReadStep(
            ...
            byte[] packedDestination)
        {
            lock (geometryLifecycleGate)
            {
                ArgumentNullException.ThrowIfNull(packedDestination);
                ...
                PhysicalMemoryBankGeometry geometry =
                    _publishedPhysicalBankGeometry;
                if (!physicalBankEnvelope.IsWellFormed ||
                ...
                for (int elementIndex = 0;
                     elementIndex < physicalBankEnvelope.Count;
                     elementIndex++)
                {
                    if (physicalBankEnvelope
                ...
                try
                {
                    if ((ulong)packedDestination.Length !=
                        checked(elementCount * (ulong)elementSize))
                    {
                        return false;
                    }
                }
                ...
                {
                    return ExecuteControllerCanonicalVectorElementReadStepUnderOwnerGate(
                        deviceId,
                        sourceAddress,
                        packedDestination);
                }
                ...
                for (ulong element = 0; element < elementCount; element++)
                {
                    ...
                    try
                    {
                        ...
                        offset = checked((int)(element * (ulong)elementSize));
                    }
                    catch (OverflowException)
                    {
                        return false;
                    ...
                    if (!ExecuteControllerCanonicalVectorElementReadStepUnderOwnerGate(
                            deviceId,
                            address,
                            elementBuffer))
                    {
        ...
        private int CalculateMemoryLatency(ulong address, int size)
        {
            ...
            ulong lastRowAddress = bankLastAccessCycle[bankIndex.Value] > 0
                ? (ulong)bankLastAccessCycle[bankIndex.Value] / (ulong)RowBufferSize
                : ulong.MaxValue;

...
namespace YAKSys_Hybrid_CPU.Arch
{
    ...
    public static class InstructionEncoder
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static uint RequireUInt32(ulong value, string paramName)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static ushort RequireUInt16(uint value, string paramName)
        {
            if (value > ushort.MaxValue)
            {
                throw new ArgumentOutOfRangeException(paramName, value,
                    "VLIW carrier field is 16 bits; silent truncation is forbidden.");
            }
        ...
        /// <summary>
        /// Encode a scalar rotate-immediate operation with the Phase 02 canonical
        /// ABI: Word1 = (rd, rs1, x0), Immediate = unsigned imm6 payload.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static VLIW_Instruction EncodeScalarRotateImmediate(
            uint opCode,
            DataTypeEnum dataType,
            ushort destReg,
            ushort srcReg,
            ...
            bool maskAgnostic = false)
        {
            if (immediate6 > 0x3F)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(immediate6),
                    immediate6,
                    "Scalar rotate-immediate payload must fit imm6 [0, 63].");
            }
        ...
        /// <summary>
        /// Encode a scalar bitfield-immediate operation with the Phase 02 canonical
        /// ABI: Word1 = (rd, rs1, x0), Immediate = unsigned imm6 bit index.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static VLIW_Instruction EncodeScalarBitfieldImmediate(
            uint opCode,
            DataTypeEnum dataType,
            ushort destReg,
            ushort srcReg,
            ...
            bool maskAgnostic = false)
        {
            if (immediate6 > 0x3F)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(immediate6),
                    immediate6,
                    "Scalar bitfield-immediate payload must fit imm6 [0, 63].");
            }
        ...
        /// <summary>
        /// Encode SLLI.UW with the Phase 03 canonical .UW immediate ABI:
        /// Word1 = (rd, rs1, x0), Immediate = unsigned imm6 shift amount.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static VLIW_Instruction EncodeScalarAddressGenerationImmediate(
            uint opCode,
            DataTypeEnum dataType,
            ushort destReg,
            ushort srcReg,
            ...
            bool maskAgnostic = false)
        {
            if (immediate6 > 0x3F)
            {
                throw new ArgumentOutOfRangeException(
                    nameof(immediate6),
                    immediate6,
                    "Scalar address-generation immediate payload must fit imm6 [0, 63].");
            }
            ...
            bool maskAgnostic = false)
        {
            ...
            inst.StreamLength = RequireUInt32(streamLength, nameof(streamLength));
            inst.Stride = stride;
            inst.TailAgnostic = tailAgnostic;
            inst.MaskAgnostic = maskAgnostic;
            ...
            bool maskAgnostic = false)
        {
            ...
            inst.Immediate = RequireUInt16(rowLength, nameof(rowLength)); // Store row length in immediate field
            inst.TailAgnostic = tailAgnostic;
            inst.MaskAgnostic = maskAgnostic;
            ...
            ulong param2)
        {
            ...
            inst.StreamLength = RequireUInt32(param2, nameof(param2));

...
namespace YAKSys_Hybrid_CPU.Core.Execution.DmaStreamCompute
{
    ...
    public enum DmaStreamComputeStatusQueryStatus : byte
    {
        Accepted = 0,
        Rejected = 1
    }

    public enum DmaStreamComputeStatusQueryRejectKind : byte
    {
        None = 0,
        InvalidToken = 1,
        OwnerDomainMismatch = 2
    ...
    public sealed record DmaStreamComputeTokenStatusSnapshot
    {
        public DmaStreamComputeTokenStatusSnapshot(
            DmaStreamComputeTokenHandle handle,
            ...
            ulong descriptorIdentityHash,
            ulong normalizedFootprintHash,
            DmaStreamComputeTokenLifecycleEvidence lifecycleEvidence)
        {
            Handle = handle;
            State = state;
            ...
            EncodedStatusWord = EncodeStatusWord(
                state,
                cancelReason,
                faultKind,
                stagedWriteCount,
        ...
        public ulong TokenId => Handle.TokenId;
        ...
        public DmaStreamComputeTokenLifecycleEvidence LifecycleEvidence { get; }

        public ulong EncodedStatusWord { get; }

        public static DmaStreamComputeTokenStatusSnapshot FromEntry(
            DmaStreamComputeActiveTokenEntry entry)
        {
            ArgumentNullException.ThrowIfNull(entry);
            DmaStreamComputeToken token = entry.Token;
            return new DmaStreamComputeTokenStatusSnapshot(
                entry.Handle,
                token.State,
                token.CancelReason,
                ...
                token.HasCommitted,
                entry.DescriptorIdentityHash,
                entry.NormalizedFootprintHash,
                token.ExportLifecycleEvidence());
        }

        private static ulong EncodeStatusWord(
            DmaStreamComputeTokenState state,
            DmaStreamComputeTokenCancelReason cancelReason,
            DmaStreamComputeTokenFaultKind faultKind,
            int stagedWriteCount,
            bool hasCommitted)
        {
            ulong encoded = (byte)state;
            encoded |= (ulong)(byte)cancelReason << 8;
            encoded |= (ulong)(byte)faultKind << 16;
            encoded |= (hasCommitted ? 1UL : 0UL) << 24;
            encoded |= ((ulong)(uint)stagedWriteCount) << 32;
            return encoded;
        }
    }

    public sealed record DmaStreamComputeStatusQueryResult
    {
        private DmaStreamComputeStatusQueryResult(
            DmaStreamComputeStatusQueryStatus status,
            DmaStreamComputeStatusQueryRejectKind rejectKind,
            DmaStreamComputeTokenStatusSnapshot? snapshot,
            string message)
        {
            Status = status;
            RejectKind = rejectKind;
            ...
            Message = string.IsNullOrWhiteSpace(message)
                ? "DSC_STATUS token query rejected."
                : message;
        }

        public DmaStreamComputeStatusQueryStatus Status { get; }

        public DmaStreamComputeStatusQueryRejectKind RejectKind { get; }

        public DmaStreamComputeTokenStatusSnapshot? Snapshot { get; }

        public string Message { get; }

        public bool IsAccepted =>
            Status == DmaStreamComputeStatusQueryStatus.Accepted && Snapshot is not null;

        public bool RequiresRetireExceptionPublication => !IsAccepted;

        public static DmaStreamComputeStatusQueryResult Accepted(
            DmaStreamComputeTokenStatusSnapshot snapshot) =>
            new(
                DmaStreamComputeStatusQueryStatus.Accepted,
                DmaStreamComputeStatusQueryRejectKind.None,
                snapshot,
                "DSC_STATUS captured store-owned token status for retire-owned writeback.");

        public static DmaStreamComputeStatusQueryResult Rejected(
            DmaStreamComputeStatusQueryRejectKind rejectKind,
            string message) =>
            new(
                DmaStreamComputeStatusQueryStatus.Rejected,
                rejectKind,
                snapshot: null,
                message);
    }
    ...
    {
        public DmaStreamComputeStatusReplayEvidence(
            ushort opcode,
            ulong tokenId,
            DmaStreamComputeStatusQueryStatus status,
            DmaStreamComputeStatusQueryRejectKind rejectKind,
            ...
            ulong encodedStatusWord)
        {
            ...
            EncodedStatusWord = encodedStatusWord;
        }
        ...
        public DmaStreamComputeTokenLifecycleEvidence TokenLifecycleEvidence { get; }
        ...
        public bool IsComplete =>
            ...
            Status == DmaStreamComputeStatusQueryStatus.Accepted &&
            RejectKind == DmaStreamComputeStatusQueryRejectKind.None &&
            OwnerBinding is not null &&
            TokenLifecycleEvidence.IsComplete &&
            LanePlacementEvidence.IsComplete;

        public static DmaStreamComputeStatusReplayEvidence Capture(
            ulong tokenId,
            DmaStreamComputeOwnerBinding ownerBinding,
            ...
            DmaStreamComputeLanePlacementEvidence lanePlacementEvidence)
        {
            ...
            DmaStreamComputeTokenStatusSnapshot? snapshot = result.Snapshot;
            return new DmaStreamComputeStatusReplayEvidence(
                Processor.CPU_Core.IsaOpcodeValues.DSC_STATUS,
                tokenId,
                result.Status,
                result.RejectKind,
                ownerBinding,
                snapshot?.LifecycleEvidence ?? default,
                lanePlacementEvidence,
                snapshot?.EncodedStatusWord ?? 0UL);
        }
    }
    ...
    public sealed class DmaStreamComputeTokenStore
    {
        ...
        public DmaStreamComputeStatusQueryResult QueryStatusByTokenId(
            ulong tokenId,
            DmaStreamComputeOwnerBinding ownerBinding)
        {
            ArgumentNullException.ThrowIfNull(ownerBinding);
            ...
            if (tokenId == 0)
            {
                return DmaStreamComputeStatusQueryResult.Rejected(
                    DmaStreamComputeStatusQueryRejectKind.InvalidToken,
                    "DSC_STATUS requires a non-zero store-owned token id.");
            }

            bool sawTokenId = false;
            foreach (DmaStreamComputeActiveTokenEntry entry in _activeTokens.Values)
            {
                if (entry.Handle.TokenId != tokenId)
                {
                    continue;
                }

                sawTokenId = true;
                if (!entry.Handle.MatchesOwner(ownerBinding))
                {
                    continue;
                }

                return DmaStreamComputeStatusQueryResult.Accepted(
                    DmaStreamComputeTokenStatusSnapshot.FromEntry(entry));
            }

            return DmaStreamComputeStatusQueryResult.Rejected(
                sawTokenId
                    ? DmaStreamComputeStatusQueryRejectKind.OwnerDomainMismatch
                    : DmaStreamComputeStatusQueryRejectKind.InvalidToken,
                sawTokenId
                    ? "DSC_STATUS owner/domain guard rejected the token status query."
                    : "DSC_STATUS token id was not present in the active lane6 token store.");
        }

...
public sealed class AcceleratorTokenStore
{
        ...
        ExternalAcceleratorConflictManager? conflictManager = null)
    {
        ...
        if (!IsCurrentRuntimeTaxonomyForTokenAdmission(
                descriptor,
                capabilityAcceptance,
                out string taxonomyMessage))
        {
            return RecordSubmitResult(
                AcceleratorTokenAdmissionResult.Reject(
                    AcceleratorTokenFaultCode.CapabilityNotAccepted,
    ...
    public AcceleratorTokenLookupResult TryStatus(
        AcceleratorTokenHandle handle,
        AcceleratorGuardEvidence? currentGuardEvidence) =>
        TryLookup(handle, currentGuardEvidence, AcceleratorTokenLookupIntent.Observe);

    public AcceleratorTokenLookupResult Status(
        AcceleratorTokenHandle handle,
        AcceleratorGuardEvidence? currentGuardEvidence) =>
        TryStatus(handle, currentGuardEvidence);
    ...
    private static bool IsCurrentRuntimeTaxonomyForTokenAdmission(
        AcceleratorCommandDescriptor descriptor,
        AcceleratorCapabilityAcceptanceResult capabilityAcceptance,
        out string message)
    {
        AcceleratorDescriptorTaxonomyKey descriptorKey = new(
        ...
        {
            message =
                "L7-SDC token admission requires a known current-runtime descriptor taxonomy entry; unknown descriptor metadata is not token authority.";
            return false;
        }

        if (!taxonomyEntry.IsCurrentRuntimeContour)
        {
            message =
                $"L7-SDC token admission rejects '{taxonomyEntry.Name}' because the taxonomy entry is {taxonomyEntry.Status} and grants no token, queue, execution, or commit authority.";
            return false;
        }

        AcceleratorCapabilityDescriptor? capabilityDescriptor =
            capabilityAcceptance.Descriptor;
        if (capabilityDescriptor is null ||
            !capabilityDescriptor.TaxonomyKey.Equals(descriptorKey) ||
            !capabilityDescriptor.TaxonomyEntry.IsCurrentRuntimeContour)
        {
            message =
                "L7-SDC token admission requires capability metadata pinned to the descriptor current-runtime taxonomy entry; metadata-only and non-current accelerator metadata are not token authority.";
            return false;
        }

        message = "L7-SDC descriptor and capability are pinned to the current runtime taxonomy entry.";
        return true;
    }

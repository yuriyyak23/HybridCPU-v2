...
public static class AcceleratorDescriptorParser
{
        ...
        out AcceleratorCarrierValidationResult? failure)
    {
            ...
            instruction.Reduction)
        {
            failure = AcceleratorCarrierValidationResult.Fail(
                AcceleratorDescriptorFault.DescriptorCarrierDecodeFault,
                $"Slot {slotIndex}: L7-SDC current command ABI accepts only a clean packed-register carrier.");
            return false;
        }
        ...
        if (rs2 is not (0 or VLIW_Instruction.NoArchReg))
        {
            failure = AcceleratorCarrierValidationResult.Fail(
                AcceleratorDescriptorFault.DescriptorCarrierDecodeFault,
                $"Slot {slotIndex}: L7-SDC current command ABI reserves rs2; use x0/NoArchReg instead of x{rs2}.");
            return false;
        }

        if ((opcode is Processor.CPU_Core.IsaOpcodeValues.ACCEL_QUERY_CAPS
                or Processor.CPU_Core.IsaOpcodeValues.ACCEL_SUBMIT) &&
            rs1 is not (0 or VLIW_Instruction.NoArchReg))
        {
            failure = AcceleratorCarrierValidationResult.Fail(
                AcceleratorDescriptorFault.DescriptorCarrierDecodeFault,
                $"Slot {slotIndex}: {OpcodeRegistry.GetMnemonicOrHex(opcode)} uses rd, x0, x0 in the current L7-SDC ABI; rs1 cannot carry authority.");
            return false;
        }
        ...
        out AcceleratorDescriptorValidationResult? failure)
    {
        ...
        {
            string message =
                AcceleratorDescriptorTaxonomyCatalog.TryGetEntry(
                    acceleratorClass,
                    acceleratorId,
                    operation,
                    shape,
                    out AcceleratorDescriptorTaxonomyEntry taxonomyEntry) &&
                taxonomyEntry.IsMetadataOnly
                    ? "L7-SDC descriptor taxonomy entry is known as metadata-only; parser acceptance, token issue, execution, commit, and compiler emission remain disabled."

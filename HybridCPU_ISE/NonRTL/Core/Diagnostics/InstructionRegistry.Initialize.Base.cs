...
namespace YAKSys_Hybrid_CPU.Core
{
    public static partial class InstructionRegistry
    {
        private static void RegisterBaseInstructionSet()
        {
            ...
            RegisterSemanticFactory((uint)Processor.CPU_Core.InstructionsEnum.DSC_STATUS, ctx =>
            {
                if (ctx.Reg3ID != 0 &&
                    ctx.Reg3ID != HybridCPU_ISE.Arch.VLIW_Instruction.NoReg)
                {
                    throw new DecodeProjectionFaultException(
                        "DSC_STATUS uses the canonical packed register ABI rd, rs1(token-id), x0.");
                }

                return new DmaStreamComputeStatusMicroOp(
                    ctx.Reg1ID,
                    ctx.Reg2ID);
            });
            RegisterOpAttributes((uint)Processor.CPU_Core.InstructionsEnum.DSC_STATUS, new MicroOpDescriptor
            {
                Latency = 2,
                MemFootprintClass = 1,
                IsMemoryOp = false,
            });
            RegisterSemanticFactory((uint)Processor.CPU_Core.InstructionsEnum.DSC_QUERY_CAPS, ctx =>
            {
                if (!IsZeroOrNoRegister(ctx.Reg2ID) ||
                    !IsZeroOrNoRegister(ctx.Reg3ID))
                {
                    throw new DecodeProjectionFaultException(
                        "DSC_QUERY_CAPS uses the canonical packed register ABI rd, x0, x0.");
                }

                return new DmaStreamComputeQueryCapsMicroOp(ctx.Reg1ID);
            });
            RegisterOpAttributes((uint)Processor.CPU_Core.InstructionsEnum.DSC_QUERY_CAPS, new MicroOpDescriptor
            {
                Latency = 2,
                MemFootprintClass = 1,
                IsMemoryOp = false,
            ...
            RegisterSystemDeviceCommandOp(
                ...
                static ctx =>
                {
                    ...
                    {
                        throw new DecodeProjectionFaultException(
                            "ACCEL_QUERY_CAPS uses the canonical packed register ABI rd, x0, x0.");
                    }

                    return new AcceleratorQueryCapsMicroOp(ctx.Reg1ID);
                });
            RegisterSystemDeviceCommandOp(
                ...
                static ctx =>
                {
                    ...
                    {
                        throw new DecodeProjectionFaultException(
                            "ACCEL_SUBMIT uses the canonical packed register ABI rd, x0, x0 plus typed descriptor sideband.");
                    }

                    return new AcceleratorSubmitMicroOp(ctx.Reg1ID);
                });
            RegisterSystemDeviceCommandOp(
                (uint)Processor.CPU_Core.InstructionsEnum.ACCEL_POLL,
                static ctx => new AcceleratorPollMicroOp(ctx.Reg1ID, ctx.Reg2ID));
            RegisterSystemDeviceCommandOp(
                (uint)Processor.CPU_Core.InstructionsEnum.ACCEL_WAIT,
                static ctx => new AcceleratorWaitMicroOp(ctx.Reg1ID, ctx.Reg2ID));
            RegisterSystemDeviceCommandOp(
                (uint)Processor.CPU_Core.InstructionsEnum.ACCEL_CANCEL,
                static ctx => new AcceleratorCancelMicroOp(ctx.Reg1ID, ctx.Reg2ID));
            RegisterSystemDeviceCommandOp(
                (uint)Processor.CPU_Core.InstructionsEnum.ACCEL_FENCE,
                static ctx => new AcceleratorFenceMicroOp(ctx.Reg1ID, ctx.Reg2ID));
            RegisterSystemDeviceCommandOp(
                (uint)Processor.CPU_Core.InstructionsEnum.ACCEL_STATUS,
                static ctx =>
                {
                    if (!IsZeroOrNoRegister(ctx.Reg3ID))
                    {
                        throw new DecodeProjectionFaultException(
                            "ACCEL_STATUS uses the canonical packed register ABI rd, rs1(token-id), x0.");
                    }

                    return new AcceleratorStatusMicroOp(ctx.Reg1ID, ctx.Reg2ID);
                });
            ...
            RegisterVmxOp((uint)Processor.CPU_Core.InstructionsEnum.VMPTRST);
            RegisterVmxOp((uint)Processor.CPU_Core.InstructionsEnum.VMCALL);
            RegisterVmxOp((uint)Processor.CPU_Core.InstructionsEnum.INVEPT);
            RegisterVmxOp((uint)Processor.CPU_Core.InstructionsEnum.INVVPID);
            RegisterVmxOp((uint)Processor.CPU_Core.InstructionsEnum.VMFUNC);
            RegisterVmxOp((uint)Processor.CPU_Core.InstructionsEnum.VMSAVEX);
            RegisterVmxOp((uint)Processor.CPU_Core.InstructionsEnum.VMRESTX);
        }

        private static void RegisterSystemDeviceCommandOp(
            uint opCode,
            Func<DecoderContext, SystemDeviceCommandMicroOp> factory)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                SystemDeviceCommandMicroOp microOp = factory(ctx);
        ...
        private static bool IsZeroOrNoRegister(ushort registerId) =>
            registerId == 0 ||
            registerId == HybridCPU_ISE.Arch.VLIW_Instruction.NoReg;
    }
}

    ...
    public static partial class InstructionRegistry
    {
            ...
            private static void RegisterPublishedVectorBinaryOp(uint opCode)
            {
                RegisterSemanticFactory(opCode, delegate(DecoderContext ctx)
                {
                    ...
                    if (ctx.Saturating)
                    {
                        if (!OpcodeRegistry.SupportsSaturatingAddPolicy(opCode))
                        {
                            throw new DecodeProjectionFaultException(
                                $"Opcode {(ushort)ctx.OpCode} reached InstructionRegistry.CreateMicroOp(...) with the saturating policy bit set, but Phase 05B opens only VADD.SAT.");
                        }

                        VectorSaturatingAddMicroOp saturatingAddMicroOp = new VectorSaturatingAddMicroOp
                        {
                            OpCode = ctx.OpCode,
                            Instruction = GetRequiredProjectedVectorInstruction(in ctx),
                            PredicateMask = ctx.PredicateMask
                        };
                        saturatingAddMicroOp.InitializeMetadata();
                        return saturatingAddMicroOp;
                    }
            ...
            private static void RegisterPublishedVectorGatherOp(uint opCode)
            {
                if (opCode != (uint)Processor.CPU_Core.InstructionsEnum.VGATHER)
                {
                    throw new InvalidOperationException(
                        "The indexed vector-gather registration helper is intentionally scoped to VGATHER only.");
                }

                RegisterSemanticFactory(opCode, delegate(DecoderContext ctx)
                {
                    if (!ctx.HasVectorAddressingContour || !ctx.HasVectorPayload)
                    {
                        throw new DecodeProjectionFaultException(
                            "VGATHER reached InstructionRegistry.CreateMicroOp(...) without canonical vector payload/addressing projection.");
                    }

                    if (!ctx.IndexedAddressing || ctx.Is2DAddressing)
                    {
                        string addressingContour = !ctx.IndexedAddressing && !ctx.Is2DAddressing
                            ? "non-indexed"
                            : DescribeNonRepresentableVectorAddressingContour(
                                ctx.IndexedAddressing,
                                ctx.Is2DAddressing);
                        if (string.IsNullOrWhiteSpace(addressingContour))
                        {
                            addressingContour = "non-indexed";
                        }

                        throw new DecodeProjectionFaultException(
                            $"Opcode {(ushort)ctx.OpCode} reached InstructionRegistry.CreateMicroOp(...) with unsupported {addressingContour} VGATHER addressing. " +
                            "Phase 01 only publishes the 1D indexed-read contour; VSCATTER, 2D, and indexed+2D remain fail closed.");
                    }

                    VectorContourLegalityStatus legalityStatus =
                        VectorLegalityMatrix.GetAddressingStatus(
                    ...
                    {
                        throw new DecodeProjectionFaultException(
                            "VGATHER indexed publication reached the registry before the runtime legality matrix opened the executable contour.");
                    }

                    GatherMicroOp gatherMicroOp = new GatherMicroOp
                    {
                        OpCode = ctx.OpCode,
                        Instruction = GetRequiredProjectedVectorInstruction(in ctx),
                        PredicateMask = ctx.PredicateMask
            ...
            private static void RegisterPublishedVectorScatterOp(uint opCode)
            {
                if (opCode != (uint)Processor.CPU_Core.InstructionsEnum.VSCATTER)
                {
                    throw new InvalidOperationException(
                        "The indexed vector-scatter registration helper is intentionally scoped to VSCATTER only.");
                }
                ...
                RegisterSemanticFactory(opCode, delegate(DecoderContext ctx)
                {
                    ...
                    {
                        throw new DecodeProjectionFaultException(
                            "VSCATTER reached InstructionRegistry.CreateMicroOp(...) without canonical vector payload/addressing projection.");
                    }
                    ...
                    if (!ctx.IndexedAddressing || ctx.Is2DAddressing)
                    {
                        ...
                        throw new DecodeProjectionFaultException(
                            $"Opcode {(ushort)ctx.OpCode} reached InstructionRegistry.CreateMicroOp(...) with unsupported {addressingContour} VSCATTER addressing. " +
                            "Phase 02 only publishes the 1D indexed-write contour; 2D and indexed+2D remain fail closed.");
                    }
                    ...
                    {
                        throw new DecodeProjectionFaultException(
                            "VSCATTER indexed publication reached the registry before the runtime legality matrix opened the executable contour.");
                    }

                    StoreScatterMicroOp scatterMicroOp = new StoreScatterMicroOp
                    {
                        OpCode = ctx.OpCode,
                        Instruction = GetRequiredProjectedVectorInstruction(in ctx),
                        PredicateMask = ctx.PredicateMask
                    };
                    scatterMicroOp.InitializeMetadata();
                    return scatterMicroOp;
                });
            ...
            private static void RegisterPublishedVectorMaskOp(uint opCode)
            {
                RegisterSemanticFactory(opCode, delegate(DecoderContext ctx)
                {
                    ...
                    if (ctx.OpCode == (uint)Processor.CPU_Core.InstructionsEnum.VMSBF &&
                        ((ctx.Immediate >> 4) & 0x0F) != 0)
                    {
                        throw new DecodeProjectionFaultException(
                            "VMSBF reached InstructionRegistry.CreateMicroOp(...) with a non-zero reserved src2 predicate nibble. " +
                            "Phase 04 publishes only the canonical unary mask-prefix ABI: src predicate in imm[3:0], dest predicate in imm[11:8].");
                    }
            ...
            private static void RegisterPublishedVectorZeroExtendOp(uint opCode)
            {
                if (opCode != (uint)Processor.CPU_Core.InstructionsEnum.VZEXT)
                {
                    throw new InvalidOperationException(
                        "The Phase 05 vector zero-extend registration helper is intentionally scoped to VZEXT only.");
                }
                ...
                RegisterSemanticFactory(opCode, delegate(DecoderContext ctx)
                {
                    ...
                    {
                        throw new DecodeProjectionFaultException(
                            $"Opcode {(ushort)ctx.OpCode} reached InstructionRegistry.CreateMicroOp(...) with unsupported {value} VZEXT addressing. " +
                            "Phase 05 only publishes the packed 1D unsigned zero-extension memory-to-memory contour.");
                    }
                    if (ctx.HasImmediate && ctx.Immediate != 0)
                    {
                        throw new DecodeProjectionFaultException(
                            "VZEXT reached InstructionRegistry.CreateMicroOp(...) with a non-zero reserved immediate sideband. " +
                            "Phase 05 publishes only the canonical packed 1D ABI: dest base in primary pointer, source base in secondary pointer, unsigned source type in DataType.");
                    }

                    VectorZeroExtendMicroOp microOp = new VectorZeroExtendMicroOp
                    {
                        OpCode = ctx.OpCode,
                        Instruction = GetRequiredProjectedVectorInstruction(in ctx),
                        PredicateMask = ctx.PredicateMask
                ...
                OpcodeInfo opcodeInfo = RequirePublishedOpcodeInfo(opCode);
                if (!OpcodeRegistry.IsWidenNarrowConvertOp(opCode))
                {
                    throw new InvalidOperationException(
                        $"Opcode {(ushort)opCode} is not a published widen/narrow/convert contour.");
                }
            ...
            private static void RegisterPublishedVectorScanSumOp(uint opCode)
            {
                if (opCode != (uint)Processor.CPU_Core.InstructionsEnum.VSCAN_SUM)
                {
                    throw new InvalidOperationException(
                        "The Phase 05A vector scan registration helper is intentionally scoped to VSCAN.SUM only.");
                }
                ...
                RegisterSemanticFactory(opCode, delegate(DecoderContext ctx)
                {
                    ...
                    {
                        throw new DecodeProjectionFaultException(
                            $"Opcode {(ushort)ctx.OpCode} reached InstructionRegistry.CreateMicroOp(...) with unsupported {value} VSCAN.SUM addressing. " +
                            "Phase 05A only publishes the packed 1D integer inclusive-prefix sum memory-to-memory contour.");
                    }
                    if (ctx.HasImmediate && ctx.Immediate != 0)
                    {
                        throw new DecodeProjectionFaultException(
                            "VSCAN.SUM reached InstructionRegistry.CreateMicroOp(...) with a non-zero reserved immediate sideband. " +
                            "Phase 05A publishes only the canonical packed 1D ABI: destination base in primary pointer, source base in secondary pointer, integer element type in DataType.");
                    }

                    VectorScanSumMicroOp microOp = new VectorScanSumMicroOp
                    {
                        OpCode = ctx.OpCode,
                        Instruction = GetRequiredProjectedVectorInstruction(in ctx),
                        PredicateMask = ctx.PredicateMask
                ...
                OpcodeInfo opcodeInfo = RequirePublishedOpcodeInfo(opCode);
                if (!OpcodeRegistry.IsScanOp(opCode))
                {
                    throw new InvalidOperationException(
                        $"Opcode {(ushort)opCode} is not a published vector scan contour.");
                }
            ...
            private static void RegisterPublishedVectorPerm2Op(uint opCode)
            {
                if (opCode != (uint)Processor.CPU_Core.InstructionsEnum.VPERM2)
                {
                    throw new InvalidOperationException(
                        "The Phase 09 permute2 registration helper is intentionally scoped to VPERM2 only.");
                }
                ...
                RegisterSemanticFactory(opCode, delegate(DecoderContext ctx)
                {
                    ...
                    {
                        throw new DecodeProjectionFaultException(
                            $"Opcode {(ushort)ctx.OpCode} reached InstructionRegistry.CreateMicroOp(...) with unsupported {value} VPERM2 addressing. " +
                            "Phase 09 only publishes the packed 1D two-source two-lane immediate-controlled contour.");
                    }

                    VectorPermute2MicroOp microOp = new VectorPermute2MicroOp
                    {
                        OpCode = ctx.OpCode,
                        Instruction = GetRequiredProjectedVectorInstruction(in ctx),
                        PredicateMask = ctx.PredicateMask
                ...
                {
                    throw new InvalidOperationException(
                        $"Opcode {(ushort)opCode} is not a published vector contour and cannot use the Phase 09 permute2 descriptor path.");
                }
            ...
            private static void RegisterPublishedVectorTransposeOp(uint opCode)
            {
                if (opCode != (uint)Processor.CPU_Core.InstructionsEnum.VTRANSPOSE)
                {
                    throw new InvalidOperationException(
                        "The Phase 09 transpose registration helper is intentionally scoped to VTRANSPOSE only.");
                }
                ...
                RegisterSemanticFactory(opCode, delegate(DecoderContext ctx)
                {
                    ...
                    {
                        throw new DecodeProjectionFaultException(
                            $"Opcode {(ushort)ctx.OpCode} reached InstructionRegistry.CreateMicroOp(...) with unsupported {value} VTRANSPOSE addressing. " +
                            "Phase 09 only publishes the packed 1D single-surface fixed 2x2 transpose contour.");
                    }

                    VectorTransposeMicroOp microOp = new VectorTransposeMicroOp
                    {
                        OpCode = ctx.OpCode,
                        Instruction = GetRequiredProjectedVectorInstruction(in ctx),
                        PredicateMask = ctx.PredicateMask
                ...
                {
                    throw new InvalidOperationException(
                        $"Opcode {(ushort)opCode} is not a published vector contour and cannot use the Phase 09 transpose descriptor path.");
                }
            ...
            private static void RegisterPublishedVectorSlideOneUpOp(uint opCode)
            {
                if (opCode != (uint)Processor.CPU_Core.InstructionsEnum.VSLIDE1UP)
                {
                    throw new InvalidOperationException(
                        "The Phase 09 slide-one registration helper is intentionally scoped to VSLIDE1UP only.");
                }
                ...
                RegisterSemanticFactory(opCode, delegate(DecoderContext ctx)
                {
                    ...
                    {
                        throw new DecodeProjectionFaultException(
                            $"Opcode {(ushort)ctx.OpCode} reached InstructionRegistry.CreateMicroOp(...) with unsupported {value} VSLIDE1UP addressing. " +
                            "Phase 09 only publishes the packed 1D fixed-one-lane single-surface contour.");
                    }

                    VectorSlideOneUpMicroOp microOp = new VectorSlideOneUpMicroOp
                    {
                        OpCode = ctx.OpCode,
                        Instruction = GetRequiredProjectedVectorInstruction(in ctx),
                        PredicateMask = ctx.PredicateMask
                ...
                {
                    throw new InvalidOperationException(
                        $"Opcode {(ushort)opCode} is not a published vector contour and cannot use the Phase 09 slide-one descriptor path.");
                }
            ...
            private static void RegisterPublishedVectorSlideOneDownOp(uint opCode)
            {
                if (opCode != (uint)Processor.CPU_Core.InstructionsEnum.VSLIDE1DOWN)
                {
                    throw new InvalidOperationException(
                        "The Phase 09 slide-one-down registration helper is intentionally scoped to VSLIDE1DOWN only.");
                }
                ...
                RegisterSemanticFactory(opCode, delegate(DecoderContext ctx)
                {
                    ...
                    {
                        throw new DecodeProjectionFaultException(
                            $"Opcode {(ushort)ctx.OpCode} reached InstructionRegistry.CreateMicroOp(...) with unsupported {value} VSLIDE1DOWN addressing. " +
                            "Phase 09 only publishes the packed 1D fixed-one-lane single-surface contour.");
                    }

                    VectorSlideOneDownMicroOp microOp = new VectorSlideOneDownMicroOp
                    {
                        OpCode = ctx.OpCode,
                        Instruction = GetRequiredProjectedVectorInstruction(in ctx),
                        PredicateMask = ctx.PredicateMask
                ...
                {
                    throw new InvalidOperationException(
                        $"Opcode {(ushort)opCode} is not a published vector contour and cannot use the Phase 09 slide-one-down descriptor path.");
                }
            ...
            private static void RegisterPublishedVectorDotProductWideOp(uint opCode)
            {
                if (opCode != (uint)Processor.CPU_Core.InstructionsEnum.VDOT_WIDE)
                {
                    throw new InvalidOperationException(
                        "The Phase 09 wide-dot registration helper is intentionally scoped to VDOT.WIDE only.");
                }
                ...
                RegisterSemanticFactory(opCode, delegate(DecoderContext ctx)
                {
                    ...
                    {
                        throw new DecodeProjectionFaultException(
                            $"Opcode {(ushort)ctx.OpCode} reached InstructionRegistry.CreateMicroOp(...) with unsupported {value} VDOT.WIDE addressing. " +
                            "The Phase 09 wide-dot carrier only publishes packed 1D scalar-footprint truth and must fail closed instead of reopening a non-representable compat surface.");
                    }

                    VectorDotWideMicroOp microOp = new VectorDotWideMicroOp
                    {
                        OpCode = ctx.OpCode,
                        Instruction = GetRequiredProjectedVectorInstruction(in ctx),
                        PredicateMask = ctx.PredicateMask
                ...
                {
                    throw new InvalidOperationException(
                        $"Opcode {(ushort)opCode} is not a published vector contour and cannot use the Phase 09 wide-dot descriptor path.");
                }

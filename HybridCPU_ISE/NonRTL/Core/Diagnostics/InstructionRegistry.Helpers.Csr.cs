...
namespace YAKSys_Hybrid_CPU.Core
{
    public static partial class InstructionRegistry
    {
            ...
            out CSRMicroOp? csrMicroOp)
        {
            ...
            switch (opcode)
            {
                case Processor.CPU_Core.IsaOpcodeValues.RDCYCLE:
                    csrMicroOp = CreatePublishedCounterReadCsrMicroOp(
                        opcode,
                        instruction,
                        CsrAddresses.Cycle);
                    return true;
        ...
        private static CsrReadCounterMicroOp CreatePublishedCounterReadCsrMicroOp(
            ushort opcode,
            in InstructionIR instruction,
            ushort csrAddress)
        {
            ...
            {
                throw new DecodeProjectionFaultException(
                    $"Opcode {OpcodeRegistry.GetMnemonicOrHex(opcode)} requires canonical rd-only counter-read payload: rs1=x0, rs2=x0, Immediate=0.");
            }

            ushort destinationRegister =
                NormalizeCanonicalOrSuppressedCsrDestinationRegister(
                    opcode,
                    ToLegacyDecoderField(instruction.Rd));

            var csrMicroOp = new CsrReadCounterMicroOp
            {
                OpCode = opcode,
                ...
                WritesRegister =
                    ...
                    destinationRegister != ArchRegisterTripletEncoding.NoReg,
            };
        ...
        private static void RegisterCounterReadCsrOp(uint opCode, ushort csrAddress)
        {
            RegisterSemanticFactory(opCode, ctx =>
            {
                ValidateCounterReadCsrDecoderContext(in ctx, csrAddress);
                ...
                var op = new CsrReadCounterMicroOp
                {
                    OpCode = ctx.OpCode,
                    CSRAddress = csrAddress,
                    DestRegID = destinationRegister,
        ...
        private static void ValidateCounterReadCsrDecoderContext(
            ...
            ushort csrAddress)
        {
            ...
            {
                throw new DecodeProjectionFaultException(
                    $"Counter-read opcode {OpcodeRegistry.GetMnemonicOrHex(ctx.OpCode)} is fixed to CSR 0x{csrAddress:X3} and requires canonical Immediate=0.");
            }

            if (!TryDecodeCanonicalOrUnpackedNoArchRegister(ctx.Reg2ID, out byte rs1) ||
                !TryDecodeCanonicalOrUnpackedNoArchRegister(ctx.Reg3ID, out byte rs2))
            {
                throw new DecodeProjectionFaultException(
                    $"Counter-read opcode {OpcodeRegistry.GetMnemonicOrHex(ctx.OpCode)} reached InstructionRegistry.CreateMicroOp(...) with non-canonical source register encoding. " +
                    "Manual publication must use rd-only canonical payload: rs1=x0, rs2=x0.");
            }

            if (rs1 is not 0 and not ArchRegisterTripletEncoding.NoArchReg ||
                rs2 is not 0 and not ArchRegisterTripletEncoding.NoArchReg)
            {
                throw new DecodeProjectionFaultException(
                    $"Counter-read opcode {OpcodeRegistry.GetMnemonicOrHex(ctx.OpCode)} requires canonical rd-only payload: rs1=x0, rs2=x0.");
            }
        }
